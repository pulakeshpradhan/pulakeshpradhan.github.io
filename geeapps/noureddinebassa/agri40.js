...
var ulb = ee.FeatureCollection("users/noureddinebassa/zone");
var data = S2.filterBounds(ulb).filterDate('2022-01-28','2022-04-12').median()
var GRENNE = data.select('B3')
var NIR = data.select('B8')
var NDVI = NIR.subtract(GRENNE).divide(NIR.add(GRENNE))
var NDVIc= NDVI.clip(ulb)
// Below line set the default background map to Google satellite
var ndviVis = {min:-1, max:1, palette: ['red', 'white', 'green'] }
 Map.addLayer(NDVIc,ndviVis)
Map.setOptions('HYBRID');
var styling ={ color :'black', fillColor:'FFFFFF00'}
Map.addLayer(ulb.style(styling))
var data = S2.filterBounds(ulb).filterDate('2022-01-28','2022-04-12').median()
var GRENNE = data.select('B3')
var NIR = data.select('B8')
var NDVI = NIR.subtract(GRENNE).divide(NIR.add(GRENNE))
var NDVIc= NDVI.clip(ulb)
// Below line set the default background map to Google satellite
var ndviVis = {min:0, max:0.5, palette: ['blue', 'white', 'green'] }
 Map.addLayer(NDVIc,ndviVis)
Map.setOptions('HYBRID');
...
var elev = {min: 1000, max: 5000, palette: elevPalette};
...
var rois = {min:-1, max:5, palette: ['blue', 'white', 'green', 'red']}
//////FUNCTIONS DEFINED////////
...
var pcp = GPM.filterDate('2018-10-01', '2022-06-10')
              .filterBounds(ulb)
...
Map.onClick(function(coords) {
  ...
  chart1.setOptions({
    title: 'Monthly Precipitation 2022',
