var urt_eez = ui.import && ui.import("urt_eez", "table", {
    ...
    gfc2019old = ui.import && ui.import("gfc2019old", "image", {
    ...
    gfc2019 = ui.import && ui.import("gfc2019", "image", {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
//////Burned Areas
var getBA = function(eneo,jina)
{
  var img_eneo = ee.Geometry.Polygon(eneo.geometry().coordinates());
  var lossImage = gfc2019.select(['loss']).clip(img_eneo);
  //var gainImage = gfc2019.select(['gain']).clip(img_eneo);
  var loss_areaImage = lossImage.multiply(ee.Image.pixelArea()).divide(10000);
  //var gain_areaImage = gainImage.multiply(ee.Image.pixelArea()).divide(10000);
  var lossStats = loss_areaImage.reduceRegion({  reducer: ee.Reducer.sum(), geometry: img_eneo, scale: 30,maxPixels: 1e9 });
  //var gainStats = gain_areaImage.reduceRegion({  reducer: ee.Reducer.sum(), geometry: img_eneo, scale: 30,maxPixels: 1e9 });
  print('Forest Loss: ', lossStats.get('loss'), 'Hectares');
  //print('Forest Gain: ', gainStats.get('gain'), 'Hectares'); 
  var visParamsLoss =  { bands: ['loss'],min: 0,  max: 10, palette: ['282828','FFBB22','FFFF4C','F096FF','FA0000'], forceRgbOutput:true };
  //var visParamsGain =  { bands: ['gain'],min: 0,  max: 10, palette: ['B4B4B4','F0F0F0','0032C8','0096A0','FAE6A0'], forceRgbOutput:true };
  map.addLayer(lossImage.updateMask(lossImage),visParamsLoss,jina + ': Forest Loss',1);
  //map.addLayer(gainImage,visParamsGain,jina + ': Forest Gain',1);
  //-----------------------------------------------------------------------------------------------
  var lossYear = gfc2019.select(['lossyear']);
  var lossByYear = loss_areaImage.addBands(lossYear).reduceRegion({
                  reducer: ee.Reducer.sum().group({ groupField: 1 }), geometry: img_eneo, scale: 30, maxPixels: 1e9 });
 //Format the output a little to make the result a dictionary
  var Loss_statsFormatted = ee.List(lossByYear.get('groups'))
                          .map(function(el) { var d = ee.Dictionary(el);
                          return [ee.Number(d.get('group')).format("20%02d"), d.get('sum')]; });
  var Loss_statsDictionary = ee.Dictionary(Loss_statsFormatted.flatten());
  print(Loss_statsDictionary);
  //===================================================================================================================
  //===================================================================================================================
  var optionsa = ui.Label('Yearly Forest Loss:- ' + jina, {textAlign: 'left', fontSize: '14px', fontWeight: 'bold'});
  panel_grafu.widgets().set(1, optionsa);   // Display the Loss
  var optionsL = { title: jina + ' - Forest Loss', fontSize: 12, 
                    hAxis: {title: 'Year', format: '####'}, vAxis: {title: 'Area (Hectares)'}, 
                    legend: 'none', lineWidth: 1, pointSize: 3 };
  //var histogramL = ui.Chart.image.histogram(lossImage, eneo, 250).setOptions(optionsL);
  var histogramL = ui.Chart.array.values({ array: Loss_statsDictionary.values(), axis: 0, xLabels: Loss_statsDictionary.keys() })
                                            .setChartType('ColumnChart').setOptions(optionsL);
  panel_grafu.widgets().set(2, histogramL);   // Display the histogram - Loss.
  //===================================================================================================================
  var lebel_jina = ui.Label('Forest Loss ' + jina, {textAlign: 'left', fontSize: '14px', fontWeight: 'bold'});
  var lebel_tarehe = ui.Label('Date: Year 2001 to 2021', {textAlign: 'left', fontSize: '14px', fontWeight: 'bold'});
  ...
  function downloadImg()
  {
    ...
    var visLoss =  { bands: ['loss'],min: 0,  max: 1, palette: ['000000','FF0000'], forceRgbOutput:true };
    var loss_to_jpeg = lossImage.updateMask(lossImage).visualize(visLoss);
    var downloadArgs = {dimensions:1024,region:img_bound, format:"jpg"};
    var url = loss_to_jpeg.getDownloadURL(downloadArgs);
    urlLabel.setUrl(url);
    urlLabel.style().set({shown: true});
    var urlForest = "http://earthenginepartners.appspot.com/science-2013-global-forest";
    urlLabelForest.setUrl(urlForest);
    urlLabelForest.style().set({shown: true});
  }
  // Add UI elements to the Map.
  var downloadButton = ui.Button('Download ' + jina +' - Forest Loss/Gain', downloadImg);
  var urlLabel = ui.Label('Download-Loss Area', {shown: false});
  var urlLabelForest = ui.Label('Link-Hansen Map', {shown: false});
  ...
  panel_takwimu.widgets().set(6, urlLabelForest);
...
var chora_sasa  = ui.Button({label: '==-> LOSS <-==', style: { margin:'5px', stretch:'horizontal'},  onClick: bonyezo,});
//==============================================================================================================================================
...
function bonyezo()
{
  panel_grafu.clear();
  panel_takwimu.clear();
  getBA(geom_cbo,geomjina);
}
//==============================================================================================================================================
var title = ui.Label('Tanzania CBNRM - Forest Loss', {
