var ls8 = ui.import && ui.import("ls8", "imageCollection", {
    ...
    mbremp = ui.import && ui.import("mbremp", "table", {
      "id": "users/lmathew/Seascape/mbremp"
    }) || ee.FeatureCollection("users/lmathew/Seascape/mbremp");
/* 
#######################################################################################################
...
var sn = mbremp.geometry().bounds(); 
Map.centerObject(mbremp,10);
var tupu = ee.Image().byte();
var jira1 = tupu.paint({featureCollection: sn, color: 1, width: 3 });
var jira2 = tupu.paint({featureCollection: mbremp, color: 3, width: 5 });
// define the image to use
//var image = ee.Image('LANDSAT/LC8_L1T_32DAY_TOA/20181119').clip(sn);
var imagea = ls8.filterDate('2019-01-01','2019-12-31').sort('CLOUD_COVER', true).select(['B[1-7]']);
var image = imagea.median().clip(mbremp);
//var image = imagea.first();
// define names for the band combinations
var bandComboNames = [
  'RGB // (B4/B3/B2)', 'RGB // (B7/B6/B4)', 'RGB // (B5/B4/B3)',
  'RGB // (B6/B5/B2)', 'RGB // (B7/B6/B5)', 'RGB // (B5/B6/B2)',
  'RGB // (B5/B6/B4)', 'RGB // (B7/B5/B3)', 'RGB // (B7/B5/B4)',
  'RGB // (B6/B5/B4)'
];
...
var chagua_mwaka = ui.Select([],'2019');    //Mwaka
//Mwaka
miaka.evaluate(function(mwakaa)
{
  chagua_mwaka.items().reset(mwakaa);
  chagua_mwaka.setPlaceholder('2019');
  chagua_mwaka.onChange(function(mwakaa)
  {
    var mwaka = mwakaa;
    alert(mwakaa);
    var imageb = ls8.filterDate(mwaka + '-01-01',mwaka + '-12-31').sort('CLOUD_COVER', true).select(['B[1-7]']);
    var imageb = imagea.median().clip(mbremp);
    bandComboNames.forEach(function(name, index)
    {
      maps[index].addLayer(imageb, visParams[index],mwaka,true);
      //maps.addLayer(jira2,{},'MBREMP',true);
    });
  })
})
...
bandComboNames.forEach(function(name, index)
{
  ...
  map.addLayer(image, visParams[index], name,true);
  map.addLayer(jira2,{},'MBREMP',true);
  map.setControlVisibility(false);
  maps.push(map);
});
// create a title.
var title = ui.Label('MBREMP Project 2 - LS8OLI-TOA(RGB) Combo-2019', {
  stretch: 'horizontal',
  textAlign: 'center',
  fontWeight: 'bold',
  fontSize: '24px'
...
ui.root.widgets().reset([title,chagua_mwaka, mapGrid]);
ui.root.setLayout(ui.Panel.Layout.Flow('vertical'));
// center the maps near Santa Cruz
ui.Map.Linker(maps, 'change-bounds');
//maps[0].setControlVisibility(true);
maps[0].centerObject(mbremp,10);
//maps[0].add(panel_mwaka);