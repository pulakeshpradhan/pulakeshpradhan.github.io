...
var showLayer = function() {
  ...
  var image_sentinel = sentinel2.filterBounds(geometry3)
                              ...
                              .map(maskS2clouds);
  var image_sentinel_clouds = sentinel2.filterBounds(geometry3)
                              .filterDate(startD, endD)
  ...
  app.intro = {
    ...
    button_panel: ui.Panel([ 
      ...
      ui.Button({
        ...
        style: {color: '#545aa7', margin: '15px 0px 0px 10px'}
      })
    ], ui.Panel.Layout.flow('horizontal')),
...
function exportImage() {
  ...
  var image_sentinel = sentinel2.filterDate(startD, endD)
                                .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
                                .select('B2','B3','B4','B8','QA60').filterBounds(geometry3).map(maskS2clouds);
  var multi = ee.Geometry(assetTable.geometry());
  print(multi);
 // Map.addLayer(multi, {}, 'mutli');
 // var Line =ee.FeatureCollection(multi); print(Line);
 // var Pimage = ee.Image(image_sentinel.median()).paint(multi,'red',5);  //Map.addLayer(Pimage,rgbVis, 'Pimage');
  var pointImage = ee.Image(image_sentinel.map(visFun).median()).paint(multi,'red', 5);
  Map.layers().set(1, pointImage);
 // app.intro.downloadImage_label.setValue(ee.data.makeThumbUrl(ee.data.getThumbId({image: pointImage, format: 'jpg', filename: 'File'})));
  app.intro.downloadImage_label.setValue(image_sentinel.map(visFun).median().getThumbURL({dimensions:700, region: multi.buffer(1000), format: 'png', bands: ['vis-red', 'vis-green', 'vis-blue']}));
}
