...
function exportImage() {
  ...
  //Map.addLayer(ee.Image(image_sentinel.median()).paint(table3, 'red'),{}, 'Points');
  //Map.addLayer(table3draw, {}, 'Table');
  //var listMaps = Map.layers();
  var multi = ee.Geometry(table3.geometry());
 // Map.addLayer(multi, {}, 'mutli');
 // var Line =ee.FeatureCollection(multi); print(Line);
 // var Pimage = ee.Image(image_sentinel.median()).paint(multi,'red',5);
  var pointImage = ee.Image(image_sentinel.map(visFun).median()).paint(multi,'red',5);
  Map.layers().set(1, pointImage);
  //Map.addLayer(Pimage,rgbVis, 'Pimage');
 // app.intro.downloadImage_label.setValue(ee.data.makeThumbUrl(ee.data.getThumbId({image: pointImage, format: 'jpg', filename: 'File'})));
  app.intro.downloadImage_label.setValue(image_sentinel.map(visFun).median().getThumbURL({dimensions:1000, region: multi.buffer(1000), format: 'png', bands: ['vis-red', 'vis-green', 'vis-blue']}));
}
Map.onClick(function(coords) {
  // Update the lon/lat panel with values from the click event.
  lat_coords.setValue(coords.lat.toFixed(10));
  long_coords.setValue(coords.lon.toFixed(10));
  point_name.setValue('');
