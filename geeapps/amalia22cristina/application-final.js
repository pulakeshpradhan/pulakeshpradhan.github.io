...
var showLayer = function() {
  ...
  var image_sentinel = sentinel2.filterDate(startD, endD)
                                ...
                                .filterBounds(geometry).map(maskS2clouds);
...
function exportMeasurePointsValues(assetTable){
  var startD = app.intro.text_sd.getValue();
  var endD = app.intro.text_ed.getValue();
  var image_sentinel = sentinel2.filterBounds(geometry3)
                              .filterDate(startD, endD);
  print("Images: ", image_sentinel.size());
  var listOfImages = image_sentinel.toList(image_sentinel.size());
...
function exportDataMP(){
  ...
  //var output = ee.Algorithms.If(condition, true, false);
  //var showoutput = ee.Algorithms.If(ee.Algorithms.IsEqual(output, false), successFunc(link));
...
function exportImage() {
  ...
  // var image5 = ee.ImageCollection([
  //   ee.Image(image1),
  //   //ee.Image(Map.layers().get(5).getEeObject()).visualize({opacity: 0.7})
  //   ]).mosaic().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
  //   print(image5);
  var image6 = Map.layers().get(6).getEeObject().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
  var choose_image6 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'All Computations'), image6, null);
  //var choose_image5 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'NDVI'), image5, choose_image6);
  var choose_image4 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'Connected Soil Pixels'), image4, choose_image6);
...
function exportMP(){
  ...
  collection_size.setValue('Image Collection Size:  ' + JSON.stringify(image_sentinel.size().getInfo()));
  //var range_json = ee.String(range.getValue()).decodeJSON();
  var one_image = ee.List.sequence(0, listOfImages.size().subtract(1)).map(function(k) {
        var pixels = ee.Image(listOfImages.get(k))
                                  .addBands(elevation)
                                  .addBands(getTemperature(startD, endD))
                                  .addBands(getInfoPrecipitation(startD, endD))
                                  .reduceRegion({
      ...
      var listGeom = assetTable.geometry().coordinates();
      var numberOfPixels = ee.List(pixels.values().get(0)).size();
      var bandNames = ee.Image(listOfImages.get(ee.Number(k)))
                                    .addBands(elevation)
                                    .addBands(getTemperature(startD, endD))
                                    .addBands(getInfoPrecipitation(startD, endD))
                                    .bandNames();
      var filterFeature = ee.Filter.inList('SCL', [5]);
      var features = ee.FeatureCollection(ee.List.sequence(0, numberOfPixels.subtract(1))
          .map(function (i) {
              return bandNames.iterate(function (bandName, feature) {
              var band = ee.String(bandName);
              var pixelValue = ee.List(pixels.get(band)).get(i);
              return ee.Feature(feature).set(band, pixelValue);
            },
            ee.Feature(ee.Geometry.Point(listGeom.get(i))));
          })
      );
  ...
  print('FeatCollection', one_image);
  // var link = ee.data.makeTableDownloadUrl(ee.data.getTableDownloadId({table: features.filter(filterFeature), format: 'CSV', filename: 'File'}));
  // var condition = features.filter(filterFeature).size();
  // app.intro.result_label.setValue('Result:  '+ JSON.stringify(condition.getInfo()));
  // app.intro.checkboxResult.onChange(function(check) {
