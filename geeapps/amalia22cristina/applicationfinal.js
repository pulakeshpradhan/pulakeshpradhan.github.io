...
var sentinel2 = ee.ImageCollection("COPERNICUS/S2_SR"),
    ...
    /* shown: false */
    ee.Geometry.Polygon(
        [[[22.75296303865695, 47.30826797263492],
          [22.75296303865695, 46.43329232913077],
          [24.25259682771945, 46.43329232913077],
          [24.25259682771945, 47.30826797263492]]], null, false),
    geometry3 = 
    /* color: #00ffff */
    /* shown: false */
    ee.Geometry.Polygon(
        [[[23.74908590078575, 46.87112961779949],
          [23.74908590078575, 46.85704518714306],
          [23.777925012113876, 46.85704518714306],
          [23.777925012113876, 46.87112961779949]]], null, false),
    geometry4 = 
    /* color: #d63000 */
    /* shown: false */
    ee.Geometry.Polygon(
        [[[23.85502692200087, 46.95335160246843],
          [23.85502692200087, 46.906930204044045],
          [23.93570776916884, 46.906930204044045],
          [23.93570776916884, 46.95335160246843]]], null, false),
    image = ee.Image("users/amalia22cristina/SIC_v2"),
    geometry5 = 
    /* color: #d63000 */
    /* displayProperties: [
      {
    ...
    ee.Geometry.Polygon(
        [[[23.886500506249412, 46.934825016538454],
          [23.886500506249412, 46.92591589667718],
          [23.902979998436912, 46.92591589667718],
          [23.902979998436912, 46.934825016538454]]], null, false);
...
var showLayer = function() {
  ...
  var image_sentinel = sentinel2.filterDate(startD, endD)
                                .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
                                .filterBounds(geometry5).map(maskS2clouds);
  var image_sentinel_clouds = sentinel2.filterBounds(geometry5)
                              .filterDate(startD, endD)
                              .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20));
  var with_clouds = sentinel2.filterDate(startD, endD).filterBounds(geometry);
  //Map.addLayer(with_clouds, {bands: ['B4', 'B3', 'B2'] }, 'clouds');
  //print("Images: ", image_sentinel.size());
  //var listOfImages = image_sentinel.toList(image_sentinel.size());
  //print('IMG', listOfImages.get(0));
  var imgSentinel = image_sentinel.map(visFun).mean();
  Map.addLayer(imgSentinel, {}, String(startD));
  ////////////////////////////////  Choose Image  //////////////////////////
  ...
  function addConnectedToLayer(conn_pixel_soil, image) {
      ...
      conn_pixel_soil = conn_pixel_soil.select('vis-red').visualize({palette: '#8a3324'});
       // {palette: 'ff3370'});
  ...
  var img_nocloud = image_sentinel.map(cloudfunction_ST2);
  var img_median = img_nocloud.median();
  var ndvi = img_median.normalizedDifference(['B8', 'B4']);
  Map.addLayer(set_threshold(ndvi), {}, 'NDVI');
  ////////////////////////////  Save Image   /////////////////////////
  ...
  //print(chart);
  app.intro.chart_panel.add(chart);
  ...
  var mosaic = ee.ImageCollection([
      imgSentinel,
      bare_soil.median().visualize({opacity: 0.6}),
      ee.Image(layerForFirst).visualize({opacity: 0.7}),
      ee.Image(layerForSoil).visualize({opacity: 0.7}),
      set_threshold(ndvi).visualize({opacity: 0.5})
    ]).mosaic();
  //print(mosaic);
  ...
  //   Export.image.toDrive({
  //   image: mosaic,
  //   description: String(startD)+'-'+String(endD),
  //   region: geometry4,
  //   scale: 30,
  //   maxPixels: 1e12
  // });
  print('Link', mosaic.getThumbURL({dimensions:1000, region: geometry4, format: 'tif', bands: ['vis-red', 'vis-green', 'vis-blue']}));
  ////////////////////////////////  Import Image as Layer  ///////////////////////////////////////
  Map.addLayer(image, {}, 'Image_Layer');
};
////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////  Bare Soil  ///////////////////////////////////////////////////
...
var set_threshold = function(image_ndvi) {
      ...
      return myndvi.visualize({palette:"66b032"});
...
var img_layers = {
  'Original Image': Map.layers().get(0),
  'Bare soil': Map.layers().get(1),
  'Connected Pixels': Map.layers().get(2),
  'Connected Soil Pixels': Map.layers().get(3), 
//  'NDVI': Map.layers().get(4),
  'All Computations': Map.layers().get(5)
};
app.createPanels = function(){
  app.intro = {
    ...
    panel_elevation: ui.Panel(
    ),
    image_label: ui.Label ({
      value: '4) Export Image',
      style: {fontWeight: 'bold', margin: '30px 0px 5px 10px'}
    }),
    export_image_panel: ui.Panel([
      image_select= ui.Select ({
      items: Object.keys(img_layers),
      style: {width: '130px'}
     }),
      ui.Button({
          label: 'Export Image',
    ...
    latitude_panel: ui.Panel([
      ...
      value: '46.93691686272266',
      style: {color:'#5f9ea0', fontSize:'9px', fontWeight: 'bold', width: '120px'}
     }) 
    ], ui.Panel.Layout.flow('horizontal')),
    ...
    button_panel: ui.Panel([ 
      ui.Panel([checkboxCSV = ui.Checkbox({
        label: 'CSV',
        value: false,
        style: {fontSize:'12px', margin: '10px 15px 0px 10px'}
      }),
      checkboxJSON =  ui.Checkbox({
        label: 'JSON',
        value: false,
        style: {fontSize:'12px', margin: '10px 15px 0px 10px'}
      })], ui.Panel.Layout.flow('vertical')),
      ui.Button({
        label: 'Add Point to File',
        onClick: addPointToTable,
        style: {color: '#545aa7', margin: '15px 10px 0px 5px'}
        }),
      ui.Button({
        label: 'Show Points on Map',
        onClick: extractValues,
        style: {color: '#545aa7', margin: '15px 0px 0px 0px'}
      })
    ], ui.Panel.Layout.flow('horizontal')),
    ...
    imageToCsv_label: ui.Label ({
      value: '6) Export Data from Measurement Points to CSV',
      style: {fontWeight: 'bold', margin: '30px 0px 5px 10px'}
    }),
    text_label: ui.Label ({
      value: 'Select an image for which to export data to CSV.',
      style: {color:'grey', fontSize:'11px', margin: '10px 15px 0px 10px'}
    }),
    select_panel: ui.Panel([ 
      collection_size = ui.Label ({
        value: 'Image Collection Size:  1',
        style: {color:'grey', fontWeight: 'bold', fontSize:'12px', margin: '15px 15px 0px 10px'}
      }),  
      // imgSize = ui.Label ({
      //   value: '12',
    ...
    csv_panel: ui.Panel([ 
      range = ui.Textbox ({
      placeholder: '[ 0 - MaxSize ]',
      style: { fontSize:'9px',  width: '120px' ,margin: '17px 15px 0px 10px'}
      }),
      ui.Button({
        label: 'Export Data to CSV',
        onClick: exportDataMP,
        style: {color: '#545aa7', margin: '15px 10px 0px 5px'}
      }),
    ], ui.Panel.Layout.flow('horizontal')),
    checkboxResult: ui.Checkbox({
        label: 'Show Link',
        value: false,
        style: {fontSize:'12px', margin: '15px 15px 0px 10px'}
    }),
    result_label: ui.Label ({
      value: '',
      style: {color:'#78184a',fontWeight: 'bold', fontSize:'12px', margin: '15px 15px 0px 10px'}
    }),
    downloadCSV_label: ui.Label ({
      value: '',
      style: {color:'grey', fontSize:'9px', margin: '15px 15px 0px 10px'}
    }),
    // urlCSV_label: ui.Label ({
    //   value: '',
    //   style: {color:'#acacac', fontSize:'9px', margin: '10px 15px 0px 10px'}
    // })
  };
...
app.boot = function(){
  ...
  var main = ui.Panel({
    widgets: [
      ...
      app.intro.panel_elevation,
      app.intro.image_label,
      app.intro.export_image_panel, 
      app.intro.downloadImage_label,
      ...
      app.intro.url_label,
      app.intro.imageToCsv_label,
      app.intro.text_label,
      app.intro.select_panel,
      app.intro.csv_panel,
      app.intro.result_label,
      app.intro.checkboxResult,
      app.intro.downloadCSV_label,
    ],
...
////////////////////////////////////   Elevation Histogram  ////////////////////////
  function printHistogram(){
   app.intro.panel_elevation.clear();
...
////////////////////////////////   Table   //////////////////////////////////////
  ...
  lat_coords.setValue(coords.lat.toFixed(15));
  long_coords.setValue(coords.lon.toFixed(15));
  point_name.setValue('');
  ...
  function extractValues(){
    ...
    Map.addLayer(centerpoints, {color: '#a81c07'}, 'Centerpoints');
...
///////////////////////////////////// Add specified points to file //////////////////////////////////// 
  var point_list = ee.List([]);
  function addPointToTable(){
    // ccreate the list with points
    ...
    if (checked) {
       ...
       app.intro.url_label.setValue(ee.data.makeTableDownloadUrl(ee.data.getTableDownloadId({table: coll, format: 'CSV', filename: 'File'})));
    }});
    // create download link for json file
    checkboxJSON.onChange(function(check) {
    if (check) {
        app.intro.download_label.setValue('Download Link: ');
    ...
    var lat_json = ee.String(lat_coords.getValue()).decodeJSON();
    var lon_json = ee.String(long_coords.getValue()).decodeJSON();
    Map.addLayer(ee.Geometry.Point([lon_json, lat_json]), {color: '#9400d3'});
   return point_list;
  }
///////////////////////////////////// Export all pixel values on each band //////////////////////////////////// 
  function exportDataMP(){
    // call the function responsible for extracting the data
    var assetTable = extractValues();
    var startD = app.intro.text_sd.getValue();
    var endD = app.intro.text_ed.getValue();
    var image_sentinel = sentinel2.filterBounds(geometry)
                                .filterDate(startD, endD);
    // create a list with all images within collection
    var listOfImages = image_sentinel.toList(image_sentinel.size());
    print('point_table', assetTable);
    collection_size.setValue('Image Collection Size:  ' + JSON.stringify(image_sentinel.size().getInfo()));
    // obtain input data from textbox
    var range_json = ee.String(range.getValue()).decodeJSON();
    // choosing the image that corresponds to the input
    var pixels = ee.Image(listOfImages.get(ee.Number(range_json)))
                              .addBands(elevation)
                              .addBands(getTemperature(startD, endD))
                              .addBands(getInfoPrecipitation(startD, endD))
                              .reduceRegion({
      reducer: ee.Reducer.toList(),
      geometry: assetTable.geometry(),
      scale: 10
    });
    var listGeom = assetTable.geometry().coordinates();
    var numberOfPixels = ee.List(pixels.values().get(0)).size();
    // create a list with all bands name
    var bandNames = ee.Image(listOfImages.get(ee.Number(range_json)))
                                  .addBands(elevation)
                                  .addBands(getTemperature(startD, endD))
                                  .addBands(getInfoPrecipitation(startD, endD))
                                  .bandNames();
    // select those elements which have value '5' for band SCL         
    var filterFeature = ee.Filter.inList('SCL', [5]);
    // for each band extract the pixel value 
      // create a feature for every pixel
    var features = ee.FeatureCollection(
      ee.List.sequence(0, numberOfPixels.subtract(1))
        .map(function (i) {
          return bandNames.iterate(function (bandName, feature) {
    ...
    print('Result', features.filter(filterFeature).size());
    // obtain a download link for file which contains all features
    var link = ee.data.makeTableDownloadUrl(ee.data.getTableDownloadId({table: features.filter(filterFeature), format: 'CSV', filename: 'File'}));
    var condition = features.filter(filterFeature).size();
    app.intro.result_label.setValue('Result:  '+ JSON.stringify(condition.getInfo()));
    app.intro.checkboxResult.onChange(function(check) {
        if (check) {
             app.intro.downloadCSV_label.setValue(link);
        }
    });
  }
///////////////////////////////////// Export choosen image //////////////////////////////////// 
  function exportImage() {
    // obtain the images from Map.Layers
    var image1 = Map.layers().get(0).getEeObject();
    var image2 = ee.ImageCollection([
      ee.Image(image1),
      ee.Image(Map.layers().get(1).getEeObject().median()).visualize({opacity: 0.8})
      ]).mosaic().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
    var image3 = ee.ImageCollection([
      ee.Image(image1),
      ee.Image(Map.layers().get(2).getEeObject()).visualize({opacity: 0.7})
      ]).mosaic().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
    var image4 = ee.ImageCollection([
      ee.Image(image1),
      ee.Image(Map.layers().get(3).getEeObject()).visualize({opacity: 0.7})
      ]).mosaic().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
    var image5 = Map.layers().get(6).getEeObject().visualize({bands: ['vis-red', 'vis-green', 'vis-blue']});
    var choose_image5 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'All Computations'), image5, null);
    var choose_image4 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'Connected Soil Pixels'), image4, choose_image5);
    var choose_image3 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'Connected Pixels'), image3, choose_image4);
    var choose_image2 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'Bare soil'), image2, choose_image3);
    var choose_image1 = ee.Algorithms.If(ee.Algorithms.IsEqual(image_select.getValue(), 'Original Image'), image1, choose_image2);
    // app.intro.downloadImage_label.setValue(ee.data.makeThumbUrl(ee.data.getThumbId({image: pointImage, format: 'jpg', filename: 'File'})));
    app.intro.downloadImage_label.setValue(ee.Image(choose_image1).getThumbURL({dimensions:1000, region: geometry4, format: 'png', bands: ['vis-red', 'vis-green', 'vis-blue']}));
  }