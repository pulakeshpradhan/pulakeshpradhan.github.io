...
var filtered = l8.filterDate('2014-01-01', '2022-10-30');
var with_lswi = filtered.map(addLSWI);
var mositure = with_lswi.qualityMosaic('nd');
...
var getLSWI8 = function(image){
  ...
  return(lswi);
    ...
    .filterBounds(SLMP_3)
    .filterDate('2021-09-01', '2021-11-25')
    .filterMetadata('CLOUD_COVER', 'less_than', 10)
    .map(getNDVI8);
   var l8BaseNDVI = ee.ImageCollection("LANDSAT/LC08/C01/T1_SR")
    .filterBounds(SLMP_3)
    .filterDate('2014-01-01', '2014-05-30')
...
var intro = ui.Panel([
  ...
  ui.Label('Click a point on the SLMP watershed  to inspect PDOs')
]);
panel.add(intro);
...
Map.onClick(function(coords) {
  ...
  // Create a LSWI chart.
  var lswiChart = ui.Chart.image.series(with_lswi.select('nd'), point);
  lswiChart.setOptions({
    title: 'Land Surface Water Index (LSWI)',
    vAxis: {title: 'LSWI', maxValue: 1},
 ...
 //
  ...
  panel.widgets().set(4, eviChart);
// print(Chart.image.series(with_lswi.select('nd'), point));
...
var Llon = ee.Image.pixelLonLat().select('latitude');
var gradient = Llon.multiply((viz.max-viz.min)/100.0).add(viz.min);
...
var Ngradient = Nlon.multiply((Nviz.max-Nviz.min)/100.0).add(Nviz.min);
...
Map.add(Nlegend);
// Adding SLMP logo
var logo = ee.Image('projects/euphoric-truth-269211/assets/png1').visualize({
    bands:  ['b1', 'b2', 'b3'],
    min: 2,
    max: 255
    });
