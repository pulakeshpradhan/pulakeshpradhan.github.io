...
Map.setCenter(-54.75357524029195, -25.789668935919924, 5)
///////////////////// Botões FAKE ////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function updateMap(BB_mask, region3, selection, fromDate, toDate, Month1, Month2) {
  ...
  var s2Clouds = ee.ImageCollection('COPERNICUS/S2_CLOUD_PROBABILITY').filterBounds(region3).filterDate('2023-10-01', eeNow);
  var s2_orig = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED').filterBounds(region3).filterDate('2023-10-01', eeNow);
  ...
  function updateMap1(BB_mask, region1, selection1, fromDate1Year, fromDate1Month, fromDate1Day, fcGeom, GRAYMAP) {
      ...
      var now = Date.now();       //getting time at UTC
      var eeNow = ee.Date(now)
      var s2Clouds = ee.ImageCollection('COPERNICUS/S2_CLOUD_PROBABILITY').filterBounds(region1).filterDate('2023-10-01', eeNow);
      var s2_orig = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED').filterBounds(region1).filterDate('2023-10-01', eeNow);
      var MAX_CLOUD_PROBABILITY = 10;
      function maskClouds(img) {
        var clouds = ee.Image(img.get('cloud_mask')).select('probability');
        var isNotCloud = clouds.lt(MAX_CLOUD_PROBABILITY);
