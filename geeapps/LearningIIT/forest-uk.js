...
var forest_mask_selection  = ui.Select({
  items: [
    ...
    {label: 'Global Forest Cover (2000 - 2012)', value: 'Hansen_map'}],
  value: 'Hansen_map',
  onChange: function(value) {
...
var label_forest_mask_year_selection = ui.Label('Year of forest map (1990 - 2020):');
...
var runFCDM = function(){
    ...
    // *********************************************************************************************************************************************************
var table = States.filter(ee.Filter.eq('stname','UTTARAKHAND'))
    ...
    if (forest_mask_select === 'No_forest_map'){
      var Hansen_map = ee.Image("UMD/hansen/global_forest_change_2020_v1_8").clip(studyarea);
    ...
    if (forest_mask_select === 'Hansen_map'){
      var Hansen_map = ee.Image("UMD/hansen/global_forest_change_2020_v1_8").unmask(0); // Hansen map 2020
    ...
    if (Sensor === 'L'){
      ...
      if (Index === 'Change'){
        ...
        Map.addLayer (NBR_difference_capped.select('NBR'),{min:[0],max:[0.3],palette:'D3D3D3,Ce0f0f'},'Delta-rNBR Landsat '+Start_second+' - '+End_second, false);
      }
      if (Index === 'NBR'){
        // Combining Landsat family  *******************************************************************************************************************************
