var CLC2002 = ui.import && ui.import("CLC2002", "table", {
    ...
    puntos = ui.import && ui.import("puntos", "table", {
      "id": "users/murillop/CH2/GED_AATB2_DANECode_to_GEE"
    }) || ee.FeatureCollection("users/murillop/CH2/GED_AATB2_DANECode_to_GEE");
// var Aubergine= 
// [
//   {
...
//   }
// ]
var dark = require('users/gena/packages:style')
dark.SetMapStyleGrey()
...
'</RasterSymbolizer>';
//Departments
var dep1 = ee.FeatureCollection("users/murillop/COL_cartography/departamentos").filter(ee.Filter.inList('DEPARTAMEN', ['CAQUETÁ', 'META', 'GUAVIARE', 'PUTUMAYO']));
// Create an empty image into which to paint the features, cast to byte.
var empty = ee.Image().byte()
// Paint the edges with different colors, display.
var dep = empty.paint({
  featureCollection: dep1, //PAs,
  color: 'blue',
  width: 2
});
...
var palette =["8a2c83","f6efc1","30791c","f59053","84b920","236493"];
 // name of the legend
var names = ['Urban (rocks, sand)', 'Ag (pasture/coca/small ag)', 'Forest', 'Grasslands(natural)', 'Secondary forest', 'Water'];
...
//Map.SetMapStyleDark().set('cursor', 'crosshair')
/////////////////////////////////////////////////////////////////
...
var thumbLabel = ui.Label('Small Video Full Study Area. Series: 1988-2019.', {whiteSpace:'pre'});
var gistLabel = ui.Label({
    value: 'Click to see TIMEMACHINE website',
  });
gistLabel.setUrl('http://emapr.ceoas.oregonstate.edu/pages/andes_amazon/view.html');
//Logos
// var ce = ui.Label({
...
var y1988 = ee.Image(lista.get(0)); 
var y2012 = ee.Image(lista.get(24));
var y2016 = ee.Image(lista.get(28));
var y2019 = ee.Image(lista.get(31));
Map.addLayer(y1988,vis1_6, '1988', true)
Map.addLayer(y2012,vis1_6, '2012-Negotiation', false)
Map.addLayer(y2016,vis1_6, '2016-Accord', false)
Map.addLayer(y2019,vis1_6, '2019', false)
...
Map.addLayer(dep, {palette:'black'}, 'Departments');
var puntos1 = puntos.filter(ee.Filter.inList('where_prec', [1]));
var buffer = function(b){
  var b = b.buffer(1000)
  return (b)
}
puntos1 = puntos1.map(buffer)
// Create an empty image into which to paint the features, cast to byte.
var empty = ee.Image().byte()
// Paint the edges with different colors, display.
puntos1 = empty.paint({
  featureCollection: puntos1,
  color: 'blue',
  width: 2
});
Map.addLayer (puntos1,{palette:'blue'},'Conflict events 1km', true)