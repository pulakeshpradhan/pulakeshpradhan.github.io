var sen = {"opacity":1,"bands":["nir","swir1","red"],"min":218.54000000000002,"max":3844.46,"gamma":1},
    ...
    depa = ee.FeatureCollection("users/murillop/COL_cartography/departamentos"),
    image3 = ee.Image("users/murillop/CH3/Colombia_1998_2008"),
    image4 = ee.Image("users/murillop/CH3/Colombia_2005_2018"),
    countries = ee.FeatureCollection("USDOS/LSIB_SIMPLE/2017"),
    st = ee.FeatureCollection("users/murillop/Amazon-Andes/study_Area_final"),
    ...
    ucdp = ee.FeatureCollection("users/murillop/CH2/UCDP"),
    control = ee.FeatureCollection("users/murillop/Amazon-Andes/FARC2013_WGS84"),
    ZRC = ee.FeatureCollection("users/murillop/CH2/ZRC_constituidas"),
    ...
    ZRC2 = ee.FeatureCollection("users/murillop/CH2/ZRC_2");
//Map.addLayer(st)
...
var z = ee.FeatureCollection(Zona1).merge(Zona2).merge(Zona3).merge(Zona4).merge(Zona8)
...
Map.addLayer(zones, {palette:'green'}, 'Zones confirmed');
...
Map.addLayer(control_FARC, {}, 'FARC_control2013')
...
//////////////////////////////////////////////////////////////////////////////////////////////
var lossByYear_Zona2= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona2,
...
//print(lossByYear);
var statsFormatted_Zona2 = ee.List(lossByYear_Zona2.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona2 = ee.Dictionary(statsFormatted_Zona2.flatten());
//////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
var lossByYear_Zona8= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona8,
...
var statsFormatted_Zona8 = ee.List(lossByYear_Zona8.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona8 = ee.Dictionary(statsFormatted_Zona8.flatten());
//Zonas para municipio
var lossByYear_Zona9= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona9,
...
var statsFormatted_Zona9 = ee.List(lossByYear_Zona9.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona9 = ee.Dictionary(statsFormatted_Zona9.flatten());
// PARA PNN
var lossByYear_Zona10= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona10,
...
var statsFormatted_Zona10 = ee.List(lossByYear_Zona10.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona10 = ee.Dictionary(statsFormatted_Zona10.flatten());
//Todo el area
...
//ZRC La perla.
var lossByYear_Zona12= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona12,
...
var statsFormatted_Zona12 = ee.List(lossByYear_Zona12.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona12= ee.Dictionary(statsFormatted_Zona12.flatten());
//Guaviare departamento
var lossByYear_Zona13= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona13,
...
var statsFormatted_Zona13 = ee.List(lossByYear_Zona13.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona13= ee.Dictionary(statsFormatted_Zona13.flatten());
//Pato Balsillas
var lossByYear_Zona14= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona14,
...
var statsFormatted_Zona14 = ee.List(lossByYear_Zona14.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona14= ee.Dictionary(statsFormatted_Zona14.flatten());
//Losada
var lossByYear_Zona15= lossAreaImage.addBands(change_1988_2018).reduceRegion({
  reducer: ee.Reducer.sum().group({
    groupField: 1
    }),
  geometry: Zona15,
...
var statsFormatted_Zona15 = ee.List(lossByYear_Zona15.get('groups'))
  .map(function(el) {
    var d = ee.Dictionary(el);
    return [ee.Number(d.get('group')).format("%01d"), d.get('sum')];
  });
var statsDictionary_Zona15= ee.Dictionary(statsFormatted_Zona15.flatten());
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//print(chart);
var chart_Zona8 = ui.Chart.array.values({
  array: statsDictionary_Zona8.values(),
  axis: 0,
  xLabels: statsDictionary_Zona8.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'La Julia',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(6, chart_Zona8);
var chart_Zona9 = ui.Chart.array.values({
  array: statsDictionary_Zona9.values(),
  axis: 0,
  xLabels: statsDictionary_Zona9.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'Pt. Leguizamo',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(7, chart_Zona9);
var chart_Zona10 = ui.Chart.array.values({
  array: statsDictionary_Zona10.values(),
  axis: 0,
  xLabels: statsDictionary_Zona10.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'La Paya',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(8, chart_Zona10);
//Toda el area  
...
//ZRC La perla
var chart_Zona12 = ui.Chart.array.values({
  array: statsDictionary_Zona12.values(),
  axis: 0,
  xLabels: statsDictionary_Zona12.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'La Perla',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(9, chart_Zona12);  
//Guaviare departamento
var chart_Zona13 = ui.Chart.array.values({
  array: statsDictionary_Zona13.values(),
  axis: 0,
  xLabels: statsDictionary_Zona13.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'Guaviare',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(10, chart_Zona13); 
//Pato Balsillas
var chart_Zona14 = ui.Chart.array.values({
  array: statsDictionary_Zona14.values(),
  axis: 0,
  xLabels: statsDictionary_Zona14.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'Pato Balsillas',
    hAxis: {title: 'Año', format: '####'},
    vAxis: {title: 'Area (Hectareas)'},
    legend: { position: "none" },
    lineWidth: 1,
  ...
  panel.widgets().set(11, chart_Zona14); 
//Losada
var chart_Zona15 = ui.Chart.array.values({
  array: statsDictionary_Zona15.values(),
  axis: 0,
  xLabels: statsDictionary_Zona15.keys()
}).setChartType('ColumnChart')
  .setOptions({
    title: 'Losada',
    hAxis: {title: 'Año', format: '####'},
  ...
  panel.widgets().set(12, chart_Zona15);
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Make legends
