var area = ui.import && ui.import("area", "geometry", {
    ...
    ee.Geometry.Polygon(
        [[[0.8308542673880703, 42.0341270121366],
          [0.8308542673880703, 41.884009207711884],
          [1.2977732127005703, 41.884009207711884],
          [1.2977732127005703, 42.0341270121366]]], null, false);
...
var dataset_post = ee.ImageCollection('COPERNICUS/S2_SR')
                  .filterDate('2022-06-25', '2022-07-30')
                  //.filterBounds(area)
                  // Pre-filter to get less cloudy granules.
                  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',20))
                  //.sort('CLOUDY_PIXEL_PERCENTAGE')
                  .filter(ee.Filter.eq('MGRS_TILE',"31TCG"))
                  .map(maskS2clouds);
print(dataset_post)
...
var dataset_pre = ee.ImageCollection('COPERNICUS/S2_SR')
                  .filterDate('2022-06-07', '2022-06-14')
                  //.filterBounds(area)
                  // Pre-filter to get less cloudy granules.
                  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',20))
                  //.sort('CLOUDY_PIXEL_PERCENTAGE')
...
print(dataset_pre)
Map.centerObject(area)
Map.addLayer(dataset_pre.first().clip(area), visualization_RGB, 'RGB_pre 09/06/2022');
Map.addLayer(dataset_post.first().clip(area), visualization_RGB, 'RGB_post 29/06/2022');
Map.addLayer(dataset_pre.first().clip(area), visualization_SNR, 'SNR_pre 09/06/2022');
Map.addLayer(dataset_post.first().clip(area), visualization_SNR, 'SNR_post 29/06/2022');
