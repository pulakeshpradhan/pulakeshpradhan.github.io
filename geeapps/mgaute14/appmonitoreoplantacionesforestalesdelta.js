Map.setCenter (  -58.824046, -34.141547,12)
...
var carto_prov = ee.FeatureCollection( 'users/mgaute/plantaciones_delta_TODO_4326_09_11_2020')
//AGREGADO DE SUP 
...
print(carto_prov.reduceColumns({
  reducer: ee.Reducer.sum(),
  selectors: ['area'],
}));
//var carto_prov = cartografia_plantaciones.filter(ee.Filter.inList('prov', "BUENOS AIRES",  "ENTRE RÍOS"))
...
//var region = ee.FeatureCollection( 'users/mgaute/provincias/provincias_ign_2015')
//var geometry = region.filter(ee.Filter.stringStartsWith('nombre',"ENTRE RÍOS"))
...
//var outline_buffer = empty.paint({
  ...
  //width: 2
//});
var outline_geometry = empty.paint({
...
/*
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////7
//Map.setCenter (  -56.090010, -28.116682,8)
//Mostrar imagen google satelital detras del mapa.
Map.setOptions("HYBRID")
//var geometry = ee.FeatureCollection('users/mgaute/limitenacional');
//var cartografia_plantaciones_corrientes = ee.FeatureCollection('users/mgaute/cartografia_corrientes');
...
//var cartografia_plantaciones = ee.FeatureCollection('users/mgaute/macizos_entrerios_29_07_2020')
var cartografia_plantaciones = ee.FeatureCollection('users/mgaute/macizos_02_07_2020')
var carto_prov = cartografia_plantaciones.filter(ee.Filter.stringStartsWith('prov',"MISIONES")) 
//AGREGADO DE SUP 
var addArea = function(feature) {
  return feature.set({areaHa: feature.geometry().area().divide(100 * 100)});
};
...
var region = ee.FeatureCollection( 'users/mgaute/provincias/provincias_ign_2015')
//var cartografia_plantaciones = ee.FeatureCollection( 'users/mgaute/macizos_mapa_fina')
var geometry = region.filter(ee.Filter.stringStartsWith('nombre',"MISIONES"))
//var geometry = ee.FeatureCollection ('users/mgaute/provincias/area_recorte_sentinel')
//Filtrar por nombres
//var geometry  = region.filter(ee.Filter.inList('nombre', 
//ee.List(['CORRIENTES', 'CHACO', 'ENTRE RÍOS','SANTIAGO DEL ESTERIO', 'SANTA FE', 'CÓRDOBA','BUENOS AIRES'])));
...
var START_actual = ee.Date("2020-11-01");
...
var START_anterior = ee.Date("2019-11-01");
...
//Map.addLayer(reclassifiedImage_actual, ndwiViz, 'Mapa Clases de Estado actual', false);
// estadistica en pie
...
var ndwiViz = {min: 1, max: 3, palette: ['red', 'yellow ', 'green ']};
//Map.addLayer(reclassifiedImage_anterior, ndwiViz, 'Mapa Clases de Estado anterior', false);
/////////////////////////////////////////
...
var ndvi_perdida_anterior =imagen_ndvi_anterior.mask(imagen_ndvi_anterior.lt (0.3))
...
// estadistica perdida actual 
var ndvi_perdida_actual =imagen_ndvi_actual.mask(imagen_ndvi_actual.lt (0.3))
var areaImage_perdida_actual = ndvi_perdida_actual.multiply(ee.Image.pixelArea())
var stats_perdida_actual = areaImage_perdida_actual.reduceRegion({
  reducer: ee.Reducer.count(),
  geometry: geometry,
...
var ndvi_incipiente_anterior =imagen_ndvi_anterior.mask (imagen_ndvi_anterior.gte (0.3).and (imagen_ndvi_anterior.lte(0.5) ))
...
var ndvi_incipiente_actual =imagen_ndvi_actual.mask(imagen_ndvi_actual.gte (0.3).and (imagen_ndvi_actual.lte(0.5) ))
...
//var diferencia_cuadrada = diferencia.pow(2);
Map.addLayer(ndvi_en_pie_anterior ,NDVIVisParam_anterior, " Plantación en Pie anterior ", false );
Map.addLayer(ndvi_en_pie_actual ,NDVIVisParam_actual, "  Plantaciónen en Pie actual  ", false );
