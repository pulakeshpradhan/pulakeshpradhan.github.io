...
leftMap.selectYear.slider.setValue(2022);
...
rightMap.selectYear.slider.setValue(2022);
...
var selectLocation = ui.Select({
  items: ee.List(aoi.sort("ID", true).distinct("Name").aggregate_array("Name")).getInfo(),
  placeholder: "Select a country / region",
  onChange: function(name) {
    var roi2 = "";
    roi2 = name;
    var roi_ = aoi.filterMetadata("Name","equals",roi2);
    if (roi2 == 'Belize') {
      leftMap.map.setCenter(-88.35, 17.256, 9)
      x.selectPercent.slider.setValue(0.70)
    }
    else if (roi2 == 'Guatemala') {
      leftMap.map.setCenter(-89.6, 16.00, 8)
      x.selectPercent.slider.setValue(0.50)
    } // -90.0987, 16.2524
    else if (roi2 == 'El Salvador') {
      leftMap.map.setCenter(-88.9575, 13.7815, 9)
      x.selectPercent.slider.setValue(0.50)
    }
    else if (roi2 == 'Honduras') {
      leftMap.map.setCenter(-86.1493, 14.8031, 8)
      x.selectPercent.slider.setValue(0.50)
    }
    else if (roi2 == 'Nicaragua') {
      leftMap.map.setCenter(-85.0666, 12.8502, 8)
      x.selectPercent.slider.setValue(0.50)
    }
    else if (roi2 == 'Costa Rica') {
      leftMap.map.setCenter(-84.1204, 10.042, 8)
      x.selectPercent.slider.setValue(0.50)
    }
    else if (roi2 == 'Panama') {
      leftMap.map.setCenter(-80.0000, 8.2, 8)
      x.selectPercent.slider.setValue(0.50)
    }
    else if (roi2 == 'Mesoamerica') {
      leftMap.map.setCenter(-84.386, 13.5, 6)
      x.selectPercent.slider.setValue(0.70)
    }
    updateMap1();
    updateMap2();
},});
selectLocation.setValue('Mesoamerica');
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function fcover(img) {
  ...
  .set('system:time_start',img.get('system:time_start')).select(['constant'],['canopy_cover'])
...
function updateMap1() {
  ...
  var roi = aoi.filterMetadata("Name","equals",selectLocation.getValue());
  var image;
  if (band == 'MODIS_imagery') {
    image = b.mcd43a4_sma_.filterDate(ee.DateRange(ee.Date.fromYMD(year, 1, 1), ee.Date.fromYMD(year, 1, 1).advance(1, 'year'))).first().updateMask(a.msk).clip(roi);
    ...
    leftMap.map.addLayer(a.pa_mes.clip(roi),{palette: "yellow"},"Protected areas", true);}
  else if (band == 'Forest_cover') {
    image = b.mcd43a4_sma_.map(fcover).filterDate(ee.DateRange(ee.Date.fromYMD(year, 1, 1), ee.Date.fromYMD(year, 1, 1).advance(1, 'year'))).first().updateMask(a.msk).clip(roi).visualize({min:0,max:1,palette:['salmon','green']});
    leftMap.map.addLayer(image, {}, 'Forest_cover', true);
    leftMap.map.addLayer(a.hill_mes.clip(roi),{},"Hillshade", true);
    leftMap.map.addLayer(a.bnds_w2,{palette: "white"},"Int'l boundaries", true);
    //leftMap.map.addLayer(a.bz_dist2,{palette: "black"},"District boundaries", true);
    leftMap.map.addLayer(a.pa_mes.clip(roi),{palette: "yellow"},"Protected areas", true);}
}
function updateMap2() {
  ...
  if (band == 'MODIS_imagery') {
    ...
    rightMap.map.addLayer(a.pa_mes.clip(roi),{palette: "yellow"},"Protected areas", true);}
  else if (band == 'Forest_cover') {
    image = b.mcd43a4_sma_.map(fcover).filterDate(ee.DateRange(ee.Date.fromYMD(year, 1, 1), ee.Date.fromYMD(year, 1, 1).advance(1, 'year'))).first().updateMask(a.msk).clip(roi).visualize({min:0,max:1,palette:['salmon','green']});
    rightMap.map.addLayer(image, {}, 'Forest_cover');
    rightMap.map.addLayer(a.hill_mes.clip(roi),{},"Hillshade");
    rightMap.map.addLayer(a.bnds_w2,{palette: "white"},"Int'l boundaries", true);
    //rightMap.map.addLayer(a.bz_dist2,{palette: "black"},"District boundaries", true);
    rightMap.map.addLayer(a.pa_mes.clip(roi),{palette: "yellow"},"Protected areas", true);}
}
//
//updateMap1(roi); // Renders the left map
//updateMap2(roi); // Renders the right map
/*
...
var chart2 = ui.Chart.image.series(b.mcd43a4_sma_.filterDate("2001-01-01","2023-01-01").map(fcover).map(b.x_pct), roi, ee.Reducer.mean(),10000)
.setOptions({title: "Mean MODIS-derived canopy cover, 2001-2022",vAxis:{title:'% canopy cover'},
...
var title = ui.Label('Forest cover change, 2001-2022:' + " " + selectLocation.getValue(), {fontWeight: 'bold', fontSize: '22px', color: 'darkgreen'});
///
...
/*
// Add chart
var chart1 = ui.Chart.image.series(b.mcd43a4_sma_.filterDate("2001-01-01","2022-01-01").map(fcover).map(b.x_pct), roi, ee.Reducer.mean(),10000)
.setOptions({title: "Mean MODIS-derived canopy cover, 2001-2022: Belize",vAxis:{title:'% canopy cover'},
pointSize:5,series:{0:{color:'green'}}});
