...
var s1_filt = ee.ImageCollection("COPERNICUS/S1_GRD").filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VV'))
                .filter(ee.Filter.eq('orbitProperties_pass', 'DESCENDING')).select('VV').filterBounds(c.gt_peten_pt);
...
Object.keys(MAP_PARAMS).forEach(function(name) {
  ...
  map.addLayer(c.pa_cam.clip(c.gt_peten_ln), {palette: "teal"},"Protected areas");
  map.addLayer(c.bnds.clip(c.gt_box),{palette: "red"},"Int'l Borders");
...
maps[0].setCenter(-90.324, 17.0287, 9);
var mapGrid = ui.Panel(
    [ ui.Panel([maps[0]], null, {stretch: 'both'}),
      ui.Panel([maps[1]], null, {stretch: 'both'})],
    ui.Panel.Layout.Flow('horizontal'), {stretch: 'both'});
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var title = ui.Label('Sentinel-1 multi-temporal SAR composites: northern Guatemala', {stretch:'horizontal',textAlign:'center',fontWeight:'bold',fontSize:'24px'});
