 ...
 * @desc [description]
 * ref: https://github.com/NASA-DEVELOP/RAiN
*/
// Centers map on ThreeGorges
Map.setCenter(109.375, 30.975, 6);
...
var LSM_NASA = ee.Image("users/wufvckshuo/Three_Gorges/ThreeGorges_LSM"), //三峡库区易发性图层
    // 三峡库区行政区划
    tabThreeGorges = ee.FeatureCollection("users/wufvckshuo/Three_Gorges/ThreeGorges_tab"),
    // 三峡库区历史滑坡分布
    ...
    // GPM降水数据
    GPM = ee.ImageCollection('NASA/GPM_L3/IMERG_V05'), //Globap Recipitation Measurement Collection
    // 坡度数据
...
var chirps_data = ui.Panel([ui.Label({value: '- 降雨, Climate Hazards Group CHIRPS, 5 km, 1981 - 现在, 动态',
    style: {color: 'black', fontWeight: 'normal'}})]);
...
var end = ui.Panel([ui.Label({value: '按“关闭”按钮开始浏览该工具。请尽情享用！',
    style: {color: 'black', fontWeight: 'bold'}})]);
panel_intro.add(end);
...
// Export.image(GPMrangeMax, 'GPMrangeMax', {});
// 导出数据，酌情使用
...
// Add a button to hide the Panel.
var CloseLayersbut = ui.Button({label: '关闭', style: {color: 'black'},
onClick: function() {
...
var intro = ui.Panel([ui.Label({value: '业务图层',style: {fontSize: '25px', fontWeight: 'bold'}}),]);
...
var toplayerbuts = ui.Panel({layout: ui.Panel.Layout.flow('horizontal'), widgets:[intro, CloseLayersbut], 
style: {width: '500px', height: '60px', position:'top-right'}});
panel_layers.add(toplayerbuts);
// panel_layers.add(CloseLayersbut);
