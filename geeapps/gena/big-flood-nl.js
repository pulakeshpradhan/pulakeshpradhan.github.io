...
App.subscribeEvents = function() {
  Map.onClick(function(pt) {
    App.infoPanel.widgets().reset([ui.Label('Selecting and analyzing area, please wait ...')])
    ...
    var selection = App.floodWaterAreas.filterBounds(ee.FeatureCollection(App.selectionClickedPoints))
    App.selectionLayer.setEeObject(selection.style({ color: 'ffff00', pointSize: 6, fillColor: 'ffff0015' }))
    selection.size().evaluate(function(s) {
      if(!s) {
        print('No area selected')
        return
      }
      App.updateChart1(selection)
    })
  })
}
App.updateChart1 = function(feature) {
  var area = feature.geometry().area(1)
  var areaStr = area.divide(1000*1000).format('%.3f').cat(' km2')
  areaStr.evaluate(function(area) {
    App.infoPanel.widgets().reset([
      ui.Label('Total selected area: ' + area, { fontSize: '11px' })
    ])
  })
  // compute Volume and Area as a function of Water Level
  var geom = feature.geometry()
  var scale = Math.max(Math.min(Map.getScale(), 20), 1)
  // compute AHN statistics within this area
  var stat = App.ahn.reduceRegion(ee.Reducer.percentile([1, 98]), geom, scale)
  var p1 = ee.Number(stat.get('elevation_p1'))
  ...
  // compute (Area, Volume)(WaterLevel) series
  var waterLevels = ee.List.sequence(p1, p1.add(1), 0.02)
  ...
  // convert volume to mm by spreading it over the overall area
  var waterDepth_mm = volume.map(function(v) {
    return ee.Number(v).divide(area).multiply(1000)
  })
  var chart2 = ui.Chart.array.values(waterLevels, 0, volume).setChartType('AreaChart')
  chart2 = chart2.setOptions({ title: 'Volume(water level), m3', height: '150px' })
  var chart2mm = ui.Chart.array.values(waterLevels, 0, waterDepth_mm).setChartType('AreaChart')
  chart2mm = chart2mm.setOptions({ title: 'Rainfall(water level), mm', height: '150px' })
  App.chartPanel2.widgets().reset([])
  App.chartPanel2.widgets().add(chart2)
  App.chartPanel2.widgets().add(chart2mm)
  function updateWaterDepthMap(z, label) {
    var waterLevel = ee.Image.constant(z)
    var floodMask = App.ahn.unmask(p1Image).lte(waterLevel)
    var waterDepth = waterLevel.subtract(App.ahn.unmask(p1Image)).updateMask(floodMask)
    waterDepth = waterDepth.clip(geom)
    label.evaluate(function(label) {
      var labelDownload = ui.Label('Download water depth image (GeoTIF)', { fontSize: '11px' }, waterDepth.getDownloadURL({ region: geom.bounds(1), scale: 5, format: 'GEO_TIFF'}))
      label = ui.Label(label, { fontSize: '11px' })
      App.infoPanel2.widgets().reset([label, labelDownload])
    })
    var zMin = waterLevels.get(0)
    var zMax = waterLevels.get(-1)
    ee.List([zMin, zMax]).evaluate(function(minMax) {
      waterDepth = waterDepth.visualize({ min: 0, max: 1, palette: palettes.cb.Blues[9] })
      App.waterLevelImageLayer.setEeObject(waterDepth)
    })
  }
  function getLabelSelectedParameters(z, z_mm, v) {
    z = ee.Number(z).format('%.2f').cat(' m')
    z_mm = ee.Number(z_mm).format('%.2f').cat(' mm')
    v = ee.Number(v).format('%.0f').cat(' m2')
    return ee.String('Level: ').cat(z).cat(', Volume: ').cat(v).cat(', Rainfall: ').cat(z_mm)
  }
  chart2.onClick(function(v) {
    var index = volume.indexOf(v)
    var z = waterLevels.get(index)
    var z_mm = ee.Number(waterDepth_mm.get(index))
    var label = getLabelSelectedParameters(z, z_mm, v)
    updateWaterDepthMap(z, label)
  })
  chart2mm.onClick(function(z_mm) {
    var index = waterDepth_mm.indexOf(z_mm)
    var z = ee.Number(waterLevels.get(index))
    var v = ee.Number(volume.get(index))
    var label = getLabelSelectedParameters(z, z_mm, v)
    updateWaterDepthMap(z, label)
  })
  // var min = stat.get('elevation_p1')
  // var max = ee.Number(stat.get('elevation_p1')).add(1)
  // var steps = 100
  // var cumulative = true
  // var hist = App.ahn.reduceRegion(ee.Reducer.fixedHistogram(min, max, steps, cumulative), geom, scale)
  // print(hist)
  // hist = ee.Array(hist.get('elevation'))
  // var histBucket = hist.slice(1, 1, 2)
  // var histCount = hist.slice(1, 0, 1)
  // var histCount = hist.slice(1, 0, 1)
  // var chart = ui.Chart.array.values(histBucket, 0, histCount)
  // chart = chart
  //   .setChartType('SteppedAreaChart')
  //   .setOptions({ pointSize: 0, lineWidth: 1 })
  // print(chart)
  // var waterLevel = ee.List.sequence(0, 0, step, count)
  // var timeSeries = App.floodWaterAreas.filterBounds(feature.geometry()).sort('system:time_start')
...
App.setupUI = function() {
  ...
  var titleMain = ui.Label({ value: 'Flood water storage capacity in the Netherlands', style: { fontSize: '20px', fontWeight: 'bold', margin: '6px 0px 0px 6px' }})
  ...
  addLayerControls(mapLayerPanelDikesPrimary, false, 0.9, function() { return App.dikesPrimaryLayer })
  var mapLayerPanelDikesRegional = ui.Panel([
    ui.Panel([
      ui.Label('Regional Dikes', { fontWeight: 'bold', position: 'top-left', margin: '0px 0px 0px 12px' }),
      ui.Panel([
        createLegendItem('a1d99b', 'dike line', ''),
      ], null, { position: 'bottom-left', margin: '10px 0px 0px 10px' })
    ], ui.Panel.Layout.flow('vertical'), { margin: '10px 0px 0px 0px' }),
    ui.Label('', { 
      margin: '0px 0px 0px 0px', 
  ...
  addLayerControls(mapLayerPanelDikesRegional, false, 0.9, function() { return App.dikesRegionalLayer })
  // peilvakken
  var mapLayerPanelFloodWaterAreas = ui.Panel([
    ui.Panel([
      ui.Label('Potential Flooding Areas', { fontWeight: 'bold', position: 'top-left', margin: '0px 0px 0px 12px' }),
      ui.Panel([
        createLegendItem('00ffff10', 'flood areas', ''),
      ], null, { position: 'bottom-left', margin: '10px 0px 0px 10px' })
    ], ui.Panel.Layout.flow('vertical'), { margin: '10px 0px 0px 0px' }),
    ui.Label('', { 
      margin: '0px 0px 0px 0px', 
  ...
  */
  ...
  var infoPanel = ui.Panel([ui.Label('Click on map to select area(s) to analyze')])
  ...
  var clearSelectionButton = ui.Button('Clear Selection', function() { 
    ...
    App.infoPanel.widgets().reset([ui.Label('Click on map to select area(s) to analyze')])
    App.infoPanel2.widgets().reset([])
    App.chartPanel1.widgets().reset([])
    App.chartPanel2.widgets().reset([])
  })
...
App.setupUrlHandling = function() {
  function parseUrlParameters() {
    ...
    if(lon !== -999 && lat !== -999) {
      ...
      Map.centerObject(App.ahn.geometry())
    }
  }
  parseUrlParameters()
...
App.addMapLayers = function() {
  ...
  Map.addLayer(sections.style({ color: '00ffff50', width: 1, fillColor: '00ffff10' }), {}, 'sections', false)
  ...
  var floodWaterAreas_Delfland = ee.FeatureCollection('users/kymoslager/Delfland')
  ...
  App.floodWaterAreas = floodWaterAreas_Noordezijlvest.merge(floodWaterAreas_Delfland).merge(floodWaterAreas_Rijnland).merge(floodWaterAreas_Waternet)
  App.floodWaterAreasLayer = ui.Map.Layer(App.floodWaterAreas.style({ color: '00ffffaa', width: 1, fillColor: '00ffff10' }), {}, 'flood areas', true)
  ...
  var dikesPrimaryLayer = ui.Map.Layer(dikesPrimary.style({ color: 'ffffff', width: 1 }), {}, 'dikes (primary)', false, 0.9)
  Map.layers().add(dikesPrimaryLayer)
  var dikesRegionalLayer = ui.Map.Layer(dikesRegional.style({ color: 'a1d99b', width: 1 }), {}, 'dikes (regional)', false, 0.9)
  Map.layers().add(dikesRegionalLayer)
  // change dike width dynamically
  Map.onChangeZoom(function(zoom) {
    if(zoom > 9) {
      dikesPrimaryLayer.setEeObject(dikesPrimary.style({ color: 'ffffff88', width: 2}))
      dikesRegionalLayer.setEeObject(dikesRegional.style({ color: 'a1d99b88', width: 2}))
    } else {
      dikesPrimaryLayer.setEeObject(dikesPrimary.style({ color: 'ffffff', width: 1 }))
      dikesRegionalLayer.setEeObject(dikesRegional.style({ color: 'a1d99b', width: 1 }))
    }
  })
  App.selectionLayer = ui.Map.Layer(ee.Image(), {}, 'selection')
  Map.layers().add(App.selectionLayer)
  App.waterLevelImageLayer = ui.Map.Layer(ee.Image(), {}, 'water depth', true)
