var jrc300m = ui.import && ui.import("jrc300m", "image", {
      "id": "users/gena/JRC_WATER_OCCURRENCE_300m_max_mean"
    }) || ee.Image("users/gena/JRC_WATER_OCCURRENCE_300m_max_mean"),
    basins = ui.import && ui.import("basins", "image", {
      "id": "users/gena/HydroBASINS"
    }) || ee.Image("users/gena/HydroBASINS"),
    reservoirsAll = ui.import && ui.import("reservoirsAll", "table", {
      "id": "projects/global-water-watch/assets/reservoirs-all-v1-0"
    }) || ee.FeatureCollection("projects/global-water-watch/assets/reservoirs-all-v1-0");
...
var waterbodies = ee.FeatureCollection('projects/global-water-watch/assets/reservoirs-v1-0')
...
var waterbodiesPoints = ee.FeatureCollection("projects/global-water-watch/assets/reservoirs-locations-v1-0")
// print('waterbodies points size', waterbodiesPoints.size())
// addLayer(waterbodiesPoints, {}, 'points', false)
var panel = ui.Panel([
  ui.Label('Validation dataset', { fontSize: '14px', 'background-color': 'ffffffcc' }, 'https://gena.users.earthengine.app/view/water-watch-validation'),
...
Map.add(panel)
var panel = ui.Panel([
  ui.Label('About Global Water Watch', { fontSize: '14px', 'background-color': 'ffffffcc' }, 'https://globalwater.watch')
], ui.Panel.Layout.flow('vertical'), { 
...
var paperLinkBox = ui.Panel({
  widgets: [
      ui.Label({ 
        value: 'https://www.nature.com/articles/s41598-022-17074-6', 
        targetUrl: 'https://www.nature.com/articles/s41598-022-17074-6',
        imageUrl: 'https://media.springernature.com/full/springer-cms/rest/v1/content/17164680/data/v2'
      })
  ],
  style: { backgroundColor: '#00000066', fontSize: '14px', position: 'bottom-center', margin: '10px', padding: '5px' }
})
Map.widgets().add(paperLinkBox)
var realTimeCheckbox = ui.Checkbox({
  label: "Real time", 
  value: false
})
var controlsBottomLeft = ui.Panel({
  widgets: [
    realTimeCheckbox,
    ui.Textbox({ 
...
Map.widgets().add(controlsBottomLeft)
...
//   'font-size': '75%'
// })
function fetchRealtime(fid) {
  // fid is the reservoir id
  ...
  var geometry = reservoirs.filter(ee.Filter.eq('fid', fid)).geometry(10)
  // water area using raw images 
  var stop = ee.Date(Date.now())
  // TODO: replace by date from last image
  var start = stop.advance(-1, 'year')
  // compute the scale that we want to compute on
  // for example: 10x10km 
  // 100000000 m2 -> 10000m -> / 200 -> 50m 
  var scale = geometry.area(10).sqrt().divide(200).max(10).getInfo()
  // Don't bother with older images used.
  // Use S2 for faster debugging
  // var missions = ['S2']
  var missions = ['L8', 'L9', 'L7', 'S2']
  // fake bayesian algorithm (this is the semi-informative prior)....
  var waterOccurrence = jrc.select('occurrence').divide(100).unmask(0).resample('bilinear')
      // Cleanup JRC data a bit....
  ...
  // This is the algorithm that determines the water area
  var waterArea = waterAlgorithms.computeSurfaceWaterArea(geometry, start, stop, scale, waterOccurrence, missions, handThreshold)
  // filter out noisy points (these have been set to pseudo-missing value 101)
  waterArea = ee.FeatureCollection(waterArea)
    .filter(ee.Filter.and(
          ee.Filter.neq('p', 101),
          // ee.Filter.gt('ndwi_threshold', -0.15),
          // ee.Filter.lt('ndwi_threshold', 0.5),
          ee.Filter.lt('filled_fraction', 0.5)
    ))
  // only store relevant properties + rename, drop geometries
  waterArea = ee.FeatureCollection(waterArea).select(['system:time_start', 'area_filled'], ['time', 'area'], false)
  return waterArea
}
// var buttonFetchRealTime = ui.Button('Realtime', onRealtimeClick, false, {
...
function show(waterbodyPoint, skipCenterWaterbody) {
  ...
  fid.evaluate(function(fid) {
    var realtimeFeatures = ee.FeatureCollection([])
    if (realTimeCheckbox.getValue()) {
      realtimeFeatures = fetchRealtime(fid) 
    }
    ui.url.set('fid', fid)
    ...
    var timeSeries = ee.Blob('gs://global-water-watch/reservoir-time-series-2022-Q3/time_series_area/' + filename)
    var timeSeriesMonthly = ee.Blob('gs://global-water-watch/reservoir-time-series-2022-Q3/time_series_area_monthly/' + filename)
    ...
    timeSeries.string().evaluate(function (sDaily) {
      ...
      timeSeriesMonthly.string().evaluate(function (sMonthly) {
        log.info('Querying realtime info ...')
        realtimeFeatures.evaluate(function(dataRealtime) {
          if(!sMonthly) {
            panelCharts.clear()
            panelCharts.add(ui.Label('No data (yet) ...'))
            showing = false
          }
          var chart1 = createChart(sMonthly, sDaily, dataRealtime)
          panelCharts.clear()
...
function createChart(sMonthly, sDaily, dataRealtime) {
  var table = 
  {
    cols: [
      ...
      {id: 'area_realtime', type: 'number', role: 'data'}
    ],
    rows: []
  }
  ...
  if (dataRealtime) {
    function feature2row (feature) {
      // who invented this format????
      return {c: [
        {v: new Date(feature.properties.time)}, 
        {v: null}, 
        {v: null}, 
        // m2 -> km2
        {v: feature.properties.area / 1000000}
      ]}
    }
    var rowsRealtime = dataRealtime.features.map(feature2row)
    allRows = allRows.concat(rowsRealtime)
  }
  ...
  var chart = ui.Chart(table)
    .setOptions({
      ...
      series: {
        ...
        2: { title: 'realtime', lineWidth: 0, pointSize: 1, color: 'green', dataOpacity: 0.3 }
      }
    })
  chart.setSeriesNames(['fit', 'raw', 'nrt'])
  chart.onClick(function(t) {
    ...
    // mosaic three least brightest images
    var image = assets.addQualityScore(images, selectedWaterbody.geometry(), { scale: Map.getScale() * 5 })
      .sort('quality_score', false).limit(3).mosaic()
