var ahn = ui.import && ui.import("ahn", "image", {
    ...
    depthProxyIntertidal = ui.import && ui.import("depthProxyIntertidal", "imageCollection", {
      "id": "projects/deltares-rws/eo-bathymetry/EMODnet_2022-intertidal"
    }) || ee.ImageCollection("projects/deltares-rws/eo-bathymetry/EMODnet_2022-intertidal"),
    depthProxySubtidal = ui.import && ui.import("depthProxySubtidal", "imageCollection", {
      "id": "projects/deltares-rws/eo-bathymetry/EMODnet_2022-subtidal"
    }) || ee.ImageCollection("projects/deltares-rws/eo-bathymetry/EMODnet_2022-subtidal");
depthProxySubtidal = depthProxySubtidal.select('green')
function App() { 
}
App.run = function() {
  App.initialize()
...
App.initialize = function() {
  ...
  // TODO: obtain times automatically from subtidal ImageCollection and insitu collection
...
App.addLayers = function() {
  ...
  App.layerIntertidal = ui.Map.Layer (intertidal3d, {}, 'SDB(intertidal, static)', false)
  Map.layers().add(App.layerIntertidal)
  // cleaned-up image for intertidal zone
  var ndwiMin = -0.1
  var ndwiMax = 0.14
  var ndwi = ee.ImageCollection('projects/deltares-rws/eo-bathymetry/EMODnet_2022-intertidal').map(function(i) { 
    return i
  ...
  var min = -25, max = 5
  // Load GEBCO
  var gebco = ee.Image("users/gena/GEBCO_2014_2D");
...
App.createUI = function() {
  ...
  App.correlationMapLayerUI = correlationMapLayerUI
...
App.updateCorrelationLayer = function(forceRefresh) {
  ...
  // update correlation map layer label
  App.correlationMapLayerUI.widgets().get(0).setValue(firstLayerName + ' vs ' + secondLayerName)
...
function estimateDepth(images, hillshade) { 
  var i = images.mosaic().setDefaultProjection(images.first().projection())
...
var invDepth = estimateDepth(depthProxySubtidal, false)
var invDepthHill = estimateDepth(depthProxySubtidal, true)
...
var i = intertidalScaled.mosaic().setDefaultProjection(depthProxyIntertidal.first().projection())
var intertidal3d = hillshadeRGB(i.visualize({min:0, max:1, palette: palette.reverse()}), i, 1.5, 1000, azimuth-180, elevation)
// Start the App
App.run()
/***
