var roi = ee.FeatureCollection('users/fqqwxc123/ARSC/xianCounty')
...
app.createPanels = function() {
  ...
  app.intro = {
    panel: ui.Panel([
      ui.Label({
        value: 'XIAN Sentinel-2 SR Explorer',
  ...
  app.filters = {
    mapCenter: ui.Checkbox({label: 'Filter to XIAN', value: true}),
    startDate: ui.Textbox('YYYY-MM-DD', '2022-09-01'),
    endDate: ui.Textbox('YYYY-MM-DD', '2022-09-28'),
    cloudCover:ui.Textbox(null,30),
  ...
  app.filters.panel = ui.Panel({
    widgets: [
      ...
      ui.Label('cloudCover',app.HELPER_TEXT_STYLE),app.filters.cloudCover,
  ...
  app.picker = {
    ...
    centerButton: ui.Button('Center on XIAN', function() {
...
app.createHelpers = function() {
  ...
  app.applyFilters = function() {
    ...
    var cloudCover = app.filters.cloudCover.getValue();
    if (cloudCover) cloudCover = parseInt(cloudCover)
    if(start) filtered = filtered.filterDate(start, end)
                                  .filter(ee.Filter.lt('CLOUD_COVERAGE_ASSESSMENT',cloudCover));
...
app.createConstants = function() {
  ...
  app.VIS_OPTIONS = {
    'False color (B8/B4/B3)': {
      ...
      visParams: {gamma: 1.3, min: 0, max: 3000, bands: ['B8', 'B4', 'B3']}
    },
    ...
    'Atmospheric (B7/B6/B5)': {
      ...
      visParams: {gamma: 1.3, min: 0, max: 3000, bands: ['B7', 'B6', 'B5']}
    }
  };
};
