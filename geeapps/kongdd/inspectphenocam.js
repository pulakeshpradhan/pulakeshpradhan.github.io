var MOD13Q1 = ee.ImageCollection("MODIS/006/MOD13Q1"),
    ...
    region = /* color: #d63000 */ee.Geometry.Polygon(
        [[[88.73931884765625, 26.44998007251706],
          [88.89106750488281, 26.295877391487558],
          [88.93535614013672, 26.332499225449443],
          [88.79219055175781, 26.461967461549143]]]),
    st_cam = ee.FeatureCollection("users/kongdd/phenofit/phenocam_st133"),
    MCD12Q1_006 = ee.ImageCollection("MODIS/006/MCD12Q1"),
    MOD13A1 = ee.ImageCollection("MODIS/006/MOD13A1"),
    ylu_full = ee.Image("projects/pml_evapotranspiration/MODIS/MOD13A1/MOD13A1_ylu");
/**
...
function showmap(){
    ...
    mapNames.forEach(function(name, index) {
        ...
        if (index === 0) {
            ...
            img = MOD13A1.select(['NDVI', 'EVI', 'SummaryQA']).filter(filterDate);
            map.addLayer(MCD12Q1_006, {min: 0, max: 17, palette:lc_colors_006}, 'MCD12Q1_006');
            map.addLayer(ylu_full, {}, 'ylu_full');
            // map.addLayer(imgcol_pmlv2, {}, 'PML_v2');
...
var maps = showmap();
// zoomToPoint(points, 'site', 14, true);
maps[0].add(zoomToPoint(points, 'site', 14, false));
// maps[0].setCenter(88.797, 26.4214, 5);
// maps[0].centerObject(region, 8);
/** max value */
...
function zoomToPoint(FeaCol, name, zoomlevel, IsPrint) {
    ...
    tool.setValue("pointreyes"); //names[0]
    if (IsPrint) {
        print(tool);
    } else {
        return tool;
