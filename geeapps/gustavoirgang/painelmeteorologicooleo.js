var aoi = ui.import && ui.import("aoi", "geometry", {
  ...
  var oleo = ee.FeatureCollection("users/gustavoirgang/oleo_Ibama_2020");
var isl = ee.FeatureCollection("users/gustavoirgang/ISL_BR");
...
var ISLvis = {
  ...
  max: 10,
   opacity: 1,
  palette: ['772673', 'AE99BF', '0097D4', '92D1F1', '98CEC9', '0095FF', 'D6BA00', 'E1E800', 'F88800', 'D60018']
};
...
var fuels = ['ISL 1', 'ISL 2', 'ISL 3', 'ISL 4', 'ISL 5', 'ISL 6', 'ISL 7', 'ISL 8', 'ISL 9', 'ISL 10'];
function addStyle(pt) {
  var size = ee.Number(1);//pt.get('variedades'));//.sqrt().divide(10).add(2);
  var color = fuelColor.get(pt.get('Name'));
  return pt.set('styleProperty',  ee.Dictionary({ 'pointSize': size, 'color': color}))
}
// Make a FeatureCollection out of the power plant data table
var pp = ee.FeatureCollection(isl).map(addStyle);
print(pp.first())
// Add power plants of a certain fuel to the map 
function addLayer(fuel) {
  print(fuel)
  Map.addLayer(pp.filter(ee.Filter.eq('Name', fuel)).style({styleProperty: 'styleProperty', neighborhood: 1}), {}, fuel, true, 0.85);
}
...
Map.addLayer(oleo.draw('000000',2), {"opacity":1}, 'Óleo')
Map.addLayer(isl.draw('772673',2), {"opacity":1}, 'ISL',false)
