...
var quality = function(image){ 
  var mask1 = image.select("relative_humidity_2m_above_ground").gt(95);
...
Map.addLayer( Hfrios.sum(), precipvis, 'soma horas 16 dias')
// Create an empty panel in which to arrange widgets.
// The layout is vertical flow by default.
var panel = ui.Panel({layout: ui.Panel.Layout.flow('vertical'),style: {width: '65%', height: '100%'}})
    .add(ui.Label('Painel Meteorológico ')).add(ui.Label('Clique no mapa para consultar os períodos com potencial de doenças fúngicas previsão 16 dias. Os resutados são apresentados em dois conjuntos temporais, de 0 a 120 horas de hora em hora e de 120 a 384 horas de previsão de 3 por 3 horas conforme padrão original do modelo. É considerado período com potencial de desenvolvimento de doença fúngica quando a umidade relativa está acima de 95%. OBS.:Caso não hajam ocorrências no ponto clicado os gráficos não aparecem'));
...
inspector00.widgets().set(1, ui.Label('Painel Meteorológico, consulta realizada em: '+ date100 ));
  ...
  var location = 'Consulta ao potencial de ocorrencência de doença fúngica por hora na coordenada lon: ' + coords.lon.toFixed(2) + ' ' +
...
Map.addLayer( dias.sum(), precipvis2, 'soma horas 5 dias')
Map.addLayer(dias1016.sum(), precipvis2, 'soma horas 6º ao 16º dia')
Map.addLayer( Hfrios.sum(), precipvis, 'soma horas 16 dias')
 var sampledPoint = dias.sum().reduceRegion(ee.Reducer.mean(), geometry, 1);
  var computedValue = sampledPoint.get('relative_humidity_2m_above_ground');
 ...
 var sampledPoint0 = dias1016.sum().reduceRegion(ee.Reducer.mean(), geometry, 1);
  var computedValue0 = sampledPoint0.get('relative_humidity_2m_above_ground');
  // Request the value from the server and use the results in a function.
  ...
  var computedValue1 = sampledPoint1.get('relative_humidity_2m_above_ground');
  // Request the value from the server and use the results in a function.
...
var Grafico50 = ui.Chart.image.series({
  ...
  scale: 100,
   xProperty:'system:index'
          ...
          title: 'Contagem de horas por dia com potencial de desenvolvimento de doenças fúngicas',
          //curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red',format: 'short'}
          },
...
var chart0 = ui.Chart.image.series({
  ...
  scale: 100,
   xProperty:'system:index'
          ...
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red'}
          },
          vAxis: {title: 'Horas por dia'},
        legend: 'true',
        trendlines: {
...
var chart = ui.Chart.image.series({
  ...
  xProperty: 'date'
          ...
          title: 'Potencial de desenvolvimento de doenças fúngicas por hora',
          //curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red'}
          },
          vAxis: {title: 'Umidade relativa hora'},
        legend: 'true',
        trendlines: {
      0: {
        type: 'polynomial',
...
var chart1 = ui.Chart.image.series({
  ...
  xProperty: 'date'
          ...
          title: 'Umidade Relattiva por hora',
          //curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red'}
...
var chart100 = ui.Chart.image.series({
  ...
  xProperty: 'date'
          ...
          title: 'Potencial de desenvolvimento de doenças fúngicas a cada 3 horas',
          //curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red'}
...
var chart11 = ui.Chart.image.series({
  ...
  xProperty: 'date'
          ...
          title: 'Umidade Relattiva a cada 3 horas',
          //curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis:  'relative_humidity_2m_above_ground' , setChartType:'ColumnChart', color: 'red'}
...
Map.layers().set(3, ui.Map.Layer(geometryyy, {color: 'FFFF00'},'Fazenda')); 
Map.setCenter(-40.712685, -9.350365, 7);  
