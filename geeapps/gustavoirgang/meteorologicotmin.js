...
var TERRACLIMATE = dataset.select('tmmn');
...
slider.setValue(2007);  
  ...
  //   print (data);
var TERRACLIMATE2 = dataset2.select(['Minimum_temperature_height_above_ground_6_Hour_Interval']);
var months = ee.List.sequence(1, 12);
var years = ee.List.sequence(1988, 2020);
...
//print(byMonthYear2)
var byMonthYear3 = byMonthYear.select(['tmmn']).merge(byMonthYear2.select(['Minimum_temperature_height_above_ground_6_Hour_Interval']));
//print(byMonthYear3)
var check2 = ee.Image(byMonthYear2.mean());
var dif = check2.subtract(check);
Map.addLayer(brasil, precipvis2 , 'Brasil');
Map.addLayer(dif, precipvis2 , 'Diferença Temperatura Minima');
Map.addLayer(check2,precipvis , 'Temperatura Minima ult. ano',false);
Map.addLayer(check, precipvis, 'Temperatura Minima Normal', false);
Map.style().set('cursor', 'crosshair', true);
  ...
  var computedValue = sampledPoint.get('tmmn');
  // Request the value from the server and use the results in a function.
  computedValue.evaluate(function(result) {
    // Add a label with the results from the server.
    inspector.widgets().set(2, ui.Label({
      value: 'Normal Temp. Min.°C: ' + result.toFixed(2),
      style: {stretch: 'horizontal'}
    }));
  });
 var sampledPoint0 = check2.reduceRegion(ee.Reducer.mean(), geometry, 4);
  var computedValue0 = sampledPoint0.get('Minimum_temperature_height_above_ground_6_Hour_Interval');
  // Request the value from the server and use the results in a function.
  computedValue0.evaluate(function(result) {
    // Add a label with the results from the server.
    inspector.widgets().set(3, ui.Label({
      value: 'Média da T. Min °C últ.ano: ' + result.toFixed(2),
      style: {stretch: 'horizontal'}
    }));
  });
  var sampledPoint1 = dif.reduceRegion(ee.Reducer.mean(), geometry, 4);
  var computedValue1 = sampledPoint1.get('Minimum_temperature_height_above_ground_6_Hour_Interval');
  // Request the value from the server and use the results in a function.
  computedValue1.evaluate(function(result) {
    // Add a label with the results from the server.
    inspector.widgets().set(4, ui.Label({
      value: 'Diferença das T. Min. °C: ' + result.toFixed(2),
      style: {stretch: 'horizontal'}
    }));
  });
...
.setOptions({
          title: 'Integração da Normal da Temperatura Minima média com histórico do último ano',
          curveType:'function',
          series: {
          // Gives each series an axis name that matches the Y-axis below.
         0: {axis: 'Minimum_temperature_height_above_ground_6_Hour_Interval',  setChartType: 'ColumnChart', color: 'red'},
          1: {axis:  'tmmn' , setChartType:'LineChart', color: 'blue'}
                  },
          vAxis: {title: 'Temperatura Minima °C mês'},
        legend: 'true',
        trendlines: {
      0: {
        type: 'polynomial',
...
}).setSeriesNames(['Temperatura Minima último ano', 'Temperatura Minima Normal']);
//print(TS5);
var Grafico2 = ui.Chart.image.series(
    TERRACLIMATE, geometry, ee.Reducer.mean(), 10).setOptions({
          title: 'Normal da Temperatura Minima Média °C por dia do ano',
          vAxis: {title: 'Temperatura Minima °C dia',
        ticks: [{v: 0, f: '0'},
                {v: 100, f: '10'},
                {v: 150, f: '15'},
                {v: 200, f: '20'},
...
var TS = ui.Chart.image.doySeries(
    TERRACLIMATE2, geometry, ee.Reducer.mean(), 10).setOptions({
          title: 'Temperatura Minima por dia do último ano',
          vAxis: {title: 'Temperatura Minima dia °C',
        ticks: [{v: 273.15, f: '0'},
                {v: 283.15, f: '10'},
                {v: 288.15, f: '15'},
                {v: 293.15, f: '20'},
          ...
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis: 'Minimum_temperature_height_above_ground_6_Hour_Interval'},
        },
        axes: {
          // Adds labels to each axis; they don't have to match the axis names.
          y: {
            Temps: {label: 'Temperatura Minima °C'},
            Daylight: {label: 'Daylight'}
          }
}
});
var T6 = ui.Chart.image.doySeriesByYear(
    TERRACLIMATE,'tmmn',geometry, ee.Reducer.mean(), 1000).setOptions({
...
.setOptions({
          ...
          series: {
          // Gives each series an axis name that matches the Y-axis below.
          0: {axis: 'Minimum_temperature_height_above_ground_6_Hour_Interval', color: 'grey'},
          1: {axis: 'tmmn', color: 'blue'},
                  },
          hAxis: {title: 'Temperatura Minima °C'},
