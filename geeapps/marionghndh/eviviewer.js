...
url values formating
url#sensor=MODIS;region=999
url#sensor=S2;region=2
*/
//-------------------------------------------------------------------
//FUNCTIONS
//-------------------------------------------------------------------
...
var updateAll = function(){
  panel_n.widgets().get(0).widgets().get(1).setValue(sensor_type);
  if (buffer_size === 0) {
    zoom = zoom_out;
    Map.centerObject(POI, zoom);
  }
  else {
    zoom = zoom_in;
    Map.centerObject(POI, zoom);
...
// ui.url.set({"sensor": "S2", "region": 44});
// ui.url.set({"region": 1});
//-------------------------------------------------------------------
//DEFAULT DATA
//-------------------------------------------------------------------
var region_id = ui.url.get("region");
var sensor_type = ui.url.get("sensor");
// var region_id = 999;
...
if (region_id < 999){
  //geometry
  var POI = ee.Geometry(table.getInfo().features[region_id].geometry);
  var buffer_size = 5000;
  var ROI = POI.buffer(buffer_size);
  zoom = zoom_in;
  //sensor setup
}
//dates
...
panel_r.widgets().get(1).onClick(function(tmp){
  if(readme_on){
    ...
    panel_r.style().set({width: '5%', height: "10%"});
  }
  else{
    print("haha");
