...
var NamKls = ['Mundur','Tetap','Maju'],PalBeda=['ff0000','00ff00','0000ff'];
var S1=[],S2=[],L8=[],EVI_S2 = [],Pd,Thn1,Thn2,Thn3,MOD13; 
//MOD13 = MOD13Q.filterDate('2018-01-01','2020-06-20')
//.select('ÉVI').divide(10000);
...
print('List S2 Apr-Jun 2020',ListImgId(S2_Apr_Jun20));
...
Map.centerObject(LBS_JwBl19,10); 
...
for(i=0; i < JumDat; i++) {
  ...
  Jum1 = Jum2 = ImgRange = xrata = yrata = count = ee.Image(0);
...
ImgRange = ymx.subtract(ymn).rename(NamBnd + '_Range').toFloat(); 
  ...
  .addBands(PosMin).addBands(PosMax).addBands(count);
}
...
else if (PosPan == 'UL') PosPnl = 'upper-left';
else if (PosPan == 'UR') PosPnl = 'upper-right';
...
function ListImgId (ImgCol) { // Id Data
...
function DetAwlTnm(IPMx,BPMx,IVMx,BIVMx,IVMx_Th,Pd,MaxAT,TglAwlDt) {
  // Img Img berisi band Pos_Max, Pd : periode 5,10,15,30
  // EVIMax_Th : Treshold nilai terendah utk EVI max tan padi -> 0.35
 // MaxAT : Max Awal tanam dr data Okt-Mar 6 bulan, klw 15 harian : MaxAt = 2*6 = 12
var UMx = 60; if ((Pd % 8) === 0) UMx = 64;
...
function KonvDat(Img,m,c,TyDt,Linier) { 
 var Hasil ;
 if (Linier == 'y') { if (TyDt == 4) Img.multiply(m).add(c).toFloat();
 else Img.multiply(m).add(c).toByte();
 }
