...
var FeaTxt = require('users/dededirgahayu11/Fungsi:DD_FeaText_ed.js');
var Grid_Indo = FeaTxt.Indo_Tile_5; // Grid 0.5 Deg
...
var Bln_Nam = ['January','February','March','April','May','June','July','August','September','October','November','December'];
...
Thn = ee.Date(Date.now()).get('year').getInfo(); Thn1 = Thn-1; Pd=10;  // Pd bisa diganti : 5,10,15,30 
FaseTampil ='S1' ; 
...
Plu,PluId
;
Pd=10; if (Pd==5 || Pd == 10 || Pd ==15) Pilih = LstPrdNam(Pd); // Pilih = Waktu.PilihPrd(Pd); 
else Pilih = PrdList;
print('Pilih',Pilih); 
...
// Buat UI
var Judul = 'SENTINEL DATA FOR MONITORING GROWTH & PHASE OF PADDY CROPS IN INDONESIA',
Author = 'Inventor: Dr Dede Dirgahayu, M.Si, PRPJ, ORPA-BRIN'
;
var TB_Thn = GUI.TxtBnm(Thn,'Tgl Awal',10,'red',200,55,''),
    ...
    PilPeriod = GUI.Pilih(LstPrdNam(10),'Select Period',13,'0000DD','-27px 0px 0px 115px','bold',120,""),
    PilProv = GUI.Pilih(PilPrv,'Select Province',13,'DD0000','8px 10px 0px 8px','bold',120,""),
    PilPulau = GUI.Pilih(Pulau,'Select Island',13,'00DD00','-28px 10px 0px 130px','bold',120,""),
...
'2px dashed #ff0000'), 
Btn_Reset = GUI.CmdBtn1('ReSet',12,'DD0000','dddddd','10px 10px 0px 8px','bold',110,'center',4,
'2px dashed #ff0000'),
Btn_Save = GUI.CmdBtn1('SAVE RESULT',14,'0000DD','dddddd','-41px 10px 0px 155px','bold',120,'center',4,
'2px dashed #ff0000')
; PilPeriod.items().reset(Pilih);
var SimLay = 'y',ResOut,SimWarna='n';
//Panel.style().set({'width' : '285px','border':'3px solid darkblue'});
var intro = UI_Pnl([Lbl_App,Lbl_Author],'278px','100px','2px solid red');
var Panel_Par = UI_Pnl([TB_Thn,TB_Prd,PilPeriod,TB_Pd,Btn_Reset,PilProv,PilPulau,Btn_Pros,Btn_Save],'278px','170px','2px solid blue');
var Panel= UI_Pnl([intro,Panel_Par],'285px','99%','3px solid darkblue'); 
UI_Margin(Panel_Par,4,0,0,0);
...
Btn_Reset.onClick(function(){
  Thn = TB_Thn.getValue(); Thn1 = Thn-1; Pd = TB_Pd.getValue();
 Prd = Waktu.TglPrd(Thn,Pd,1); PrdList = Waktu.TglPrd(Thn,Pd,2);
  if (Pd == 10 || Pd ==15) Pilih = LstPrdNam(Pd); // Pilih = Waktu.PilihPrd(Pd); 
  else Pilih = PrdList; PilPeriod.items().reset(Pilih);
});
PilPeriod.onChange(
  function (key) { 
...
NamPrvPil = PilProv.getValue(); PrvId = PilPrv[NamPrvPil];
 ...
 ft = Shp_Adm[0].filter(ee.Filter.eq('NAMA_PROV',NamPrv));
...
Prd1 = Waktu.TglPrd(Thn1,Pd,1);PrdList1 = Waktu.TglPrd(Thn1,Pd,2);
//Periode = Pilih[PilPeriod.getValue()]; // IdxPrd = Waktu.GetIdxPrd(Pilih[Periode]);
Periode = Pilih.indexOf(PilPeriod.getValue()) + 1;
IdxPrd = Periode-1;  
...
var FaseS1_LayO = Fase_S1.mask(Mask).addBands(Feat2_Img(Shp_Kab, 8, 1,'')).addBands(Feat2_Img(ft, 9, 2,''))
.addBands(Feat2_Img(Grid_Indo, 10, 1,''));
...
//Map.addLayer(Feat2_Img(Indo_Kc, 8, 1,''), {min:8,max:8,palette : '000000'},'Batas Kecamatan',0);
print('Shp prv',ft);
var AdmImg ;
Map.addLayer(Feat2Img(Shp_Kab, 8, 2), {min:8,max:8,palette : '880088'},'Admin Boundary');
//Map.addLayer(Feat2_Img(ft, 9, 3,''), {min:9,max:9,palette : 'red'},'Batas Provinsi');
//Map.addLayer(Feat2Img(Bts1Prv, 3, 2), {palette : 'blue'},'Batas Area');
Map.addLayer(Feat2Img(Grid_Indo,1,1),{},'Grid Tiles 0.5 Deg');
Map.addLayer(Jln_Indo,{color : 'red'},'Primary Road',0);
//Adm_Img = FTxt.Feat2Imgs([Kec_Shp,Kab_Shp,Prv_Shp],[1,2,3],[1,2,3],'');
...
Btn_Save.onClick(
 ...
 // FaseS2_Prv =  FaseS2_Prv.addBands(Feat2Img(Shp_Kab, 8,2)).addBands(Feat2Img(ft, 9, 3)); 
...
return ee.ImageCollection([Feat2_Img(Shp_Admin[2], Val[0], 1,''),Feat2_Img(Shp_Admin[1], Val[1], 2,''),Feat2Img(Shp_Admin[0], Val[3], 3,'')]).max();
}
...
function LstPrdNam(Pd){
  var i,j,k,JumLst,Lst=[],JumPd,JumPdBl,Nam,NamPd;
  JumPdBl=Math.floor(30/Pd);
  for(i=0; i < 12; i++) {
    for(j=0; j < JumPdBl; j++) {
      k=i*JumPdBl+ j; 
      if(Pd==10)Nam='_d' + (j+1); else Nam='_p' + (j+1);
      NamPd=Bln_Nam[i]  + Nam; Lst[k] = NamPd;  
    }} return Lst;
}
// ************ GUI
//ui.Panel(widgets, layout, style)
function UI_Pnl(Wgts,Lx,Ly,Bord) {//Unit : "%" atw "px"
  var UI = ui.Panel({widgets: Wgts,style:{width: Lx || Lx + "px",
      height:Ly || Ly + "px" ,border : Bord || ""}});
  return UI;
}
//ui.Label(value, style, targetUrl, imageUrl)
function UI_Lbl(Val,Warna,Sz,Pad,Bold) {
var UI = ui.Label({value :Val,style :{color:Warna||'darkblue',fontSize:Sz+'px' || '11px',
  padding:Pad + 'px' || "",fontWeight : Bold || 'normal'}}) ;
return UI;
}
//ui.Textbox(placeholder, value, onChange, disabled, style);
function UI_TB(Val,Warna,Sz,Wd,Unt,PH) { // Unt :'px','pt','%'
...
var UI =  ui.Textbox({placeholder: PH || "",value : Val,style: {color:Warna||'darkblue', 
    fontSize:Sz+Unt || '11px',width:Wd + Unt || '48px'}});
return UI;
}
//ui.Button(label, onClick, disabled, style, imageUrl)
function UI_Btn(Lbl,Warna,Sz,Pad,Bord) {
var UI = ui.Button({label : Lbl,style:{color:Warna||'red',fontSize:Sz+'px',
  padding:Pad + 'px' || "2px",border : Bord || "1px dashed blue"}});
return UI;
}
//ui.Select(items, placeholder, value, onChange, disabled, style)
function UI_Pil(Itm,PH,Warna,Wd,Val) {
var UI =  ui.Select({items: Itm,placeholder: PH || "",value : Val || null,style: {color:Warna||'blue',width:Wd || ""}});
return UI;
}
// Style set
function UI_Sty(UI,Warna,Wb,Siz,Bold,Align,Bord,Wd) {
  return UI.style().set({
  color:Warna || 'darkblue',backgroundColor:Wb || 'white',
