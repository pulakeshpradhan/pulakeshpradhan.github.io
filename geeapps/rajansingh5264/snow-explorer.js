...
MAKE MAIN PANEL
*/
var colors = {'cyan': '#000000', 'transparent': '#11ffee00', 'gray': '#008080'};
var TITLE_STYLE = {
  fontWeight: '100',
  fontSize: '38px',
  padding: '8px',
...
var BORDER_STYLE = '4px solid rgba(97, 97, 97, 0.05)';
  ...
  var titleLabel = ui.Label('Snow Explorer, By: Rajan Singh', TITLE_STYLE);
...
mapPanel.drawingTools().onDraw(function (geometry) {
  ...
  //Setup a function to caclulate the Cloud and Cloud       
...
//Setup a function to caclulate Snow Cover Extent (SCE) mask  
function CalculateSCE (image){
  var SCE = image.expression(
      "((b('CloudMask') == 0 && (b('NDSI') >= 0.3) && (b('B4') >= 1000))) ? 2" +
       ": (b('CloudMask') > 0) ? 1" +
        ": 0").rename('SCE');
    return image.addBands(SCE);
}
//Setup a function to mask the No snow pixels
function MaskSCE (image){
        var SCEband = image.select("SCE");
        var SCE_masked = SCEband.updateMask(SCEband.gt(0));
        return image.addBands(SCE_masked)}
 ...
 // print (nbr_images);
  ...
  //Create a Band Composite image (RGB)
  var BandComposite = mosaic.select('B4', 'B3', 'B2').clip(AOI);
  //Create a SCE image
  var SCE = mosaic.select('SCE_1');
...
var sld_ramp =
  ...
  mapPanel.addLayer(BandComposite.clip(AOI), BandCompViz, 'Sentinel-2 Imagery');
  mapPanel.addLayer(SCE.clip(AOI), SCEkViz, 'Snow Cover Extent');
   /*
ADD DATE PANEL
*/
