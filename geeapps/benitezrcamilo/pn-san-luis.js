var pnsl = ui.import && ui.import("pnsl", "table", {
      "id": "users/benitezrcamilo/pn_sanLuis"
    }) || ee.FeatureCollection("users/benitezrcamilo/pn_sanLuis"),
    ...
    mtpi = ui.import && ui.import("mtpi", "image", {
      "id": "CSP/ERGo/1_0/Global/SRTM_mTPI"
    }) || ee.Image("CSP/ERGo/1_0/Global/SRTM_mTPI"),
    chili = ui.import && ui.import("chili", "image", {
      "id": "CSP/ERGo/1_0/Global/SRTM_CHILI"
    }) || ee.Image("CSP/ERGo/1_0/Global/SRTM_CHILI"),
    ndvi = ui.import && ui.import("ndvi", "imageCollection", {
      "id": "LANDSAT/LC08/C01/T1_ANNUAL_NDVI"
    }) || ee.ImageCollection("LANDSAT/LC08/C01/T1_ANNUAL_NDVI"),
    ...
    ee.Geometry.Polygon(
        [[[-57.5311929778959, -22.514221366736678],
          [-57.5311929778959, -22.748083968581632],
          [-57.31215306090371, -22.748083968581632],
          [-57.31215306090371, -22.514221366736678]]], null, false);
//Map.addLayer(cuencas,{color: '416790'},  "Cuencas del Paraguay", false)
//var manduvira = cuencas.filterMetadata("NOMBRE", "equals", "RIO MANDUVIRA");
//var yhaguy = hidro.filterMetadata ("nombre", "equals", "Rio Yhaguy");
var region = pnsl
Map.addLayer(region, {color: '74b8ff'} , "PN San Luís", true);
...
var cloudMasked = sentinel.filterBounds(region).map(maskS2clouds);
var median = cloudMasked.median();
var median = median.clip(region)
Map.addLayer(median, {bands: ['B11', 'B8A', 'B4'], min :394, max: 4223}, 'S2 median');
//Map.addLayer(yhaguy, {color: '0414d6'} , "Río Yhaguy", false);
//var roi = YhaguyBuffer.geometry().bounds()
//Export.image.toDrive({
//  image:ndviMasked,
//  description: 'ndviMasked',
//  scale: 10,
// region: roi,
//  maxPixels: 1e13
//})
...
var sl = region;
//Map.addLayer(sl, {color: '74b8ff'} , "PN San Luís", true);
var geometry = sl;
//*******************************************************************************************
//                                     SET TIME FRAME
...
var prefire_end = '2019-06-30';
// Now set the same parameters for AFTER the fire.
var postfire_start = '2019-06-30';
var postfire_end = '2019-12-31';
//*******************************************************************************************
...
Map.addLayer(post_cm_mos, vis,'Post-fire True Color Image - Clouds masked',false);
...
Map.addLayer(slope, {min: 0, max: 13, palette:['00ff00','efff00','ff8508', 'ff0000']}, 'slope',false);
///LANDFORMS
var landforms = land.select('constant').clip(region);
var landformsVis = {
  min: 11.0,
  max: 42.0,
  palette: [
...
Map.addLayer(landforms, landformsVis, 'Landforms',false);
//NDVI LANDSAT 8 (ANUAL)
var dataset = ndvi
                  .filterDate('2017-01-01', '2017-12-31');
var colorized = dataset.select('NDVI').max().clip(region);
var colorizedVis = {
  min: 0.1486634761095047,
  max: 0.35640498995780945,
  palette: [
...
Map.addLayer(colorized, colorizedVis, 'NDVI 2017 Composite',false);
//MTPI
var srtmMtpi = mtpi.select('elevation').clip(region);
var srtmMtpiVis = {
  min: -20.0,
  max: 60.0,
  palette: ['0b1eff', '4be450', 'fffca4', 'ffa011', 'ff0000'],
};
Map.addLayer(srtmMtpi, srtmMtpiVis, 'SRTM mTPI',false);
/////CONTOURS
...
Map.addLayer(contourlines.clip(region), {min: 100, max: 350, palette:['00ff00', 'ff0000']}, 'contours')
