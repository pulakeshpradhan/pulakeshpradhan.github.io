...
/*
Como acceder a los datos que el container "data" tiene:
data.label.title
data.panel.main
data.imagery = coleccion anual de CHIRPS
*/
function App(){
  var data = {
    panel : {
      ...
      legendImageSingle: ui.Panel({style:{position: "bottom-right"}}),
      legendImageAggregate: ui.Panel({style: {position: "bottom-right"}})
      },
    label : {
      title : ui.Label({value: 'Rainfall Data Exploration',
    ...
    point: ee.Geometry.Point([2.1578, 41.3818]),
    button: {
    ...
    slider: {
      imageSingle: ui.Slider({min:1981, max:2019, step:1, onChange: AddImageSingle, disabled:true, style:{stretch: "horizontal"}})
    },
    select: {
      aggregate: ui.Select({
    ...
    reducer: { aggregate: ee.Reducer.mean()},
  ...
  function AddImageSingle (thisYear){
    ...
    stats.evaluate(function (clientStats){
      ...
      var thisLegend = CreateLegend( clientStats.rain_min, clientStats.rain_max, thisYear + " Rainfall (mm)")
      data.panel.legendImageSingle.add(thisLegend)
      data.map.add(data.panel.legendImageSingle)
      data.map.addLayer(thisImage, {min: clientStats.rain_min, max: clientStats.rain_max, palette: data.palette.rain}, "Rain (Single)")
    })
  }
  ...
  function AddImageAggregate(){
    RemoveMapLayer("Rain (Aggregate)")
    data.map.remove(data.panel.legendImageAggregate)
    var thisImage = data.imagery.select("rain")
                                .reduce(data.reducer.aggregate)
                                .rename("rain")
    var stats = thisImage.reduceRegion(ee.Reducer.minMax(), data.point.buffer(100000), 5000)
    stats.evaluate(function (clientStats){
      data.panel.legendImageAggregate.clear()
      var thisLegend = CreateLegend(clientStats.rain_min, clientStats.rain_max, "Aggregate Rainfall (mm) "+ data.select.aggregate.getValue())
  ...
  SetUpMouseClick ();
  AddWidgets();
  data.map.centerObject(data.point, 4)
  ui.root.clear();
