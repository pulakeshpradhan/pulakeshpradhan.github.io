...
function App(){
  var data = {
    ...
    point: ee.Geometry.Point([2.1756, 41.389]),
    map : ui.Map()
  };
  ...
  function SetupMouseClick (){
    function Click (coordinates){
      ...
      data.map.addLayer(data.point, {color: "red"}, "Point")
    }
    data.map.onClick(Click)
  }
  ...
  function AddImageSingle (thisYear){
    ...
    stats.evaluate( function (clientSide){
    data.panel.legendImageSingle.clear();
    var thisLegend = CreateLegend(clientSide.rain_min, clientSide.rain_max, "Rainfall (mm)");
    data.panel.legendImageSingle.add(thisLegend)
    data.map.add(data.panel.legendImageSingle)
    data.map.addLayer(thisImage, {min: clientSide.rain_min, max: clientSide.rain_max, palette:data.palette.rain}, "Rain (Single)")
    })
  }
  ...
  function AddImageAggregate(){
    ...
    stats.evaluate( function (ClientSide){
      ...
      var thisLegend = CreateLegend(ClientSide.rain_min, ClientSide.rain_max, "Aggregate Rainfall (mm)")
      data.panel.legendImageAggregate.add(thisLegend)
      data.map.add(data.panel.legendImageAggregate)
      data.map.addLayer(thisImage, {min: ClientSide.rain_min, max: ClientSide.rain_max, palette:data.palette.rain}, "Rain (Aggregate)")
    })
  }
  ...
  data.map.centerObject(data.point, 7)
  AddWidgets();
  SetupMouseClick();
  ui.root.clear();
