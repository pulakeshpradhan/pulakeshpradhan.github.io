var geometry = ui.import && ui.import("geometry", "geometry", {
      ...
      "shown": true,
      "locked": false
    }) || /* color: #23cba7 */ee.Geometry.Point([78.70880990863675, 33.73021880417881]);
...
// panel widgets
// app title
var header = ui.Label('Hydrological Properties Time Series', 
    {fontSize: '25px', fontWeight: 'bold', color: '4A997E'});
// app summary
var text = ui.Label(
  'This toolbox plots hydrological properties of wetlands in India.' +
...
var Temp=ui.Button({label: 'Plot Temperature'});
var plotPanel = ui.Panel({
          widgets: [wextnt,color,Temp],
          layout: ui.Panel.Layout.Flow('horizontal'),
        });
panel.add(plotPanel);
wextnt.onClick(waterextent);
color.onClick(watercolor);
Temp.onClick(waterTemp);
...
var data7=ee.ImageCollection("LANDSAT/LE07/C01/T1_SR");
...
// combine data of landsat 5 7 and 8
var dataset=dataset57.merge(dataset8);
dataset=dataset.map(water);
var Wdataset=ee.ImageCollection(dataset).select('water');
...
/////////////// plot hist ////////////////////
var hist = ui.Chart.image.series(MIC,wetland,ee.Reducer.sum(),30)
.setOptions({title: 'Wetland water extent',
  hAxis: {title: 'Date'},
  vAxis: {title: 'Water Area (ha)'}})
  .setChartType('ColumnChart')
;
chartPanel.widgets().reset([hist]);
// inspector1.widgets().set(0, [ui.Label('Name: '+ WetlandName),
//             ui.Label('Description: '+ WetlandDes)
//             ]);
...
dataset57=dataset57.map(function (image){
  return image.addBands(((image.select('HueC').multiply(-0.5974)).add(604.66)).rename('DomWav'));});
var dataset8=ee.ImageCollection("LANDSAT/LC08/C01/T1_SR")
.filter(ee.Filter.date(startDate,endDate))
.filterBounds(wetland)
...
dataset8=dataset8.map(function (data){  
 return data.addBands(((data.select('HueC').multiply(-0.5974)).add(604.66)).rename('DomWav'));});   
var dataset=dataset57.merge(dataset8);
...
Map.addLayer(outlineW,{palette:'FF0000'},'selected_wetland');
// select data set based on user AOI and dates
var data=ee.ImageCollection("MODIS/006/MOD11A2")
.filter(ee.Filter.date(startDate,endDate))
.filterBounds(wetland)
.select(['LST_Day_1km']); 
var modLSTc = data.map(function(img) {
  return img
    .multiply(0.02)
    .subtract(273.15)
 ...
 /////////////// plot hist ////////////////////
var hist = ui.Chart.image.series(modLSTc,wetland,ee.Reducer.mean(),30)
.setOptions({title: 'Wetland Surface temperature',
  hAxis: {title: 'Date'},
