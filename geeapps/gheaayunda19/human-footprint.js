...
// Membuat Panel
var panel = ui.Panel({style: {width: '210px'}})
...
// panel.add(thumbnail)
// Header
var header1 = ui.Label('BVT Academy', {fontWeight: 'bold', fontSize: '20px', margin: '4px 10px'})
var header2 = ui.Label(' Mapping Our Human Footprint', {fontWeight: 'bold', fontSize: '15px', margin: '2px 10px'})
panel.add(header1).add(header2)
// Description
var description = ui.Label('Google Earth Engine merupakan salah satu platform berbasis cloud untuk analisa data geospatial terutama data dalam bentuk raster. Pada materi ini akan dipaparkan mengenai proses pembuatan GEE App untuk pemetaan human footprint menggunakan data-data citra opensource. Dalam GEE App ini akan ditampilkan layers Built Up Area dan Nighlight Data', {fontSize: '12px', textAlign: 'justify', margin: '2px 10px'})
...
function renderSlider_bua(dates) {
  var slider = ui.DateSlider({
    ...
    style: {height:'120px' ,width:'200px', position:'top-center'},
...
function renderDateRange_bua(dateRange) {
  ...
  var vis_bua = {min: 0, max: 1, palette: ['#ffbb22']}  
...
function renderDateRange_nd(dateRange) {
  var image_nd = nightlight_data
    .filterDate(dateRange.start(), dateRange.end())
    .mean()
  var vis_nd = {min:1,max:10, opacity: 0.7, palette:['#41416b','#845a7e','#de7482','#f9a397','#f5e3b8','ffffff']}  
...
var vis_nd = {min:1,max:10,palette:['#41416b','#845a7e','#de7482','#f9a397','#f5e3b8','ffffff']}
// Thumbnail Legenda
