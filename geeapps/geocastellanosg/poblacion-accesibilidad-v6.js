var censales = ee.FeatureCollection("users/geocastellanosg/censales"),
    Arg = ee.FeatureCollection("users/geocastellanosg/Vectores/Arg_provincial_no_preciso"),
...
var fc_censales = ee.FeatureCollection(censales)
print(fc_censales,'fc_censales')
var buff = fc_censales.map(function (vec) 
{return vec.buffer(3000)})
print(buff,'buff')
print(censales)
//Map.addLayer(censales);
...
var ACCES_STYLE = {
  min: 0,
  max: 0.1,
  palette: ['#FEF9E7','#F7DC6F','#FFC107', '#F4511E', '#D32F2F','#880E4F']
};
...
Map.add(ui.Label('poblacion v6', {fontWeight: 'bold', fontSize: '18px'}));
// A list of points the user has clicked on, as [lon,lat] tuples.
var selectedPoints = [];
// Returns the list of countries the user has selected.
function getSelectedcenso() {
  return buff.filterBounds(ee.Geometry.MultiPoint(selectedPoints));
}
// Makes a bar chart of the given FeatureCollection of provinces by names.
function makeResultsBarChart(buff) {
  var chart = ui.Chart.feature.byFeature(buff, 'localidad');
...
// Makes a table of the given FeatureCollection of censos by name.
function makeResultsTable(buff) {
  var table = ui.Chart.feature.byFeature(buff, 'localidad');
...
function updateOverlay() {
  var overlay = getSelectedcenso().style(HIGHLIGHT_STYLE);
...
function updateChart() {
  ...
  var chart = chartBuilder(getSelectedcenso());
...
function clearResults() {
  ...
  var instructionsLabel = ui.Label('Seleccionar Localidad');
...
var chartTypeToggleButton = ToggleButton(
    [
      {
        label: 'Resultado como Tabla',
        value: makeResultsBarChart,
      },
      {
        label: 'Resultado como Gráfico',
...
Map.addLayer(buff,{}, 'Localidades')
Map.addLayer(censales.style({color: '#1E88E5', width: 1}), {}, 'censales')
