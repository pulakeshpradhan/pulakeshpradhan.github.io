...
Map.addLayer(RF2018, {min: 0, max: 3, palette: palette}, 'Penutupan Lahan Pra 2018');
Map.addLayer(RF2019, {min: 0, max: 3, palette: palette}, 'Penutupan Lahan Pra 2019');
...
print('Hasil Perhitungan Luas Tahun 2020', arealist2018);
...
print(grafhotspot);
var title = ui.Label({
  value: 'Forest Fire Mapping using Normalized Burned Ratio and Cloud Computing to Calculate the Losses Incurred in Mount Lawu, Magetan Regency',
  style: {fontSize: '20px',
          fontWeight: 'bold'
  }
});
...
var dataTable = {
  ...
  rows: [{c: [{v: '2018 Case'}, {v: 284.88}]},
         {c: [{v: '2019 Case'}, {v: 146.03}]}]
};
// Define a dictionary of customization options.
var options = {
  title: 'Area Affected by Fire',
  vAxis: {title: 'Area (ha)'},
...
var b = ui.Label({
  value: '2019 case : Rp 42,732,000.00', 
  style: {
    fontSize: '14px'
  }
});
var c = ui.Label({
  value: '2019 case : Rp  21,094,500.00', 
  style: {
    fontSize: '14px'
  }
});
sidebar.add(chart).add(a).add(b).add(c);
...
Map.onClick(function(coords) {
  ...
  lat.setValue('Latitude: ' + coords.lat);
});
