...
function setThumbs (){
  ...
  yearsList.forEach(function(year){
      ...
      var thumb_geometria = ui.Thumbnail({
        ...
        params:{
          region:options.geometry.bounds().buffer(10000)
        },
        onClick:function(){
          var position = 0
...
function init (){
  ...
  options.geometry = ee.FeatureCollection("WWF/HydroSHEDS/v1/Basins/hybas_6")
    .filter(ee.Filter.eq('HYBAS_ID',6060450950))
    .geometry();
  ...
  options.mapp.addLayer(options.line,{},'limite da subbacia do rio São João da Barra');
...
options.panel.add(
  ...
  value:'clique aqui para acessar o código',
    style:{fontSize:'10px'},
  targetUrl:'https://code.earthengine.google.com/41b78cbca49acc9c2ce58a1cc1ac62cf?noload=1'
})
  ],
