...
// --- initial options
  ...
  fire_1: 'colecao 1', // 'colecao beta', 'colecao 1','colecao 1 noMask' 'MCD64A1', 'FIRMS' or 'FireCCI'.
  fire_2: 'colecao 1 noMask', // 'colecao beta', 'colecao 1 v1', 'colecao 1 v4', 'colecao 1 v5', 'colecao 1 v6', 'MCD64A1', 'FIRMS' or 'FireCCI'.
  fires:[
    'colecao beta',
    'colecao 1',
    'colecao 1 noMask',
...
var dataset = {
  // raster:{
    ...
    mapbiomas:'projects/mapbiomas-workspace/COLECAO6/mapbiomas-collection60-integration-v0-12', // Brasil - 1985 à 2019
    'colecao beta':'projects/mapbiomas-workspace/FOGO1/monthly-coverage-collection-beta',// Brasil - 2000 à 2019
    'colecao 1 noMask':'projects/mapbiomas-workspace/FOGO1/monthly-coverage-collection-1', // 1985 - 2020
    'colecao 1':'projects/mapbiomas-workspace/FOGO1/monthly-coverage-collection-1-v9',// 1985 - 2020
...
function setControl (mapp) {
  ...
  var dataStudio = ui.Label({
    value:'Painel Data Studio com valores de area em km²',
    style:{
      fontSize:'11px',
      backgroundColor:'ffffff00',
      margin:'0px 0px 0px 0px ',
    ...
    targetUrl:'https://datastudio.google.com/s/mpYJLCE2iwo'
  });
  var panel = ui.Panel({
  widgets:[ 
...
function aktualizer (nregion,mapp){
  ...
  plotLayer(obj,position);
  var mapbiomas = ee.ImageCollection(dataset.mapbiomas)
    .select('classification_'+(options.year))
    .mosaic()
    .clip(region);
  var name_mapbiomas = preNames['back'] + ' | ' + (options.year-1) +' || Cobertura Mapbiomas col 6 '+ (options.year)
  ...
  plotLayer(obj,position);
  var estados = ee.FeatureCollection(dataset.estados)
    .filterBounds(region);
  estados = ee.Image(1).paint(estados,'000000',0.5);
  estados = estados.updateMask(estados.eq(0));
  position = position + 1;
  obj = {
      eeObject: estados,
      name: preNames['auxiliar'] + ' | || Estados ',
      visParams: visParams.lines,
    };
  plotLayer(obj,position);
  var areasProtegidas = ee.FeatureCollection(dataset.areasProtegidas)
    .filterBounds(region);
  var areasProtegidas_image = ee.Image(1).paint(areasProtegidas_image,'000000',0.5);
  areasProtegidas_image = areasProtegidas_image.updateMask(areasProtegidas_image.eq(0));
  position = position + 1;
  obj = {
      eeObject: areasProtegidas_image,
      name: preNames['auxiliar'] + ' | || Áreas Protegidas ',
      visParams: visParams.lines,
    };
  plotLayer(obj,position);
  position = position + 1;
  ...
  obj = {
      eeObject: setRegions(),
      name: preNames['auxiliar'] + ' | || Regiões de classifição',
      visParams: visParams.line,
    };
  plotLayer(obj,position);
  ...
  plotLayer(obj,position);  
    ...
    if (options.year >= options.dictYears['focos'].start){
      if (options.year <= options.dictYears['focos'].end ) {
        focosName = preNames['referencia'] + ' | || Focos de Calor';
