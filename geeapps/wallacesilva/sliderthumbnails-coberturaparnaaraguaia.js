var geometry = ui.import && ui.import("geometry", "geometry", {
      ...
      "locked": false
        ...
        [[[-50.632657127580316, -9.761726922989828],
          [-50.632657127580316, -11.28469740751083],
          [-49.731778221330316, -11.28469740751083],
          [-49.731778221330316, -9.761726922989828]]], null, false),
...
function getLandcover(year){
  return ee.Image('projects/mapbiomas-workspace/public/collection5/mapbiomas_collection50_integration_v1')
...
function setThumbs (){
  ...
  var yearsList = ee.List.sequence(1985,2019,1).getInfo();
  yearsList.forEach(function(year){
      ...
      var thumb_geometria = ui.Thumbnail({
        ...
        params:{
          region:options.geometry.centroid().buffer(80000).bounds()
        },
        onClick:function(){
          var position = 0
      ...
      var thumb_geometria_buffer = ui.Thumbnail({
        image:landcover
          ...
          .blend(options.lineBuffer),
        params:{
            region:options.geometry.centroid().buffer(140000).bounds()
        },
        onClick:function(){
          var position = 1
          if ( options.mapp.layers().get(position).getShown() === false){
      ...
      var thumbs = ui.Panel({
        widgets:[
          thumb_geometria,thumb_geometria_buffer,
          ],
...
function init (){
  ...
  options.geometry = ee.FeatureCollection('projects/mapbiomas-workspace/AUXILIAR/areas-protegidas')
    .filter(ee.Filter.eq('NomeUC','PARNA do Araguaia'))
    .geometry();
  ...
  options.mapp.addLayer(options.line,{},'limite do parque');
  options.mapp.addLayer(options.lineBuffer,{},'limite do parque');
...
options.panel.add(
  ...
  [
    ui.Label({
    value:'Toolkit interativos -> clique nos mapas para ligar e desligar as layers || ',
    style:{fontSize:'10px'},
  ...
  value:'clique aqui para acessar o código',
    style:{fontSize:'10px'},
  targetUrl:'https://code.earthengine.google.com/f318d86a437fe4d0850f93d007b74347?noload=1'
})
  ],
