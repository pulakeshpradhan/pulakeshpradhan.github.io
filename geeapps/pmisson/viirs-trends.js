...
alert('This app is only as visualization tool and should not be used for scientific analisys.\n For more details about this app e-mail to the author.\nAlejandro Sánchez de Miguel: alejasan@ucm.es');
...
var mask1=med.gt(5)
var diff=VIIRS201204.select('avg_rad').subtract(med).divide(med).mask(mask1);
var diff2=VIIRS202205.select('avg_rad').subtract(med).divide(med).mask(mask1);
diff.unmask(0)
...
*/
alert('Images:SNPP VIIRS-DNB NOAA-NASA Version:EOG Colorado School of Mines, Interface:Gabriel Marbán Paine, Alberto Jiménez González, A. Sánchez de Miguel School supervisor: Julio Galarón Touriño PI: A.Sánchez de Miguel/Cities at Night')
...
var MODIS = collectionFromConstructor;
//print(MODIS)
...
// Parametrizamos el timelapse con proyección, resolución, AOI, valores de pixel y frames/seg
  ...
  var func = function(image, list){
    var reduce = image.select('avg_rad').reduceRegion({
    ...
    var values = ee.Dictionary(reduce.get('avg_rad')).keys();
  ...
  var columnHeader = ee.List([[
    ...
    {label: 'avg_rad', role: 'data', type: 'number'},
  ]]);
  ...
  dataTableServer.evaluate(function(dataTableClient) {
    ...
    var chart = ui.Chart.image.series({imageCollection:MODIS.select('avg_rad'),region:ZonaAOI,reducer: ee.Reducer.mean(),scale:30, xProperty: 'system:time_start'}).setOptions(options);
...
var controlPanel = ui.Panel({
  widgets: [
    ...
    ui.Label(
        ...
        {whiteSpace: 'pre'}),
    ui.Label(
        'The date indicates the \nbeginning of the period(one month).',
        {whiteSpace: 'pre'})
  ],
