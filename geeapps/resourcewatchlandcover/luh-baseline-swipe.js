...
var select_labels = ['Agriculture','Cropland','Grazing Land',
...
var left_title = ui.Label({
  value: 'Enter year (850 - 2015) and select land use variable.',
  style: {
  fontWeight: 'bold',
  }
});
...
var left_viz = {min:0, max:1, palette:['feff9c','ffdb00']};
// create the legend image
var lon = ee.Image.pixelLonLat().select('longitude');
var left_gradient = lon.multiply((left_viz.max-left_viz.min)/100.0).add(left_viz.min);
var left_legendImage = left_gradient.visualize(left_viz);
// Create legend title
...
var left_thumbnail = ui.Thumbnail({
  image: left_legendImage,
  params: {
    bbox:'0,0,150,10', 
    dimensions:'150x10'
  },
...
// create vizualization parameters
var right_viz = {min:0, max:1, palette:['feff9c','ffdb00']};
// create the legend image
var lon = ee.Image.pixelLonLat().select('longitude');
var right_gradient = lon.multiply((right_viz.max-right_viz.min)/100.0).add(right_viz.min);
var right_legendImage = right_gradient.visualize(right_viz);
// Create legend title
...
// create thumbnail from the image
var right_thumbnail = ui.Thumbnail({
  image: right_legendImage,
  params: {
    bbox:'0,0,150,10', 
    dimensions:'150x10'
  },
...
var leftaddGrazing = function(){
  ...
  left_variable_description.setValue('Grazing Land: fraction of grid cell of grazing land. This layer is the sum of the rangeland and pastureland layers.');
  var label = ee.String(year).cat(' Grazing Land');
  leftmap.clear();
  leftmap.add(left_panel);
  leftmap.add(left_legend_panel);
...
var leftaddPrimary = function(){
  ...
  leftmap.clear();
  left_variable_description.setValue('Primary Vegetation: fraction of grid cell of primary vegetation. This layer is the sum of the primary forest and primary non-forest layers. Primary vegetation is land that is either naturally forested or non-forested and has not been utilized by humans since the beginning of the model year.');
...
var rightaddGrazing = function(){
  ...
  right_variable_description.setValue('Grazing Land: fraction of grid cell of grazing land. This layer is the sum of the rangeland and pastureland layers.');
  var label = ee.String(year).cat(' Grazing Land');
  rightmap.clear();
  rightmap.add(right_panel);
  rightmap.add(right_legend_panel);
...
left_textbox.setValue('850')
left_select.setValue('Primary Vegetation')
right_textbox.setValue('2015')
right_select.setValue('Primary Vegetation')