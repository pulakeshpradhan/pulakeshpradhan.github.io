...
var S2_RGB = ee.ImageCollection('COPERNICUS/S2_SR')
                  ...
                  .mosaic()
                  .clip(NomEdoSeleccionado);
//Colección de imágenes Sentinel-2, SAVI
...
var SAVI = Col_SAVI.expression ('float (((NIR - RED) / (NIR + RED + L))* (1+L))',
    {
      ...
      'NIR': Col_SAVI.select ('B8'),
      'RED': Col_SAVI.select ('B4')
    });
...
var afterVH = collectionVH.filterDate(AfInicio, AfFin)
                  .map(function(image) {
                   // var Fecha = ee.Date(image.get('system:time_start'));
                   //     //Fecha = ee.Number(FechaFin);
                   //     image.set(image.metadata('system:time_start'));
                    return image.addBands(image.metadata('system:time_start'));
                  })
                  .mosaic()
                  .clip(NomEdoSeleccionado);//.mosaic();
...
var SAVI_STYLE = {max: 1, min: 0, // IMPORTANTE variar rangos en funcion del indice
...
//Filtro de predios para cultivo
var Predios_Cultivos = ee.FeatureCollection('users/jsilverio/VGeoOperacionPV_2021_2021');
var Campo_Cultivo = 'NombreCult';
var Texto_Inicio = ' Selecciona Cultivo';
// Function to build states list
var build_select = function(Coleccion_Predios, NombreCampo, TextoInicio) {
  var Lista = ee.Dictionary(Coleccion_Predios.aggregate_histogram(NombreCampo)).keys().insert(0, TextoInicio);
  var ids = ee.List(Lista).distinct();
  var names = ee.List(Lista).distinct();
  // Build dictionary from each image ID and name
  return ee.Dictionary.fromLists(ids,names);   //var out_dict = ee.Dictionary.fromLists(ids,names);
};
panel.CULTIVO = build_select(Predios_Cultivos, Campo_Cultivo, Texto_Inicio);
...
panel.Filtro_Cultivo = {
    ...
    select: ui.Select({
      items: Object.keys(panel.CULTIVO.getInfo()),
    onChange: function() {
         var option = panel.Filtro_Cultivo.select.getValue();
         Cultivo = option;//.Nombre;
      }
    }),
  };
  ...
  var FiltroCultivo = ui.Panel({
    widgets: [
      ui.Panel([
        ...
        panel.Filtro_Cultivo.select,
      ], ui.Panel.Layout.flow('horizontal'))
    ],
    style: panel.SECTION_STYLE_FONDO
  });
  panel.add(FiltroCultivo);
  panel.Filtro_Cultivo.select.setValue(panel.Filtro_Cultivo.select.items().get(0));
//*********Fin Cultivo
//********* Modalidad
...
panel.filters = {
    ...
    endDate: ui.Textbox('YYYY-MM-DD', '2021-07-08'),
...
panel.refreshMapLayer = function() {
  ...
  //NDVI
   ...
   var SAVI_Coll = filtered
                ...
                .mosaic()
                .clip(NomEdoSeleccionado);
    var SAVI = SAVI_Coll.expression ('float (((NIR - RED) / (NIR + RED + L))* (1+L))',
        {
          'L': 0.5, // Cobertura vegetacion 0-1
          'NIR': SAVI_Coll.select ('B8'),
          'RED': SAVI_Coll.select ('B4')
        });                
   //FIN SAVI
    //TEMPERATURA
...
//
 ...
 //***************************************************Comentado para agilizar los filtros.***********************
  ...
  //*******************FIN Limpiamos y agregamos los valores del estado**************************
    ...
    Map.addLayer(SAVI.clip(NomEdoSeleccionado), SAVI_STYLE,'SAVI_SENTINEL2 FILTROS', 0);
    ...
    Map.addLayer(ndvi_Layer.clip(NomEdoSeleccionado), SAVI_STYLE, 'NDVI_SENTINEL2 FILTROS', 0);
    Map.addLayer(MODIS_NDVI_LAYER.clip(NomEdoSeleccionado), SAVI_STYLE, 'NDVI_MODIS FILTROS', 0);
...
function updateOverlay() {
  ...
  Map.layers().set(13, ui.Map.Layer(overlay, {}, 'Predio seleccionado'));
}
...
function clearResults() {
  ...
  DatosPredio = '';
  Map.layers().remove(Map.layers().get(13));
