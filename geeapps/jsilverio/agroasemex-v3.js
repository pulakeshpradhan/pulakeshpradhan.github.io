...
var temp = ee.ImageCollection('MODIS/006/MOD11A1')
    ...
    .map(function(img){
      ...
      var unmasked = img.gt(-999).reduceRegion(ee.Reducer.count(), Mexico, 1000);
      return mulitplied.set('count', unmasked.get('LST_Day_1km'))
      .copyProperties(img,['system:time_start','system:time_end']);
    });
var Temp = temp;
var ModisImagen = ee.Image(temp.median());
var TempModis = ModisImagen.clip (Mexico);
...
var Temp_STYLE = {  min: -5,  max: 35,  palette: [
        ...
        'd1f2ff', '89ddec', '4384d4', '0602ff', '006847', '1dcc00', 'f39703', 'ff0000'  //'d1f2ff', '89ddec', '4384d4', '0602ff', '006847', '1dcc00', 'f39703', 'ff0000'
      ],
    };
...
Map.addLayer(TempModis.clip(Mexico), Temp_STYLE, 'Temp de la Superficie Terrestre', 0);
...
panel.filters = {
 ...
 function applyFilters() {
    ...
    if (panel.filters.mapCenter.getValue()) {
      ...
      filtroTEMP = filtroTEMP.filterBounds(Mexico);
    }
...
panel.refreshMapLayer = function() {
  ...
  //NDVI
   ...
   //FIN RGB
    ...
    TempModis = ModisImagen.clip(Mexico);
    // FIN TEMPERATURA
    Map.addLayer(S2_RGB.clip(Mexico), RGB_STYLE, 'RGB_SENTINEL2 FILTROS', 0);
    ...
    Map.addLayer(ndvi_Layer.clip(Mexico), NDVI_STYLE, 'NDVI_SENTINEL2 FILTROS', 0);
    Map.addLayer(TempModis.clip(Mexico), Temp_STYLE, 'Temp de la Superficie Terrestre FILTROS', 0);
    Map.addLayer(Predios.style(Predios_STYLE), {}, 'OI 2020/2021 SEGUIMIENTO AGROASEMEX');
...
function updateOverlay() {
  ...
  Map.layers().set(6, ui.Map.Layer(overlay, {}, 'Predio seleccionado'));
}
...
function clearResults() {
  selectedPoints = [];
  removeLayer('Predio seleccionado');
  Map.layers().remove(Map.layers().get(6));
