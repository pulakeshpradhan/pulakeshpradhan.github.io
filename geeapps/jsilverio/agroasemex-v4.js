...
//print(NomEdoSeleccionado.select('NOM_ENT').getInfo());
...
var Predios = ee.FeatureCollection('users/jsilverio/SIOF_20_04_TABLERO_FINAL')
              .filter(ee.Filter.or(ee.Filter.eq('Cve_EdoGeo', '28')));   //01
...
var SupTotalPredios = Predios.aggregate_sum('SEPERFICIE');
var SumaAsegTotalPredios = Predios.aggregate_sum('PRIMA_REAS');
var SupTotalPrediosVigentes = PrediosVigentes.aggregate_sum('SEPERFICIE');
var SumaAsegTotalPrediosVigentes = PrediosVigentes.aggregate_sum('PRIMA_REAS');
var SupTotalPrediosSeguimiento = PrediosNDVI.aggregate_sum('SEPERFICIE');
var SumaAsegTotalPrediosSeguimiento = PrediosNDVI.aggregate_sum('PRIMA_REAS');
...
Map.addLayer(S2_RGB.clip(NomEdoSeleccionado), VEGETACION_STYLE, 'Vegetación_SENTINEL2');
Map.addLayer(S2_RGB.clip(NomEdoSeleccionado), AGRICULTURA_STYLE, 'Agricultura_SENTINEL2', 0);
Map.addLayer(ndvi_Layer.clip(NomEdoSeleccionado), NDVI_STYLE, 'NDVI_SENTINEL2', 0);   //MODIS_NDVI
...
Map.addLayer(Predios.style(Predios_STYLE), {}, 'OI 2020/2021 AGROASEMEX', 0); //PrediosVigentes
Map.addLayer(PrediosVigentes.style(PrediosVigentes_STYLE), {}, 'OI 2020/2021 AGROASEMEX VIGENTES', 1);
Map.addLayer(PrediosNDVI.style(PrediosNDVI_STYLE), {}, 'OI 2020/2021 AGROASEMEX SEGUIMIENTO', 0);
...
//ESTADOS
panel.ESTADOS = {
  "Selecciona un estado":{ Nombre: 'Selecciona un estad', CveEdo: '00', Coord: [24.2910115597328, -98.640894391325]},
...
panel.COLLECTION_ID_Predios = 'users/jsilverio/SIOF_20_04_TABLERO_FINAL';
  ...
  panel.picker = {
    ...
    onChange: function() {
        // Refresh the map layer.
         ...
         //print('CveEdo', option.CveEdo);
         Map.setCenter(option.Coord[1], option.Coord[0], 7);
...
panel.filters = {
    ...
    endDate: ui.Textbox('YYYY-MM-DD', '2021-04-28'),
 ...
 function applyFilters() {
    ...
    if(CveEdoSeleccionado !== '00'){
      ...
      Predios = ee.FeatureCollection(panel.COLLECTION_ID_Predios)
                .filter(ee.Filter.or(ee.Filter.eq('Cve_EdoGeo', CveEdoSeleccionado)));  //ee.Number.parse(CveEdoSeleccionado.slice(0,2))
    }
...
panel.refreshMapLayer = function() {
   ...
   removeLayer('OI 2020/2021 AGROASEMEX');
   ...
   removeLayer('OI 2020/2021 AGROASEMEX SEGUIMIENTO');
   removeLayer('OI 2020/2021 AGROASEMEX VIGENTES');
  //NDVI
   var S2 = filtered
            .filterBounds(NomEdoSeleccionado)
            .filterMetadata('CLOUDY_PIXEL_PERCENTAGE', 'Less_Than', 30);
  ...
  PrediosNDVI = PrediosARevisar.filter(ee.Filter.eq('Valor', 1));
    //FIN Diferencias de NDVI entre sentinel y Modis - una desviación ************************************************************
  //*******************Cálculo de Superficies totales**************************
  SupTotalPredios = Predios.aggregate_sum('SEPERFICIE');
  SumaAsegTotalPredios = Predios.aggregate_sum('PRIMA_REAS');
  SupTotalPrediosVigentes = PrediosVigentes.aggregate_sum('SEPERFICIE');
  SumaAsegTotalPrediosVigentes = PrediosVigentes.aggregate_sum('PRIMA_REAS');
  SupTotalPrediosSeguimiento = PrediosNDVI.aggregate_sum('SEPERFICIE');
  SumaAsegTotalPrediosSeguimiento = PrediosNDVI.aggregate_sum('PRIMA_REAS');
  //*******************FIN Cálculo de Superficies totales**************************
  ...
  //*******************FIN Limpiamos y agregamos los valores del estado**************************
    ...
    Map.addLayer(S2_RGB.clip(NomEdoSeleccionado), VEGETACION_STYLE, 'Vegetación_SENTINEL2 FILTROS');
...
function makeResultsBarChart(getSelectedPredios) {
  ...
  //var properties = getSelectedPredios.propertyNames();
      ...
      EdoGeo = getSelectedPredios.reduceColumns(ee.Reducer.toList(), ['EdoGeo']).get('list');
...
function updateOverlay() {
  ...
  Map.layers().set(9, ui.Map.Layer(overlay, {}, 'Predio seleccionado'));
}
...
function updateChart() {
  ...
  {
    var  efchart = chartBuilder(getSelectedPredios());
    clearResults();
  }
}
...
function clearResults() {
  ...
  DatosPredio = '';
  Map.layers().remove(Map.layers().get(9));
...
function ValoresSuperficie() {
  ...
  SuperficiesTotales = ui.Panel({
    widgets: [
      ...
      ui.Label('Superficie Asegurada: ' + SupTotalPrediosVigentes.format("%,d").getInfo(0) + ' ha.', panel.SECTION_PREDIOS_STYLE),
      ...
      ui.Label('Superficie Asegurada: ' + SupTotalPrediosSeguimiento.format("%,d").getInfo(0) + ' ha.', panel.SECTION_PREDIOS_STYLE),
 ...
 function ValoresPredio() {
  ...
  DatosPredio = ui.Panel({
    widgets: [
      ...
      ui.Label('Estado: ' + EdoGeo.getInfo(0), panel.SECTION_PREDIOS_STYLE),
