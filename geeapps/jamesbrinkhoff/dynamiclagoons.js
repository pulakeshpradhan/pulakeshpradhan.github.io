var demh = ee.Image("AU/GA/DEM_1SEC/v10/DEM-S"), 
  ...
  // Applies scaling factors - specific to collection2.
  ...
  //cloud mask for Landsat c 2
  var maskL8sr_c02 = function(image) {
    // Bits 3 and 4 are cloud shadow and cloud, respectively, bit 2 is cirrus.
  ...
  //function to get landsat image collection from collection 2
  var get_ls_ic = function(cloud,start,stop){ //get year collection centered at pt, and reset date to last date in image
    ...
    var l8 = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2').filterDate(start,stop).map(applyScaleFactors).select(['SR_B2','SR_B3','SR_B4','SR_B5','SR_B6','SR_B7','ST_B10','QA_PIXEL'],['b','g','r','nir','swir1','swir2','tc','pixel_qa']);
    var l7 = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2').filterDate(start,stop).map(applyScaleFactors).select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7','ST_B6','QA_PIXEL'],['b','g','r','nir','swir1','swir2','tc','pixel_qa']);
    var l5 = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2').filterDate(start,stop).map(applyScaleFactors).select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7','ST_B6','QA_PIXEL'],['b','g','r','nir','swir1','swir2','tc','pixel_qa']);
    var ls_ic_all = l8.merge(l7).merge(l5);
    var ls_ic = ls_ic_all
                .filterMetadata('CLOUD_COVER','less_than',cloud)
                .map(maskL8sr_c02)
                .map(vis);
    return ls_ic.sort('system:time_start');
  };
  /*   */
  ...
  var newDate = function(xValue,yValue,seriesValue)
  {
    ...
    var image = ee.Image(ls.filterDate(date,endy.toString()+'-12-31')
                .filterBounds(feature.geometry()).first());
    var rgb = ui.Map.Layer(image,{bands:['r','g','b'],min:0,max:0.3},'RGB',true);
