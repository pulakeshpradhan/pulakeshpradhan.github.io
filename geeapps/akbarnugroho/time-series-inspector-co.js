...
// mengatur citra dan parameter visualisasi
var sentinelMap = ee.ImageCollection('COPERNICUS/S5P/OFFL/L3_CO');
var sst = 
        sentinelMap.select('CO_column_number_density').filterDate('2020-03-20','2020-06-23');
var vis = {min:0.00,max:0.05,palette:"lightblue,yellow,orange,red,purple"};
var composite = sst.mean().visualize(vis).clip(batas);
var compositeLayer = ui.Map.Layer(composite).setName('CO Composite');
// membuat peta utama dan mengatur layer CO
var mapPanel = ui.Map();
var layers = mapPanel.layers();
layers.add(compositeLayer, 'CO Covid-19 Composite');
/*
* pengaturan panel
*/
...
var intro = ui.Panel([
  ui.Label({
    value: 'Sentinel-5P CO Concentrate - Time Series Inspector',
    style: {fontSize: '20px', fontWeight:'bold'}
  }),
  ui.Label('Pilih lokasi untuk melihat time series konsentrasi CO')
  ]);
...
* pengaturan grafik
*/
// menghasilkan bagan deret waktu CO untuk koordinat yang dipilih
var generateChart = function (coords){
  ...
  .setOptions({
    title: 'CO column number density: time series',
    vAxis: {title: 'CO (Mol/m2)'},
    hAxis: {title: 'Date',
    format: 'YYYY-MM',
            gridlines: {count:7},
...
var legendTitle = ui.Label({
  value: 'Legenda Peta: rerata CO column number density (mol/m2)',
...
var text_7 = ui.Label( 
'Email: akbar.nugroho@mail.ugm.ac.id',{fontSize: '11px',color:'gray',margin: '0 0 0 11px',padding: '0'}); 
var text_8 = ui.Label(
