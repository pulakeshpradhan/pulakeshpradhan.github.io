...
var gauge_q = ee.FeatureCollection("users/rriggs/qGauges")
var gauge_w = ee.FeatureCollection("users/rriggs/wGauges")
var missSD = ee.FeatureCollection("users/rriggs/eGauges")
...
var L5 = ui.Label('Riggs, R., Allen, G.H., David, C.H., Lin, P., Pan, M., Yang, X., Gleason, C., "RODEO: An algorithm and Google Earth Engine application for river discharge retrieval from Landsat."', {margin: '1px 15px 10px 15px'}, 'https://github.com/Ryan-Riggs/RODEO');
...
var bias = ee.Array(ee.Number(stats.get('Bias_e')).abs())
var stde = ee.Array(stats.get('STDE_e'))
var rmse = ee.Array(stats.get('RMSE_e'))
var bias_a = ee.Array(ee.Number(stats.get('Bias')).abs())
var stde_a = ee.Array(stats.get('STDE'))
var rmse_a = ee.Array(stats.get('RMSE'))
var data = ee.List([bias,bias_a, stde,stde_a, rmse, rmse_a]);
var labs = ee.List(['Estimated Absolute Bias','Absolute Bias', 'Estimated STDE','STDE', 'Estimated RMSE', 'RMSE'])
...
var NRMSE = ee.Array(stats.get('NRMSE'))
var rBias = ee.Array(stats.get('rBias'))
var RRMSE = ee.Array(stats.get('RRMSE'))
var KGE = ee.Array(stats.get('KGE'))
var R = ee.Array(stats.get('R'))
var metrics = ee.List([NRMSE, rBias, RRMSE]);
var labs1 = ee.List(['NRMSE', 'rBias', 'RRMSE'])
var metricsChart = ui.Chart.array.values(metrics, 0, labs1)
  .setChartType('BarChart').setOptions({legend: {position: 'none'}, hAxis:{'title':'Percentage(%)'}});
var metrics1 = ee.List([R, KGE]);
var labs2 = ee.List(['R', 'KGE'])
var metricsChart1 = ui.Chart.array.values(metrics1, 0, labs2)
  .setChartType('BarChart').setOptions({legend: {position: 'none'}, hAxis:{'title':'R/KGE'}});
...
//Assign previously designed charts to panel. 
  var StatsPanel =ui.Panel([ui.Label('Uncertainty/Error', {fontWeight: 'bold'}), statsChart], ui.Panel.Layout.flow('vertical'));
    widgetList
  .set(2, StatsPanel)
  var metricsPanel =ui.Panel([ui.Label('Statistics', {fontWeight: 'bold'}), metricsChart], ui.Panel.Layout.flow('vertical'));
    widgetList
  .set(3, metricsPanel)
    var metricsPanel1 =ui.Panel([ui.Label('', {fontWeight: 'bold'}), metricsChart1], ui.Panel.Layout.flow('vertical'));
    widgetList
  .set(4, metricsPanel1)
