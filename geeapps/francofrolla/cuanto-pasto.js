...
//var select_escena = ui.Select({placeholder:"Escena", items: Object.keys(escenas)});
var computedIds = ee.List([""])
...
function setDisplay(dictionary) {
  visParams = {
    bands: ['NDVI'], 
    min: [dictionary['NDVI_p5'],],
    max: [dictionary['NDVI_p95'],],
    palette: ["FFFFFF", "CE7E45", "DF923D", "F1B555", "FCD163", "99B718", "74A901", "66A000", "529400", "3E8601", "207401", "056201", "004C00", "023B01", "012E01", "011D01", "011301"]
  };
  Map.addLayer(composite, visParams, 'NDVI');
  Map.centerObject(composite,15)
  var min  = ee.Number(params .get("NDVI_p5"))
  var max  = ee.Number(params .get("NDVI_p95"))
...
//var indice_S2 = (image.select("B8").subtract(image.select("B4"))).divide(image.select("B8").add(image.select("B4"))).rename('NDVI');
//var indice_L8 = (image.select("B5").subtract(image.select("B4"))).divide(image.select("B5").add(image.select("B4"))).rename('NDVI');
 if (que_satelite === "Sentinel_2") {
    ...
    print("imagenes observadas", coleccion.size())
      ...
      var ndvi = (image.select("B8").subtract(image.select("B4"))).divide(image.select("B8").add(image.select("B4"))).rename('NDVI');
...
var options = {
  title: 'Historial de NDVI promedio para el lote',
  hAxis: {title: 'Fechas'},
  vAxis: {title: 'NDVI'},
  lineWidth: 2,
  series: {
...
var chart1 = ui.Chart.image.series({
  imageCollection: coleccion.select('NDVI'),
  region: lote,
  reducer: ee.Reducer.mean(),
  scale: escala,
}).setOptions(options);
inspectorPanel.add(chart1);
var  list = coleccion.toList(coleccion.size());
//ACA SE CALCULA LA DISTANCIA ENTRE DIAS CORRECTAMENTE
//En esta variable yo tengo todas las capas con NDVI calculadas.
//var actual = listOfImages.indexOf(imagen1);
//var previous = ee.Image(listOfImages.get(actual.subtract(1)));
//var list = listOfImages
var accumulate = function(imagen1) {
  // Get the latest cumulative anomaly image from the end of the list with
  // get(-1).  Since the type of the list argument to the function is unknown,
  // it needs to be cast to a List.  Since the return type of get() is unknown,
  // cast it to Image.
  var actual = list.indexOf(imagen1);
  var previa = ee.Image(list.get(actual.subtract(2)));
  // Add the current anomaly to make a new cumulative anomaly image.
  var dif_ndvi = imagen1.subtract(previa)
  var media = dif_ndvi.select(["NDVI"]).reduceRegion({
    reducer: ee.Reducer.mean(), 
    geometry: lote,
    scale: escala
     });
  media = ee.Number(media.get("NDVI"))
  media = ee.Algorithms.If(media,media,9999)
  var date = ee.Date(previa.get('system:time_start'));
  var date1 = ee.Date(imagen1.get('system:time_start'));
  var dif_dias = date1.difference(date,"day").round()
  var tasa_ndvi = ee.Number(media).divide(dif_dias)  
  return imagen1.set({tasa_ndvi: tasa_ndvi})
};
coleccion = coleccion.map(accumulate)
coleccion = coleccion.filter(ee.Filter.lt('tasa_ndvi', 1));
coleccion = coleccion.filter(ee.Filter.gt('tasa_ndvi', -1));
print(coleccion)
var y = coleccion.aggregate_array("tasa_ndvi")
var x = coleccion.aggregate_array("system:time_start")
            var chart = ui.Chart.array.values(y,0,x)
            .setSeriesNames(["Tasa NDVI"])
            .setOptions({
            title: 'Tasa NDVI (DifNDVI/Difdias)',
            hAxis: {'title': 'Fechas'},
            vAxis: {'title': 'Tasa'},
            lineWidth: 1,
            pointSize: 6
 ...
 */
texto_escena.style().set(estilo_rojo)
select_escena.style().set(estilo2)
computedIds = coleccion
        .reduceColumns(ee.Reducer.toList(), ['system:index'])
        .get('list');
computedIds.evaluate(function(ids) {
      select_escena.items().reset(ids);
      select_escena.setPlaceholder("Ahora si, elija una Fecha")
       //se selecciona la primera imagen por default
       //select_escena.setValue(select_escena.items().get(0));
    });
//bueno aca habria que pasarlo a un combobox, 
/*
...
var coeficientes = function(feature) {
  var estimado = ee.Number(feature.get("NDVI"));
  var real = ee.Number(feature.get('profundida'));
...
function leyenda(valor_inicial, valor_final) {
  var labelPanel = ui.Panel({
      widgets: [
        ui.Label(valor_inicial + " ", {margin: '4px 8px', backgroundColor: "#ffffff"}),
        ui.Label(valor_final + " ", {margin: '4px 8px',stretch: "horizontal", textAlign: "right", backgroundColor: "#ffffff" })
      ],
...
valor_inicial = valor_inicial.toFixed(2)
...
var estilo_rojo = {shown: true, color: "red",backgroundColor: "#35373a", stretch : "horizontal",textAlign: "justify"} 
...
var texto_escena = ui.Label({value: "Eliga una fecha", style: estilo_apagado })
var select_escena = ui.Select({
      //items: Object.keys(lista_satelites),
      style: estilo_apagado,
      placeholder: 'Cargando...',
      onChange:   function() {
            var imageId = select_escena.getValue();
            if (imageId) {
            // si una imagen es encontrada, se grafica
            var image = ee.Image(COLLECCION + '/' + imageId);
            composite = image.clip(lote);
            print(composite)
            if (que_satelite === "Sentinel_2") {
            var ndvi = (composite.select("B8").subtract(composite.select("B4"))).divide(composite.select("B8").add(composite.select("B4"))).rename('NDVI');
            //En realidad esto es MSAVI pero lo hago asi para no correr todo el codigo
            //var ndvi = composite.select("B8").multiply(2).add(1).subtract(composite.select("B8").multiply(2).add(1).pow(2).subtract(composite.select("B8").subtract(composite.select("B4")).multiply(8)).sqrt()).divide(2).rename('NDVI');
            composite = composite.addBands(ndvi)
            }
            if (que_satelite != "Sentinel_2") {
            var ndvi = (composite.select("B5").subtract(composite.select("B4"))).divide(composite.select("B5").add(composite.select("B4"))).rename('NDVI');
            //idem antes
            //var ndvi = image.select("B5").multiply(2).add(1).subtract(image.select("B5").multiply(2).add(1).pow(2).subtract(image.select("B5").subtract(image.select("B4")).multiply(8)).sqrt()).divide(2).rename('NDVI')
            composite = composite.addBands(ndvi)
            }
            //print(composite)
            training1 = composite.select("NDVI").sampleRegions({
            collection: pts,
            properties: ['profundida'],
            scale: escala
            });
           //Train a CART classifier with default parameters.
            //trained = ee.Classifier.svm().setOutputMode('REGRESSION').train(training, 'profundida', bandas1);
            //Classify the input imagery.
            //classified = composite.select("NDVI").classify(trained);
            //training1 = classified.sampleRegions({
            //collection: pts,
            //properties: ['profundida'],
            //scale: escala
            //});
            var y = ee.Array(training1.aggregate_array("profundida"))
            var x = ee.Array(training1.aggregate_array("NDVI"))
            var chart = ui.Chart.array.values(y,0,x)
            .setSeriesNames(["Serie"])
            .setOptions({
            title: 'NDVI y Biomasa del cultivo (Datos Entrenamiento)',
            hAxis: {'title': 'NDVI'},
            vAxis: {'title': 'Biomasa (kg/ha)'},
            pointSize: 3,
            });
            //Map.add(inspectorPanel);
            inspectorPanel.clear()
            inspectorPanel1.clear()
            inspectorPanel.add(chart);
            params = composite.select(["NDVI"]).reduceRegion({
            reducer: ee.Reducer.percentile([5, 95]), 
            geometry: lote,
            scale: escala
            });
...
var inspectorPanel1 = ui.Panel({style: {width: '30%',position:"bottom-left" }});
var panel_auxiliar2 = ui.Panel({style: {width: '30%',position:"bottom-right" }});
var panel_inicio =  ui.Panel({
  widgets: [ 
  ui.Label({
        value: '¿Cuanto pasto? es una herramienta del grupo  "Carnes a pasto" para inferir relaciones de biomasa con NDVI',  
        style: estilo1, 
      }),
    ui.Label({
        value: "Contacto: frolla.franco@inta.gob.ar",
        style: estilo1, 
      }),
      terminos_url,
      que_mision,
...
var boton_estadisticos = ui.Button({
  ...
  onClick: function() {
    ...
    var pearson_print =ui.Label({value: "Coeficiente de correlación (R²)", style: estilo4})
    var pearson_print1 =ui.Label({value: "Calculando...", style: estilo4})
...
var boton_descargas = ui.Button({
  ...
  onClick: function() {
   ...
   var url = composite.select('NDVI').getDownloadURL({name: "Mapa",scale: 10, region: region }) 
  var a = composite.getThumbURL({
    params: {
    bands:   ['NDVI'],
    min: visParams["max"],
    max: visParams["min"],
