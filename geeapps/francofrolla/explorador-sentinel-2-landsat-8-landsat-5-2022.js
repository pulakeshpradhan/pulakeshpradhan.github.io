var poligono_lote = ui.import && ui.import("poligono_lote", "geometry", {
      "geometries": [
        {
          "type": "Point",
          "coordinates": [
      ...
      "shown": true,
      "locked": false
    }) || /* color: #d63000 */ee.Geometry.Point([-63.0932910131134, -37.76775296985981]);
 /* 
 ...
 |_____|_| \_|  |_/_/    \_\
*/
Map.setCenter(-63.0932910131134, -37.76775296985981,15)
...
app.createPanels = function() {
  ...
  /* Filtros. */
  app.filters = {
    geojson: ui.Textbox({placeholder:"Ingrese GeoJSON", style:{color: 'black', backgroundColor: "#35373a",width: '160px'} }),
    botongeo: ui.Button({
      label: 'Ingrese GeoJSON (WGS84)',
      onClick: function() {
        var json = app.filters.geojson.getValue();
        var datos = (json.split('"coordinates":'))
        var coordendas = ee.String((datos[1]))
        var largo = coordendas.length()
        var slice = coordendas.slice(0,largo.subtract(4))
        var sinaltura = slice.replace(ee.String(",0"),ee.String(""),"g")
        var cordsfinales = ee.List(sinaltura.decodeJSON())
        poligono_lote = ee.Geometry.Polygon(cordsfinales)
        Map.addLayer(poligono_lote)
        Map.centerObject(poligono_lote,12)
        app.applyFilters()}}),
    titulo: ui.Label('1) Seleccionar casilla para dibujar lote o ingrese KMZ via http://lmingenieria.com.ar/kmzageojson/', {fontWeight: 'bold', color: "white", backgroundColor: "#35373a"}),
    ...
    startDate: ui.Textbox('YYYY-MM-DD', '2021-11-01'),
    endDate: ui.Textbox('YYYY-MM-DD', '2021-12-01'),
    //applyButton: ui.Button('Aplicar filtros', app.applyFilters),
    applyButton: ui.Button({
       label: 'Aplicar filtros',
       style: {color: "black", backgroundColor: "#35373a"},
       onClick: function() { Map.clear(), Map.add(panel_auxiliar), Map.add(panel_auxiliar2),panel_auxiliar.clear(),panel_auxiliar2.clear(),  Map.setOptions("HYBRID"),app.applyFilters() }}),
  ...
  app.dibujarlote()
   ...
   que_satelite = app.filters.que_mision.getValue();
  /* Visualizacion de filtros. */
  app.filters.panel = ui.Panel({
    widgets: [
      app.filters.titulo,
      app.filters.geojson,
      app.filters.botongeo,
      app.filters.dibujarlote,
      app.filters.limpiar,
      ui.Label('Seleccionar satelite', {fontWeight: 'bold', color: "white", backgroundColor: "#35373a"}),
      app.filters.que_mision,
      ...
      ui.Label("Landsat 5: 1984 al 2013.", {fontWeight: 'bold', color: "white", backgroundColor: "#35373a"}),
...
var satelite = app.filters.que_mision.getValue();  
if (satelite === "Landsat_8") {
  app.VIS_OPTIONS = app.VIS_OPTIONS_L8;
  app.vis.select.items().reset(Object.keys(app.VIS_OPTIONS));
  app.vis.select.setValue(app.vis.select.items().get(0));
}
if (satelite === "Sentinel_2") {
  app.VIS_OPTIONS = app.VIS_OPTIONS_S2;
  app.vis.select.items().reset(Object.keys(app.VIS_OPTIONS));
  app.vis.select.setValue(app.vis.select.items().get(0));
}
if (satelite === "Landsat_5") {
  app.VIS_OPTIONS = app.VIS_OPTIONS_L5;
  app.vis.select.items().reset(Object.keys(app.VIS_OPTIONS));
  app.vis.select.setValue(app.vis.select.items().get(0));
}
};
...
if (indice_seleccionado == "NDVI" && que_satelite === "Sentinel_2") {var indice = (image.select("B8").subtract(image.select("B4"))).divide(image.select("B8").add(image.select("B4"))).rename('NDVI'); app.vis.label2.setValue("Índice Normalizado de diferencia de Vegetación. Rouse et al., 1973")}
if (indice_seleccionado == "MSAVI2" && que_satelite === "Sentinel_2") {var indice = image.select("B8").multiply(2).add(1).subtract(image.select("B8").multiply(2).add(1).pow(2).subtract(image.select("B8").subtract(image.select("B4")).multiply(8)).sqrt()).divide(2).rename('MSAVI2'); app.vis.label2.setValue("Índice  modificado de vegetación suelo ajustado. Qi et al., 1994")}
if (indice_seleccionado == "NDWI" && que_satelite === "Sentinel_2")  {var indice = (image.select("B3").subtract(image.select("B8"))).divide(image.select("B3").add(image.select("B8"))).rename('NDWI'); app.vis.label2.setValue("Índice  de agua por diferencia normalizada. Gao, 1995")};
if (indice_seleccionado == "EVI" && que_satelite === "Sentinel_2")  {var indice = (image.select("B8").subtract(image.select("B4").multiply(2.5))).divide((image.select("B8").add(image.select("B4").multiply(6)).subtract(image.select("B2").multiply(7.5)).add(1))).rename("EVI"); app.vis.label2.setValue("Índice  de vegetación mejorado. Huete et al., 2002")}  
if (indice_seleccionado == "MSI" && que_satelite === "Sentinel_2")  {var indice = ((image.select("B11").divide(image.select("B8")))).rename("MSI"); app.vis.label2.setValue("Índice de estrés hídrico. Hunt y Rock, 1989")} 
if (indice_seleccionado == "GNDVI" && que_satelite === "Sentinel_2") {var indice = (image.select("B8").subtract(image.select("B3"))).divide(image.select("B8").add(image.select("B3"))).rename('GNDVI'); app.vis.label2.setValue("Índice Verde Normalizado de diferencia de Vegetación. Gitelson et al. 1996")}
//Landsat 8
if (indice_seleccionado == "NDVI" && que_satelite === "Landsat_8") {var indice = (image.select("B5").subtract(image.select("B4"))).divide(image.select("B5").add(image.select("B4"))).rename('NDVI'); app.vis.label2.setValue("Índice Normalizado de diferencia de Vegetación. Rouse et al., 1973")}
if (indice_seleccionado == "MSAVI2" && que_satelite === "Landsat_8") {var indice = image.select("B5").multiply(2).add(1).subtract(image.select("B5").multiply(2).add(1).pow(2).subtract(image.select("B5").subtract(image.select("B4")).multiply(8)).sqrt()).divide(2).rename('MSAVI2'); app.vis.label2.setValue("Índice  modificado de vegetación suelo ajustado. Qi et al., 1994")}
if (indice_seleccionado == "NDWI" && que_satelite === "Landsat_8")  {var indice = (image.select("B5").subtract(image.select("B6"))).divide(image.select("B5").add(image.select("B6"))).rename('NDWI'); app.vis.label2.setValue("Índice  de agua por diferencia normalizada. Gao, 1995")}
if (indice_seleccionado == "EVI" && que_satelite === "Landsat_8")  {var indice = ((image.select("B5").subtract(image.select("B4"))).divide(image.select("B5").add((image.select("B4").multiply(6)).subtract((image.select("B2").multiply(7.5)).add(1)))).multiply(2.5)).rename("EVI"); app.vis.label2.setValue("Índice  de vegetación mejorado. Huete et al., 2002")}  
if (indice_seleccionado == "MSI" && que_satelite === "Landsat_8")  {var indice = ((image.select("B5").divide(image.select("B6")))).rename("MSI"); app.vis.label2.setValue("Índice de estrés hídrico. Hunt y Rock, 1989")} 
if (indice_seleccionado == "GNDVI" && que_satelite === "Landsat_8") {var indice = (image.select("B5").subtract(image.select("B3"))).divide(image.select("B5").add(image.select("B3"))).rename('GNDVI'); app.vis.label2.setValue("Índice Verde Normalizado de diferencia de Vegetación. Gitelson et al. 1996")}
if (indice_seleccionado == "NDVI" && que_satelite === "Landsat_5") {var indice = (image.select("B4").subtract(image.select("B3"))).divide(image.select("B4").add(image.select("B3"))).rename('NDVI'); app.vis.label2.setValue("Índice Normalizado de diferencia de Vegetación. Rouse et al., 1973")}
if (indice_seleccionado == "MSAVI2" && que_satelite === "Landsat_5") {var indice = image.select("B4").multiply(2).add(1).subtract(image.select("B4").multiply(2).add(1).pow(2).subtract(image.select("B4").subtract(image.select("B3")).multiply(8)).sqrt()).divide(2).rename('MSAVI2'); app.vis.label2.setValue("Índice  modificado de vegetación suelo ajustado. Qi et al., 1994")}
if (indice_seleccionado == "NDWI" && que_satelite === "Landsat_5")  {var indice = (image.select("B4").subtract(image.select("B5"))).divide(image.select("B4").add(image.select("B5"))).rename('NDWI'); app.vis.label2.setValue("Índice  de agua por diferencia normalizada. Gao, 1995")}
if (indice_seleccionado == "EVI" && que_satelite === "Landsat_5")  {var indice = ((image.select("B4").subtract(image.select("B3"))).divide(image.select("B4").add((image.select("B3").multiply(6)).subtract((image.select("B1").multiply(7.5)).add(1)))).multiply(2.5)).rename("EVI"); app.vis.label2.setValue("Índice  de vegetación mejorado. Huete et al., 2002")}  
if (indice_seleccionado == "MSI" && que_satelite === "Landsat_5")  {var indice = ((image.select("B4").divide(image.select("B5")))).rename("MSI"); app.vis.label2.setValue("Índice de estrés hídrico. Hunt y Rock, 1989")} 
if (indice_seleccionado == "GNDVI" && que_satelite === "Landsat_5") {var indice = (image.select("B4").subtract(image.select("B2"))).divide(image.select("B4").add(image.select("B2"))).rename('GNDVI'); app.vis.label2.setValue("Índice Verde Normalizado de diferencia de Vegetación. Gitelson et al. 1996")}
 ...
 var p90 = ee.Number(indice.reduceRegion({
  ...
  var p10 = ee.Number(indice.reduceRegion({
    ...
    //14-09 sacaba los lansdsat con resolucion 10 m
    var escala = 10
    if (que_satelite !== "Sentinel_2") {escala = 30}
...
app.vis = {
    ...
    descarga1:ui.Label("Descargar escena"),
    descarga2:ui.Label("  Descargar indice  "),
    label: ui.Label(),
...
coleccion1 = coleccion1.filter(ee.Filter.gt(indice_seleccionado, umbralcete));
...
var map_indice_l5 = function(indice_seleccionado) { //1
var indice = ee.Image();
if (indice_seleccionado == "NDVI"){
    coleccion = coleccion.map (function(image) {
  indice = image.normalizedDifference(['B4', 'B3']).rename('NDVI'); 
  return image.addBands(indice);
  });
 }
if (indice_seleccionado == "NDWI"){
   print("hasta aca llegue")
  coleccion = coleccion.map (function(image) {
  var indice = (image.select("B4").subtract(image.select("B5"))).divide(image.select("B4").add(image.select("B5"))).rename('NDWI'); 
  return image.addBands(indice);
  });
}
if (indice_seleccionado == "EVI"){
  coleccion = coleccion.map (function(image) {
  var indice = ((image.select("B4").subtract(image.select("B3"))).divide(image.select("B4").add((image.select("B3").multiply(6)).subtract((image.select("B1").multiply(7.5)).add(1)))).multiply(2.5)).rename("EVI"); 
  return image.addBands(indice);
  });
}
if (indice_seleccionado == "MSI"){
  coleccion = coleccion.map (function(image) {
  var indice = ((image.select("B4").divide(image.select("B5")))).rename("MSI") 
  return image.addBands(indice);
  });
}
if (indice_seleccionado == "MSAVI2"){
  coleccion = coleccion.map (function(image) {
  var indice = (image.select("B4").multiply(2).add(1).subtract(image.select("B4").multiply(2).add(1).pow(2).subtract(image.select("B4").subtract(image.select("B3")).multiply(8)).sqrt()).divide(2)).rename('MSAVI2') 
  return image.addBands(indice);
  });
 }
var coleccion1 = coleccion.map(function(image) {
...
//app.setLoadingMode(false);
  ...
  app.media = {
    ...
    onClick: function() {
       ...
       if (que_satelite === "Sentinel_2") {panel_auxiliar.clear(), map_indice_s2(indice_elegido)}
       if (que_satelite === "Landsat_8") {panel_auxiliar.clear(), map_indice_l8(indice_elegido)}
       if (que_satelite === "Landsat_5") {panel_auxiliar.clear(), map_indice_l5(indice_elegido)}
             } else {
...
app.createHelpers = function() {
  ...
  app.applyFilters = function() {
    ...
    que_satelite = app.filters.que_mision.getValue();
    app.select_VIS();
      if (que_satelite === "Landsat_8") {app.COLLECTION_ID = 'LANDSAT/LC08/C01/T1_RT_TOA'}
      if (que_satelite === "Sentinel_2") {app.COLLECTION_ID = 'COPERNICUS/S2'}
      if (que_satelite === "Landsat_5") {app.COLLECTION_ID = 'LANDSAT/LT05/C01/T1'}
    ...
    var agregar_nubes_L5 = function(image) {
    var qa = image.select('BQA');  
    var mask = qa.bitwiseAnd(1 << 4).eq(0);
    var nubes = image.reduceRegion({
    reducer: ee.Reducer.mean(),
    ...
    if (que_satelite === "Sentinel_2" && app.filters.filtrar_nubes.getValue() === true) {
    filtered = filtered.map(agregar_nubes);
    filtered = filtered.filterMetadata('mascara', 'equals', 0);
    }
     if (que_satelite === "Landsat_8" && app.filters.filtrar_nubes.getValue() === true) {
    print("si pasa L8")
    filtered = filtered.map(agregar_nubes_L8);
    filtered = filtered.filterMetadata('mascara', 'less_than', 5000);
    }
     if (que_satelite === "Landsat_5" && app.filters.filtrar_nubes.getValue() === true) {
    print("si pasa L5")
    filtered = filtered.map(agregar_nubes_L5);
    filtered = filtered.filterMetadata('mascara', 'less_than', 5000);
    }
    coleccion = filtered;
    ...
    scale: Map.getScale(),
      ...
      var visParams = {
         ...
         params.get('B6_p5').getInfo(), params.get('B5_p5').getInfo(), params.get('B2_p5').getInfo()
          ...
          if (cuales == 'Color Natural L5 (B3/B2/B1)') {
      var visParams = {
          bands: ['B3', 'B2', 'B1'], 
          min: [
         params.get('B3_p5').getInfo(), params.get('B2_p5').getInfo(), params.get('B1_p5').getInfo()
            ],
            max: [
            params.get('B3_p95').getInfo(), params.get('B2_p95').getInfo(), params.get('B1_p95').getInfo()
               ]
            };
      }
      if (cuales == 'False color infrarrojo L5 (B4/B3/B2)') {
      var visParams = {
          bands: ['B4', 'B3', 'B2'], 
          min: [
         params.get('B4_p5').getInfo(), params.get('B2_p5').getInfo(), params.get('B2_p5').getInfo()
            ],
            max: [
            params.get('B4_p95').getInfo(), params.get('B3_p95').getInfo(), params.get('B2_p95').getInfo()
               ]
       ...
       if (cuales == 'Agricultura L5 (B5/B4/B3)') {
      var visParams = {
          bands: ['B5', 'B4', 'B3'], 
          min: [
         params.get('B5_p5').getInfo(), params.get('B4_p5').getInfo(), params.get('B3_p5').getInfo()
...
//  if (que_satelite === false) {
  ...
  app.VIS_OPTIONS_L5 = {
            'Color Natural L5 (B3/B2/B1)': {
            description: 'Apariencia similar a la del ojo humano ',
  visParams: {gamma: 1.3, min: 0.1, max: 0.3, bands: ['B3', 'B2', 'B1']}
   },
  'False color infrarrojo L5 (B4/B3/B2)': {
   description: 'Vegetaciones en roja, zonas urbanas  ' +
                'en azul claro, y suelos en marron.',
  visParams: {bands: ['B4', 'B3', 'B2'],min:0.1, max:0.3}
  },
  'Agricultura L5 (B5/B4/B3)': {
    description: 'Vegetacion sana en tonos de verde, tierra arrada ' +
                'o sin cultivo en colores magenta',
  visParams: {gamma: 1.3, min: 0.1, max: 0.3, bands: ['B5', 'B4', 'B3']}
  }
 }; 
 app.VIS_OPTIONS = app.VIS_OPTIONS_S2;
};
...
app.boot = function() {
 ...
 var eeNow = ee.Date(Date.now());
  ...
  app.filters.endDate.setValue(dateStr);
  })
  var eeNow = ee.Date(Date.now()).advance(-30,'day');
  eeNow.format('YYYY-MM-dd').evaluate(function(dateStr){
  app.filters.startDate.setValue(dateStr);
  })
 Map.centerObject(poligono_lote, 14);
