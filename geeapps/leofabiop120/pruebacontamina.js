...
//  * Componente:  SIG
//  *
//  * Propósito:- Mapeo de Contaminantes
...
// Note style.
var noteStyle1 = {backgroundColor: 'rgba(0, 0, 0, 0.0)', color:'DimGrey', fontSize: '9px', fontWeight: '500', margin: '8px 8px 1px 8px'};
...
var notesPanel1 = ui.Panel({
  widgets: [
  ui.Label({value:'DATA-Contaminación, es una herramienta de procesamiento inteligente basado en el uso de sensores que proveen servición de monitoreo sobre la contaminación', style: noteStyle1}),
  ui.Label({value:'Para su uso adecuado, siga los siguientes pasos', style: noteStyle1}),
  ui.Label({value:'1) En el Panel izquierdo: Defina los parámetros del Mapa', style: noteStyle1}),
  ui.Label({value:'*El Rango de fechas corresponde al promedio que será calculado en el mapa ', style: noteStyle1}),
  ui.Label({value:'2) Luego de definir los parámetros, haga clic en el botón Run (parte superior del visor de mapa)', style: noteStyle1}),
  ui.Label({value:'3) Haga clic en Layer, para activar o desactivar las capas', style: noteStyle1}),
  ui.Label({value:'4) En el Panel derecho: Encontrará información Estadística del área de estudio seleccionada', style: noteStyle1}),
    ui.Label({value:'* Espere a que cargue las estadísticas...', style: noteStyle1}),
...
// Note style.
var noteStyle2 = {backgroundColor: 'rgba(0, 0, 0, 0.0)', color:'DimGrey', fontSize: '9px', fontWeight: '500', margin: '8px 8px 1px 8px'};
// Show/hide note button.
var notesButton2 = ui.Button({label: 'Fuente de Datos', onClick: notesButtonHandler2, style: {margin: '0px'}});
var notesPanel2 = ui.Panel({
  widgets: [
  ui.Label({value:'* Fuente y Descripción de Datos', style: noteStyle2}),
  ui.Label({value:'________________________________________________', style: noteStyle2}),
  ui.Label({value: '• Leer sobre los Datos de NO2.', style: noteStyle2, targetUrl: 'https://sentinel.esa.int/web/sentinel/user-guides/sentinel-5p-tropomi'}),
  ui.Label({value: '• Leer sobre los Datos de CO.', style: noteStyle2, targetUrl: 'https://sentinel.esa.int/web/sentinel/user-guides/sentinel-5p-tropomi'}),
  ui.Label({value: '• Leer sobre los Datos de SO2.', style: noteStyle2, targetUrl: 'https://sentinel.esa.int/web/sentinel/user-guides/sentinel-5p-tropomi'}),
  ui.Label({value: '• Leer sobre los Datos de CH4.', style: noteStyle2, targetUrl: 'https://sentinel.esa.int/web/sentinel/user-guides/sentinel-5p-tropomi'}),
  ui.Label({value: '• Leer sobre los Datos de O3.', style: noteStyle2, targetUrl: 'https://sentinel.esa.int/web/sentinel/user-guides/sentinel-5p-tropomi'}),
  ui.Label({value: '• Consulte la descripción de los datos en el catálogo de datos de Earth Engine.',
    ...
    ui.Label({value:'Referencia Sugerida', style: noteStyle2}),
    ui.Label({value:'DATA-I, 2021. ', style: noteStyle2}),
...
var Col_NO2 = sentinel5_NO2;
var modC_NO2 = Col_NO2.map(function(image) {
...
function dailyImages_NO2 (images,startdate,enddate){
 ...
 var imagePerDay_NO2 = ee.ImageCollection(dailyImages_NO2 (modC_NO2,start,end)).filter(ee.Filter.gt('nbands',0));
// a bit more sophisticated, print a chart with median, 5%, 95% over geometry
var timeSeries_NO2 = imagePerDay_NO2.sort('system:time_start').map(function(i) {
var values_NO2 = i.select('NO2_column_number_density').reduceRegion({
    reducer: ee.Reducer.minMax().combine({ reducer2: ee.Reducer.median(), sharedInputs: true}), 
    geometry: studyarea3, 
    scale: 5000
  });
  return ee.Feature(null, values_NO2)
    .set({ 'system:time_start': i.get('system:time_start') })
});
// evaluate values, convert into a data table and print chart
timeSeries_NO2.evaluate(function(ts) {
  var dataTable = {
    cols: [{id: 'x', type: 'date'},
           {id: 'y', type: 'number'},
  ...
  var chart_NO2 = ui.Chart(dataTable, 'LineChart', options);
  //panel2.add(chart);
  panel2.widgets().set(2, chart_NO2);
});
//----------------------------------------------------------Generar Histograma---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var options_NO2 = {
...
var Data_NO2 = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_NO2')
  ...
  .mean()
  .multiply(1e6);
var Histograma_NO2=  ui.Chart.image.histogram(Data_NO2, studyarea3, 1000, null, null, 100).setOptions(options_NO2);
//----------------------------------------------------Generar Grafico doble---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start1 = '2020-01-01';
var end2 = '2021-12-01';
...
var Data_NO2_Col = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_NO2");
var filtered_No2 = Data_NO2_Col.filterBounds(studyarea3)
.filterDate(Fecha_1,Fecha_2)
.map(function(image){return image.clip(studyarea3)});
var Coll_NO2 = filtered_No2.select("tropospheric_NO2_column_number_density").mean().multiply(1e6);
// combine two images
var Combine_Pob_NO2 = ghslPop.addBands(Coll_NO2);
// sample N points from the 2-band image
var scale = Map.getScale();
var values_NO2 = Combine_Pob_NO2.sample({ region: studyarea3, scale: scale, numPixels: 2000, geometries: true});
var chart_Pob_NO2 = ui.Chart.feature.byFeature(values_NO2, 'population_count', ['tropospheric_NO2_column_number_density'])
...
//print(chart);
var Cargangando_NO2 = ui.Label({value: "Cargando...."});
panel2.widgets().set(4, Cargangando_NO2);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start_SO2 = ee.Date(Fecha_1);
var end_SO2 = ee.Date(Fecha_2);
var Col_SO2 = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_SO2");
var modL_SO2 = Col_SO2.filterDate(start_SO2,end_SO2).select('SO2_column_number_density');
var modC_SO2 = modL_SO2.map(function(image) {
  return image.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
...
function dailyImages_SO2 (images,startdate,enddate){
   ...
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = end_SO2.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDay_SO2 = ee.ImageCollection(dailyImages_SO2 (modC_SO2,start_SO2,end_SO2)).filter(ee.Filter.gt('nbands',0));
// a bit more sophisticated, print a chart with median, 5%, 95% over geometry
var timeSeries_SO2 = imagePerDay_SO2.sort('system:time_start').map(function(i) {
...
// evaluate values, convert into a data table and print chart
timeSeries_SO2.evaluate(function(ts) {
  var dataTable = {
    cols: [{id: 'x', type: 'date'},
           {id: 'y', type: 'number'},
           {id: 'i0', type: 'number', role: 'interval'},
  ...
  //print(ui.Chart(dataTable, 'LineChart', options));
  var chart_SO2 = ui.Chart(dataTable, 'LineChart', options);
  //panel2.add(chart);
  panel2.widgets().set(2, chart_SO2);
});
//----------------------------------------------------------Generar Histograma---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var options_SO2 = {
...
var Data_SO2 = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_SO2')
  ...
  .mean()
  .multiply(1e6);
var Histograma_SO2=  ui.Chart.image.histogram(Data_SO2, studyarea3, 1000, null, null, null).setOptions(options_SO2);
//----------------------------------------------------Generar Grafico doble---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start1_SO2 = '2020-01-01';
var end2_So2 = '2021-12-01';
var collectionSO2 = sentinel5_SO2.select('SO2_column_number_density')
                   .filterDate(start1_SO2,end2_So2)
                   .filter(ee.Filter.calendarRange(60, 364, 'day_of_year'));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The GHSL global population density dataset for 2015.
var ghslPop_So2 = ee.Image('JRC/GHSL/P2016/POP_GPW_GLOBE_V1/2015');
var Datas_SO2 = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_SO2")
var filtered_So2 = Datas_SO2.filterBounds(studyarea3)
.filterDate(Fecha_1,Fecha_2)
.map(function(image){return image.clip(studyarea3)});
var Coll_SO2 = filtered_So2.select("SO2_column_number_density").mean().multiply(1e6);
// combine two images
var Combine_Pob_So2 = ghslPop_So2.addBands(Coll_SO2);
// sample N points from the 2-band image
scale = Map.getScale();
var values_SO2 = Combine_Pob_So2.sample({ region: studyarea3, scale: scale, numPixels: 2000, geometries: true}) 
var chart_Pob_SO2 = ui.Chart.feature.byFeature(values_SO2, 'population_count', ['SO2_column_number_density'])
  .setChartType('ScatterChart')
  .setOptions({ 
    pointSize: 2, 
    pointColor: 'red', 
...
//print(chart);
var Cargando_So2 = ui.Label({value: "Cargando...."});
panel2.widgets().set(4, Cargando_So2);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start_CO = ee.Date(Fecha_1);
var end_CO = ee.Date(Fecha_2);
var Col_CO = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_CO");
var modL_CO = Col_CO.filterDate(start_CO,end_CO).select('CO_column_number_density');
var modC_CO = modL_CO.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function dailyImages_CO (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = end_CO.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDay_CO = ee.ImageCollection(dailyImages_CO (modC_CO,start_CO,end_CO)).filter(ee.Filter.gt('nbands',0));
// a bit more sophisticated, print a chart with median, 5%, 95% over geometry
var timeSeries_CO = imagePerDay_CO.sort('system:time_start').map(function(i) {
...
// evaluate values, convert into a data table and print chart
timeSeries_CO.evaluate(function(ts) {
  var dataTable = {
    cols: [{id: 'x', type: 'date'},
           {id: 'y', type: 'number'},
           {id: 'i0', type: 'number', role: 'interval'},
  ...
  //print(ui.Chart(dataTable, 'LineChart', options));
  var chart_CO = ui.Chart(dataTable, 'LineChart', options);
  //panel2.add(chart);
  panel2.widgets().set(2, chart_CO);
})
//----------------------------------------------------------Generar Histograma---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var options_CO = {
...
var Data_CO = ee.ImageCollection('COPERNICUS/S5P/NRTI/L3_CO')
  ...
  .mean()
  //.multiply(1e6);
var Histograma_CO=  ui.Chart.image.histogram(Data_CO, studyarea3, 1000, null, null, null).setOptions(options_CO);
//----------------------------------------------------Generar Grafico doble---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start1_CO = '2020-01-01'
var end2_CO = '2021-12-01'
var collectionCO = sentinel5_CO.select('CO_column_number_density')
                   .filterDate(start1_CO,end2_CO)
                   .filter(ee.Filter.calendarRange(60, 364, 'day_of_year'));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The GHSL global population density dataset for 2015.
var ghslPop_CO = ee.Image('JRC/GHSL/P2016/POP_GPW_GLOBE_V1/2015');
var Datas_CO = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_CO")
var filtered_CO = Datas_CO.filterBounds(studyarea3)
.filterDate(Fecha_1,Fecha_2)
.map(function(image){return image.clip(studyarea3)});
var Coll_CO = filtered_CO.select("CO_column_number_density").mean()//.multiply(1e6);
// combine two images
var Combine_Pob_CO = ghslPop_CO.addBands(Coll_CO)
// sample N points from the 2-band image
var scale = Map.getScale()
var values_CO = Combine_Pob_CO.sample({ region: studyarea3, scale: scale, numPixels: 2000, geometries: true}) 
var chart_Pob_CO = ui.Chart.feature.byFeature(values_CO, 'population_count', ['CO_column_number_density'])
  .setChartType('ScatterChart')
  .setOptions({ 
    pointSize: 2, 
    pointColor: 'red', 
...
//print(chart);
var Cargando_CO = ui.Label({value: "Cargando...."});
panel2.widgets().set(4, Cargando_CO);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Collection
var Col_CH4 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_CH4");
var studyarea3 = studyarea3;
print(studyarea3)
//Date
var start_CH4 = ee.Date(Fecha_1);
var end_CH4 = ee.Date(Fecha_2);
//Filter Collection
var modL_CH4 = Col_CH4.filterBounds(studyarea3)
              .filterDate(start_CH4,end_CH4)
              .select('CH4_column_volume_mixing_ratio_dry_air');
var modC_CH4 = modL_CH4.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
...
function dailyImages_CH4 (images,startdate,enddate){
   ...
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = end_CH4.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDay_CH4 = ee.ImageCollection(dailyImages_CH4 (modC_CH4,start_CH4,end_CH4)).filter(ee.Filter.gt('nbands',0));
// a bit more sophisticated, print a chart with median, 5%, 95% over geometry
var timeSeries_CH4 = imagePerDay_CH4.sort('system:time_start').map(function(i) {
...
// evaluate values, convert into a data table and print chart
timeSeries_CH4.evaluate(function(ts) {
  var dataTable = {
    cols: [{id: 'x', type: 'date'},
           {id: 'y', type: 'number'},
           {id: 'i0', type: 'number', role: 'interval'},
  ...
  //print(ui.Chart(dataTable, 'LineChart', options));
  var chart_CH4 = ui.Chart(dataTable, 'LineChart', options);
  panel2.widgets().set(2, chart_CH4);
})
//----------------------------------------------------------Generar Histograma---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var options_CH4 = {
...
var Data_CH4 = ee.ImageCollection('COPERNICUS/S5P/OFFL/L3_CH4')
  ...
  .mean()
  //.multiply(1e6);
var Histograma_CH4=  ui.Chart.image.histogram(Data_CH4, studyarea3, 1000, null, null, null).setOptions(options_CH4);
//----------------------------------------------------Generar Grafico doble---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start1_CH4 = '2020-01-01'
var end2_CH4 = '2021-12-01'
var collectionCH4 = sentinel5_CH4.select('CH4_column_volume_mixing_ratio_dry_air')
                   .filterDate(start1_CH4,end2_CH4)
                   .filter(ee.Filter.calendarRange(60, 364, 'day_of_year'));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The GHSL global population density dataset for 2015.
var ghslPop_CH4 = ee.Image('JRC/GHSL/P2016/POP_GPW_GLOBE_V1/2015');
var Datas_CH4 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_CH4")
var filtered = Datas_CH4.filterBounds(studyarea3)
.filterDate(Fecha_1,Fecha_2)
.map(function(image){return image.clip(studyarea3)});
var Coll_CH4 = filtered.select("CH4_column_volume_mixing_ratio_dry_air").mean()//.multiply(1e6);
// combine two images
var Combine_Pob_Ch4 = ghslPop_CH4.addBands(Coll_CH4)
// sample N points from the 2-band image
var scale = Map.getScale()
var values_CH4 = Combine_Pob_Ch4.sample({ region: studyarea3, scale: scale, numPixels: 2000, geometries: true}) 
var chart_Pob_CH4 = ui.Chart.feature.byFeature(values_CH4, 'population_count', ['CH4_column_volume_mixing_ratio_dry_air'])
  .setChartType('ScatterChart')
  .setOptions({ 
    pointSize: 2, 
    pointColor: 'red', 
...
//print(chart);
var Cargando_Ch4 = ui.Label({value: "Cargando...."});
panel2.widgets().set(4, Cargando_Ch4);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start_O3 = ee.Date(Fecha_1);
var end_O3 = ee.Date(Fecha_2);
var Col_O3 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_O3");
var modL_O3 = Col_O3.filterDate(start_O3,end_O3).select('O3_column_number_density');
var modC_O3 = modL_O3.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function dailyImages_O3 (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = end_O3.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDay_O3 = ee.ImageCollection(dailyImages_O3 (modC_O3,start_O3,end_O3)).filter(ee.Filter.gt('nbands',0));
// a bit more sophisticated, print a chart with median, 5%, 95% over geometry
var timeSeries_O3 = imagePerDay_O3.sort('system:time_start').map(function(i) {
...
// evaluate values, convert into a data table and print chart
timeSeries_O3.evaluate(function(ts) {
  var dataTable = {
    cols: [{id: 'x', type: 'date'},
           {id: 'y', type: 'number'},
           {id: 'i0', type: 'number', role: 'interval'},
  ...
  //print(ui.Chart(dataTable, 'LineChart', options));
  var chart_O3 = ui.Chart(dataTable, 'LineChart', options);
  //panel2.add(chart);
  panel2.widgets().set(2, chart_O3);
})
//----------------------------------------------------------Generar Histograma---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var options_O3 = {
...
var Data_O3 = ee.ImageCollection('COPERNICUS/S5P/OFFL/L3_O3')
  ...
  .mean()
  //.multiply(1e6);
var Histograma_O3=  ui.Chart.image.histogram(Data_O3, studyarea3, 1000, null, null, null).setOptions(options_O3);
//----------------------------------------------------Generar Grafico doble---------------------------------------------//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var start1_O3 = '2020-01-01'
var end2_O3 = '2021-12-01'
var collectionO3 = sentinel5_O3.select('O3_column_number_density')
                   .filterDate(start1_O3,end2_O3)
                   .filter(ee.Filter.calendarRange(60, 364, 'day_of_year'));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The GHSL global population density dataset for 2015.
var ghslPop_O3 = ee.Image('JRC/GHSL/P2016/POP_GPW_GLOBE_V1/2015');
var Datas_O3 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_O3")
var filtered_O3 = Datas_O3.filterBounds(studyarea3)
.filterDate(Fecha_1,Fecha_2)
.map(function(image){return image.clip(studyarea3)});
var Coll_O3 = filtered_O3.select("O3_column_number_density").mean()//.multiply(1e6);
// combine two images
var Combine_Pob_O3 = ghslPop_O3.addBands(Coll_O3)
// sample N points from the 2-band image
var scale = Map.getScale()
var values_O3 = Combine_Pob_O3.sample({ region: studyarea3, scale: scale, numPixels: 2000, geometries: true}) 
var chart_Pob_O3 = ui.Chart.feature.byFeature(values_O3, 'population_count', ['O3_column_number_density'])
  .setChartType('ScatterChart')
  .setOptions({ 
    pointSize: 2, 
    pointColor: 'red', 
...
//print(chart);
var Cargando_O3 = ui.Label({value: "Cargando...."});
panel2.widgets().set(4, Cargando_O3);
...
if (contaminacion === "NO2"){ 
var Coleccion_NO2 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_NO2");
var startNO2 = ee.Date(Fecha_1);
var endNO2 = ee.Date(Fecha_2);
//var dateRange = ee.DateRange(start, start.advance(3, 'month'));
var FiltroNO2 = Coleccion_NO2.filterDate(startNO2,endNO2).select('NO2_column_number_density');
//var modL = NO2.filterDate(dateRange).select('NO2_column_number_density');
var Property_NO2 = FiltroNO2.map(function(image) {
  return image.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function ImagenDiariaNO2 (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = endNO2.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDayNO2 = ee.ImageCollection(ImagenDiariaNO2 (Property_NO2,startNO2,endNO2));
// Chart the time-series
var Grafico_NO2 = ui.Chart.image.series({
  imageCollection: imagePerDayNO2,
  region: point,
...
lowerPanel.widgets().reset([Grafico_NO2]); 
}
if (contaminacion === "SO2"){ 
var Coleccion_SO2 = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_SO2");
var startSO2 = ee.Date(Fecha_1);
var endSO2 = ee.Date(Fecha_2);
//var dateRange = ee.DateRange(start, start.advance(3, 'month'));
var FiltroSO2 = Coleccion_SO2.filterDate(startSO2,endSO2).select('SO2_column_number_density');
var Property_SO2 = FiltroSO2.map(function(image) {
  return image.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function dailyImagesSO2 (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = endSO2.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDaySO2 = ee.ImageCollection(dailyImagesSO2 (Property_SO2,startSO2,endSO2));
// Chart the time-series
var Grafico_SO2 = ui.Chart.image.series({
  imageCollection: imagePerDaySO2,
  region: point,
  reducer: ee.Reducer.mean(),
  scale:1000,
   xProperty: 'system:time_start'})
...
lowerPanel.widgets().reset([Grafico_SO2]); 
} 
if (contaminacion === "CO") { 
var Coleccion_CO = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_CO");
var startCO = ee.Date(Fecha_1);
var endCO = ee.Date(Fecha_2);
//var dateRange = ee.DateRange(start, start.advance(3, 'month'));
var FiltroCO = Coleccion_CO.filterDate(startCO,endCO).select('CO_column_number_density');
var PropertyCO = FiltroCO.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
...
function dailyImages (images,startdate,enddate){
   ...
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = endCO.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDayCO = ee.ImageCollection(dailyImages (PropertyCO,startCO,endCO));
// Chart the time-series
var Grafico_CO = ui.Chart.image.series({
  imageCollection: imagePerDayCO,
  region: point,
  reducer: ee.Reducer.mean(),
  scale:1000,
   xProperty: 'system:time_start'})
...
lowerPanel.widgets().reset([Grafico_CO]); 
}
if (contaminacion === "CH4") { 
var Coleccion_CH4 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_CH4");
var startCH4 = ee.Date(Fecha_1);
var endCH4 = ee.Date(Fecha_2);
//var dateRange = ee.DateRange(start, start.advance(3, 'month'));
var Filtro_CH4 = Coleccion_CH4.filterDate(startCH4,endCH4).select('CH4_column_volume_mixing_ratio_dry_air');
var Property_CH4 = Filtro_CH4.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function dailyImagesCH4 (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = endCH4.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDayCH4 = ee.ImageCollection(dailyImagesCH4 (Property_CH4,startCH4,endCH4));
// Chart the time-series
var Grafico_CH4 = ui.Chart.image.series({
  imageCollection: imagePerDayCH4,
  region: point,
  reducer: ee.Reducer.mean(),
  scale:1000,
   xProperty: 'system:time_start'})
...
lowerPanel.widgets().reset([Grafico_CH4]); 
}
if (contaminacion === "O3") { 
var Coleccion_O3 = ee.ImageCollection("COPERNICUS/S5P/OFFL/L3_O3");
var startO3 = ee.Date(Fecha_1);
var endO3 = ee.Date(Fecha_2);
//var dateRange = ee.DateRange(start, start.advance(3, 'month'));
var FiltroO3 = Coleccion_O3.filterDate(startO3,endO3).select('O3_column_number_density');
var PropertyO3 = FiltroO3.map(function(image) {
  return image//.multiply(1e6)
    .copyProperties(image, ['system:time_start'])
    .set({date: image.date().format('yyyy-MM-dd * HH-mm-ss')});
});
// create one image per day
function dailyImagesO3 (images,startdate,enddate){
   startdate = ee.Date(startdate);
   enddate = ee.Date(enddate);
   var originDate = startdate;
   var count = endO3.difference(startdate, 'day').round();
   var listOfYear = ee.List.sequence(1, count, 1);
   return ee.ImageCollection(listOfYear.map(function(i){
     i = ee.Number(i);
     var date_start = originDate.advance(i.add(-1), 'day');
 ...
 var imagePerDayO3 = ee.ImageCollection(dailyImagesO3 (PropertyO3,startO3,endO3));
// Chart the time-series
var Grafico_O3 = ui.Chart.image.series({
  imageCollection: imagePerDayO3,
  region: point,
  reducer: ee.Reducer.mean(),
  scale:1000,
   xProperty: 'system:time_start'})
...
lowerPanel.widgets().reset([Grafico_O3]); 
}
}
