var imgSoc = ui.import && ui.import("imgSoc", "image", {
    ...
    ftc2 = ui.import && ui.import("ftc2", "table", {
      "id": "users/projectgeffao/Jordan/PreCalLevel2_Jordan_v1"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/PreCalLevel2_Jordan_v1"),
    ...
    imgCombined3Cat = ui.import && ui.import("imgCombined3Cat", "image", {
      "id": "users/projectgeffao/World/Coper_LC2019x100_plus_LPDx10_plus_Mont1_world"
    }) || ee.Image("users/projectgeffao/World/Coper_LC2019x100_plus_LPDx10_plus_Mont1_world"),
    imgTerrain = ui.import && ui.import("imgTerrain", "image", {
      "id": "users/projectgeffao/Jordan/Terrain_rgb3_30m_Jordan"
    }) || ee.Image("users/projectgeffao/Jordan/Terrain_rgb3_30m_Jordan"),
    ...
    ftc0 = ui.import && ui.import("ftc0", "table", {
      "id": "users/projectgeffao/Jordan/JOR_Precal_Level0_v3_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/JOR_Precal_Level0_v3_Fix"),
    ftc1 = ui.import && ui.import("ftc1", "table", {
      "id": "users/projectgeffao/Jordan/JOR_Precal_Level1_v3_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/JOR_Precal_Level1_v3_Fix"),
    ftcProjectUnits = ui.import && ui.import("ftcProjectUnits", "table", {
      "id": "users/projectgeffao/Jordan/JOR_Precal_Pilots_v3_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/JOR_Precal_Pilots_v3_Fix"),
    ftcLandscape = ui.import && ui.import("ftcLandscape", "table", {
      "id": "users/projectgeffao/Jordan/JOR_Precal_Landscape_Area_v3_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/JOR_Precal_Landscape_Area_v3_Fix"),
    ftcWholeArea = ui.import && ui.import("ftcWholeArea", "table", {
      "id": "users/projectgeffao/Jordan/JOR_Precal_Landscape_WholeArea_v3_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Jordan/JOR_Precal_Landscape_WholeArea_v3_Fix");
// Authors for this App are:
// Eugenia Raviolo - eugenia.raviolo@gmail.com
// Cesar Garcia - cesarnon@gmail.com
// Ingrid Teich - ingridteich@gmail.com  g
/** Modules */ 
var mdlLegends = require('users/projectgeffao/modules:legends.js');
var mdlNdvi = require('users/projectgeffao/modules:ndvi.js');
var mdlLocalization = require('users/projectgeffao/Jordan:Apps/localization.js');
/** Global variables */
...
ftcKBA = ftcKBA.filterMetadata('ISO3', 'equals', 'JOR');
ftcPA = ftcPA.filterMetadata('ISO3', 'equals', 'JOR');
...
var referenceDocUrl = 'https://drive.google.com/file/d/1Pe-P7iRya0WqUvQOlsyP3uW-qo1tRtj4/view?usp=sharing';
var contactEmail = 'cesar.garcia@fao.org';
var logoAssetId = 'users/projectgeffao/Jordan/Logo_Jordan3';
var logoDimensions = '1986x633';
...
var ftcClickOn = ftcProjectUnits;
var levelNDVI;
...
var vis = {
    ...
    projectAreas: { color: "purple", fillColor: "d13eff20" },
    landscape: { color: '#11ff00', fillColor: "ffffff30" },
    highlight: { color: 'red', fillColor: '00000000' }, // a422ff
    soc: { min: 0, max: 60, palette: paletteSOC, },
    custom: { max: 1, min: 1, opacity: 1, palette: ['#FF00FF'] },//DF8F05 FF7EF2
    terrain: { min: -300, max: 1800, palette: paletteTerrain },
    pa: { color: 'green' },
    kba: { color: 'orange' },
...
function initApp(lan) {
    ...
    var pnlTerrain = mdlLegends.createColorRampLegendPanel(labels.lblTerrain + ' (m)', vis.terrain);
    pnlTerrain.widgets().get(2).widgets().get(1).setValue('700');
    ...
    layerEntries = [
        ...
        },
        {
            image: imgSoc,
            style: vis.soc,
            name: labels.lblSoilOrganicCarbon,
            visible: false,
        ...
        },
        {
            ...
            style: vis.fgl,
            name: labels.lblForestGainsLosses + ' ' + defaultInitialLCYear + '-' + defaultFinalLCYear,
            visible: false,
...
function initControlPanel(pnlControl, lan) {
    ...
    pnlControl.add(chbOnlyProjectAreas)
        .add(selectLevel1);
    //.add(selectLevel2);
    var selAssetClick = ui.Select({
        items: [labels.lblLandscape, labels.lblProjectAreas, labels.lblLevel1],
        style: { width: '70%' },
        value: labels.lblProjectAreas,
    });
    selAssetClick.onChange(function (v) {
        if (v === labels.lblLandscape) {
            ftcClickOn = ftcLandscape;
        }
        else if (v === labels.lblProjectAreas) {
            ftcClickOn = ftcProjectUnits;
        }
        else {
            ftcClickOn = ftc1;
        }
    });
    pnlControl.add(ui.Label({
        value: labels.lblAssetClick,
        style: { fontSize: '12px', margin: '5px 0px 0px 10px' },
    }));
    pnlControl.add(selAssetClick);
    var listLayersChb = [];
    ...
    var initControlPanelWelcome = function () {
        ...
        var pnlWelcome = ui.Panel({
            ...
            widgets: [
                ...
                ui.Label(labels.lblCloseWarning, { color: 'grey', fontSize: '10px' }),
            ]
        });
        map.add(pnlWelcome);
...
function initMapPanel(pnlMap) {
    ...
    var handleClickMap = function (coords) {
        map.widgets().remove(pnlCombinedLegend);
        ftcAoi = ftcClickOn.filterBounds(ee.Geometry.Point(coords.lon, coords.lat));
    ...
    var btnSelectWholeArea = ui.Button({
        label: labels.lblSelectContainer2,
        style: {
            position: "bottom-right",
        },
        onClick: function () {
            selectContainer(ftcWholeArea);
        }
    });
    map.add(btnSelectContainer);
    map.add(btnSelectWholeArea);
...
function showInfoSelectedAoi() {
    ...
    selectedArea.get('areaHa').getInfo(function (pArea) {
        ...
        selectedArea.get('Forest_Gain').getInfo(function (pValue) {
            var aux = pArea > 0 ? (pValue * 100 / pArea) : 0;
            infoEntries['forestGain'].lblValue.setValue(formatNumber(pValue, 0) + " ha. (" + aux.toFixed(1) + "%)");
        });
        selectedArea.get('Forest_Loss').getInfo(function (pValue) {
            var aux = pArea > 0 ? (pValue * 100 / pArea) : 0;
...
function hotspots() {
    ...
    if (sectionIndex === -1) {// use combined image           
        ...
        // Calculate image        
        imgCustom = getFilteredImage(imgCombined3Cat, catNumbers);
