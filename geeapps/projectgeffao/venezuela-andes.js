var imgSoc = ui.import && ui.import("imgSoc", "image", {
    ...
    imgTransitions = ui.import && ui.import("imgTransitions", "image", {
      "id": "users/projectgeffao/World/ESA_LC_Transitions_World"
    }) || ee.Image("users/projectgeffao/World/ESA_LC_Transitions_World"),
    imgForestChange = ui.import && ui.import("imgForestChange", "image", {
      "id": "users/projectgeffao/World/ESA_ForestChange92_18_1loss_2gain"
    }) || ee.Image("users/projectgeffao/World/ESA_ForestChange92_18_1loss_2gain"),
    imgCombined3 = ui.import && ui.import("imgCombined3", "image", {
      "id": "users/projectgeffao/Venezuela/CoperLC2019x100_plus_LPD2020x10_plus_Montx1_Venezuela"
    }) || ee.Image("users/projectgeffao/Venezuela/CoperLC2019x100_plus_LPD2020x10_plus_Montx1_Venezuela"),
    imgCombined2 = ui.import && ui.import("imgCombined2", "image", {
      "id": "users/projectgeffao/Venezuela/CoperLC2019x10_plus_LPD2020_Venezuela"
    }) || ee.Image("users/projectgeffao/Venezuela/CoperLC2019x10_plus_LPD2020_Venezuela"),
    imgLcAll9Cat = ui.import && ui.import("imgLcAll9Cat", "image", {
      "id": "users/projectgeffao/Venezuela/Copernicus_LC_9cat_2015_2019_Venezuela"
    }) || ee.Image("users/projectgeffao/Venezuela/Copernicus_LC_9cat_2015_2019_Venezuela"),
    imgLcAll = ui.import && ui.import("imgLcAll", "image", {
      "id": "users/projectgeffao/World/ESA_LC_UNCCD_1992_2018_World"
    }) || ee.Image("users/projectgeffao/World/ESA_LC_UNCCD_1992_2018_World"),
    ftcBasins = ui.import && ui.import("ftcBasins", "table", {
      "id": "users/projectgeffao/Venezuela/RegionesHidrograficasVenezuela"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/RegionesHidrograficasVenezuela"),
    ftcPaMN = ui.import && ui.import("ftcPaMN", "table", {
      "id": "users/projectgeffao/Venezuela/SistemaMN"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/SistemaMN"),
    ftcPa = ui.import && ui.import("ftcPa", "table", {
      "id": "users/projectgeffao/Venezuela/SistemaPN"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/SistemaPN"),
    imgTerrain = ui.import && ui.import("imgTerrain", "image", {
      "id": "users/projectgeffao/Venezuela/Terrain_rgb3b_Venezuela"
    }) || ee.Image("users/projectgeffao/Venezuela/Terrain_rgb3b_Venezuela"),
    ftc0 = ui.import && ui.import("ftc0", "table", {
      "id": "users/projectgeffao/Venezuela/PreCalLevel0_VEN_v6_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/PreCalLevel0_VEN_v6_Fix"),
    ftc1 = ui.import && ui.import("ftc1", "table", {
      "id": "users/projectgeffao/Venezuela/PreCalLevel1_VEN_v6_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/PreCalLevel1_VEN_v6_Fix"),
    ftc2 = ui.import && ui.import("ftc2", "table", {
      "id": "users/projectgeffao/Venezuela/PreCalLevel2_VEN_v6_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/PreCalLevel2_VEN_v6_Fix"),
    ftcLimit = ui.import && ui.import("ftcLimit", "table", {
      "id": "users/projectgeffao/Venezuela/PreCalProjectCota_VEN_v6_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/PreCalProjectCota_VEN_v6_Fix"),
    ftcProject = ui.import && ui.import("ftcProject", "table", {
      "id": "users/projectgeffao/Venezuela/PreCalProjectUnidades_VEN_v6_Fix"
    }) || ee.FeatureCollection("users/projectgeffao/Venezuela/PreCalProjectUnidades_VEN_v6_Fix");
...
var mdlLocalization = require('users/projectgeffao/Venezuela:Apps/localization.js');
/** Global variables */
...
ftcKba = ftcKba.filterMetadata("ISO3", "equals", "VEN");
...
var defaultInitialLCYear = '1992';
var defaultFinalLCYear = '2018';
var defaultFinalLCYear9Cat = '2019';
var lcInitialYears = ['1992', '2000', '2005', '2010', '2015'];
var referenceDocUrl = 'https://www.thegef.org/project/integrated-management-multiple-use-landscapes-and-high-conservation-value-forest-sustainable';
var contactEmail = 'ingrid.teich@fao.org';
var logoAssetId = 'users/projectgeffao/Venezuela/LogoVenezuela';
...
var namesFGL;
// Visualization
var paletteLC9Cat = ['#377e3f', '#42c94c', '#c19511', '#fcdb00', '#da78e6', '#d7191c', '#cfdad2', '#18eebe', '#4458eb'];
...
var paletteMountainsHotspots = ['orange', '#c5fff8', '#95dbd3', '#92db9c', '#55c364', '#8b9c15', '#d99c22', '#9e7219'];
...
var vis = {
    ...
    lc9Cat: { min: 1, max: 9, opacity: 1, palette: paletteLC9Cat },
    ...
    projectAreas: { color: 'red', fillColor: "ffffff00" },
    watersheds: { color: 'blue', fillColor: "ffffff00", width: 1 },
    limit: { color: 'yellow', fillColor: "ffffff00" },
...
function initApp(lan) {
    ...
    ftcLimit=ftcLimit.map(function(f){return f.set('name',labels.lblLimit)});
    // Categories labels
    namesLC9Cat = [
        labels.lblForestClose,
        labels.lblForestOpen,
        labels.lblShrubland,
        labels.lblGrassland,
        labels.lblCropland,
        labels.lblUrban,
    ...
    layerEntries = [
        ...
        {
            image: imgLcAll9Cat.select('y' + defaultFinalLCYear9Cat).clip(ftc0),
            style: vis.lc9Cat,
            name: labels.lblLandCover + ' ' + defaultFinalLCYear9Cat,
            visible: true,
            legend: mdlLegends.createDiscreteLegendPanel(labels.lblLandCover + ' ' + defaultFinalLCYear9Cat, namesLC9Cat, vis.lc9Cat.palette, false, false),
            group: 'GENERAL',
        },
        {
            image: imgLpd,
        ...
        {
            image: ftcBasins,
            style: vis.watersheds,
            name: labels.lblWatersheds,
            visible: false,
            legend: null,
            group: 'AREAS',
            singleColor: 'SQUARE',
        ...
        },
        {
            image: ftcPa,
            style: vis.pa,
            name: labels.lblProtectedAreasNP,
            visible: false,
            legend: null,
            group: 'AREAS',
            singleColor: 'SQUARE',
        },
        {
            image: ftcPaMN,
            style: vis.paMN,
            name: labels.lblProtectedAreasMN,
            visible: false,
            legend: null,
            group: 'AREAS',
            singleColor: 'SQUARE',
    ...
    hotspotsEntries = [
        {
            ...
            palette: vis.lc9Cat.palette,
            names: namesLC9Cat,
            prefix: 'lc_',
            sufix: '_' + defaultFinalLCYear9Cat,
            addZeroCategory: false,
            image: imgLcAll.select('y' + defaultFinalLCYear9Cat),
        ...
        sldOpacity.onSlide(function (value) {
            ...
            for (var i = layersArray.length - 1; i >= 0; i--) {
                ...
                if (lastShown === null
                    ...
                    && l.getName() !== labels.lblWatersheds
                    && l.getName() !== labels.lblLimit
                    && l.getShown()
                ) {
...
function initMapPanel(pnlMap) {
    ...
    var handleClickMap = function (coords) {
        map.widgets().remove(pnlCombinedLegend);
        ftcAoi = ftcProject.filterBounds(ee.Geometry.Point(coords.lon, coords.lat));
    ...
    var limitView = function () {
        resetNamesLevel1();
        levelNDVI = true;
        ftcAoi = ftcLimit; // TODO CHANGE TO limit
        map.centerObject(ftcAoi);
        showInfoSelectedAoi();
        clearCombinedLayerAndLegend();
    }
    ...
    var btnSelectLimit = ui.Button({
        label: labels.lblSelectContainer2,
        style: {
            position: "bottom-right",
        },
        onClick: limitView,
...
function showInfoSelectedAoi() {
    ...
    selectedArea.get('gsoc_mean').getInfo(function (pValue) {
        infoEntries['socMean'].lblValue.setValue(pValue.toFixed(0) + ' tonnes/ha.');
    });
    map.centerObject(ftcAoi);
    ...
    setupGeneralCharts(pnlGeneralCharts, defaultFinalLCYear9Cat);
    setupHotspotsCharts(pnlHotspotsCharts, defaultFinalLCYear9Cat);
...
function hotspots() {
    ...
    if (sectionIndex === -1) {// use combined image           
        ...
        print(combinedCatNames, catNumbers)
    }
    else {
...
function setupCombined(pImage, pLegendTitle, pLegendText, pFromChart) {
    ...
    pImage.getDownloadURL(options, function (url) {
        lblDownloadText.setValue(labels.lblGeneratingDownloadLink);
        if (url !== null) {
...
function setupGeneralCharts(pPnlContainer, pLCFinalYear) {
    ...
    if (true) {
        var lstFeatLC = namesLCColumns.map(function (pName, i) {
            var lstValues = ee.List([namesLC9Cat[i], ftcAoi.first().get(pName + '_' + pLCFinalYear)]);
    ...
    // HANSEN Forest loss
    if (true) {
        var yearsHansen = [2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010,
            2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019];
        var lstFeatForestLossByYear = yearsHansen.map(function (i) {
            var v = ftcAoi.first().get('Han_' + i);
            var lstValues = ee.List([i, v]);
            return ee.Feature(null, { row: lstValues });
        });
        ...
        var optionsForestLossByLC = {
            title: labels.lblDeforestation,
            legend: { position: 'none' },
        };
        createChart(lstHeaderForesLossByYear.cat(ee.FeatureCollection(lstFeatForestLossByYear).aggregate_array('row')), optionsForestLossByLC, 'ColumnChart', createChartPanel(pPnlContainer));
    }
    //  NDVI
    if (true && levelNDVI) {
        // NDVI monthly values
    ...
    if (true) {
        var lstFeatLCTransTable = catsLC.map(function (i) {
            ...
            var values = catsLC.map(function (c) {
                return ee.Number(ftcAoi.first().get(transition + '_' + c)).format('%.0f');
            });
            var lstValues = ee.List([namesLC[i - 1]]).cat(values);
            return ee.Feature(null, { row: lstValues });
        });
