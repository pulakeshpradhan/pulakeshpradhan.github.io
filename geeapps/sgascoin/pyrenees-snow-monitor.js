...
var minSnowDays = 10;
// time period
var t1i = ee.Date('2000-11-01'); // start date climatology + 1 day
...
var s2 = ee.String('Current')
         .cat(t2i.format(' MMM-dd').cat(' to ').cat(t2f.format('MMM-dd'))).getInfo();
var maxLeg = t2f.difference(t2i,'days');
// color palette 
var p=["edf8fb","b3cde3","8c96c6","8856a7","810f7c"];
var vis={min:minSnowDays, max:maxLeg.getInfo(), palette:p};
...
Map1.addLayer(im1.updateMask(m1), vis, 'SCD climatology',true,0.7);
...
Map2.addLayer(im2.updateMask(m2), vis, 'SCD Jan-Jun this year',true,0.7);
