var qapt = ui.import && ui.import("qapt", "table", {
      "id": "users/durvaljd/qa_reserv"
    }) || ee.FeatureCollection("users/durvaljd/qa_reserv"),
    ...
    ee.Geometry.Polygon(
        [[[-54.88055526194514, -24.691683030910287],
          [-54.88055526194514, -25.455416174302297],
          [-54.08404647288264, -25.455416174302297],
          [-54.08404647288264, -24.691683030910287]]], null, false),
    itaipu = ui.import && ui.import("itaipu", "table", {
      "id": "users/durvaljd/bracos_princ"
    }) || ee.FeatureCollection("users/durvaljd/bracos_princ");
...
//var geometry= ee.Feature(ee.FeatureCollection("users/odraitaipu/bracos/saoFranciscoVerdadeiro").first()).set('nome','São Francisco Verdadeiro').geometry();
var itaipu=itaipu.first().set('nome','braços').geometry();
print(itaipu)

Map.centerObject(itaipu, 10)
// Import Collections //
// landsat 8 raw dn
...
var startYear = 2013;
var endYear = 2017;
var forMask = SRP.filterBounds(itaipu).select('B6').filterMetadata('CLOUD_COVER', "less_than", 10).filter(ee.Filter.calendarRange(startMonth, endMonth, 'month')).filter(ee.Filter.calendarRange(startYear, endYear, 'year'));
var mask = ee.Image(forMask.select('B6').median().lt(600)) 
mask = mask.updateMask(mask)
// filter landsat 8 collection
var FC_OLI = OLI_DN.filterDate(iniDate, endDate).filterBounds(itaipu).filterMetadata('CLOUD_COVER', "less_than", oliCloudPerc);
...
Map.addLayer(sd_coll.mean(), {min: 1, max: 3, palette: ['800000', 'FF9700', '7BFF7B', '0080FF', '000080']}, 'Zsd [m]', false);
Map.addLayer(tsi_coll.mean(), {min: 48, max: 60, palette: ['cyan','skyblue','blue' ,'limegreen','yellow','orange','orangered','red']}, 'TSI Landsat', true);
//Map.addLayer(tsi_collR.mode(), {min: 1, max: 7, palette: ['purple', 'blue', 'limegreen', 'yellow', 'orange', 'orangered', 'darkred']}, 'TSI Reclass', true);
Map.addLayer(itaipu,{},'SFV',false)

// Time Series //
...
var lstTimeSeries = ui.Chart.image.seriesByRegion(
    lst_coll, itaipu, ee.Reducer.mean())
...
var sdTimeSeries = ui.Chart.image.seriesByRegion(
    sd_coll, itaipu, ee.Reducer.mean())
        .setChartType('ScatterChart')
        .setOptions({
...
var tsiTimeSeries = ui.Chart.image.seriesByRegion(
    tsi_coll, itaipu, ee.Reducer.mean())
        .setChartType('ScatterChart')
        .setOptions({
...
var tsiRTimeSeries = ui.Chart.image.seriesByRegion(
    tsi_collR, itaipu, ee.Reducer.mean())
        .setChartType('ScatterChart')
        .setOptions({
...
//entrei com :
var funcao = function(image, list) {
  var img = ee.Image(image);
  var ftColect = img.reduceRegions(qapt,ee.Reducer.mean(),10);
  var eedate = ee.Date(image.get('system:time_start'));
  var dateTime = eedate.format("dd/MM/YYYY");
  return ee.List(list).add(ftColect.set('data',dateTime));
};
var listaChuva = ee.List(tsi_coll.iterate(funcao, ee.List([])));
print(listaChuva)
var ids = ee.List(ee.FeatureCollection(listaChuva.get(0)).reduceColumns(ee.Reducer.toList(),['cd_ponto']).get('list'));
//var funcao = ;
var final = ee.List(listaChuva.iterate(function(item, list){
  var ft = ee.FeatureCollection(item); 
  var reduced = ee.List(ft.reduceColumns(ee.Reducer.toList(),['mean']).get('list'));
  var dt = ft.get('data');
  var keys = ee.List([ids.join(';'),'data']);
  var values = ee.List([reduced.join(';'), dt]);
  var dict = ee.Dictionary.fromLists(keys, values);
  return ee.List(list).add(ee.Feature(null,dict));
}, ee.List([])));
print(ee.FeatureCollection(final));
//Export.table.toDrive(ee.FeatureCollection(final));