...
var dicobjetos = {
  ...
  select:{
    ...
    Map.addLayer(table.filter(ee.Filter.inList('DPA_DESPRO', [i])))
    Map.centerObject(table.filter(ee.Filter.inList('DPA_DESPRO', [i])), 8 )
   }}),
  }
  //label: ui.Label('Min: ', {}),
...
var layer
 if(aux == "CO_column_number_density") {print()
  ...
  var COClip = COData.clip (puntos_CO);
    Map.addLayer (COClip, {
    max: 0.05, 
    min: 0.0, 
  ...
  Titulo = ui.Label({
    ...
    style: {fontWeight: 'bold', fontSize: '1em', margin: '0px 0px 15px 0px',}}); // Estilo y dimensiones
  Leyenda = ui.Panel({
  ...
  for ( i = 0; i < 7; i++) {Leyenda.add(Simbolos(Simbologia[i], Etiquetas[i]));} 
  ...
  var NO2TotalClip = NO2TotalData.clip (puntos_NO2);
  ...
  return ui.Panel({
    ...
    layout: ui.Panel.Layout.Flow('horizontal')});};
  for (i = 0; i < 7; i++) {Leyenda.add(Simbolos(Simbologia[i], Etiquetas[i]));} 
  Map.centerObject (NO2TotalClip, 10);
  Map.add(Leyenda);
  ...
  var O3TotalClip = O3TotalData.clip (puntos_O3);
  ...
  return ui.Panel({
    ...
    layout: ui.Panel.Layout.Flow('horizontal')});};
  for (i = 0; i < 7; i++) {Leyenda.add(Simbolos(Simbologia[i], Etiquetas[i]));} 
  Map.centerObject (O3TotalClip, 10);
  Map.add(Leyenda);
}
}
})
