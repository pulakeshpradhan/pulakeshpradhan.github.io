...
var image = ee.Image('users/adriadescals/shared/MS_pheno_2019_MODIS_Lai_p50_wSize30_scale5000')
var SoS_MS = image.select('SoS');
var EoS_MS = image.select('EoS');
// var SoS_MS = image.select('SoS_mean').rename('SoS');
// var EoS_MS = image.select('EoS_mean').rename('EoS');
var phenoPalette = ['ff0000','ff8d00','fbff00','4aff00','00ffe7','01b8ff','0036ff','fb00ff']
var visDate_SoS1 = {min:60 ,max:300, "palette":phenoPalette};
var visDate_EoS1 = {min:60 ,max:300, "palette":phenoPalette};
...
var blankImage2 = ee.Image(60).set('doy',ee.Number(SoSdict.get('SoS')).add(0.5)).rename('SoS').int()
var blankImage3 = ee.Image(0).set('doy',EoSdict.get('EoS')).rename('EoS').int()
var blankImage4 = ee.Image(60).set('doy',ee.Number(EoSdict.get('EoS')).add(0.5)).rename('EoS').int()
...
Map.add(ui.Panel(
    [
      ui.Label('End of Season (EoS)', LEGEND_TITLE_STYLE), makeLegend(visDate_EoS1['min'],visDate_EoS1['max']),
...
Map.add(ui.Panel(
    [
      ui.Label('Start of Season (SoS)', LEGEND_TITLE_STYLE), makeLegend(visDate_SoS1['min'],visDate_SoS1['max']),
      ui.Label('(Day of Year)', LEGEND_FOOTNOTE_STYLE)
    ],
    ui.Panel.Layout.flow('vertical'),
    {width: '230px', position: 'bottom-left'}));
var resultsPanel = ui.Panel({style: {position: 'bottom-right',width: '500px'}});
  var instructionsLabel = ui.Label('Click on a pixel to inspect the LAI time series and the SoS and EoS metrics');
...
.setOptions({title: 'MODIS LAI (MCD15A3Hv6)', 
      ...
      series: {
        0: {pointSize: 0, lineWidth: 2, color: '2800ff'},
        1: {pointSize: 2, lineWidth: 0, color: '000000'},
        2: {pointSize: 0, lineWidth: 2, color: '3eff00'},
      }})
