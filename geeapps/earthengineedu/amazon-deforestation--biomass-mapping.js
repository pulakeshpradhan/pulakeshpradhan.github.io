var Amazon_Basin = ee.FeatureCollection("users/lgoldberg8000/GEE_Tutorials/Amazon_Basin_Outline"),
    indigenous_lands = ee.FeatureCollection("users/lgoldberg8000/GEE_Tutorials/Indigenous_Land_Rights_Brazil");
...
Map.addLayer(series.sum().clipToCollection(indigenous_lands),
    {min: -60000, max: 60000, palette: ['FF0000', '000000', '00FF00']}, 'NDVI/EVI Layer');
...
Map.addLayer(allLosses, {palette: ['FF0000']}, "Hansen's Forest Loss Layer");
// *********************************************************************************************************************
// Carbon stock
// *********************************************************************************************************************
...
Map.addLayer(carbonStats, carbonVisParams, 'MODIS Biomass Map');
// *********************************************************************************************************************
// Land cover
// *********************************************************************************************************************
...
Map.addLayer(landCover01_05, landCoverVisParams, 'Land Cover Classification 2001-2005');
Map.addLayer(landCover06_10, landCoverVisParams, 'Land Cover Classification 2006-2010');
Map.addLayer(landCover11_15, landCoverVisParams, 'Land Cover Classification 2011-2015');
Map.addLayer(landCover16_17, landCoverVisParams, 'Land Cover Classification 2016-2017');
// *********************************************************************************************************************
// Start of UI script
// *********************************************************************************************************************
// Legend maker
...
var title = ui.Panel([
  ui.Label({
    value: 'Amazon Deforestation & Biomass Mapping',
    style: TITLE_STYLE,
  })
]);
panel.add(title);
...
var forestLoss = ui.Panel([
  ui.Label({
    value:  'Deforestation in the Brazilian Amazon has significant implications on both carbon emissions and preservation of indigenous lands. '+
            'Use this tool to determine the locations and drivers of forest loss throughout the region, and consider how large-scale forest disturbance has '+
...
var redBlurb = ui.Label({
  value: 'Click on a polygon to plot deforestation trends over time.',
  style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '3b7042',
    padding: '0px 0px 0px 8px',
...
var eviCheckbox = ui.Checkbox('NDVI/EVI Layer', true);
...
var eviLegendTitle = ui.Label({
  value: 'Forest Greenness Change Legend',
  style: {
    fontWeight: 'bold',
    fontSize: '12px',
    margin: '0 0 4px 0',
...
var eviClassifications = ['Forest Loss', 'No Change', 'Forest Gain'];
for (var i = 0; i < 3; i++) {
  eviLegend.add(makeRow(eviPalette[i], eviClassifications[i]));
}
var fgaPanel = ui.Panel([
  ui.Label({
    value: 'Forest Greenness Analysis',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px',
  ...
  ui.Label({
    value:  'Changes in forest cover or greenness can be measured by means of the Normalized Difference Vegetation Index (NDVI), which uses satellite images to '+
            'calculate the reflection and absorbance of sunlight on leaves. Explore this map layer to determine regions of growth or loss in NDVI from 2000-2016.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '8px',
...
var hansenCheckbox = ui.Checkbox("Hansen's Forest Loss Layer", true);
...
var deforestationPanel = ui.Panel([
  ...
  ui.Label({
    value: 'The Hansen Global Forest Change dataset v1.6 provides a yearly distribution of the extent of forest change'+
    'on the global scale. Click on a polygon to graph the total area of forest loss each year from 2001 to 2018.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
var carbonCheckbox = ui.Checkbox('MODIS Biomass Map', true);
...
var carbonPanel = ui.Panel([
  ui.Label({
    value: 'Biomass Implications of Deforestation',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px'}
  ...
  ui.Label({
    value:  'Deforestation leads to drastic carbon emission due to the release of significant carbon stocks held in tropical forests. '+
            'Biomass represents the total weight of woody matter in a tree, while carbon represents only a portion of this matter- usually around 50% of a tree\'s total weight.'+
            'Use the MODIS biomass map to plot the total biomass lost through deforestation during each epoch of time. ',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
var landCoverButton = ui.Button({
  label: 'Land Cover Change Analysis',
  style: {padding: '0px 0px 0px 8px' },
 });
...
var landCover01_05Checkbox = ui.Checkbox('Land Cover Classification 2001-2005', true);
...
var landCover06_10Checkbox = ui.Checkbox('Land Cover Classification 2006-2010', true);
...
var landCover11_15Checkbox = ui.Checkbox('Land Cover Classification 2011-2015', true);
...
var landCover16_17Checkbox = ui.Checkbox('Land Cover Classification 2016-2017', true);
...
// landCoverLegend
var landCoverClassifications = ['Evergreen Broadleaf Forest', 'Grasslands', 'Savannas', 'Croplands', 'Natural Vegetation Mosaics'];
...
var landCoverPanel = ui.Panel([
  ui.Label({
    value: 'Land Cover Change Impacts',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px'}
  ...
  ui.Label({
    value: 'Land cover change across the Amazon region results largely from human action, particularly as consequence of agricultural development. '+
    'Use the MODIS land cover change dataset to map the spatial distribution of each land use type over time. Focus especially on the growth of agriculture and grasslands at the expense of forest.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
Map.onClick(function(coords) {
  ...
  if (carbonBool) {
    ...
    var carbonChart = ui.Chart.array.values({
      ...
      .setOptions({
        title: 'Biomass Lost over Time',
        hAxis: {title: 'Year', format: '####'},
        vAxis: {title: 'Biomass (Mg/Ha)'},
        legend: { position: "none" },
        lineWidth: 1,
        pointSize: 3,
        colors: ['3b7042']
