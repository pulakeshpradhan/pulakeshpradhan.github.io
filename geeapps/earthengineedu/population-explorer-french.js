...
Map.add(ui.Label(
    'Explorateur de la Population Mondiale', {fontWeight: 'bold', fontSize: '24px'}));
/*
...
function makeResultsBarChart(countries) {
  ...
  chart.setOptions({
    ...
    vAxis: {title: null},
    hAxis: {title: 'Population Approximative en 2015', minValue: 0}
  });
...
function clearResults() {
  ...
  var instructionsLabel = ui.Label('Sélectionnez des régions pour comparer la population.');
...
var chartTypeToggleButton = ToggleButton(
    [
      {
        label: 'Afficher les résultats sous forme de tableau',
        value: makeResultsBarChart,
      },
      {
        label: 'Afficher les résultats sous forme de graphique',
...
var buttonPanel = ui.Panel(
    [ui.Button('Effacer les résultats', clearResults), chartTypeToggleButton],
...
Map.add(ui.Panel(
    [
      ui.Label('Densité de la Population', LEGEND_TITLE_STYLE), makeLegend(),
      ui.Label(
          '(milliers de personnes par kilomètre carré)', LEGEND_FOOTNOTE_STYLE),
      ui.Label(
          'Source: Global Human Settlement Layer (JRC)', LEGEND_FOOTNOTE_STYLE),
      ui.Label('Source des frontières du pays: USDOS LSIB', LEGEND_FOOTNOTE_STYLE)
    ],
