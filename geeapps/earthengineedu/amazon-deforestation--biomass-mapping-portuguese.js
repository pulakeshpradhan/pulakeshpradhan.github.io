...
Map.addLayer(series.sum().clipToCollection(indigenous_lands),
    {min: -60000, max: 60000, palette: ['FF0000', '000000', '00FF00']}, 'Camada NDVI/EVI');
...
Map.addLayer(allLosses, {palette: ['FF0000']}, "Camada de Perda de Floresta de Hansen");
// *********************************************************************************************************************
// Carbon stock
// *********************************************************************************************************************
...
Map.addLayer(carbonStats, carbonVisParams, 'Mapa de Biomassa MODIS');
// *********************************************************************************************************************
// Land cover
// *********************************************************************************************************************
...
Map.addLayer(landCover01_05, landCoverVisParams, 'Classificação da Cobertura do Solo 2001-2005');
Map.addLayer(landCover06_10, landCoverVisParams, 'Classificação da Cobertura do Solo 2006-2010');
Map.addLayer(landCover11_15, landCoverVisParams, 'Classificação da Cobertura do Solo 2011-2015');
Map.addLayer(landCover16_17, landCoverVisParams, 'Classificação da Cobertura do Solo 2016-2017');
// *********************************************************************************************************************
// Start of UI script
// *********************************************************************************************************************
// Legend maker
...
var title = ui.Panel([
  ui.Label({
    value: 'Desmatamento na Amazônia e Mapeamento de Biomass',
    style: TITLE_STYLE,
  })
]);
panel.add(title);
...
var forestLoss = ui.Panel([
  ui.Label({
    value:  'O desmatamento na Amazônia brasileira tem implicações significativas tanto nas emissões de carbono quanto na preservação das terras indígenas. '+
            'Use esta ferramenta para determinar os locais e causas da perda de florestas em toda a região e considere como o distúrbio florestal em larga escala tem impactos ambientais em larga escala.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
...
var redBlurb = ui.Label({
  value: 'Clique em um polígono para traçar as tendências de desmatamento ao longo do tempo.',
  style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '3b7042',
    padding: '0px 0px 0px 8px',
...
var eviCheckbox = ui.Checkbox('Camada NDVI/EVI', true);
...
var eviLegendTitle = ui.Label({
  value: 'Lenda da Verde Florestal Mudança',
  style: {
    fontWeight: 'bold',
    fontSize: '12px',
    margin: '0 0 4px 0',
...
var eviClassifications = ['Perda Florestal', 'Sem Alteração', 'Ganho Florestal'];
for (var i = 0; i < 3; i++) {
  eviLegend.add(makeRow(eviPalette[i], eviClassifications[i]));
}
var fgaPanel = ui.Panel([
  ui.Label({
    value: 'Análise do Verde da Floresta',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px',
  ...
  ui.Label({
    value:  'Mudanças na cobertura florestal ou no verde podem ser medidas por meio do Índice de Vegetação de Diferença Normalizada (NDVI), que usa imagens de satélite para calcular a reflexão e absorção da luz solar nas folhas. ' + 
            'Explore esta camada do mapa para determinar as regiões de crescimento ou perda de NDVI de 2000 a 2016.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '8px',
...
var hansenCheckbox = ui.Checkbox("Camada de Perda de Floresta de Hansen", true);
...
var deforestationPanel = ui.Panel([
  ui.Label({
    value: 'Desmatamento com o Tempo',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px',
  ...
  ui.Label({
    value:  'O conjunto de dados v1.6 das alterações florestais globais de Hansen fornece uma distribuição anual da extensão das alterações florestais à escala global. ' +
            'Clique em um polígono para representar graficamente a área total da perda florestal a cada ano de 2001 a 2018.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
var carbonCheckbox = ui.Checkbox('Mapa de Biomassa MODIS', true);
...
var carbonPanel = ui.Panel([
  ui.Label({
    value: 'Implicações do Desmatamento na Biomassa',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px'}
  ...
  ui.Label({
    value:  'O desmatamento leva à emissão drástica de carbono devido à liberação de estoques significativos de carbono mantidos em florestas tropicais. '+
            "A biomassa representa o peso total da matéria lenhosa numa árvore, enquanto o carbono representa apenas uma parte desta matéria - normalmente cerca de 50% do peso total de uma árvore. "+
            'Use o mapa de biomassa do MODIS para plotar a biomassa total perdida por desmatamento durante cada época de tempo.',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
var landCoverButton = ui.Button({
  label: 'Análise de Classificação da Cobertura do Solo',
  style: {padding: '0px 0px 0px 8px' },
 });
...
var landCover01_05Checkbox = ui.Checkbox('Classificação da Cobertura do Solo 2001-2005', true);
...
var landCover06_10Checkbox = ui.Checkbox('Classificação da Cobertura do Solo 2006-2010', true);
...
var landCover11_15Checkbox = ui.Checkbox('Classificação da Cobertura do Solo 2011-2015', true);
...
var landCover16_17Checkbox = ui.Checkbox('Classificação da Cobertura do Solo 2016-2017', true);
...
var landCoverLegendTitle = ui.Label({
  value: 'Lenda da Cobertura do Solo',
  style: {
    fontWeight: 'bold',
    fontSize: '12px',
    margin: '0 0 4px 0',
...
// landCoverLegend
var landCoverClassifications = ['Floresta de Folha Larga Evergreen', 'Prados e Pastagens', 'Savanas', 'Cultivos', 'Mosaicos de Vegetação Natural'];
...
var landCoverPanel = ui.Panel([
  ui.Label({
    value: 'Impactos da Mudança Cobertura do Solo',
    style: {fontSize: '16px',
    fontWeight: 'bold',
    color: '#616161',
    padding: '0px 0px 0px 8px'}
  ...
  ui.Label({
    value:  'A mudança da cobertura do solo na região amazônica resulta em grande parte da ação humana, particularmente como consequência do desenvolvimento agrícola. '+
            'Use o conjunto de dados de mudança de cobertura do solo do MODIS para mapear a distribuição espacial de cada tipo de uso da terra ao longo do tempo. Enfoque especialmente no crescimento da agricultura e dos campos às custas da floresta. ',
    style: {fontSize: '14px',
    fontWeight: 'bold',
    color: '#9E9E9E',
    padding: '0px 0px 0px 8px',
...
Map.onClick(function(coords) {
  ...
  selection.get('Name').getInfo(function(i) {
    id.setValue('Indigenous Territory Name: ' + i);
  });
  // add it as a layer
  ...
  ndviChart.setOptions({
    title: 'Anomalia EVI com o Tempo',
    vAxis: {title: 'EVI'},
  ...
  if (deforestationBool) {
    ...
    var deforestationChart = ui.Chart.array.values({
      ...
      .setOptions({
        title: 'Perda de Florestas por Ano',
        hAxis: {title: 'Ano', format: '####'},
        vAxis: {title: 'Área (metros quadrados)'},
  ...
  if (carbonBool) {
    ...
    var carbonChart = ui.Chart.array.values({
      ...
      .setOptions({
        title: 'Biomassa Perdida com o Tempo',
        hAxis: {title: 'Ano', format: '####'},
        vAxis: {title: 'Biomassa (Mg/Ha)'},
        legend: { position: "none" },
        lineWidth: 1,
        pointSize: 3,
        colors: ['3b7042']
