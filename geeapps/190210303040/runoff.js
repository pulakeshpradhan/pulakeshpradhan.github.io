var samarinda = ui.import && ui.import("samarinda", "table", {
      "id": "projects/gee-190210303040/assets/SHP_Samarinda"
    }) || ee.FeatureCollection("projects/gee-190210303040/assets/SHP_Samarinda");
var shp = ee.FeatureCollection(samarinda)
...
var lulc = modis.select('LC_Type1').first().clip(samarinda)
            .rename('lulc');
...
var CN2 = CN_whole.clip(samarinda).rename('CN2');
...
var runoff_func = function(image) {
  ...
  return Q3.clip(samarinda).rename('runoff')
...
// the bands are referred to as the 'primary' and 'secondary' properties
var MergeBands = function(aRow) {
  var anImage = ee.Image.cat(aRow.get('primary'), aRow.get('secondary')).clip(samarinda);
  return anImage;
};
