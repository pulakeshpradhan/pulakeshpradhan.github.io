...
S1_3 =[],S2_3 =[],L8_3 =[],TglPrd3,Trend,RPISlop,Bts_Plu=BtsPlu(),AOI,Pilih,MaxPd
;
...
Tgl = Harini.get('day').getInfo(); Thn1 = Thn-1;
...
print('Ketersediaan Data Sentinel-2 di Jawa Barat dari 01-01-'+ Thn + ' : ',ExDatS2);
 Thn = 2020; Thn1 = Thn-1; Pd = 15; MaxPd = Math.floor(365/Pd);  // Pd bisa diganti : 5,10,15,30 harian
...
Plu,PluId
;
if (Pd >= 10 && Pd <=31) Pilih = Waktu.PilihPrd(Pd); else Pilih = PrdList;
...
; Judul = 'MULTISENSOR for AGRICULTURAL CROP MONITORING';
Judul = 'SENTINEL for AGRICULTURAL CROP MONITORING';
var TB_Thn = GUI.TxtBnm(Thn,'Tgl Awal',10,'red',200,55,''),
    TB_Prd = GUI.TxtB(3,'Tgl Awal',10,'red','-33px 0 0 210px',200,40,''),
...
Periode = Pilih[PilPeriod.getValue()];  
...
for (k=0; k < 3; k++) {
Idx = IdxPrd-k; 
if (Idx >= 0) TglPrd3 = PrdList[Idx]; else TglPrd3 = PrdList1[MaxPd + Idx];
S1_3[k]= (Pilih_S1(TglPrd3,'','VH-VV','MeanMed','RPI',Bts1Prv)).rename('RPI_' + (k+1));   
S2_3[k]=(Pilih_S2k(TglPrd3,'EVI','Qual','EVI',Bts1Prv)).rename('EVI_' + (k+1));
}
// Hitung Trend/Slope : Lama
S1_3 = ee.Image.cat(S1_3); S2_3 = ee.Image.cat(S2_3); 
//dPI = S1_3.select('RPI_3').subtract(S1_3.select('RPI_3')).rename('dPI');
Trend = Citra.Slop3(S1_3,'n'); TrnS2 = Citra.Slop3(S2_3,'n');
S1 = S1.addBands(Trend); S1_3 = []; S2 = S2.addBands(TrnS2); S2_3 = [];
print('Trend S2',S2);
Fase_S1 = Citra.Det1Fase_S1(S1,'Slope',Air_Th,Bera_Th).focal_mode();
Fase_S2 = Citra.DetFase_S2(S2).focal_mode();
Map.clear(); 
//Map.addLayer(L8.clip(ft).mask(Mask),VisRGB_L8,'RGB L8 '+ Tgl_Fase[1],0);
Map.addLayer(S2.clip(ft),VisRGB_S2,'RGB S2 '+ Tgl_Fase[1],0);
Map.addLayer(S1.clip(ft),VisRGB_S1,'RGB S1 '+ Tgl_Fase[1],0);
//Map.addLayer(S2.select('SCL').clip(ft).mask(Mask),Vis_SCL,'Lahan S2 '+ Tgl_Fase[1],0);
//Map.addLayer(Fase_S2.clip(ft).mask(Mask),VisFase,'Fase S2 '+ Tgl_Fase[1],0);
...
var LegendaFase = Legenda('Crop Phase',Waktu.TglKis(Tgl_Fase,'Eng'),Nama_Fase_Kls,Fase_palette,7); 
Map.add(LegendaFase);
//Panel.add(LegendaFase);
...
function BtsPlu() {
var Batas_Plu = [
[94.972336,6.077241,109.167427,-6.2],[105,-5.5,116,-8.9],[115.756277,-7.4309423,127.632498,-11.019763],
