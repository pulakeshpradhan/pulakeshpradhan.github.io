...
var app_version = ui.Label({value: 'Version: Temperate Basic Collection2',
...
\nThe basic version requires a forest mask with a positive \
\nvalue (or values) for forest and non-forest being masked.',{whiteSpace: 'pre'});
var ab_thre_label = ui.Label('Threshold (abrupt)      ', {width: '200px'});
...
var output_full_t = ui.Textbox({
    ...
    value: 'users/shijuanchen32/georgia_public/test/basic_full_C2',
    style:{stretch: 'horizontal', backgroundColor: 'rgba(255, 255, 255, 0.0)'}
  });
...
var output_year_t = ui.Textbox({
    ...
    value: 'users/shijuanchen32/georgia_public/test/basic_first_year_C2',
    style:{stretch: 'horizontal', backgroundColor: 'rgba(255, 255, 255, 0.0)'}
  });
...
view_full_Button.onClick(function() {
  ...
  mapPanel.addLayer(region_draw, {palette: 'red'}, 'study_region');
  mapPanel.centerObject(region_fc);
  var seg_dis = ee.Image(view_full_t.getValue()).updateMask(ee.Image(view_results_t.getValue()));
...
--------------------------------------------------------------------------------*/
  ...
  var forest_mask = ee.Image(forest_mask_t.getValue()).selfMask();
  ...
  var classify = ccdc_SMA_utlis.classifySegments_basic(saveRegion, ccdc_sma, forest_mask, ab_threshold, gr_threshold, ch_prob);
//  print(classify);
  //get annual results
