var geometry = ui.import && ui.import("geometry", "geometry", {
      ...
      "shown": true,
      "locked": false
    }) || /* color: #23cba7 */ee.Geometry.Point([97.77081788521446, 3.6541856926814376]);
//Indeks Kekeringan Tanamman
//Input Data NDDI
var dataset = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
.map(function(image){return image})
var NDDI = dataset.map(function(image) { var NDDI2 = image.expression(
'(((band5 - band4)/(band5 + band4))-((band5 - band6)/(band5 + band6)))/(((band5 - band4)/(band5 + band4))+((band5 - band6)/(band5 + band6)))', {
'band4': image.select('SR_B4'), 'band6': image.select('SR_B6'), 'band5': image.select('SR_B5')
})//.rename('NDDI1');
...
.seriesByRegion({ imageCollection: NDDI, regions: aoi,
...
title: 'Indeks Kekeringan Tanaman', hAxis: {title: 'Date'},
vAxis: {title: 'Indeks Kekeringan Tanaman'}, series: {0: {color: '23cba7'}}
});
...
value: 'Monitoring Indeks Kekeringan Tanaman', style: {fontWeight: 'bold',
...
function makeColorBarParams(palette) { return {
bbox: [0, 0, 1, 0.1],
dimensions: '100x10', format: 'png',
min: 0,
max: 1,
palette: palette,
};
...
image: ee.Image.pixelLonLat().select(0), params: makeColorBarParams(nddiVis.palette),
style: {stretch: 'horizontal', margin: '0px 8px', maxHeight: '24px'},
});
var legendLabels = ui.Panel({ widgets: [
ui.Label(nddiVis.min, {margin: '4px 8px'}), ui.Label(
(nddiVis.max / 2),
{margin: '4px 8px', textAlign: 'center', stretch: 'horizontal'}), ui.Label(nddiVis.max, {margin: '4px 8px'})
],
layout: ui.Panel.Layout.flow('horizontal')
});
var legendTitle = ui.Label({ value: 'Rata-rata NDDI ', style: {fontWeight: 'bold'}
});
var legendPanel = ui.Panel([legendTitle, colorBar, legendLabels]);
legendPanel.style().set({ width: '400px', position: 'top-left'
});
Map.add(legendPanel);