...
var years_ = '2021', years = {
...
}, seasons_ = 'Growing season', seasons = {
  'Growing season': 1,
  'Non-growing season': 0
}, locations_ = 'sample2: Shanghai, CN.', bymarker_ = 'By marker...', locations = {
  "By marker...": [0, 0, 0, 0],
...
], types_ = 'RGB_432 (RED/GREEN/BLUE)', types = {
  ...
  ui.Label({
    value: 'A Batch Pixel-based Compositing Algorithm Demo',
    style: { fontSize: '20px', fontWeight: 'bold' }
  }),
  ui.Label({
    value: 'Li, Jianzhou, Jinji Ma, and Xiaojiao Ye. 2022. "A Batch Pixel-Based Algorithm to Composite Landsat Time Series Images" Remote Sensing 14, no. 17: 4252.',
    style: { fontSize: '10px', fontWeight: 'bold' }
  }),
  ui.Label({
    value: 'For details of the method, please refer to this article.',
    style: { fontSize: '10px' },
    targetUrl: 'https://doi.org/10.3390/rs14174252'
  })
])
  ...
  , get_landsat_by_year = function (year) {
    ...
    return sn;
  }, linkedMap = ui.Map(), panel = ui.Panel(), wrs = ee.FeatureCollection("users/leejianzhou2080/WRS2_descending_0");
// button action
var btn_composite = ui.Button({
  ...
  onClick: function () {
    ...
    if (!path) {
      ...
      var p = ee.Geometry.Point(l, t), w = wrs.filterBounds(p).first();
      path = w.get("PATH")
      row = w.get("ROW")
    ...
    var ref, compare, ref_vis, com_vis, _bands, ref_stat, compare_stat, _t = 'nd', percentile = [0.3, 99.7], _sensor = get_landsat_by_year(selected_year), _reducer = {
    ...
    if (selected_type.color >= 0) {//gray image
      var _ex = bandlist[selected_type.band], _bands = [_t];
...
function get_reference_image(_image_id, sensor, year, season, wrs_path, wrs_row) {
  ...
  var sts = "system:time_start", filter1, filter2, filter3, filter4, filter5, filter6, filter7, filter8, filterD, filterQ, ic;
...
function geeComposite(sensor, year, season, wrs_path, wrs_row, method) {
  var sts = "system:time_start", year = (season == 0 && year == 2013) ? 2014 : year, filter1, filter2, filter3, filter4, filter5, filter6, filter7, filter8, filterD, filterQ, ic;
...
var _qa_dic = {
  ...
  bands_rename_raw = {
    '8': [['B2', 'B3', 'B4', 'B5', 'B6', 'B7'], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']],
    '7': [['B1', 'B2', 'B3', 'B4', 'B5', 'B7'], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']],
    '5': [['B1', 'B2', 'B3', 'B4', 'B5', 'B7'], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']],
    '4': [['B1', 'B2', 'B3', 'B4', 'B5', 'B7'], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']],
  },
  bands_rename_toa = [
    ['B2', 'B3', 'B4', 'B5', 'B6', 'B7', "BQA", "cloud"], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2', "BQA", "cloud"],
    ['B1', 'B2', 'B3', 'B4', 'B5', 'B7', "BQA", "cloud"], ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2', "BQA", "cloud"]
  ];
function landsat_cloud_mask(image) {
  var max_cloud_score = ee.Number(20), qa_dic = ee.Dictionary(_qa_dic), spaceid = image.get("SPACECRAFT_ID");
...
function landsat_cloud_mask_simple(image) {
  var qa_dic = ee.Dictionary(_qa_dic), spaceid = image.get("SPACECRAFT_ID");
  return image.updateMask(image.select("BQA").eq(qa_dic.get(spaceid)).reduce("max").not())
}
function getClipped(img) {
  var MOVE_STEP = 0.02, footprint = ee.Geometry(img.get('system:footprint'))
