...
var MineFeature1 = ee.FeatureCollection("users/matthepler/M1101914"),
    ...
    imageVisParam5 = {"opacity":1,"bands":["B4","B3","B2"],"min":446,"max":1718,"gamma":1},
    ...
    Creek = ee.FeatureCollection("projects/direct-link-253221/assets/Big_Sandy"),
    Guy = ee.FeatureCollection("projects/direct-link-253221/assets/Guyandote_Crayfish1"),
    imageVisParam = {"opacity":1,"bands":["B4","B3","B2"],"min":1377.56,"max":7356.44,"gamma":1};
...
Map.addLayer(lastImage, imageVisParam,'True Color Satellite Image',shown,opacity3);
Map.addLayer(lastImage, imageVisParam2,'Near Infrared Satellite Image',shown,opacity3);
Map.addLayer(MineFeature, visParams,'Doe Branch Mine Permit Boundary',shown,opacity); 
        print(lastImage.date());
        Map.addLayer(CFX,visParams3,"VA_CFX");
        Map.addLayer(Creek, {color: '00FFFF'},"Big Sandy Crayfish Habitat");
         Map.addLayer(Guy, {color: '0000FF'},"Guyandotte Crayfish Habitat");
