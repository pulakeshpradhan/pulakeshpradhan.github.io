...
var ndviVis = {
  ...
  max: 0.7,
  palette: [
        '#762a83', '#af8dc3', '#e7d4e8', '#f7f7f7', '#d9f0d3', '#7fbf7b',
        '#1b7837'
      ]
};
...
var intro = ui.Panel([
  ...
  ui.Label(
      'Click a point on the map to get: Landsat 8 median TOA spectrum, ' +
      'Landsat 8 TOA time series.')
]);
panel.add(intro);
...
var chartFunction = function(coords) {
  ...
  var spectrumChart =
      ui.Chart.image
          ...
          .setOptions({
            ...
            curveType: 'function',
            series: {0: {visibleInLegend: false, color: '555555'}},
          });
  ...
  // Transpose 1xP vector of coefficients.
  ...
  ndviChart.setOptions({
    ...
    series: {
      ...
      1: {lineWidth: 2, color: '4575b4', pointsVisible: false}
    },
    trendlines: {
      0: {
        type: 'linear',
...
chartFunction({
  lon: aPoint.coordinates().get(0).getInfo(),
  lat: aPoint.coordinates().get(1).getInfo()
});
// LEGEND SETUP ---------------------------------------------------
/**
...
var legendTitle =
    ui.Label({value: 'Map Legend: median NDVI', style: {fontWeight: 'bold'}});
