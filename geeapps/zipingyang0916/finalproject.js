...
var get_legend_for_make_map = function(){
  var legend_panel = ui.Panel({
    style:{
      height : '200px',
      width : '120px',
...
var get_image = function(start_date,end_date,geometry,collection){
  var image = ee.Algorithms.If({
    condition:collection == landsat5,
    trueCase:landsat5.filterDate(start_date,end_date).filterBounds(geometry)
                     .map(maskL578cloud).map(L57addFeature)
                     .median(),
    falseCase:ee.Algorithms.If({
      condition:collection == landsat7,
      trueCase:landsat7.filterDate(start_date,end_date).filterBounds(geometry)
                       .map(maskL578cloud).map(L57addFeature)
                       .median(),
      falseCase:ee.Algorithms.If({
        condition:collection == landsat7,
        trueCase:landsat8.filterDate(start_date,end_date).filterBounds(geometry)
                         .map(maskL578cloud).map(L8addFeature)
                         .median(),
        falseCase:sentinel2.filterDate(start_date,end_date).filterBounds(geometry)
...
var get_raw_image = function(start_date,end_date,geometry,collection){
  var image = ee.Algorithms.If({
    condition:collection == landsat5,
    trueCase:landsat5.filterDate(start_date,end_date).filterBounds(geometry)
                     .map(L57addFeature).median(),
    falseCase:ee.Algorithms.If({
      condition:collection == landsat7,
      trueCase:landsat7.filterDate(start_date,end_date).filterBounds(geometry)
                       .map(L57addFeature).median(),
      falseCase:ee.Algorithms.If({
        ...
        trueCase:landsat8.filterDate(start_date,end_date).filterBounds(geometry)
                         .map(L8addFeature).median(),
        falseCase:sentinel2.filterDate(start_date,end_date).filterBounds(geometry)
                           .map(S2addFeature).median()
      })
    })
  });
};
var vis = {min:0,max:80,palette:'0000FF,00FFFF,00FF00,FFFF00,FF7F00,FF0000'},
...
var select_lake = ui.Select({
  items:[{label:'印旛沼',value:inbanuma},{label:'琵琶湖',value:biwako},
         {label:'太湖',value:tako},{label:'洱海',value:eruhai}],
  placeholder:'オプション',
  style:{
    position:'top-left',
    padding:'0px',
...
var select_satellite = ui.Select({
  items:[{label:'Landsat-5 (1985-2011) 30mの解像度',value:landsat5},
         {label:'Landsat-7 (2001-2020) 30mの解像度',value:landsat7},
         {label:'Landsat-8 (2015-2020) 30mの解像度',value:landsat8},
         {label:'Sentinel-2 (2018-2020) 10mの解像度',value:sentinel2}],
  placeholder:'衛星を選択し',
  style:{
    width:'230px',
    padding:'0px',
...
var button_ok_show_image = ui.Button({
  ...
  onClick:function(){
    ...
    var start_date = ee.Date(dateslider.getValue()[0]),
        ...
        image = ee.Image(get_image(start_date,end_date,geometry,collection)),
        raw_image = ee.Image(get_image(start_date,end_date,geometry,collection)),
        band_size = image.bandNames().size();
    var layer_image = ee.Image(ee.Algorithms.If({
      condition:band_size.neq(0),
      trueCase:image.updateMask(image.select('mndwi').gt(0)).updateMask(image.select('nir').lt(0.2))
    ...
    var raw_layer_image = ee.Image(ee.Algorithms.If({
      condition:band_size.neq(0),
      trueCase:raw_image.updateMask(lake_mask),
      falseCase:ee.Image.constant(1).select(['constant'],['tss'])
                                    .clip(waste_mask)
    }));
    var layer = ui.Map.Layer(layer_image.select('tss'),vis,'tss'),
        layer_rgb = ui.Map.Layer(layer_image,vis_rgb,'雲なし'),
        layer_raw_rgb = ui.Map.Layer(raw_layer_image,vis_rgb,'生データ').setOpacity(0);
