...
var inspector_srtm =
ui.Panel([ui.Label('Дізнатися висоту')]);
...
var computedValue =
sample.first().get('elevation');
computedValue.evaluate(function(result) {
inspector_srtm.widgets().set(0,
ui.Label({
...
var inspector_ndvi =
ui.Panel([ui.Label('Дізнатися NDVI')]);
...
var computedValue =
sample.first().get('NDVI_mean');
computedValue.evaluate(function(result) {
inspector_ndvi.widgets().set(0,
ui.Label({
value: 'Середній NDVI: ' +
result.toFixed(2),
}));
...
right.addLayer(dataset.median().clip(UA),
visualization, 'True Color (432)');
left.addLayer(dataset.median().clip(UA),
visualization, 'True Color (432)');
var geometry =
ee.Geometry.Point([24.502225469789003, 48.156867664966825]),
