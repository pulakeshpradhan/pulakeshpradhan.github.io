...
var villageName ='Narsapur';
var village = villages.filter(ee.Filter.eq("NAME",villageName)); 
print(village)
var region = narsapurMandal;
Map.centerObject(narsapurMandal,12);
Map.addLayer(village, null, villageName + " Village",true,0.2);
Map.addLayer(region, null,"Narsapur Mandal",true,0.1);
...
var composite = image.clip(region);
print(composite);
...
Map.addLayer(cropLayer.updateMask(cropLayer).clip(village), {min: 0, max: 1, palette: cropPalette}, "Paddy");
var cottonLayer = classified.eq(6);
var cottonPalette = ['white','ffff00'];
Map.addLayer(cottonLayer.updateMask(cottonLayer).clip(village), {min: 0, max: 1, palette: cottonPalette}, "Cotton");
var maizeLayer = classified.eq(7);
var maizePalette = ['white','00ffff'];
Map.addLayer(maizeLayer.updateMask(maizeLayer).clip(village), {min: 0, max: 1, palette: maizePalette}, "Maize");
var redgramLayer = classified.eq(10);
var redgramPalette = ['white','FFAE00'];
Map.addLayer(redgramLayer.updateMask(redgramLayer).clip(village), {min: 0, max: 1, palette: redgramPalette}, "Redgram");
...
var names = [
  'Water',
  'Paddy (' + CalculateArea(cropLayer.clip(village)) + " ha)",
  'Cotton (' + CalculateArea(cottonLayer.clip(village)) + " ha)",
  'Maize (' + CalculateArea(maizeLayer.clip(village)) + " ha)",
  'Redgram (' + CalculateArea(redgramLayer.clip(village)) + " ha)",
  'field data'
];
...
var MapTitle = villageName + ' Village' + ' (' + date.getInfo() + ')';
