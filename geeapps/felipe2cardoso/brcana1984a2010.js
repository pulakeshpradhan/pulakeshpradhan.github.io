...
/*************************************DEFINIÇÃO DA ÁREA DE ESTUDO****************************************/
var area_estudo = ee.FeatureCollection('users/felipe2cardoso/Usina_BA');
...
Map.centerObject(area_estudo,12); 
...
inspector.add(ui.Label('Clique para obter o NDVI médio'));
...
Map.onClick(function(coords) {
  ...
  var sampledPoint = temporalMean.reduceRegion(ee.Reducer.mean(), point, 60);
...
var intro = ui.Panel([
  ui.Label({
    value: 'Série Temporal',
    style: {fontSize: '20px', fontWeight: 'bold'}
  }),
  ui.Label('Clique em um ponto no mapa para inspecionar a área da Usina.')
]);
panel.add(intro);
...
Map.onClick(function(coords) {
  ...
  var ndviChart = ui.Chart.image.series(NDVI, point, ee.Reducer.mean(), 60);
...
Map.addLayer(NDVI.reduce(ee.Reducer.mean()).clip(area_estudo), {palette: ['black','blue','red','orange','yellow','green','lime'],min: -1, max: 1}, 'NDVI');