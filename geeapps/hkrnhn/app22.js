// Landsat-5&7_app
// Landsat-5,7 画像検索・植生指数オーバーレイ
// USGS Landsat Level 2, Collection 2対応
var app = {}; //空のオブジェクト生成
// UIメインパネル作成開始
app.createPanels = function() {
  ...
  app.filters = {
    ...
    selectYear: ui.Select({ // 検索開始年選択
      ...
      value: app.dataset[Object.keys(app.dataset)[0]].yearList[0],
      placeholder: '年選択',
      style: app.WIDGETS_STYLE,
    }),
  ...
  app.filters.panel = ui.Panel({
    widgets: [
      ...
      ui.Label('Landsat-5 & 7', app.HEADER_TEXT_STYLE),
...
app.initialSetup = function() {
  // 衛星データセット
  app.collectionL5 = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2');
  app.collectionL7 = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2');
  app.collectionL5and7 = (app.collectionL5).merge(app.collectionL7);
  app.dataset = {
    'Landsat-5 & 7': {
      collection: app.collectionL5and7,
      since: 1984,
      to: 2011,
      yearList: [],
    },
    'Landsat-5': {
      collection: app.collectionL5,
      since: 1984,
      to: 2011,
      yearList: [],
    },
    'Landsat-7': {
      collection: app.collectionL7,
      since: 1999,
      to: 2003,
      yearList: [],
    },
  ...
  app.dataset[Object.keys(app.dataset)[0]].yearList = 
    serialNumber(app.dataset[Object.keys(app.dataset)[0]].since,
    app.dataset[Object.keys(app.dataset)[0]].to);
  app.dataset[Object.keys(app.dataset)[1]].yearList = 
    serialNumber(app.dataset[Object.keys(app.dataset)[1]].since,
    app.dataset[Object.keys(app.dataset)[1]].to);
  app.dataset[Object.keys(app.dataset)[2]].yearList = 
    serialNumber(app.dataset[Object.keys(app.dataset)[2]].since,
    app.dataset[Object.keys(app.dataset)[2]].to);
  // 土壌腐植推定値(g/kg)
  ...
  app.ScaleResample = function(image) {
    var scaledImg = image
      .select(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7'], // old names
       ['B1', 'B2', 'B3', 'B4', 'B5', 'B7']) // new names
