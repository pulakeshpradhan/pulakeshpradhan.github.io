...
app.createPanels = function() {
  ...
  app.filters = {
    ...
    geoButton: ui.Button({ // 端末位置情報により現在地点へ
      label: '現在地',
      onClick: function() {
  ...
  app.filters.panel = ui.Panel({
    widgets: [
      ...
      ui.Label('Sentinel-2', app.HEADER_TEXT_STYLE),
...
app.initialSetup = function() {
  ...
  app.imgCollection = //イメージコレクション初期値セット
    ee.ImageCollection(app.dataset[Object.keys(app.dataset)[0]].id);
  // 検索開始年・月リスト
  app.today = new Date();
  ...
  // Sentinel-2雲確率画像
  app.s2Clouds = ee.ImageCollection('COPERNICUS/S2_CLOUD_PROBABILITY');
  // 土壌腐植推定値(g/kg)
  ...
  var Fude = ui.Map.Layer(app.Fudeoutline, {palette: '#808080'}, '筆ポリゴン', false);
  // 湛水筆ポリゴン2021
  ...
  Map.layers().set(1, ui.Map.Layer({name:'植生指数・反射率', shown: false}));
  ...
  app.calcYellowness = function(image) { //Yellowness
  ...
  app.ScaleResample = function(image) {
    ...
    var yellowness = app.calcYellowness(scaledImg);
    return scaledImg.addBands(ndvi).addBands(gndvi)
      .addBands(evi2).addBands(WDRVIg).addBands(WDRVIre).addBands(yellowness)
      .resample('bilinear');
  };
  // カラー合成画像の表示オプション
  app.VIS_OPTIONS = {
  ...
  app.VIS_OPTIONS2 = {
    ...
    'WDRVIre': {
      description: 'WDRVIre=(0.1*NIR-RE)/(0.1*NIR+RE)+0.818',
      visParams: {
        'bands': ['WDRVIg'],
        'min': 0.1,'max':0.9,
      },
    ...
    'Yellowness': {
      description: 'Yellowness=RED+GREEN-2*BLUE',
      visParams: {
        'bands': ['Yellowness'],
        'min': 0.0,'max':0.25,
      },
  ...
  app.refreshMapLayer1 = function() {
    if (app.mosaicimg) {
      ...
      var mapImg = app.mosaicimg;
        // .select('B4', 'B11', 'B12', 'NDVI', 'GNDVI','EVI2', 'WDRVIg', 'OM');
...
　　  Map.layers().set(5, {shown: false});
    ...
    var labelFcFc = ui.Label('● FeatureCollectionを作業領域に変換（属性を保持）',
      app.LEGEND_TEXT_STYLE);
...
　　  Map.layers().set(5, aoiBoundsLayer);
  ...
  app.setFudeFc = function() {
    ...
    if (drawingTools.layers().get(0)){
      // 描画オブジェクトをジオメトリに変換
      var aoiGeo = drawingTools.layers().get(0).toGeometry();
      // 筆ポリゴンから描画オブジェクトと重なる個所を抽出
...
　  var pcValLabel = ui.Panel({
  ...
  app.aoiScatterPlot = function(aoiFc) {
    ...
    var dispPlot = function(){
      var obsDate = app.imageDateStr;
      var currentScale = Math.max(Map.getScale()*5, 10);
  ...
  app.showFcMeanUnion = function(aoiFc, aoiGeo) {
    ...
    var sumUpData = function(){
      ...
      var bandsList = ['NDVI', 'GNDVI','EVI2', 'OM'];
...
　　}
  ...
  app.scaleVI = function(image) {
    ...
    var yellowness = app.calcYellowness(scaledImg);
    return scaledImg.addBands(ndvi).addBands(gndvi)
       .addBands(evi2).addBands(WDRVIre).addBands(WDRVIg).addBands(yellowness)
  ...
  app.aoiChart = function(aoiFc) {
      ...
      var currentScale = Math.max(Map.getScale(), 10);
      app.setChartCollection('VI'); // 画像コレクション作成
      var chartTitle = '反射率・植生指数推移: ' + datasetName
          + ', 雲量上限' 
          + app.filters.cloudcover.getValue() + '%';
  ...
  // 雲確率チャート
  app.cloudChart = function(aoiFc) {
    var currentScale = Math.max(Map.getScale()*5, 10);
      app.setChartCollection('Cloud'); // チャート用画像コレクション作成
      var chart = ui.Chart.image.seriesByRegion({
        imageCollection: app.chartCollection,
        regions: aoiFc,
        reducer: ee.Reducer.mean(),
