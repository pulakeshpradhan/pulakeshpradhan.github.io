...
var geometry = /* color: #d63000 */ee.Geometry.Polygon(
        [[[-42.4072265625, -2.535915069920843],
          [-42.558370983787825, -2.979133331970038],
          [-42.35782446232008, -3.6418234443557247],
          [-42.88000103666718, -4.30919939338001],
          [-43.53013024461302, -5.249578394136768],
          [-43.171217523645055, -5.971655677060058],
          [-43.84883946583193, -6.900535088520186],
          [-44.615892457243945, -7.042783782704756],
          [-45.078282497345185, -8.139992929884686],
          [-46.95556640625, -11.338948153729447],
          [-45.85693359375, -18.13436771227032],
          [-41.8359375, -17.779028221663985],
          [-38.759765625, -15.90745193647626],
          [-38.78173828125, -13.506087472485149],
          [-37.9248046875, -12.842866807960249],
          [-36.826171875, -11.144988727157784],
          [-34.87060546875, -8.89484084259505],
          [-34.5849609375, -6.958053880399314],
          [-35.39794921875, -4.925684320606877],
          [-36.8701171875, -4.684834812464392],
          [-39.1552734375, -2.8212472140890483],
          [-40.84716796875, -2.49201205316291]]]);
...
var version = '1'
var ano = '2010'
var parametros = require('users/CartasSol/package:Arida/GuiVisual_Vrs0')
...
var VisualizaMaps = function(ano, chBox){
    ...
    //manejar os box
    if (chBox.prod){
        Map.addLayer(ndvimax, visNDVI, '(v) Mapa de produtividade primária líquida ('+ano+')', true);
        DictlayerAnos = (DictlayerAnos).set('productividade_', ndvimax)        
    }
    if(chBox.carb){
        Map.addLayer(CO2flux, visCO2f, '(iv) Mapa de carbono ('+ano+')', true);
        DictlayerAnos = (DictlayerAnos).set('carbono_', CO2flux)
    }
    if (chBox.tcob){
        Map.addLayer(mapaTransicao, visTrans, '(iii) Mapas de Transições da Cobertura Vegetal ('+ano1+'/'+ano2+')', true);
        DictlayerAnos = ee.Dictionary(DictlayerAnos).set('transcition_', mapaTransicao)
    }
    if (chBox.CobVeg){
        Map.addLayer(mapaUSO, visUSO, '(ii) Mapa de cobertura vegetal e uso do solo ('+ano+')', true);
        DictlayerAnos = (DictlayerAnos).set('uso_', mapaUSO)
    }
    if (chBox.Wsurf){
        Map.addLayer(mapaAgua, visclass, '(i) Mapa de Superfície de água ('+ano+')', true);
        DictlayerAnos = (DictlayerAnos).set('agua_', mapaAgua)
    }
...
var ExportarMapsLoaded = function(ano){
    if (exportado === false){
        ...
        var DictmapLoaded = ee.Dictionary(dicExp.get(ano))
        var listNomMapas = DictmapLoaded.keys()
        listNomMapas.evaluate(function(secuencia){
            ...
            secuencia.forEach(function(nome){
                var MapaImg = ee.Image(DictmapLoaded.get(nome))
                paramExp.image = MapaImg
                paramExp.description = nome + "_" + ano
                Export.image.toDrive(paramExp)
            })
        })
    }
