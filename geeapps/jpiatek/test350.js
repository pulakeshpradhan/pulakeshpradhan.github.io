// Decorrelation Stretch - copied from example scripts (left window)
...
// DC function must go first, now we can add and process our image
// Load image from ImageCollection - this is Landsat 8 surface reflectance data
var image = ee.Image('LANDSAT/LC08/C01/T1_SR/LC08_034035_20180102');
Map.addLayer(image, {min: -100, max:2000}, 'Original Image');
    Map.setCenter(-106.5646, 35.8935, 10);
// Choose the bands to DCS (must be 3). SUBTRACT ONE FROM EACH (Band 1 = 0, Band 2 = 1)
var rgb = [4, 5, 6];
Map.addLayer(image.select(rgb), {min: -200, max: 2000}, 'Original DCS Bands');
// Define an interesting region: only stretch that part.
// If there is an error about "too many pixels", make this smaller.
var region = ee.Geometry.Rectangle(-106.7, 36.0, -106.3, 35.8);
