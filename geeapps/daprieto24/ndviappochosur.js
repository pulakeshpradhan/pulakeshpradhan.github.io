...
var collection_20 = ee.ImageCollection("COPERNICUS/S2_SR") // Carga el Collection de Sentinel 2. REVISAR
  .filter(ee.Filter.lt("CLOUDY_PIXEL_PERCENTAGE", 20)) // ...Filtra el porcentaje de nubosidad
  .filterDate('2020-08-25' ,'2020-08-30') //... selecciona el rango de tiempo 
  .filterBounds(geometry) // Imagenes dentro de estos limites
  .sort("CLOUDY_PIXEL_PERCENTAGE")
...
var ndvi2020_topanel = ui.Map.Layer(NDVI_20.clip(geometry),Simbologia,'NDVI',true);
...
var Vegetacion_topanel = ui.Map.Layer(Vegetacion.clip(geometry),SimbologiaIMG,'Img 2020',true);
//// add layer to map
/*Map.add(ndvi2017_topanel);
Map.add(ndvi2018_topanel);
Map.add(ndvi2019_topanel);*/
Map.add(Vegetacion_topanel);
...
var intro = ui.Panel([
  ...
  ui.Label('This tool shows different vegetation indexes to 2020 year, you can select and display the index that you want clicking on below.',{
    textAlign: 'center',
    fontSize: '16px',
    color: 'black'}),
  ui.Label({
  ...
  ui.Label('Value indexes',{stretch: 'horizontal',
   textAlign: 'center',
   fontWeight: 'bold',
   fontSize: '14px',
   color:'black'
...
var extCheck4 = ui.Checkbox('NDVI').setValue(true);
//Add legend to map
var legend = ui.Panel({
style: {
...
// Create legend title
var legendTitle = ui.Label(' ',{stretch: 'horizontal',
  textAlign: 'center',
  fontSize: '14px'
});
...
var gradient = lon.multiply((Simbologia.max-Simbologia.min)/100.0).add(Simbologia.min);
var legendImage = gradient.visualize(Simbologia);
...
var text2 = ui.Label('Select indexes to display (2020).',{stretch: 'horizontal',
   textAlign: 'center',
   fontWeight: 'bold',
   fontSize: '15px',
   color:'black'
...
var extCheck13 = ui.Checkbox('IMG-2020').setValue(true);
//Add widgets to app
panel.add(extCheck4)
     .add(extCheck5)
     .add(extCheck6)
...
var doCheckbox13 = function(){
  ...
  Vegetacion_topanel.setShown(checked)} 
  )}
  doCheckbox13();