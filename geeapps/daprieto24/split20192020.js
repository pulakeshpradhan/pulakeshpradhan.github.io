var AOI = ui.import && ui.import("AOI", "table", {
    ...
    imageVisParam = ui.import && ui.import("imageVisParam", "imageVisParam", {
      "params": {
        ...
        "min": 0.05837000034749508,
        "max": 0.1865299990028143,
        "gamma": 1
      }
    }) || {"opacity":1,"bands":["B4","B3","B2"],"min":0.05837000034749508,"max":0.1865299990028143,"gamma":1};
...
var series_20 = dataset.filterMetadata('Year','equals', arr[a])
             .map(maskS2clouds)
var Image_2020 = series_20.min().clip(AOI).visualize({bands: ['B4', 'B3', 'B2'], min: 0.047, max: 0.24, gamma: 1.25, opacity: 1});
//Print Image 2020
...
var dataset1 = ee.ImageCollection('COPERNICUS/S2')
              ...
              .filterDate('2019-06-01', '2019-06-30') //Revisar
...
var series_19 = dataset1.filterMetadata('Year','equals', arr[a])
             .map(maskS2clouds)
var Image_2019 = series_19.min().clip(AOI).visualize({bands: ['B4', 'B3', 'B2'], min: 0.04, max: 0.20, gamma: 1.20, opacity: 1});
//print(final_image)
...
var series_18 = dataset2.filterMetadata('Year','equals', arr[a])
             .map(maskS2clouds)
var Image_2018 = series_18.min().clip(AOI).visualize({bands: ['B4', 'B3', 'B2'], min: 0.04, max: 0.20, gamma: 1.20, opacity: 1});
//print(final_image)
...
var dataset3 = ee.ImageCollection('COPERNICUS/S2')
              .filterBounds(AOI)
              .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 5))
              .filterDate('2017-10-30', '2017-11-05') //Revisar
              .sort("CLOUDY_PIXEL_PERCENTAGE")
              .map(function(image){
                return image.set('Year',ee.Image(image).date().get('year'))
              })
