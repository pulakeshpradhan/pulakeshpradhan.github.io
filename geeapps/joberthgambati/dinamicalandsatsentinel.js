...
//var rgbTs = require('users/joberthgambati/Mosaic_Generator:rgb-timeseries.js')
...
var zoomTo
var ATUALIZAR = ui.Button({
  label: "ZOOM"})
var zoomToField = ui.Textbox({
  value: "",
  placeholder: 'LON, LAT (DECIMAL)',
  style: {position: 'bottom-left',
          stretch: 'vertical'
  },
  onChange: function(value) {
  zoomToField.setValue(value).toString();
  return(value);
  }})
// #############################################################################
// ### DEFINE UI ELEMENTS ###
...
ATUALIZAR.onClick(function() {
 try{ 
   zoomTo = zoomToField.getValue().split(",");
  var lat = parseFloat(zoomTo[0]);
  var lon = parseFloat(zoomTo[1]);
  print("Latitude Inserida: " + lat);
  print("Longitude Inserida: " + lon);
  map.setCenter(lat, lon, 15);
  map.addLayer = ee.Geometry.Point(lon, lat);
} 
 catch (e) {print("Você não Inseriu Lat/Lon válidos, verifique se os dados estão corretos e separados por espaço")}
});
// Map/chart panel
...
var LCB_PROPS = {
  startYear: 1984,
  endYear: 2023,///new Date().getFullYear(), 
 ...
 */
 /*
 function displayBrowseImg2(aoiBox, aoiCircle, start, end) {
  clearImgs();
  waitMsgImgPanel.style().set('shown', true);
  imgCardPanel.add(waitMsgImgPanel);
  var visParams = SRGB_PARAMS[rgbSelect.getValue()];
  var dates = ee.List([2015,2016,2017,2018,2019,2020,2021])
  //var dates = col.aggregate_array('composite_year');
  dates.evaluate(function(dates) {
    waitMsgImgPanel.style().set('shown', false);
    dates.forEach(function(date) {
      print(date)
      if (ee.Date(Date.now()) > ee.Date(date + "-" + startDayBox.getValue())) {
      if (date > 2015){
      var img =  ee.ImageCollection("COPERNICUS/S2").filterBounds(aoiBox).filterDate(date + "-" + startDayBox.getValue(), date + "-" + endDayBox.getValue()).first()
      print(ee.Date(Date.now()))
      print(ee.Date(date + "-" + endDayBox.getValue()))
      //col.filter(ee.Filter.eq('year', date)).first();
...
var visParams = SRGB_PARAMS[rgbSelect.getValue()];
print("oi", ee.Date(Date.now()) < ee.Date(2023 + "-" + startDayBox.getValue())); 
var dates = ee.List([2015,2016,2017,2018,2019,2020,2021,2022,2023]);
  //var dates = col.aggregate_array('composite_year');
  dates.evaluate(function(dates) {
    waitMsgImgPanel.style().set('shown', false);
    dates.forEach(function(date) {
      ...
      var img =  ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED").filterBounds(aoiBox).filterDate(date + "-" + startDayBox.getValue(), date + "-" + endDayBox.getValue()).filter(ee.Filter.lte('CLOUDY_PIXEL_PERCENTAGE', 5)).first()
      //col.filter(ee.Filter.eq('year', date)).first();
      var aoiImg = ee.Image().byte()
        .paint(ee.FeatureCollection(ee.Feature(aoiCircle)), 1, 2)
        .visualize({palette: AOI_COLOR});
      if (ee.Image(img.id()) != null) {
     var thumbnail = ui.Thumbnail({
        image: img.visualize(visParams).blend(aoiImg),
        params: {
          region: aoiBox,
...
function renderGraphics(coords) {
  ...
  var aoiCircle = point.buffer(250);
  ...
  print(startDayBox.getValue())
  print(endDayBox.getValue())
  ...
  //var SentinelCol = ee.ImageCollection("COPERNICUS/S2").filterBounds(aoiBox).filterDate("2015" + startDayBox.getValue(), "30-" + endDayBox.getValue())
  // Display the image chip time series.
  print("1")
  displayBrowseImg(col, aoiBox, aoiCircle)
  print("2")
  //displayBrowseImg2(aoiBox, aoiCircle, startDayBox.getValue(), endDayBox.getValue())
