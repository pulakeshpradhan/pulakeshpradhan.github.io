...
var JsonTxt = ui.Textbox({value:'',style: {width: '225px'}})
var controlPanel = ui.Panel({
  widgets: [
 ui.Label('1. Select drawing mode & Draw geometry on map \nor provide GeoJSON text.',
            {whiteSpace: 'pre'}),
    ui.Panel([ui.Button({
...
var ExportArea = PLK //PLK.filter(ee.Filter.eq('Name', 'D6')).geometry() //Map.setCenter(57,30,3)
...
var CloudCoverageLbl = ui.Label({value: 'Includes Images with Max Cloud Coverage (%):',style: { width: '170px'}});
...
var RemoveCloudyPixelsCheck = ui.Checkbox('Remove Cloudy Pixels').setValue(true);
...
//Map.style().set('cursor', 'crosshair');
    ...
    ExportArea = drawingTools.layers().get(0).getEeObject();
    var layers = drawingTools.layers();
    if (layers.get(0).geometries().length()){
      JsonTxt.setValue(ee.Serializer.toJSON(ExportArea))
    } else {
     ExportArea = ee.Deserializer.fromJSON(JsonTxt.getValue());
     Map.centerObject(ExportArea)
    } 
...
MainPanel.add(ui.Label('4. Choose image type once analysis performed successfully. Then, click button to export image and wait for download link.'))
...
var ExportButton = ui.Button({
  ...
  if (ExportData===1){
    ...
    var Download_Image = ui.Label('Error: Less than 125 km2 area allowed to download the Natural/True, False and Land&Water color imagery while 350 km2 for others. Clear the defined area and redraw it with less size.')
    DownloadPanel.add(Download_Image)
    var Download_Image = ui.Label(SelectMap.getValue()).setUrl(ExportImg.getDownloadURL({
    image: ExportImg.serialize(),scale: 10, name:SelectMap.getValue(), filename: SelectMap.getValue(),
  ...
  if (ExportData===2){
    ...
    var n = colList.size().getInfo();
      ...
      if (SelectMap.getValue()==='Natural/True Color' || SelectMap.getValue()==='False Color' || SelectMap.getValue()==='Land & Water Color' || SelectMap.getValue()==='NDVI' || SelectMap.getValue()==='NDWI'){
        imageLabel.setUrl(img.select(bnd).getDownloadURL({image: img.select(bnd).serialize(),scale: 10, 
