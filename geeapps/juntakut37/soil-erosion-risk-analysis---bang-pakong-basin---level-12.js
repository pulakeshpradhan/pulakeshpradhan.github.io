...
var aoi = ee.FeatureCollection("users/juntakut37/Basin_TH/bang_pakong_lv12"), 
...
ee.List(aoi.distinct('HYBAS_ID_s').aggregate_array('HYBAS_ID_s'))
     .sort()
...
function showFloodMap(amphoe) {
  ...
  Map.drawingTools().setShown(false);
  var amphoe = aoi.filterMetadata('HYBAS_ID_s', 'equals', amphoe);
...
Map.addLayer(K, {min: 0, max: 0.60, palette: ['a52508','ff3818','fbff18','25cdff','2f35ff','0b2dab']}, 'KFactor Map', 0);
...
var LS3 = ee.Image(slope.multiply(4.56));
var LS2 = ee.Image(slope).multiply(ee.Image(slope).multiply(65.41));
var LS1 = ee.Image(LS3).add(LS2).add(0.065);
