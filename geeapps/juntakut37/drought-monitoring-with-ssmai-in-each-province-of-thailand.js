 ...
 * for a selected province of Thailand going back to the year 2015.  
 * Click on a province polygon and wait a minute to view the time series.
...
// Messages.
c.clickMessage = ui.Label('🖱️ Click a province to view its Standardized Soil Moisture Anomaly Index (SSMAI) time series.');
...
function drawChart(coords) {
  ...
  var aoi = m.provinces.filterBounds(point);
  var aoiName = aoi.first().get('PROV_NAME');
  var aoiViz = aoi.draw({color: 'black', strokeWidth: 2});
  var aoiVizLayer = ui.Map.Layer(aoiViz, null, 'Selected Province');
...
function checkPoint(coords) {
  ...
  var aoi = m.provinces.filterBounds(geom)
...
function drawRecentSsmai() {
  var recent = m.drought.sort('system:time_start', false).first()
    .select(m.droughtBand).clip(m.provinces);
  ...
  var recentRelLayer = ui.Map.Layer(
    recent, {min: m.palette.minVal, max: m.palette.maxVal,
      ...
      'Surface Soil Moisture Anomaly Index for ' + date, true, 0.7);  
  c.map.layers().set(0, recentRelLayer);
  drawLegend(date);
}
// Overlays provinces of Thailand on the map.
function drawProvinces() {
  var provincesViz = ee.Image().byte().paint({
    featureCollection: m.provinces.filterBounds(m.drought.first().geometry()),
    color: 1,
    width: 1.5
  });
  var provincesVizLayer = ui.Map.Layer(provincesViz, {palette: ['484848']}, 'Provinces of Thailand');
  c.map.layers().set(1, provincesVizLayer);
}
