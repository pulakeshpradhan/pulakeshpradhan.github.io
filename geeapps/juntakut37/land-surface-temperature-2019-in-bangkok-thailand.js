...
var bangkok = ee.FeatureCollection("users/juntakut37/bangkok");
// #############################################################################
var IMG = ee.ImageCollection("COPERNICUS/S2_SR")
            ...
            .clip(bangkok);
print(IMG)
Map.addLayer(IMG,{bands:['B4','B3','B2'],min:0,max:350},'IMG')
// ##################################################
...
var clippedLSTc = modLSTc.mean().clip(bangkok);
...
Export.image.toDrive({
  image: clippedLSTc,
  description: 'LST_Celsius_BK_2019',
  folder: 'LST_folder',
  region: bangkok,
  scale: 1000,
  crs: 'EPSG:4326',
