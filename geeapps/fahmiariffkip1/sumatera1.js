var geometry = ui.import && ui.import("geometry", "geometry", {
    ...
    ee.Geometry.Polygon(
        [[[104.8291651979421, -2.6608656753051516],
          [104.8291651979421, -2.906394524939088],
          [105.13815567645773, -2.906394524939088],
          [105.13815567645773, -2.6608656753051516]]], null, false);
...
var lulc = modis.select('LC_Type1').first().clip(geometry)
            .rename('lulc');
...
var CN2 = CN_whole.clip(geometry).rename('CN2');
...
// the bands are referred to as the 'primary' and 'secondary' properties
var MergeBands = function(aRow) {
  var anImage = ee.Image.cat(aRow.get('primary'), aRow.get('secondary')).clip(geometry);
  return anImage;
};
