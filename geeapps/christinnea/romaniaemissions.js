var romania = ui.import && ui.import("romania", "table", {
      "id": "users/christinnea/Romania"
    }) || ee.FeatureCollection("users/christinnea/Romania"),
    termo = ui.import && ui.import("termo", "table", {
      "id": "users/christinnea/termo"
    }) || ee.FeatureCollection("users/christinnea/termo"),
    timisoara = ui.import && ui.import("timisoara", "table", {
      "id": "users/christinnea/timisoara"
    }) || ee.FeatureCollection("users/christinnea/timisoara"),
    bucuresti = ui.import && ui.import("bucuresti", "table", {
...
var locationDict = {
  'București': {lon:26.100919, lat: 44.428695, zoom: 12},
  'Brașov':{lon:25.605299, lat: 45.651681, zoom:12},
  'Cluj':{lon:23.589345, lat: 46.767983, zoom:12},
  'Iași':{lon: 27.591432, lat:47.154248, zoom:12},
...
var defaultLocation = locationDict['București'];
...
var no2_2019 = ee.ImageCollection('COPERNICUS/S5P/OFFL/L3_NO2')
  ...
  .map(function(image){return image.clip(romania)})
...
//var title_prior= mapPanel.add(ui.Label(
   // 'Concentrații medii de NO2 în perioada premergătoare epidemiei '+ s1, {fontWeight: 'bold', fontSize: '10px', position: 'bottom-left', color: 'slateGrey'}));
var title_during= linkedMap.add(ui.Label(
    'Concentrații medii de NO2 în perioada epidemiei '+s2, {fontWeight: 'bold', fontSize: '10px', position: 'bottom-right', color: 'slateGrey'}));
var title_2019= mapPanel.add(ui.Label(
    'Concentrații medii de NO2 în perioada '+s3, {fontWeight: 'bold', fontSize: '10px', position: 'bottom-left', color: 'slateGrey'}));
...
var header = ui.Label('Harta comparativă a concentrațiilor de dioxid de azot (NO2) troposferic în perioada epidemiei de COVID-19', {fontSize: '15px', fontWeight:'bold', color: 'darkSlateGrey'});
var text_1 = ui.Label(
    'Harta prezintă concentrația de dioxid de azot (NO2) măsurată în troposferă (0-10 km), mediată în perioada de dinaintea începutului epidemiei de COVID-19 (panou dreapta) și timpul scurs în zilele de epidemie (panou stânga). Data de referință este 20 februarie 2020. ',
    {fontSize: '11px'});
var text_2 = ui.Label(
    'Sursa datelor: Sentinel-5P Near Real Time Data (European Comission/ESA/Copernicus)/ Google Earth Engine',
    {fontSize: '11px'});    
var text_3 = ui.Label(
    'Datele sunt actualizate zilnic pentru intervalul de referință. Ultima actualizare: '+ now.format('YYYY-MMM-dd').getInfo(),
    {fontSize: '11px'}); 
var toolPanel = ui.Panel([header, text_1, text_2, text_3], 'flow', {width: '300px'});
//Create external reference with link
var link1 = ui.Label(
    'Informații despre cum măsurăm NO2 la sol (română)', {fontSize:'11px', color:'blue'},
    'https://aerlive.ro/oxizi-de-azot-nox-monoxidul-de-azot-no-dioxidul-de-azot-no2/');
var link2 = ui.Label(
    'Informații despre cum măsurăm NO2 din satelit (engleză)', {fontSize:'11px', color:'blue'},
    'http://www.tropomi.eu/data-products/nitrogen-dioxide');
var linkPanel = ui.Panel(
    [ui.Label('Mai multe informații despre generarea dioxidului de azot:', {fontWeight: 'bold'}), link1, link2]);
...
var locationPanel = ui.Panel([
  ui.Label('Selectează localitate:', {'font-size': '15px', 'fontWeight':'bold' }), locationSelect
]);
...
var legendTitle = ui.Label({
  value: 'Concentrația de NO2(μmol/m^2)',
...
var title_op_1= ui.Label('Opacitate panou drept',
...
var title_op_2= ui.Label('Opacitate panou stâng',
    {fontSize: '14px', position:'bottom-center', color:'darkSlateGrey'});
var viewPanel =
...
var intro= ui.Label('Panou de statistică', {fontSize: '15px', fontWeight:'bold', color:'darkSlateGrey'});
var info= ui.Label('Statisticile prezentate se generează automat la actualizarea zilnică a hărților. Pentru vizualizare și descărcare, utilizați butonul atașat fiecărui grafic.', {fontSize:'11px'})
...
var no2duringTS = ui.Chart.image.seriesByRegion(
  no2_during, bbox, ee.Reducer.mean(), 'tropospheric_NO2_column_number_density', 1.1, 'system:time_start', 'label')
        ...
        .setOptions({
          title: 'Concentratii troposferice de NO2 in perioada 2020', fontSize:'3px',
          vAxis: {title: '(mol/m^2)'},
          hAxis: {format: 'dd-MM'},
          lineWidth: 1,
...
var no2TS2019 = ui.Chart.image.seriesByRegion(
    no2_2019, bbox, ee.Reducer.mean(), 'tropospheric_NO2_column_number_density', 1.1, 'system:time_start', 'label')
        .setChartType('LineChart')
        .setOptions({
          title: 'Concentratii troposferice de NO2 in perioada 2019', fontSize:'3px',
          vAxis: {title: '(mol/m^2)'},
          hAxis: {format: 'dd-MM'},
          lineWidth: 1,
          interpolateNulls: true,
...
var no2_all = ui.Chart.image.seriesByRegion(
    final, bbox, ee.Reducer.mean(), 'tropospheric_NO2_column_number_density', 1.1, 'system:time_start', 'label')
        .setChartType('LineChart')
        .setOptions({
          title: 'Concentratii troposferice de NO2 2019-2020', fontSize:'3px',
          vAxis: {title: '(mol/m^2)'},
          hAxis: {format: 'dd-MM'},
          lineWidth: 1,
          interpolateNulls: true,
