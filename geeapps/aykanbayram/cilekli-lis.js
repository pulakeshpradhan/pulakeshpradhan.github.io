...
Map.add(ui.Label(
    'Land Information System for Cilekli Village in Trabzon-Turkey', {fontWeight: 'bold', fontSize: '24px'}));
print(NM17)
print(NM14)
...
function clearResults() {
  ...
  var instructionsLabel = ui.Label('Select area to compare normalized difference index');
...
var chartTypeToggleButton = ToggleButton(
    [
      {
        label: 'Display parcels attributes',
...
var intro = ui.Panel([
  ui.Label({
    value: 'Index Analysis of the Selected Area',
    style: {fontSize: '20px', fontWeight: 'bold'}
  }),
  ui.Label('**Select a parcel on cadastral map and wait for the application to analyze it. 1-NDVI : It is the index that determines the density of vegetation. 2-NDTI : It is the index that distinguishes residential areas from soil areas by emphasizing them. 3-NPCRI: It is an index that compares the chlorophyll ratio of plants. 4-NDMI : It is the index used for drought monitoring and determination of humidity change in vegetation areas.' )
]);
panel.add(intro);
...
var npcrıChart = ui.Chart.image.series(npcrı, point, ee.Reducer.mean(), 500);
  ...
  panel.widgets().set(4, npcrıChart);
// NDMI tablosu
