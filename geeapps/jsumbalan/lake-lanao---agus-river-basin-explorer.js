//var lanao = ee.FeatureCollection('users/jsumbalan/Bendum_Gaup');
var lanao = ee.FeatureCollection('users/jsumbalan/ALLWS_Watershed_Boundary');
var gain = ee.Image('UMD/hansen/global_forest_change_2020_v1_8').select(['gain']);
var cover = ee.Image('UMD/hansen/global_forest_change_2020_v1_8').select(['treecover2000']).gte(80);
var loss = ee.Image('UMD/hansen/global_forest_change_2020_v1_8').select(['lossyear']);
//var center = lanao.centroid({'maxError': 1});
Map.centerObject(lanao,10.8);
Map.getCenter();
//Map.getZoom();
...
var gains = ui.Map.Layer(gain.updateMask(gain).clip(lanao),
...
//App title
var header = ui.Label('Lake Lanao - Agus River Basin Forest Loss Explorer(Development)', {fontSize: '25px', fontWeight: 'bold', color: '4A997E'});
//App summary
var text = ui.Label(
...
var opacityshd = ui.Slider({
       ...
       var waterdesc = ui.Label({value: 'Lake Lanao-Agus River Basin Boundary'});
...
//Gain
       ...
       var gaindesc = ui.Label({value: 'Forest Gain 2000–2012'});
