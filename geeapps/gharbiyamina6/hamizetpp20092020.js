...
var hamiz = ee.FeatureCollection("users/gharbiyamina6/hamiz");
...
var elev = {min: 0, max: 200, palette: elevPalette};
Map.centerObject(hamiz,10,5);
//sélectionner la première bande 'DSM' et mosaïque puis clip le DSM à la limite de la zone d'étude hamiz
var dsm_hamiz= dsm.select('DSM').filterBounds(hamiz).mosaic().clip(hamiz);
print(dsm_hamiz);
//  ajouter le DSM découpé à la vue de la carte
Map.addLayer(dsm_hamiz, elev, 'Elevation (ALOS)');
...
// Filtrer de 20009 à septembre 2020 
var pcp = GPM.filterDate('2009-10-01', '2020-09-30')
              .filterBounds(hamiz)
...
// Filtrer ETIa à la zone d'étude hamiz
// Filtrer de 2009 à 2020
var ETIa_filt = ETIa.filterDate('2009-10-01', '2020-09-30')
                  .filterBounds(hamiz);
// Créer une fonction pour mettre les valeurs à l'échelle avec un facteur d'échelle de 0,1
function scaleETIa(image) {
  var scaled = image.multiply(0.1);
  return scaled;
}
// Mapper la fonction à l'imageCollection
var scaledImage = ETIa_filt.map(scaleETIa);
...
//  pour l'application web, créer un panneau  qui affichera des séries temporelles en cliquant sur la carte à l'intérieur de hamiz
...
Map.onClick(function(coords) {
  ...
  var sample = dsm_hamiz.sample(point, 30);
  ...
  chart1.setOptions({
    title: 'Monthly Precipitation 2009-2020',
