...
app.createPanels = function() {
  ...
  app.intro = {
    panel: ui.Panel([
      ui.Label({
        value: 'Explorando Sentinel 2',
        style: {fontWeight: 'bold', fontSize: '24px', margin: '10px 5px'}
      }),
      ui.Label('Esta aplicación te permite filtrar y exportar imágenes.' +
               'de la colección Sentinel 2.')
    ])
  };
  /* The collection filter controls. */
  app.filters = {
    mapCenter: ui.Checkbox({label: 'Filtrar al centro del mapa', value: true}),
    ...
    button: ui.Button({
      ...
      onClick: function() {
        ...
        Export.image.toDrive({
          ...
          description: 'Sentinel_2_Export' + imageIdTrailer,
        });
      }
    })
  };
...
app.createConstants = function() {
  ...
  app.VIS_OPTIONS = {
    'Falso Color (B8/B4/B3)': {
      description: 'La vegetación es de tonos rojos, las áreas urbanas son' +
                   'azul cian y suelos marrones',
      visParams: {gamma: 1, min: 0, max: 3000.0, bands: ['B8', 'B4', 'B3']}
    },
    'Color natural (B4/B3/B2)': {
      ...
      visParams: {gamma: 1.0, min: 0.0, max: 3000.0, bands: ['B4', 'B3', 'B2']}
    },
    'Atmosférica (B12/B8/B3)': {
      description: 'Lineas de costas definidas ' +
                   'la vegetacion aparece azul.',
      visParams: {gamma: 1.0, min: 0, max: 4000.0, bands: ['B12', 'B8', 'B3']}
    }
  };
};
