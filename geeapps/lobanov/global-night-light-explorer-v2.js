...
var maskClouds = function(img) {
  var mask = img.select(cloudBand).gt(cloudThreshold);
...
var updateMap = function() {
  ...
  var countryBorderLayer = ui.Map.Layer(country.style(borderStyle), {}, 'country border');
  Map.layers().set(0, compositeLayer);
  Map.layers().set(1, countryBorderLayer);
};
var countrySelectorHandler = function(name) {
  print('User selected:', name);
  countryName = name;
  updateMap();
  chartingPanel.widgets().set(0, ui.Label('Click on the map to chart time-serires...'));
  Map.centerObject(country, 6);
};
var startSliderDateHandler = function(dateRange) {
  var date = ee.DateRange(dateRange).start();
  dateStart = date;
  updateMap();
};
var endSliderDateHandler = function(dateRange) {
  var date = ee.DateRange(dateRange).start();
  dateEnd = date;
  updateMap();
};
var mapClickHandler = function(coords) {
  var clickedPoint = ee.Geometry.Point([coords.lon, coords.lat]).buffer(5000);
  var clickedPointLayer = ui.Map.Layer(clickedPoint, {color: 'yellow'}, 'point');
  Map.layers().set(2, clickedPointLayer);
  var chart = ui.Chart.image.series({
    imageCollection: maskedColl.select(dataBand), 
    region: clickedPoint, 
    reducer: ee.Reducer.mean().combine(ee.Reducer.minMax(), '', true), 
    scale: maskedColl.first().projection().nominalScale(), 
  })
  // .setSeriesNames([
  //         'Max', 'Mean', 'Min'
  //       ])
  ...
  chartingPanel.widgets().set(0, chart);
};
// #############################################################################
// ### 3 IMPLEMENTATION ###
// #############################################################################
...
var startDateSelector = ui.DateSlider({
  ...
  value: dateStart, 
  // period, 
  onChange: startSliderDateHandler, 
  style: {width: '72%'}
});
