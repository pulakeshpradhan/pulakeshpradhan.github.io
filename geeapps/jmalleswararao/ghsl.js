var built_up = ee.Image("JRC/GHSL/P2016/BUILT_LDSMT_GLOBE_V1"),
    ...
    district = ee.FeatureCollection("users/saurabh/India_Map"),
...
var state_dict = tools.featureCollection.toDictionary(district, {idField:'NAME_1'} ).getInfo();
...
function addLayer_builtup(start_year, end_year, state){
  ...
  if(start_year==1975 && end_year == 1975){
      var builtUp1975 = {
        min: 1.0,
        max: 3.0,
        palette: ['0c1d60', 'FFFFFF', 'FFA07A'],
      };
    var x = builtUp.updateMask(builtUp.eq(6).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(x.clip(state), builtUp1975, 'Built-Up till 1975');
  }
  else if(start_year==1975 && end_year == 1990){
    var builtUp1975_1990 = {
      min: 1.0,
      max: 4.0,
      palette: ['0c1d60', 'FFFFFF', 'F08080', 'FFA07A'],
    };
    var y = builtUp.updateMask(builtUp.eq(6).add(builtUp.eq(5)).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(y.clip(state), builtUp1975_1990, 'Built-Up 1975 to 1990');
  }
  else if(start_year==1975 && end_year == 2000){
    var builtUp1975_2000 = {
      min: 1.0,
      max: 5.0,
      palette: ['0c1d60', 'FFFFFF', 'FA0000', 'F08080', 'FFA07A'],
    };
    var z = builtUp.updateMask(builtUp.eq(6).add(builtUp.eq(5)).add(builtUp.eq(4)).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(z.clip(state), builtUp1975_2000, 'Built-Up 1975 to 2000');
  }
   ...
   Map.addLayer(builtUp.clip(state), builtUp1975_2015, 'Built-Up 1975 to 2015');
  }
  else if(start_year==1990 && end_year == 1990){
    var builtUp1990 = {
      min: 1.0,
      max: 3.0,
      palette: ['0c1d60', 'FFFFFF', 'F08080'],
    };
    var p = builtUp.updateMask(builtUp.eq(5).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(p.clip(state), builtUp1990, 'Built-Up 1990');
  }
  else if(start_year==1990 && end_year == 2000){
    var builtUp1990_2000 = {
      min: 1.0,
      max: 3.0,
      palette: ['0c1d60', 'FFFFFF', 'FA0000'],
    };
    var q = builtUp.updateMask(builtUp.eq(5).add(builtUp.eq(4)).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(q.clip(state), builtUp1990_2000, 'Built-Up 1990 to 2000');
  }
  else if(start_year==1990 && end_year == 2015){
    var builtUp1990_2015 = {
      min: 1.0,
      max: 4.0,
      palette: ['0c1d60', 'FFFFFF', '8B0000', 'FA0000'],
    };
    var r = builtUp.updateMask(builtUp.eq(5).add(builtUp.eq(4)).add(builtUp.eq(3)).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(r.clip(state), builtUp1990_2015, 'Built-Up 1990 to 2015');
  }
    ...
    var s = builtUp.updateMask(builtUp.eq(4).add(builtUp.eq(2)).add(builtUp.eq(1)));
    Map.addLayer(s.clip(state), builtUp2000, 'Built-Up upto 2000');
  }
  else{
    var builtUp2000_2015 = {
      min: 1.0,
      max: 3.0,
      palette: ['0c1d60', 'FFFFFF', '8B0000'],
    };
    var t = builtUp.updateMask(builtUp.eq(3).add(builtUp.eq(2)).add(builtUp.eq(1)));
     Map.addLayer(t.clip(state), builtUp2000_2015, 'Built-Up upto 2000 to 2015');
  }  
}
...
function addLayer(year, state){
  Map.addLayer(year.clip(state), populationCountVis, 'Population Count');
}
var color = function(){
  ////////////////////////////////////////////////////////////////
...
//Variable for storing State Date
var search = null ;
...
var chart = null;
//Variable for return the data for start and end Year
var return_addLayer = function(sy,ey,state)
{
 ...
 function addImage(image) { // display each image in collection
      ...
      Map.addLayer(Layer_image.clip(state), populationCountVis, 'PopulationCount'+s[i]);
      i=i+1;
    }
  ...
  .setOptions({
          ...
          vAxis: {title: 'Population in state '},
          hAxis: {title: 'YEARS'},
          color: '00FF00',
...
//Selecting the States by Dictionary
var select_state = ui.Select({
  placeholder: 'Select State',
  items: Object.keys(state_dict),
  onChange: function(key) {
  var data =   centerobject_state(key);
  search = data;
  }
});
...
builtup_button.onClick(function(){
  addLayer_builtup(year_start, year_end, search);
  builtUp_chart_panel.widgets().set(0,builtup_chart(search));
  color();
});
...
population_button.onClick(function(){
  return_addLayer(start_year, end_year, search);
  color_population();
});
...
var centerobject_1 = function(district0){
  var object = district.filter(ee.Filter.eq('NAME_2', district0));
  Map.centerObject(object);
  return object;
};
...
var district_fun = function(state){
  ...
  var district_data = district.filter(ee.Filter.eq('NAME_1', state));
  var district_dict = tools.featureCollection.toDictionary(district_data, {idField:'NAME_2'} ).getInfo();
  return district_dict;
};
////// Function for State Data to Retrieve District
var state_data = function(state){
  var s_data = tools.featureCollection.toDictionary(district, {idField:'NAME_1'} ).getInfo();
  return s_data;
};
...
gsw_panel.add(sd_label);
gsw_panel.add(split_panel);
