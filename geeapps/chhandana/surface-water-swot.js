var table = ui.import && ui.import("table", "table", {
      "id": "projects/ee-chhandana/assets/SWORD/SWOT_FAST_TRACK"
    }) || ee.FeatureCollection("projects/ee-chhandana/assets/SWORD/SWOT_FAST_TRACK"),
    ...
    image = ui.import && ui.import("image", "image", {
      "id": "projects/ee-chhandana/assets/HLS_T16TGP_20200402T162137Z_20220806T024706Z_L8_30_v01_B01_WTR"
    }) || ee.Image("projects/ee-chhandana/assets/HLS_T16TGP_20200402T162137Z_20220806T024706Z_L8_30_v01_B01_WTR"),
    image2 = ui.import && ui.import("image2", "image", {
      "id": "projects/ee-chhandana/assets/toulouse_par_swot_0_HIGH_projectedV13f"
    }) || ee.Image("projects/ee-chhandana/assets/toulouse_par_swot_0_HIGH_projectedV13f"),
    image3 = ui.import && ui.import("image3", "image", {
      "id": "projects/ee-chhandana/assets/SWOT_ClassV3"
    }) || ee.Image("projects/ee-chhandana/assets/SWOT_ClassV3"),
    image4 = ui.import && ui.import("image4", "image", {
      "id": "projects/ee-chhandana/assets/SWOT_ClassV3_exp"
    }) || ee.Image("projects/ee-chhandana/assets/SWOT_ClassV3_exp"),
    image5 = ui.import && ui.import("image5", "image", {
      "id": "projects/ee-chhandana/assets/SWOT_ClassV3_exp_4326"
    }) || ee.Image("projects/ee-chhandana/assets/SWOT_ClassV3_exp_4326"),
    image6 = ui.import && ui.import("image6", "image", {
      "id": "projects/ee-chhandana/assets/toulouse_projectedV13f_4326"
    }) || ee.Image("projects/ee-chhandana/assets/toulouse_projectedV13f_4326"),
    table3 = ui.import && ui.import("table3", "table", {
      "id": "projects/ee-chhandana/assets/swordreach_eu_hb23_v14"
    }) || ee.FeatureCollection("projects/ee-chhandana/assets/swordreach_eu_hb23_v14");
//SWOT_FAST_TRACK
// Time Series
// SWOT-WATER-MARCH-2023
// March-2019
// cg
...
//Map.addLayer(table,{color: 'FF0000'},'SWOT_FAST_TRACK');
Map.setCenter(1.4332, 43.5742,12)
var shown = true; // true or false, 1 or 0 
var opacity = 0.5; // number [0-1]
var nameLayer = 'SWOT_FAST_TRACK'; // string
...
var updateRoi = function(roi, startDate, endDate) {
  ...
  var filterSpeckles = function(img) {
    ...
    var vv_smoothed = vv.focal_median(20,'circle','meters').rename('VV_Filtered'); //Apply a focal median filter
  ...
  Map.addLayer(S1.first(),{bands: 'VV_Filtered',min: -18, max: 0}, 'Filt SAR image');
  ...
  var classifyWater = function(img) {
    ...
    var water = vv.lt(-15.8).rename('Water');  // -16 Identify all pixels below threshold and set them equal to 1. All other pixels set to 0
  ...
  ClassChart.onClick(function(xValue, yValue, seriesName) {
      ...
      var classSWOT = ee.Image(image5).clip(roi).select('b1'); 
      ...
      var visParams = {
        min: 0,
        max: 1,
        palette: ['#FFFFFF','#0000FF'] //#3cdfff
      };
      var visParams2 = {
        min: 0,
        max: 1,
        palette: ['#003366','#B22222'] //#3cdfff  '#003366' 
      };
      //Add water classification on top of SAR image
      Map.addLayer(table,{color: 'FF0000'},'SWOT_FAST_TRACK',shown,opacity);
      Map.addLayer(image6.clip(roi), {bands: 'b1',color: 'blue'},'SWOT image');
      Map.addLayer(image5.clip(roi), {bands: 'b1',color: '#FFFFFF', min: 1, max: 2},'WaterSWOT2');      
//      Map.addLayer(table2,{color: '0000FF'},'SWORD-Congo');
      Map.addLayer(table3,{color: '0000FF'},'SWORD-Europe');
      Map.addLayer(classSWOT,visParams2,'SWOT KaRIn Water map');
      Map.addLayer(classification,visParams,'Sentinel-1 SAR Water map');
//Map.addLayer(image5.mask(image6.select('b1')), {bands: 'b1',color: 'blue', min: 1, max: 2},'WaterSWOT2');     Map.addLayer(image5, {bands: 'b1',color: 'red', min: 1, max: 2},'WaterSWOT2');
...
changeRoi();
print('image3',image3);
print('image5-water', image5);
//Map.addLayer(table,visParams, nameLayer,shown,opacity);
//Map.addLayer(table2,{color: '0000FF'},'SWORD-Congo');
//Map.addLayer(image2, {color: '0000FF'},'Filtered SAR image');
//Map.addLayer(image3, {bands: 'b1',color: '1E05FF',min: 0, max: 1},'Filtered SAR image');
//{bands: 'b1',min: 0, max: 2}
//Map.addLayer(image3, {bands: 'b1',color: 'blue', min: 0, max: 1},'WaterSWOT');
// Make a water image out of the mask.
//var water = image6.mask.not;
// Mask water with itself to mask all the zeros (non-water).
//image5 = image5.mask(water);
//Map.addLayer(image6, {bands: 'b1',color: 'blue'},'SWOT image');
//Map.addLayer(image5.mask(image6.select('b1')), {bands: 'b1',color: 'blue', min: 1, max: 2},'WaterSWOT2');
//Map.addLayer(image5, {bands: 'b1',color: 'red', min: 1, max: 2},'WaterSWOT2');