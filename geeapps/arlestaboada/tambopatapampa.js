...
var bounds=ee.Geometry.Polygon([[
    [-70.38872286624553,-13.079108729185426],
    [-70.38940951175334,-13.106529353522282],
    [-70.34958407230022,-13.107198111096572],
  ]])
  var recorte=ee.Geometry.Polygon([[
   ...
   var recorte1=ee.FeatureCollection('users/arlestaboada/SUP_CON_MINERIA_ILEGAL').geometry();
var cuenca=recorte;
var ICfilter;
var ndwi;
var mascara_sst;
   ...
   app.refreshMapLayer = function() {
    ...
    if (imageId) {
      ...
      image = ee.Image(COLLECTION_ID + '/' + imageId).clip(cuenca);
       image1 = ee.Image(COLLECTION_ID + '/' + imageId).clip(recorte1);
     ...
     app.picker.fecha_ad.setValue(date_ad.getInfo())
      if(opcion_vis.valor<2){
        ...
        Map.addLayer(image1,visParams, imageId);
      }else if( opcion_vis.valor===2){
        if(menor_2012){
          ...
          Map.addLayer(image,visParams_I, imageId);
           Map.addLayer(image1,visParams_I, imageId);
          ...
          if(menor_2012){
            ...
            mascara_sst.visualize({min:0,max:1,palette:[ 'b2ffff',"0000ff",'ffff00',"ff0000"]})
             ...
             Map.addLayer(image1,visParams_N, imageId);
