var punto_interes=ee.FeatureCollection('users/arlestaboada/app/Rumialba');
var recorte=punto_interes;
...
collectionLCFinal.evaluate(function(){
 // Map.addLayer( collectionLCFinal.first(),{},'Tragadero');
});
// Create an intro panel with labels.
var intro = ui.Panel([
  ui.Label({
    value: 'Serie temporal de  Rumialba',
...
/***codigo****/
var area=ee.FeatureCollection('users/arlestaboada/app/Rumialba');
var bosque=area.filter(ee.Filter.eq('DESUSO','BOSQUE'));
var nobosque=area.filter(ee.Filter.eq('DESUSO','NO BOSQUE'));
//Map.addLayer(bosque,{'color':'green'},'bosque');
//Map.addLayer(nobosque,{'color':'red'},'no bosque');
var cloud_masks = require('users/fitoprincipe/geetools:cloud_masks');
//'options (list): cloud, shadow, snow'  =>["snow"]
var  landsatTOAfunction = cloud_masks.landsatTOA ();
var coleccionLandsat=ee.ImageCollection("LANDSAT/LT05/C01/T1_TOA")
                       .filterBounds(area)
                       .filterDate("2010-01-01","2010-12-31")
                       //filtrar por metadatos
                       .filterMetadata("CLOUD_COVER","less_than",23)
                        .sort("CLOUD_COVER")
                        .map(landsatTOAfunction);
var mediana=coleccionLandsat.median();
var bosqueColeccionLandsat=mediana.clip(bosque).normalizedDifference(['B4','B3']);    
var noBosqueColeccionLandsat=mediana.clip(nobosque).normalizedDifference(['B4','B3']) ;  
Map.addLayer( bosqueColeccionLandsat,{'palette':['red','black','green']},'Bosque 2010');
Map.addLayer(noBosqueColeccionLandsat,{'palette':['red','black','green']},'No Bosque 2010');
var artesanas=ee.FeatureCollection('users/arlestaboada/app/artesanas');
var apicultores=ee.FeatureCollection('users/arlestaboada/app/apicultores_zava');
var apicultores_za=ee.FeatureCollection('users/arlestaboada/app/apicultores_zava_za');
var apicultores_ptos=ee.FeatureCollection('users/arlestaboada/app/apicultores_zava_ptos');
Map.addLayer( artesanas,{color:'orange'},'Artesanas');
Map.addLayer(apicultores,{color:'blue'},'Apicultores');
Map.addLayer(apicultores_za,{color:'blue'},'Apicultores za');
//Map.addLayer(apicultores_ptos,{color:'blue'},'Apicultores ptos');
/*codigo****/
//Map.addLayer(punto_interes,{},'shape');
