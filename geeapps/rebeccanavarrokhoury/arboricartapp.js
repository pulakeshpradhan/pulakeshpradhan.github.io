...
Map.drawingTools().addLayer([], "Cultures d'hiver");
Map.drawingTools().addLayer([], 'Cultures de printemps');
Map.drawingTools().addLayer([], "Cultures d'été");
Map.drawingTools().addLayer([], 'Double saison');
Map.drawingTools().addLayer([], 'Arbustes'); 
Map.drawingTools().addLayer([], 'Autres types de végétation');
Map.drawingTools().addLayer([], 'Sol imperméable');
Map.drawingTools().addLayer([], 'Sans végétation');
Map.drawingTools().addLayer([], 'Vergers à feuillage caduc');
Map.drawingTools().addLayer([], 'Vergers à feuillage persistant');
...
var urlGeom= ui.Label('Télécharger le fichier KML', {shown: false})
// Download function
...
var exportDataButton = ui.Button('Générer un lien de téléchargement');
...
var controlPanel = ui.Panel({
  widgets:[
    ui.Label('Collecte de données de référence', TITLE_STYLE),
    ui.Label("1) Zoomez sur votre zone d'intérêt."),
    ui.Label("2) Sélectionnez une année pour l'analyse des séries chronologiques NDVI."),
    dropdown,
    ui.Label('3) Cliquez sur un champ et observez le tracé de la série chronologique NDVI.'),
    ui.Label("4) Sélectionnez un mode de dessin, choisissez ou créez une classe LULC (voir 'Geometry Imports'), et commencez à dessiner. Quittez le mode dessin pour tracer d'autres séries temporelles NDVI."),
    ui.Panel({
      widgets:[
    ui.Button({
    ...
    layout: ui.Panel.Layout.flow('horizontal', true) 
     ...
     ui.Button({
      label:  'Quitter le mode de dessin',
      onClick: quitDrawing,
      style: {stretch: 'horizontal'}
    }),
    ui.Label('5) Une fois terminé, générez un lien de téléchargement.'),
    exportDataButton,
     urlGeom,
