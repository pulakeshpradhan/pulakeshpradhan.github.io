var geometry = ui.import && ui.import("geometry", "geometry", {
    ...
    ee.Geometry.Polygon(
        [[[103.741629601982, -1.5499531809754037],
          [103.741629601982, -1.71021997091968],
          [103.91603756096637, -1.71021997091968],
          [103.91603756096637, -1.5499531809754037]]], null, false);
//===========================================================================================
...
var dataset = ee.ImageCollection('FIRMS').filter(
    ee.Filter.date('2019-08-04', '2019-08-05')).mean().clip(geometry);
...
var DIFF_UPPER_THRESHOLD = -0.6201658993959427;
...
Map.addLayer(diff_smoothed, {min:-10,max:10}, 'Different Threshold', 0);
...
// Add the image to the map using both the color ramp and interval schemes.
Map.addLayer(dNBR.sldStyle(sld_intervals2), {}, 'dNBR classified2');
// Seperate result into 8 burn severity classes
var thresholds = ee.Image([-500, -251, -101, 99, 269, 439, 659, 2000]);
var kelas = dNBR.expression(
  'dNBR<=-251?1: dNBR<= -101?2:dNBR<=99?3:dNBR<= 269?4:dNBR<=439?5:dNBR<=659?6:dNBR<=2000?7:8',{
  'dNBR':dNBR
  }).clip(geometry).rename('area');
var classified = dNBR.lt(thresholds)//.reduce('sum').toInt();
print ('kelas NBR',kelas);
Map.addLayer(kelas,{palette:['7a8737', 'acbe4d', '0ae042', 'fff70b', 'ffaf38', 'ff641b', 'a41fd6', 'ffffff']}, 'kelas');
var thresholds2 = ee.Image([439, 659, 2000]);
var classified2 = dNBR.lt(thresholds2)//.reduce('sum').toInt();
//==========================================================================================
//                              ADD BURNED AREA STATISTICS
// count number of pixels in entire layer
var allpix =  classified2.updateMask(classified2);  // mask the entire layer
...
var areacount = function(cnr, name) {
 var singleMask =  classified2.updateMask(classified2.eq(cnr));  // mask a single class
...
Export.image.toDrive({image: fires.updateMask(fires), scale: 100, description: id, fileNamePrefix: 'hotspot',
