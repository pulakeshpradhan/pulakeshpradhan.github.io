...
var geometry = 
    ...
    /* shown: false */
    ee.Geometry.Polygon(
        [[[120.09403007523889, -2.347100925267569],
          [120.09403007523889, -2.989101025939675],
          [121.06632011430139, -2.989101025939675],
          [121.06632011430139, -2.347100925267569]]], null, false);
...
var after_start='2020-07-10';
var after_end='2020-07-17';
/********************************************************************************************
...
var difference_threshold = 1.2; /*threshodl to be applied on the difference image (after flood
 ...
 */
/**
function maskS2clouds(imageS2) {
  var qa = imageS2.select('QA60');
  ...
  return imageS2.updateMask(mask).divide(10000);
}
// Map the function over one year of data and take the median.
// Load Sentinel-2 TOA reflectance data.
var S2 = ee.ImageCollection('COPERNICUS/S2');
// Terapkan Filter Awan
var S2Cld = S2
            // Pre-filter to get less cloudy granules.
          .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
          .map(maskS2clouds);
// Penerapan rentan waktu
var S2_bfr = S2Cld.filterDate(before_start, before_end);
var S2_aftr = S2Cld.filterDate(after_start, after_end);
var rgbVis = {
  min: 0.0,
  max: 0.3,
  bands: ['B5', 'B3', 'B2'],
};
Map.addLayer(S2_bfr.median().clip(aoi), rgbVis, 'RGB_Bfr');
Map.addLayer(S2_aftr.median().clip(aoi), rgbVis, 'RGB_Aftr');
*/
//------------------------------  DISPLAY PRODUCTS  ----------------------------------//
// Before and after flood SAR mosaic
Map.setCenter(120.3543, -2.6846, 11);
...
var title = ui.Label('Banjir Masamba', titleTextVis);
...
// Edited by...
var text8 = ui.Label('Script edited by: RS & GIS Laboratory Politeknik Pertanian Negeri Samarinda July 2020', subTextVis);
