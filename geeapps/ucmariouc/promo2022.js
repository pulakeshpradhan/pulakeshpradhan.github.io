Map.style().set('cursor', 'crosshair');
Map.setOptions('HYBRID');
Map.drawingTools().setShown(false);
var cuenca = ee.FeatureCollection('users/ucmariouc/BUSTILLOS');
var Bienvenida=ui.Panel({
    style: {
    position: 'top-center',
    border: '3px solid white',
    backgroundColor: 'rgba(8, 97, 24, 0.5)'
    }
  });
var Mensaje=ui.Label({
      value: 'Lic. en Geoinformática - D.M. UACJ, en cd. Cuauhtémoc, Chih.',
      style: {
        //margin: '1px 1px 1px 10px',
        fontSize: '17px',
        fontWeight: 'bold',
        color: 'white',
        textAlign: 'center',
        backgroundColor: 'rgba(46, 123, 167, 0)',
        stretch: 'horizontal',
        }
    });
var linkedl=ui.Label({value:'Visitanos en: https://www.facebook.com/GeoUACJ',
      style: {
        margin: '1px 10px 10px 10px',
        textAlign: 'center',
        fontSize: '12px',
        color: 'white',
        fontWeight: 'bold',
        backgroundColor: 'rgba(46, 123, 167, 0)',
        stretch: 'horizontal'
        }});
var ayuda=ui.Label({value:'Serie de tiempo de la vegetación: Conocenos',
      style: {
        margin: '1px 10px 10px 10px',
        textAlign: 'center',
        fontSize: '11px',
        color: 'white',
        fontWeight: 'bold',
        backgroundColor: 'rgba(46, 123, 167, 0)',
        stretch: 'horizontal',
        }});
var cred=ui.Label({value:'Creditos: http://www.gisandbeers.com/crear-timelapses-en-google-earth-engine/',
      style: {
        margin: '1px 10px 10px 10px',
        textAlign: 'center',
        fontSize: '7px',
        color: 'white',
        fontWeight: 'bold',
        backgroundColor: 'rgba(46, 123, 167, 0)',
        stretch: 'horizontal',
        }});
var Geo = ui.Chart([
  ['<a target="_blank" href=https://www.facebook.com/GeoUACJ>' +
   '<b> Dame Clic y Visita Facebook ó...!!! 🔍</b>'],
  ['<a target="_blank" href=https://api.whatsapp.com/send?phone=526562053203/>' +
   '<center><b> <img src=data:image/jpeg;base64, width=40px> </b></center>' +
   '<center><b> Da clic y envia un mensaje por Whatsapp </b></center>' + 
   '<center><h> Para mas Información </h><center>']
  ],
   'Table', {allowHtml: true});
Bienvenida.add(Mensaje);
Bienvenida.add(linkedl);
Bienvenida.add(ayuda);
Bienvenida.add(Geo);
Bienvenida.add(cred)
Map.add(Bienvenida);
var mde = ee.Image("USGS/SRTMGL1_003");
var sombras = ee.Terrain.hillshade(mde, 270, 45);
// Llamamos a la colección de imágenes y filtramos fechas de análisis
var MODIS = ee.ImageCollection('MODIS/006/MOD13Q1')
  .filterDate('2022-03-01', '2023-03-14');print(MODIS)
// Hacemos una llamada a la banda del índice o lo componemos a través del juego de bandas
var IndiceEVI = MODIS.select(['EVI']);
// Definimos las coordenadas máximas y mínimas de visualización en el timelapse
var ZonaAOI = ee.Geometry.Rectangle({
  coords: [[-107.6098, 29.1641], [-106.4398, 28.3599]]});
// Parametrizamos el timelapse con proyección, resolución, AOI, valores de pixel y frames/seg
var Timelapse = {
  crs: 'EPSG:4326',
  dimensions: '400',
  region: ZonaAOI,
  min: 0,
  max: 3000,
  palette: '#4FDECC, #EFF4EE, #ECFF00, #038112, #DE714F, #19BD08, #60DE4F',
  framesPerSecond: 2,};
// Creamos la animación con la colección de imagenes y parámetros del timelapses
var Animacion = ui.Thumbnail({
  image: IndiceEVI ,
  params: Timelapse,
  style: {
    position: 'bottom-right', 
    width: '400px'}});
Map.add(Animacion);
// Adicionalmente visualizamos una imagen compuesta del timelapse sobre el visor
var MODIS_Composicion = ee.Image(IndiceEVI .median());
Map.addLayer (MODIS_Composicion, {
  min: 0,
  max: 3000,
  palette: '#4FDECC, #EFF4EE, #ECFF00, #038112, #DE714F, #19BD08, #60DE4F',
  bands: ['EVI']}, 
  'Indice EVI');
Map.centerObject(ZonaAOI, 8);
Map.addLayer(cuenca,{},'Cuenca')
var visSombras = {"opacity":0.4,"bands":["hillshade"],"min":152,"max":209,"gamma":1};
Map.addLayer(sombras, visSombras, 'Sombras')