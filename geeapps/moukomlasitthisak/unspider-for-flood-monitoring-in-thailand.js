...
var geometry = ee.FeatureCollection('USDOS/LSIB_SIMPLE/2017').filter(ee.Filter.eq('country_na', 'Thailand'));
...
var polarization = "VH"; /*or 'VV' --> VH mostly is the prefered polarization for flood mapping.
                           ...
                           as well.*/ 
/*var pass_direction = "ASCENDING"; /* "ASCENDING">> when images are being compared use only one 
...
var LC = ee.ImageCollection("MODIS/006/MCD12Q1")
  .filterDate('2001-01-01',after_end)
...
Map.addLayer(before_filtered, {min:-25,max:0}, 'ข้อมูลก่อนน้ำท่วม',0);
Map.addLayer(after_filtered, {min:-25,max:0}, 'ข้อมูลหลังน้ำท่วม',1);
// Difference layer
//Map.addLayer(difference,{min:0,max:2},"Difference Layer",0);
// Flooded areas
Map.addLayer(flooded,{palette:"0000FF"},'พื้นที่น้ำท่วม');
// Population Density
...
Map.addLayer(population_count, populationCountVis, 'ความหนาแน่นประชากร',0);
// Exposed Population
...
Map.addLayer(population_exposed, populationExposedVis, 'คาดการประชากรที่ได้รับผลกระทบ');
// MODIS Land Cover
var LCVis = {
  min: 1.0,
  max: 17.0,
...
Map.addLayer(LC, LCVis, 'สิ่งปกคุลมดิน',0);
// Cropland
var croplandVis = {
  min: 0,
  max: 14.0,
...
Map.addLayer(cropland, croplandVis, 'พื้นที่เกษตรกรรม',0)
// Affected cropland
Map.addLayer(cropland_affected, croplandVis, 'พื้นที่เกษตรกรรมที่ได้รับผลกระทบ'); 
// Urban
var urbanVis = {
  min: 0,
  max: 13.0,
...
Map.addLayer(urban, urbanVis, 'พื้นที่เมือง',0)
// Affected urban
Map.addLayer(urban_affected, urbanVis, 'พื้นที่เมืองที่ได้รับผลกระทบ'); 
//------------------------------------- EXPORTS ------------------------------------//
...
// Create legend title
var legendTitle = ui.Label('สัญลักษณ์',titleTextVis);
...
var palette =['#0000FF', 'yellow', 'red'];
// name of the legend
var names = ['พื้นที่น้ำท่วม','พื้นที่เกษตรงที่ได้รับผลกระทบ', 'พื้นที่เมืองที่ได้รับผลกระทบ'];
...
var title = ui.Label('ผลการวิเคราะห์', titleTextVis);
//var text1 = ui.Label('พื้นที่น้ำท่วม',textVis);
...
var text2 = ui.Label('พื้นที่น้ำท่วมโดยประมาณ',textVis);
...
flood_area_ha.evaluate(function(val){number2.setValue(val+' ไร่')}),numberVIS;
// Estimated number of exposed people
var text3 = ui.Label('ประมาณการจำนวนประชากรที่ได้รับผลกระทบ:' ,textVis);
var text3_2 = ui.Label('ข้อมูลประชากรจาก WorldPop (100 เมตร)',subTextVis);
...
var text4 = ui.Label('พื้นที่เกษตรที่ได้รับผลกระทบ',textVis);
...
crop_area_ha.evaluate(function(val){number4.setValue(val+' ไร่')}),numberVIS;
// Estimated area of affected urban
var text5 = ui.Label('พื้นที่เมืองที่ได้รับผลกระทบ', textVis);
...
urban_area_ha.evaluate(function(val){number5.setValue(val+' ไร่')}),numberVIS;
// Disclaimer
var text6 = ui.Label('Disclaimer: ผลิตภัณฑ์นี้ได้รับมาโดยอัตโนมัติโดยผู้ผลิตจะไม่รับผิดชอบใด ๆ เกี่ยวกับเนื้อหาหรือการใช้เนื้อหาดังกล่าว',subTextVis)
// Produced by...
var text7 = ui.Label('Script produced by: UN-SPIDER December 2019 ปรับปรุงโดย GeographyLounge 2021 ', subTextVis)
...
//value: 'ความหนาแน่นของประชากรที่จะได้รับผลกระทบ',
//style: {
//fontWeight: 'bold',
//fontSize: '12px',
//margin: '10px 0 0 0',
