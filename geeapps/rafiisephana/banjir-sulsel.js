var geometry = ui.import && ui.import("geometry", "table", {
      "id": "users/rafiisephana/Sulsel"
    }) || ee.FeatureCollection("users/rafiisephana/Sulsel");
//Menentukan periode sebelum banjir
var before_start= '2020-07-01';
var before_end='2020-07-10';
//Menentukan waktu setelah banjir
var after_start='2020-07-13';
var after_end='2020-07-23';
...
var pass_direction = "DESCENDING"; /* atau 'ASCENDING', bisa diganti jika image collection kosong atau tidak ditemukan.*/
var difference_threshold = 1.1; /* ambang batas ini didapatkan dari proses trial and error, jika pada wilayah studi terlihat kesalahan, anda bisa mengubah nilai ini*/
...
//Menghitung populasi terdampak banjir 
...
//Membuat layer raster populasi terdampak banjir yang telah disesuaikan dengan layer banjir
var population_exposed = population_count
  .updateMask(flooded_res1)
  .updateMask(population_count);
//Menjumlahkan nilai piksel dari populasi terdampak banjir 
...
//Mendapatkan angka populasi terdampak banjir dalam format integer
var number_pp_exposed = stats.getNumber('population_count').round();
//Area lahan pertanian terdampak banjir 
...
//Menghitung area terdampak banjir 
var cropland_affected = flooded_res
  .updateMask(cropland)
//Mendapatkan area piksel dari area terdampak banjir
var crop_pixelarea = cropland_affected
  .multiply(ee.Image.pixelArea());
//Menjumlahkan piksel area terdampak banjir
...
//Area banjir
Map.addLayer(flooded,{palette:"16a1e4"},'Flooded areas');
//Kepadatan populasi
...
// Export data populasi terdampak banjir
Export.image.toDrive({
...
//Membuat Judul
var title = ui.Label({
  value: 'Banjir Bandang Sulawesi Selatan, 2020',
  style:{
  fontWeight: 'bold',
  fontSize: '25px',
  color: '1f9bbf'
...
var title = ui.Label('Analisis Banjir ', titleTextVis);
var text1 = ui.Label('Periode Kejadian Banjir :',textVis);
var number1 = ui.Label(after_start.concat(" sampai ",after_end),numberVIS);
//Estimasi luasan banjir
var text2 = ui.Label('Estimasi Luasan Banjir :',textVis);
var text2_2 = ui.Label('processing...',subTextVis);
dates(after_collection).evaluate(function(val){text2_2.setValue('Sumber : Data Klasifikasi Sentinel-1 ESA Copernicus'+val)});
var number2 = ui.Label('processing...',numberVIS);
flood_area_ha.evaluate(function(val){number2.setValue(val+' ha')}),numberVIS;
//Estimasi jumlah populasi terdampak banjir
var text3 = ui.Label('Estimasi Populasi Terdampak Banjir : ',textVis);
var text3_2 = ui.Label('Source: Data classification GHSL 2015 (Resolution 250m)',subTextVis);
var number3 = ui.Label('processing...',numberVIS);
number_pp_exposed.evaluate(function(val){number3.setValue(val+' ha')}),numberVIS;
// Estimasi lahan pertanian terdampak banjir
var MODIS_date = ee.String(LC.get('system:index')).slice(0,4);
var text4 = ui.Label('Estimasi Lahan Pertanian Terdampak:',textVis);
var text4_2 = ui.Label('Processing', subTextVis)
MODIS_date.evaluate(function(val){text4_2.setValue('Source : Data classification MODIS Land Cover'+val +' (Resolution 500m)')}), subTextVis;
var number4 = ui.Label('processing...',numberVIS);
crop_area_ha.evaluate(function(val){number4.setValue(val+' ha')}),numberVIS;
// Estimated area perkotaan terdampak banjir
var text5 = ui.Label('Estimasi Permukiman Terdampak :',textVis);
var text5_2 = ui.Label('Processing', subTextVis)
MODIS_date.evaluate(function(val){text5_2.setValue('Source : Data classification MODIS Land Cover '+val +' (Resolution 500m)')}), subTextVis;
var number5 = ui.Label('processing...',numberVIS);
urban_area_ha.evaluate(function(val){number5.setValue(val+' ha')}),numberVIS;
// Diolah oleh
var text7 = ui.Label ('Script bersumber dari UN-SPIDER & U-Inspire serta dimodifikasi', subTextVis)
...
var palette =['#16a1e4', '#8bcd79', '#ff218c'];
//Nama legenda
var names = ['Area Banjir','Area Pertanian Terdampak' , 'Area Permukiman Terdampak '];
