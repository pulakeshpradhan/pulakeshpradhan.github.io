//thêm panel
var vnp = ee.FeatureCollection("users/thanhntrung/vn_province");
Map.setCenter(108,17,6);
...
var ts_title = ui.Label("Nhập các thông số thời gian, Tỉnh thành phố và quỹ đạo ảnh để tính ngập-lụt trên ảnh vệ tinh Sentinel-1",titleTextVisB);
var label_ngaytr1 = ui.Label("Thời gian trước ngập lụt 01: yyyy-mm-dd",textVis);
var label_ngaytr2 = ui.Label("Thời gian trước ngập lụt 02: yyyy-mm-dd",textVis);
var label_ngaysau1 = ui.Label("Thời gian xảy ra ngập lụt 01: yyyy-mm-dd",textVis);
var label_ngaysau2 = ui.Label("Thời gian xảy ra ngập lụt 02: yyyy-mm-dd",textVis);
////////////////////////////////////////////////////////DATE SET UP//////////////////////////
   //Today´s date as the default end date of the regression. 
   var now = new Date();
   var nowStr = now.toLocaleDateString('en-CA'); 
   var endDate2 = ui.Textbox({
     value: nowStr,
...
//Today-12m´s date as the default end date of the regression. 
   var lastMonth1 = dateNow.advance(-2, 'month').format ("YYYY-MM-dd");
   var lastMonth2 = dateNow.advance(-3, 'month').format ("YYYY-MM-dd");
   var endDate1 = ui.Textbox({
     value: lastMonth1.getInfo(),
     placeholder: 'yyyy-mm-dd',
     onChange: function(value) {
    return(value);
  }
   ...
   //Get the starting date 5 years before today's day date.
   var startDate1 = ui.Textbox({
     value: lastMonth2.getInfo(),
     placeholder: 'yyyy-mm-dd.',
     onChange: function(value) {
    return(value);
  }
    });
var tinh = vnp.aggregate_array('ADM1_VI')
var tinh_lst = ui.Select([], '...')
  tinh.evaluate(function(states){
  tinh_lst.items().reset(states)
  tinh_lst.setPlaceholder('Chọn Tỉnh')
  tinh_lst.onChange(function(state){
    print(tinh_lst.getValue())
    })
  })
//var tinh_name = tinh_lst.getValue();
//print(tinh_name);
//làm mới
//print(endDate1.getValue());
var text_qd = ui.Label('Chọn quỹ đạo bay của vệ tinh Seltinel-1',titleTextVis);
var text_ttitle = ui.Label('Chọn tỉnh, thành phố',titleTextVis);
var quydao = ui.Select(['DESCENDING','ASCENDING'],'DESCENDING','DESCENDING');
//xủ lý ảnh////////////////////////////////////////////////////////
var submit = ui.Button({
  ...
  onClick: function() {
    ...
    //chartPanel.clear()
    //ui.root.clear();
    //ui.root.widgets().get(0)
 Map.clear();   
var tinh_name = tinh_lst.getValue();
var title_tinh = ui.Label(tinh_name,titleTextVis);
// thêm ngày vào đây
    var filter = ee.Filter.inList('ADM1_VI', [tinh_name]);
    var aoi = vnp.filter(filter);
    //Map.addLayer(aoi, {color: 'green'}, 'filteredArea');
/*===========================================================================================
...
//var before_end='2020-09-01';
var before_start = startDate1.getValue();
var before_end = endDate1.getValue();
// Now set the same parameters for AFTER the flood. 
//var after_start='2020-10-01';
//var after_end='2020-10-30';
var after_start = startDate2.getValue();
var after_end = endDate2.getValue();
/********************************************************************************************
...
var pass_direction = quydao.getValue();
/********************************************************************************************
...
//var aoi = ee.FeatureCollection(aoi1);
...
//to export
var desc_before = ee.Image.cat(
        //desc.filter(spring).mean(),
        //desc.filter(lateSpring).mean(),
        before_collection.mean());
var desc_after = ee.Image.cat(
        //desc.filter(spring).mean(),
        //desc.filter(lateSpring).mean(),
        after_collection.mean());
...
//Dem
var demmr = ee.Image("MERIT/DEM/v1_0_3").updateMask(flooded);
//var demmr_pxarea = demmr.multiply(ee.Image.pixelArea());
//Map.addLayer(demmr_pxarea,{},'DEM');
...
var minMaxValues = demmr.reduceRegions({reducer: ee.Reducer.minMax(),
                                 collection: flooded_vec
                                 });
//print('minMaxValues', minMaxValues);
// Make an image out of the land area attribute.
var theMax = minMaxValues
  .filter(ee.Filter.notNull(['max']))
  .reduceToImage({
    properties: ['max'],
    reducer: ee.Reducer.first()
});
var theDiff = theMax.subtract(demmr).multiply(100).divide(2);
// Compute the volumn
/*var vol = image.expression(
    '((maxe - dem) * 900)/2', {
      'maxe': theMax,
      'dem': demmr
});*/
var vol = theDiff.reduceRegion({
...
var realValue = ee.Number(vol.get("first")).int();
//var flood_m3 = vol
 // .multiply(ee.Image.pixelArea());
 //var thetich = realValue.int();
//print(realValue.int());
...
Map.addLayer(before_filtered, {min:-25,max:0}, 'Ảnh S1 trước khi bị ngập',0);
Map.addLayer(after_filtered, {min:-25,max:0}, 'Ảnh S1 sau khi bị ngập',1);
// Difference layer
//Map.addLayer(difference,{min:0,max:2},"Difference Layer",0);
// Flooded areas
Map.addLayer(flooded,{palette:"0000FF"},'Vùng bị ngập');
// Population Density
...
Map.addLayer(population_exposed, populationExposedVis, 'Số người dân bị ảnh hưởng');
// MODIS Land Cover
var LCVis = {
  min: 1.0,
  max: 17.0,
...
Map.addLayer(LC, LCVis, 'Lớp phủ',0);
// Cropland
var croplandVis = {
  min: 0,
  max: 14.0,
...
Map.addLayer(cropland, croplandVis, 'Vùng đất trồng',0)
// Affected cropland
Map.addLayer(cropland_affected, croplandVis, 'Vùng ảnh hưởng đến dất trồng'); 
// Urban
var urbanVis = {
  min: 0,
  max: 13.0,
...
Map.addLayer(urban, urbanVis, 'Đô thị',0)
// Affected urban
Map.addLayer(urban_affected, urbanVis, 'Vùng ảnh hưởng đến đô thị'); 
//------------------------------------- EXPORTS ------------------------------------//
...
var title = ui.Label('Kết quả ngập lụt tỉnh'+ " - " + title_tinh.getValue(), titleTextVis);
//var text1 = ui.Label('Thời xảy ra gian ngập lụt:',textVis);
//dates(after_collection).evaluate(function(val){text1.setValue(val)});
//var number1 = ui.Label(after_start.concat(" đến ",after_end),numberVIS);
//var number1 = ui.Label("",numberVIS);
//(after_collection).evaluate(function(val){number1.setValue(val)});
...
var text2 = ui.Label('Vùng ngập lụt:',textVis);
var text2_2 = ui.Label('Please wait...',subTextVis);
dates(after_collection).evaluate(function(val){text2_2.setValue('Nguồn ảnh: Senintel-1 imagery '+val)});
var number2 = ui.Label('Please wait...',numberVIS); 
flood_area_ha.evaluate(function(val){number2.setValue(val+' ha')}),numberVIS;
// Estimated number of exposed people
var text3 = ui.Label('Số người bị ảnh hưởng: ',textVis);
var text3_2 = ui.Label('Nguồn ảnh: GHSL 2015 (250m)',subTextVis);
...
var text4 = ui.Label('Diện tích ảnh hưởng đến đất trồng:',textVis);
var text4_2 = ui.Label('Please wait', subTextVis)
MODIS_date.evaluate(function(val){text4_2.setValue('Nguồn ảnh: MODIS Land Cover '+val +' (500m)')}), subTextVis;
...
var text5 = ui.Label('Diện tích ảnh hưởng đến đô thị:',textVis);
var text5_2 = ui.Label('Please wait', subTextVis)
MODIS_date.evaluate(function(val){text5_2.setValue('Nguồn ảnh: MODIS Land Cover '+val +' (500m)')}), subTextVis;
var number5 = ui.Label('Please wait...',numberVIS);
urban_area_ha.evaluate(function(val){number5.setValue(val+' ha')}),numberVIS;
//
var text8 = ui.Label('Thể tích ngập nước (m3):',textVis);
var text8_2 = ui.Label('Please wait', subTextVis);
var number8 = ui.Label('Please wait...',numberVIS);
realValue.evaluate(function(val){
  //var n = val.getInfo();
  number8.setValue(val)}),numberVIS;
//number8.setValue(ee.Number(realValue));
text8_2 = ui.Label('Nguồn DEM MERIT', subTextVis);
// Disclaimer
var text6 = ui.Label('Thông tin được chiết xuất từ các nguồn ảnh khác nhau chỉ mang tính chất nghiên cứu hoa học, chưa được kiểm chứng về độ chính xác. Thận trọng khi công bố kết quả này!',subTextVis)
// Produced by...
var text6_2 = ui.Label('Mã nguồn: UN-SPIDER December 2019 và được VTGEO-IGS (ntthanh@igsvn.vast.vn) hiệu chỉnh theo lãnh thổ Việt Nam', subTextVis)
//vol.first(function(val){text8_2.setValue(val+' m3')}),numberVIS;
...
// Create legend title
var legendTitle = ui.Label('Chú giải',titleTextVis);
...
var palette =['#0000FF', '#30b21c', '#ff80ed'];
// name of the legend
var names = ['Vùng ngập lụt','Vùng đất trồng bị ảnh hưởng','Vùng đô thị bị ảnh hưởng'];
