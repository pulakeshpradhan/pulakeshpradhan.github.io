...
var states = ee.FeatureCollection("TIGER/2018/States")
  .filterMetadata('DIVISION', 'not_equals', '0'); // filtering out non-state territories
var counties = ee.FeatureCollection("TIGER/2018/Counties");
// var averageMultiplier = 2; // country average multiplier for diff. color stretch
var aoi = states; // default AOI
// var initialAoi = null; // state name e.g. 'California' OR null for USA;
var initialAoi = 'California'; // state name e.g. 'California' OR null for USA;
var stateZoom = 6;
...
var descriptionText = 'This Earth Engine app uses Sentinel-5P satellite data to display average air pollution levels, greenhouse gas levels and other atmospheric data in the U.S.A. over a selected time period. Toggle the select dataset drop down menu to choose between air pollutants, greenhouse gases and other data. Use the state drop down to select a state and then the county drop down to select a county, if interested. Then use the time slider to set the date range of interest. This will then return a map of the average value for that dateset over the selected period of time. Check the comparison mode checkbox to view an anomaly map where each pixel is compared to the region average. Click anywhere on the map to view the value of the data at a point. Export the data at each pixel as a CSV by clicking the export button.';
//var creditText = 'Created by James Poetzscher for the California Air Resources Board'
var image;
...
var show_query_point = true;
var query_point_style = {color: 'red'}; //color, pointSize, pointShape, width, fillColor, styleProperty, neighborhood, lineType
// Define vis palettes:
...
// # UI Handlers
function aoiIsCountryMode(checked) {
  if (checked) {
    aoi = states;
    imageOpacity = countryOpacity;
    refreshMap();
    Map.centerObject(aoi);
    stateSelector.setValue(null, false);
    // stateSelector.setDisabled(true);
    countySelector.items().reset();
    countySelector.setDisabled(true);
  } else {
    imageOpacity = stateOpacity;
    // stateSelector.setDisabled(false);
    // countySelector.setDisabled(false);    
  }
}
function stateSelectorHandler(stateFP) {
  aoiIsCountryCheckbox.setValue(false, false);
  countySelector.setDisabled(false);
  countySelector.setPlaceholder('Loading...');
  countySelector.items().reset();
  var stateCounties = counties.filterMetadata("STATEFP", 'equals', stateFP);
  aoi = stateCounties;
  refreshMap();
  Map.centerObject(aoi, stateZoom);
  var countyDict = makeNameIdDict(stateCounties, 'NAME', 'GEOID');
  countyDict.evaluate(function(counties) {
    var items = [];
    for (var key in counties) {
      items.push({'label': key, 'value': counties[key]});
    }
    countySelector.items().reset(items);
    countySelector.setPlaceholder('Select a County');
  });
}
function countySelectorHandler(countyGEOID) {
  var county = counties.filterMetadata("GEOID", 'equals', countyGEOID);
  aoi = county;
  refreshMap();
  Map.centerObject(aoi);
}
...
function aggregateStats(localMeanImage, regionMeanValue, regionName) {
  ...
  var diffLabel = ['Difference from', regionName.replace(/_/g, ', '), 'average,', thisData.unitsLabel].join(' ');
  print(diffLabel)
...
function computeExports() {
  ...
  // var regionMeanImage = ee.Image(ee.Number(regionMeanValue));
  // var differenceImage = currentImage.subtract(regionMeanImage);
  var countyName = getLabelByValue(countySelector, countySelector.getValue());
  var stateName = getLabelByValue(stateSelector, stateSelector.getValue());
  var regionNamePrefix = [countyName, stateName, 'USA'].filter(Boolean).join('_');
...
function setStates() {
  var statesDict = makeNameIdDict(states, 'NAME', 'STATEFP');
  statesDict.evaluate(function(states) {
    var items = [];
    for (var key in states) {
      items.push({'label': key, 'value': states[key]});
    }
    stateSelector.items().reset(items);
    if (initialAoi) {
      if (initialAoi in states) {
        stateSelector.setValue(states[initialAoi]);
      } else {
        alert('Invalid state name: ' + initialAoi);
      }
    }
  });
}
function getLabelByValue (selector, value) { // get selector label by value
  ...
  return null; // if selecor is empty or does not contain the requested value
}
...
var introPanel = ui.Panel([
  ...
  ui.Label({
    value: subText,
    style: {fontSize: '12px', fontWeight: 'bold', textAlign: 'center', stretch: 'horizontal',}
  }),
   ui.Label({
    value: descriptionText,
    style: {fontSize: '12px', textAlign: 'center', stretch: 'horizontal'}
  }),
]);
var aoiIsCountryCheckbox = ui.Checkbox({
  label: 'All of the USA', 
  value: true, 
  onChange: aoiIsCountryMode, 
  // disabled, 
  // style
});
var stateSelector = ui.Select({
  ...
  // value, 
  onChange: stateSelectorHandler, 
  // disabled: true, 
  style: {width: '45%'}
});
var countySelector = ui.Select({
  placeholder: 'Select a County', 
  // value, 
  onChange: countySelectorHandler, 
  disabled: true, 
  style: {width: '45%'}
});
var aoiSelectPanel = ui.Panel({
  widgets: [
    ui.Label({value: 'Select region of interest:'}),
    aoiIsCountryCheckbox,
    ui.Panel([stateSelector, countySelector], ui.Panel.Layout.flow('horizontal'), {stretch: 'horizontal'})
    ],
});
// setStates();
