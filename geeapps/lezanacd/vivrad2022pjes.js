var table = ui.import && ui.import("table", "table", {
    ...
    table4 = ui.import && ui.import("table4", "table", {
      "id": "users/lezanacd/ign_paraje_munic"
    }) || ee.FeatureCollection("users/lezanacd/ign_paraje_munic");
/*
...
var dicobjetos = {
  select:{
    ...
    (table3.filter(ee.Filter.inList('NOMMUNI_C_', [k]))),
    (table4.filter(ee.Filter.inList('NOM_MUNIC', [k])))
...
var boton = ui.Button({label:'ver municipio', onClick:function(k){
  ...
  Map.addLayer(table3.filter(ee.Filter.inList('NOMMUNI_C_', [k]))),
  Map.addLayer(table4.filter(ee.Filter.inList('NOM_MUNIC', [k])).style(pje_style)),
...
var radios = ee.FeatureCollection('users/lezanacd/vivxrad_22')
            .filterBounds(table3);
var pjes = ee.FeatureCollection('users/lezanacd/ign_paraje_munic')
            .filterBounds(table3);
