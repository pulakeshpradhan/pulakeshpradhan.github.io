var table2 = ui.import && ui.import("table2", "table", {
      "id": "users/lezanacd/Dptos_22"
    }) || ee.FeatureCollection("users/lezanacd/Dptos_22"),
    table = ui.import && ui.import("table", "table", {
      "id": "users/lezanacd/frac_2220"
    }) || ee.FeatureCollection("users/lezanacd/frac_2220"),
...
var dicobjetos = {
  select:{
    dptos: ui.Select({
    items: Object.keys(dic_Deptos).sort(dic_Deptos), 
    placeholder:'Seleccione Departamento', 
    onChange: function(k){
    ...
    frac: ui.Select({
    items: Object.keys(dic_fraccion).sort(dic_fraccion), 
   ...
   //Map.centerObject(table.filter(ee.Filter.inList('FRAC', [i])), 10)
    ...
    rad: ui.Select({
    items: Object.keys(dic_radio).sort(dic_radio),
