var table2 = ui.import && ui.import("table2", "table", {
      "id": "users/lezanacd/2021_viv_munic"
    }) || ee.FeatureCollection("users/lezanacd/2021_viv_munic"),
    table = ui.import && ui.import("table", "table", {
      "id": "users/lezanacd/2022_viv_munic"
    }) || ee.FeatureCollection("users/lezanacd/2022_viv_munic");
/*
...
var dicobjetos = {
  select:{
    ...
    onChange: function(k){
    (table.filter(ee.Filter.inList('NOM_MUNIC', [k])))
    //Map.centerObject(table2.filter(ee.Filter.inList('NOM_MUNIC', [k])), 8)
    }}),
...
var boton = ui.Button({label:'ver municipio', onClick:function(k){
  ...
  print(k),
  Map.addLayer(table.filter(ee.Filter.inList('NOM_MUNIC', [k])).style(dpto_style)),
  Map.centerObject(table.filter(ee.Filter.inList('NOM_MUNIC', [dicobjetos.select.munic.getValue()])))
}})
/*
-----------------------------------
Boton de limpiar
...
Map.clear();
// Map.add(panel);
Map.addLayer(munic.style(COUNTRIES_STYLE));
//Map.addLayer(pjes);
...
//dicobjetos.label,
//dicobjetos.label2,
//SAQUE BOTON "boton2" de la vista por redundancia con el limpiar del grafico
boton,boton2], layout: ui.Panel.Layout.Flow('vertical'), style: {backgroundColor: '00005555', position: 'bottom-right'}})
Map.add(panel)
/*SCRIP COMPARACION DE MUNICIPIO*/
var munic = ee.FeatureCollection('users/lezanacd/2022_viv_munic')
            .filterBounds(table);
...
var HIGHLIGHT_STYLE = {fillColor :Math.floor(Math.random()*16777215).toString(16)};
...
function getSelectedMunic() {
  return munic.filterBounds(ee.Geometry.MultiPoint(selectedPoints));
}
// Makes a bar chart of the given FeatureCollection of countries by name.
function makeResultsBarChart(munic) {
  var chart = ui.Chart.feature.byFeature(munic,'NOM_MUNIC','CANT_VIV');
  chart.setChartType('BarChart');
  //chart.setSeriesNames();
  chart.setOptions({
    title: 'Comparación de viviendas',
    vAxis: {title: null},
    hAxis: {title: 'Viviendas relevadas por Municipio 2021-22 (CANT)', minValue: 0},
    colors:[
  ...
  //chart.style({fillcolor:'FF0000'});
  return chart;
}
// Makes a table of the given FeatureCollection of countries by name.
function makeResultsTable(munic) {
  var tablemn = ui.Chart.feature.byFeature(munic,'CODMUNI_C_');
  tablemn.setChartType('Table');
  tablemn.setOptions({allowHtml: true, pageSize: 5});
  tablemn.style().set({stretch: 'both'});
  return tablemn;
}
// Updates the map overlay using the currently-selected countries.
function updateOverlay() {
  var overlay = getSelectedMunic().style(HIGHLIGHT_STYLE);
...
function updateChart() {
  ...
  var chart = chartBuilder(getSelectedMunic());
...
function clearResults() {
  ...
  var instructionsLabel = ui.Label('Seleccione el Municipio para comparar cant. viviendas.');
