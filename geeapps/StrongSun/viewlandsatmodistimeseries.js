...
var createYearPanel = function(){
  ...
  var startYearslider = ui.Slider({min:2000, max:2019, value:2001, step:1});
  startYearslider.style().set({stretch:'horizontal'});
  var endYearLabel = ui.Label('End Year:');
  var endYearslider = ui.Slider({min:2000, max:2019, value:2010, step:1});
...
var lon_lat_panel=function()
{
  ...
  var scale_label=ui.Label('scale:')
  ...
  var panel=ui.Panel([label1,panelA,panelB,day_panel,scale_panel],ui.Panel.Layout.flow('vertical'))
  panel.style().set('width','400px')
  return panel
}
...
map1.onClick(function(coords)
{
  ...
  mapPanel.widgets().get(1).widgets().get(1).widgets().get(1).setValue(lon.toFixed(2))
  mapPanel.widgets().get(1).widgets().get(2).widgets().get(1).setValue(lat.toFixed(2))
  map1.centerObject(point,10)
})
//Creat charts and generate Landsat NDVI and MODIS NDVI
var creat_charts=require('users/StrongSun/View_MODIS_Landsat_NDVI_Timeseries:Landsat_MODIS_TimeSeries_Chart_generate')
var generateNDVI=require('users/StrongSun/View_MODIS_Landsat_NDVI_Timeseries:generateNDVI')
submit_btn.onClick(function()
{
  //get the point
  var lon=parseInt(mapPanel.widgets().get(1).widgets().get(1).widgets().get(1).getValue())
  var lat=parseInt(mapPanel.widgets().get(1).widgets().get(2).widgets().get(1).getValue())
  var point=ee.Geometry.Point(lon,lat)
  //get the start_date and end_date
  var startDate=ee.Date(mapPanel.widgets().get(0).widgets().get(1).widgets().get(1).getValue().toString())
  var endDate=ee.Date(mapPanel.widgets().get(0).widgets().get(2).widgets().get(1).getValue().toString())
  ...
  var kernelSize=parseInt(mapPanel.widgets().get(1).widgets().get(4).widgets().get(1).getValue())
  //get the charts
  var charts=creat_charts.generate_Landsat_Modis_chart(point,kernelSize,LandsatNDVI,MODISNDVI)
