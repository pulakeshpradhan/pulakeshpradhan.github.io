...
app.createPanels = function() {
  ...
  app.intro = {
    panel: ui.Panel([
      ...
      ui.Label('This app allows you to filter, visualize and export images ' +
               'from the Landsat 8 collection.'+
               'This app is implemented from Earth Engine Example Scripts')
    ])
  };
  ...
  app.filters = {
    ...
    endDate: ui.Textbox('YYYY-MM-DD', '2018-09-01'),
...
app.createHelpers = function() {
  ...
  app.applyFilters = function() {
    ...
    var filtered = ee.ImageCollection(app.COLLECTION_ID).filterMetadata("CLOUD_COVER_LAND","less_than",5);
  ...
  app.refreshMapLayer = function() {
    ...
    if (imageId) {
      ...
      var thermal   = image.select( ['B11'] );
      var maxReducer = ee.Reducer.max();
      var theMax = thermal.reduceRegion({
        reducer:maxReducer,
        bestEffort:true
      });
      var minReducer = ee.Reducer.min();
      var theMin = thermal.reduceRegion({
        reducer:minReducer,
        bestEffort:true
      })
      var minimum = theMin.get('B11').getInfo()
      var maximum = theMax.get('B11').getInfo()
      // Add the image to the map with the corresponding visualization options.
      var visOption = app.VIS_OPTIONS[app.vis.select.getValue()];
      var visParamsthermal= {"opacity":1,"bands":["B11"],min:minimum,max:maximum,"palette":["ff0000","00ff2a","3d00ff"]};
      Map.addLayer(image, visParamsthermal, imageId);
    }
    Map.addLayer(danish_samples)
  };
};
...
app.createConstants = function() {
  ...
  app.VIS_OPTIONS = {
    'Thermal (B11)':{
      description: 'Band 11 ------ \n' +
      'Lower reflectance values appear red & higher values appear blue',
      visParams: {opacity:1,bands:["B11"],min:2500,max:2900,palette:["ff0000","00ff2a","3d00ff"]}
    }
  };
...
app.boot = function() {
  ...
  Map.centerObject(danish_samples,8);
