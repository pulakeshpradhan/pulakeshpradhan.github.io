var imageVisParam = ui.import && ui.import("imageVisParam", "imageVisParam", {
    ...
    }) || {"opacity":1,"bands":["B4","B3","B2"],"min":1632,"max":3950.75,"gamma":1},
    imageVisParam2 = ui.import && ui.import("imageVisParam2", "imageVisParam", {
      "params": {
        ...
        "min": 41.57142857142857,
        "max": 109.6,
        "gamma": 1.5000000000000002
      }
    }) || {"opacity":1,"bands":["Oa08_radiance","Oa06_radiance","Oa04_radiance"],"min":41.57142857142857,"max":109.6,"gamma":1.5000000000000002},
...
app.createPanels = function() {
  ...
  app.intro = {
    panel: ui.Panel([
      ui.Thumbnail({
        image: ee.Image('users/crystalwhite4real2015/KWOKBS').visualize({bands:  ['b1','b2','b3'],min: 0,max: 255}),
      params: {
          dimensions: '2201x709',
          format: 'png'
        },
      ...
      ui.Label('A rapid earth remote sensing data wrapper to view and download small scale, near real time and archival satellite imagery.  Zoom to an area on the planet, select your time frame, and hit process!  Note the file downloaded may be cropped to the center of the screen depending on the scale requested.'),
      ui.Label({
        value: 'Built on Google Earth Engine, development funded by the Kansas Water Office and managed by the Kansas Biological Survey',
  ...
  app.helper = {
    ...
    row_MOD_data:ui.Label({value:'',style:{fontSize:'10px',margin:'2px 5px'}}),
    row_MYD_data:ui.Label({value:'',style:{fontSize:'10px',margin:'2px 5px'}}),
...
app.createHelpers = function() {
  ...
  app.process_aoi = function() {
    ...
    // optic_label: ui.Label('Optical:', {fontWeight: 'bold'}),
    // optic_low_label: ui.Label('Optical (low spatial/temporal res):', {fontWeight: 'bold'})
    if(app.filtered_sent1.size().getInfo()>0) { 
...
app.createConstants = function() {
  ...
  app.MOD09_DATA = ee.ImageCollection('MODIS/006/MOD09GA');
  app.MYD09_DATA = ee.ImageCollection('MODIS/006/MYD09GA');
...
app.boot = function() {
  ...
  var main = ui.Panel({
    widgets: [
      ...
      ui.Panel({widgets: [app.helper.row_sent1_header,app.helper.row_sent1_data],layout: ui.Panel.Layout.flow("horizontal")}),
      ...
      ui.Panel({widgets: [app.helper.row_MOD_header,app.helper.row_MOD_data],layout: ui.Panel.Layout.flow("horizontal")}),
      ui.Panel({widgets: [app.helper.row_MYD_header,app.helper.row_MYD_data],layout: ui.Panel.Layout.flow("horizontal")}),
  ...
  // app.aoi_def.aoi_select.setValue(app.AOI_SELECT_MAPBOUNDS);
  Map.setCenter(-95.25, 38.95, 12);
  app.process_aoi();
};
app.boot();