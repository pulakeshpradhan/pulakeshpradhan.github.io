...
var selama = ee.ImageCollection("COPERNICUS/S5P/NRTI/L3_CO")
  .select('CO_column_number_density')
...
'Peta Emisi Karbon Monoksida (CO)', {fontWeight: 'bold', fontSize: '20px', position: 'top-center', color: 'black'}));
...
//Membuat grafis CO sebelum PSBB (Kota Medan)
...
grafSebelum.setOptions({
  title: 'Emisi CO di Kota Medan dan Sekitarnya sebelum PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
);
print(grafSebelum);
//Membuat grafis CO selama PSBB (Kota Medan)
...
grafSelama.setOptions({
  title: 'Emisi CO di Kota Medan dan Sekitarnya selama PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSelama);
//Membuat grafis CO sebelum PSBB (Kota Jakarta)
var grafSebelum = ui.Chart.image.seriesByRegion({
  imageCollection: sebelum,
...
grafSebelum.setChartType('ColumnChart');
grafSebelum.setOptions({
  title: 'Emisi CO di Kota Jakarta dan Sekitarnya sebelum PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSebelum);
//Membuat grafis CO selama PSBB (Kota Jakarta)
var grafSelama = ui.Chart.image.seriesByRegion({
  imageCollection: selama,
  regions: buffer_Jakarta,
  reducer: ee.Reducer.mean(),
...
grafSelama.setChartType('ColumnChart');
grafSelama.setOptions({
  title: 'Emisi CO di Kota Jakarta dan Sekitarnya selama PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSelama);
//Membuat grafis CO sebelum PSBB (Kota Balikpapan)
var grafSebelum = ui.Chart.image.seriesByRegion({
  imageCollection: sebelum,
...
grafSebelum.setChartType('ColumnChart');
grafSebelum.setOptions({
  title: 'Emisi CO di Kota Balikpapan dan Sekitarnya sebelum PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSebelum);
//Membuat grafis CO selama PSBB (Kota Balikpapan)
var grafSelama = ui.Chart.image.seriesByRegion({
  imageCollection: selama,
  regions: buffer_Balikpapan,
  reducer: ee.Reducer.mean(),
...
grafSelama.setChartType('ColumnChart');
grafSelama.setOptions({
  title: 'Emisi CO di Kota Balikpapan dan Sekitarnya selama PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSelama);
//Membuat grafis CO sebelum PSBB (Kota Makassar)
var grafSebelum = ui.Chart.image.seriesByRegion({
  imageCollection: sebelum,
...
grafSebelum.setChartType('ColumnChart');
grafSebelum.setOptions({
  title: 'Emisi CO di Kota Makassar dan Sekitarnya sebelum PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSebelum);
//Membuat grafis CO selama PSBB (Kota Makassar)
var grafSelama = ui.Chart.image.seriesByRegion({
  imageCollection: selama,
  regions: buffer_Makassar,
  reducer: ee.Reducer.mean(),
...
grafSelama.setChartType('ColumnChart');
grafSelama.setOptions({
  title: 'Emisi CO di Kota Makassar dan Sekitarnya selama PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSelama);
//Membuat grafis CO sebelum PSBB (Kota Jayapura)
var grafSebelum = ui.Chart.image.seriesByRegion({
  imageCollection: sebelum,
...
grafSebelum.setChartType('ColumnChart');
grafSebelum.setOptions({
  title: 'Emisi CO di Kota Jayapura dan Sekitarnya sebelum PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
print(grafSebelum);
//Membuat grafis CO selama PSBB (Kota Jayapura)
var grafSelama = ui.Chart.image.seriesByRegion({
  imageCollection: selama,
  regions: buffer_Jayapura,
  reducer: ee.Reducer.mean(),
...
grafSelama.setChartType('ColumnChart');
grafSelama.setOptions({
  title: 'Emisi CO di Kota Jayapura dan Sekitarnya selama PSBB',
  vAxis: {title: 'Kadar CO (mol/m^2)'},
  lineWidth: 1,
  pointSize: 4,
  }
...
var header = ui.Label('Peta Emisi Karbon Monoksida (CO) Sebelum dan Selama Regulasi PSBB COVID-19 di Kota-kota Besar di Indonesia', {fontSize: '25px', color: 'darkSlateGrey'});
var text_1 = ui.Label(
'Peta polusi CO di kota-kota besar di Indonesia selama PSBB dimonitor melalui satelit Sentinel-5. Diasumsikan bahwa kebijakan PSBB diterapkan serentak pada tanggal 1 April 2020.',
{fontSize: '15px'});
var text_2 = ui.Label(
...
var legendLabels = ui.Panel({
  widgets: [
    ...
    ui.Label(0.05, {margin: '4px 8px'})
  ],
  layout: ui.Panel.Layout.flow('horizontal')
});
var legendTitle = ui.Label({
  value: 'Kadar CO (mol/m^2) ',
