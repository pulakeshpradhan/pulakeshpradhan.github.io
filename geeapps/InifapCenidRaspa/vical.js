...
// Pide al servidor conjunto de datos Landsat 5, 7, 8 y 9
function ColeccionImagenSR(fecha, recorte, umbral)
{
  ...
  // Une las series de imágenes
  var serieT =L7a.merge(L5a).merge(L8a).merge(L9a).sort('system:time_start');
	return serieT;
}
// Pide al servidor conjunto de datos Landsat 8 y 9
function ColeccionImagenSR_L89(fecha, recorte, umbral)
{
  var L9sr = imp.ColeccionLandsatSR(fecha, 'LC09', recorte, umbral);
  var L8sr = imp.ColeccionLandsatSR(fecha, 'LC08', recorte, umbral);
  var L8a = L8sr.select(['blue', 'green', 'red', 'nir', 'swir1', 'swir2']);
  var L9a = L9sr.select(['blue', 'green', 'red', 'nir', 'swir1', 'swir2']);
  // Une las series de imágenes
  var serieT =(L8a).merge(L9a).sort('system:time_start');
	return serieT;
}
// Pide al servidor conjunto de datos Landsat 5 y 7
function ColeccionImagenSR_L7(fecha, recorte, umbral) //pARA landsar 5 y 7
{
  var L7sr = imp.ColeccionLandsatSR(fecha, 'LE07', recorte, umbral);
  var L5sr = imp.ColeccionLandsatSR(fecha, 'LT05', recorte, umbral);
   var serieT =(L7sr).merge(L5sr).sort('system:time_start');
	return serieT;
}
...
function getName(imagen){
 ...
 function getURLRGB (image, image2, indice, recorte, escala){ //descargar imaen RGB
    var Name=getName(image2);
    var recorte2=image;
    var NameMap=Name.cat(ee.String('_'+'RGB'));
    var NameDownload=NameMap.getInfo();
    var downloadUrl = recorte2.getDownloadURL({ 
 ...
 function getURLNDVI2 (image, indice, recorte, escala,tipo){ 
    var recorte2=image.clip(recorte.geometry());
    var NameMap=(ee.String('StatisticalValues_'+indice+tipo));
    var NameDownload=NameMap.getInfo();
...
// ===============================================================================***************************************************
// Function for indices that require adjustment
// ===============================================================================
...
var ImgButton2 = ui.Button({
  label: 'Maximizar Panel',
  imageUrl: St.IconName('launch'),
  style: {position: 'top-left'},
  onClick: function(){panel.style().set('shown', true),ImgButton2.style().set('shown', false)}//function(){ui.util.getCurrentPosition(imp.current_position,imp.Error)}
});
Map.add(ImgButton2)
ImgButton2.style().set('shown', false)
var ImgButton3 = ui.Button({
  label: 'minimize',
  imageUrl: St.IconName('minimize'),
  style: {textAlign: 'top-center'},
  onClick: function(){panel.style().set('shown', false), ImgButton2.style().set('shown', true)}//function(){ui.util.getCurrentPosition(imp.current_position,imp.Error)}
});
panel.add(ImgButton3)
//Agrega texto de version y titulo de APP
panel.add( ui.Label({value: 'Version 1.0 - last update: August 2022', style: St.version}));
...
var VIHelp_Button = ui.Button({
  ...
  imageUrl: St.IconName('help_outline'),
...
var STHelp_Button = ui.Button({
  label: 'Help',
  style: St.HelpGeneral,
  imageUrl: St.IconName('help_outline'),
  onClick: function(){Map.add(ST_PANEL)}
});
// ===============================================================================
...
var FAQ_PANEL=ui.Panel({
    ...
    widgets:[
        ...
        ui.Label({ value:'The VICAL help section is available here',style: St.conte, targetUrl:'https://cenidraspariego.github.io/vical_en/'}),
        ...
        ui.Label({ value:'2 – Select a set of images: 1) Landsat (5, 7,8 and 9), 2) Sentinel-2, 3) LandSat (5, 7, 8 and 9) and sentinel-2, 4) Landsat (4 and 5)', style: St.conte}),
        ui.Label({ value:'3 – Select a vegetation index', style: St.conte}),
...
var ABOUT_PANEL=ui.Panel({
    ...
    widgets:[
        ...
        ui.Label({ value:'If you use this tool, please cite: Jiménez-Jiménez et al. (2022), VICAL: Global Calculator to Estimate Vegetation Indices for Agricultural Areas with Landsat and Sentinel-2 Data', style: {fontWeight: 'bold', color:'#949494', textAlign: 'left',padding: '5px',  margin: 'auto'},targetUrl:'https://www.mdpi.com/2073-4395/12/7/1518'}),
...
var SAT_PANEL=ui.Panel({
    ...
    widgets:[
        ...
        ui.Label({ value:'Vegetation indices can be extracted from: (1) Landsat (5, 7, 8 and 9), (2) Sentinel-2, (3) Landsat (5, 7, 8 and 9) and Sentinel-2 and 4) Landsat (4 and 5). VICAL used the atmospherically corrected land surface reflectance images', style: St.conte}),
        ui.Label({ value:'For option “(1) Landsat (7, 8 and 9)”, if the options “L-5 and l-7” and “L-8 and L-9” is selected, ETM+ data are spectrally adjusted to OLI spectral bands using Roy et al., (2016). If only one of the two options (“L-5 and L-7” and “L-8 and L-9”) is selected, no spectral adjustment is made.', style: St.conte}),
...
var ST_PANEL=ui.Panel({
    layout: ui.Panel.Layout.flow('vertical'),
    widgets:[
        ui.Label({ value:'STATISTICAL VALUES',style: St.titleWidwet}), 
        ui.Label({ value:'Statistical values of the vegetation index in each pixel are calculated, all the images found in the defined interval are used.', style: St.conte}),
        ui.Label({ value:'The options are:', style: St.conte}),        
        ui.Label({ value:'1. None: The vegetation index (VI) is calculated for each pixel and for each image found in the defined interval. A list of images is displayed, therefore it is possible to calculate the VI time series and download the images by date ', style: St.conte}),
        ui.Label({ value:'2. Maximum: the maximum VI values are calculated for each pixel, all the images found in the interval are used and the result is a single image that can be downloaded', style: St.conte}),
        ui.Label({ value:'3. Minimum: the minimum VI values are calculated for each pixel, all the images found in the interval are used and the result is a single image that can be downloaded', style: St.conte}),
        ui.Label({ value:'4. Mean: the mean VI values are calculated for each pixel, all the images found in the interval are used and the result is a single image that can be downloaded', style: St.conte}),        
        ui.Label({ value:'5. Median: the median VI values are calculated for each pixel, all the images found in the interval are used and the result is a single image that can be downloaded', style: St.conte}),  
    ],
...
var INDICE_PANEL=ui.Panel({
    ...
    widgets:[
        ...
        ui.Label({ value:'24. GCVI: Green Chlorophyll Vegetation Index ', style: {fontSize: '13px'}}),
        ui.Label({ value:'->    NIR / G - 1', style: {fontSize: '13px', color:'#727171'}}),   
        ui.Label({ value:'25. AWEI: Automated Water Extraction Index ', style: {fontSize: '13px'}}),
        ui.Label({ value:'->   B+2.5*G-1.5*(NIR+SWIR1)-0.25*SWIR2', style: {fontSize: '13px', color:'#727171'}}),  
        ui.Label({ value:'26. MNDWI: Modified Normalized difference water index ', style: {fontSize: '13px'}}),
        ui.Label({ value:'->   (G - SWIR1)/(G + SWIR1)', style: {fontSize: '13px', color:'#727171'}}),  
        ui.Label({ value:'27. WI: water index ', style: {fontSize: '13px'}}),
        ui.Label({ value:'->   1.7204+171*G+3*R-70*NIR-45*SWIR1-71*SWIR2', style: {fontSize: '13px', color:'#727171'}}),  
        ui.Label({ value:'28. ANDWI: Augmented Normalized Difference Water Index ', style: {fontSize: '13px'}}),
        ui.Label({ value:'->   (B+G+R-NIR-SWIR1-SWIR2)/(B+G+R+NIR+SWIR1+SWIR2)', style: {fontSize: '13px', color:'#727171'}}),  
        ui.Label({ value:' ---------------------------------------------------------------------------', style: {fontSize: '13px'}}),  
...
var CloseButtonABOUT = ui.Button({label: 'Close',imageUrl: St.IconName('close'), style: St.closeButton,
  onClick: function(){
    Map.remove(ABOUT_PANEL);
  }
});
var CloseButtonSAT = ui.Button({label: 'Close',imageUrl: St.IconName('close'), style: St.closeButton,
  onClick: function(){
    Map.remove(SAT_PANEL);
  }
});
var CloseButtonINDICE = ui.Button({label: 'Close',imageUrl: St.IconName('close'), style: St.closeButton,
  onClick: function(){
    Map.remove(INDICE_PANEL);
  }
});
var CloseButtonPR = ui.Button({label: 'Close',imageUrl: St.IconName('close'), style: St.closeButton,
  onClick: function(){
    Map.remove(PR_PANEL);
  }
});
var CloseButtonST = ui.Button({label: 'Close',imageUrl: St.IconName('close'), style: St.closeButton,
  onClick: function(){
    Map.remove(ST_PANEL);
  }
});
...
var Sensor = ui.Select({ 
 items: [ 
        { label:"1. LANDSAT (5, 7, 8 and 9)", value: 0 }, 
        { label:"2. SENTINEL-2", value: 1}, 
        { label:"3. LANDSAT (5, 7, 8 and 9) and SENTINEL-2", value: 2}, 
  ...
  onChange: function(value) {
    if (value === 0) { //es para el ARVI
        PolTL.style().set('shown', true);
    }
    else {
        PolTL.style().set('shown', false);
    }
  }  
});  
var indice = ui.Select({ 
 items: [ 
        ...
        { label:"24. GCVI",   value: 24 },
        { label:"25. AWEI",   value: 25 },
        { label:"26. MNDWI",   value: 26 },
        { label:"27. WI",   value: 27 },
        { label:"28. ANDWI",   value: 28 },
 ],
...
// Estadisticas *******************************************************************************
var Statistical=ui.Label({value:'Statistical values',style: {color: '#000000', width: '130px'}});
var StatisticalSelect = ui.Select({ 
  items: [ 
          { label:"1. None", value: 0 }, 
          { label:"2. Maximum", value: 1}, 
          { label:"3. Minimum", value: 2}, 
          { label:"4. Mean", value: 3}, 
          { label:"5. Median", value: 4},       
          { label:"6. Percentile [25]", value: 5},  
          { label:"7. Percentile [75]", value: 6},  
          { label:"8. Animation ", value: 7},
  ],
  placeholder : 'Select an option',
  value: 0,
  style: {width: '125px', border: '1px solid darkgray'},
  onChange: function(value) {
    if (value === 7) { //es para el ARVI
        value_Anima.style().set('shown', true);
    }
    else { //es para el ATSAVI
        value_Anima.style().set('shown', false);
    }
  }
}); 
var Animation_text= ui.Label({value: 'Text scale(date):', style: St.styleTexTConf});
var Animation_in= ui.Textbox({placeholder: '1', value: '1', style: {maxWidth: '70px'}});
var value_Anima= ui.Panel({
   layout: ui.Panel.Layout.flow('horizontal'),
   style: {backgroundColor: "#eceaea", margin: '2px 5px'},
   widgets: [Animation_text,Animation_in]
});
var StatisticalT= ui.Panel({
      layout: ui.Panel.Layout.flow("horizontal"),
      style: {position: 'top-center'},
      widgets: [Statistical, StatisticalSelect,STHelp_Button]
  });
//7. LinearRegresion (alpha)******************************************************************************************************
var Regresion=ui.Checkbox({label:'regression map',value: false,style: {color: '#dc7a55', width: '117px'}})
var RegressionSelect = ui.Select({ 
  ...
  value: 0,
  style: {width: '195px', border: '1px solid darkgray'},
}); 
...
//=====================================*******************************************************************************************
//SE CREAN LOS TEXTBOX DE LANDSAT 7, 8 Y 9 
//====================================
var PolL7=ui.Checkbox({label:'L-5 and L-7',value: true,style: {color: '#000000', width: '120px'}})
var PolL8=ui.Checkbox({label:'L-8 and L-9',value: true,style: {color: '#000000', width: '120px'}})
var PolTL= ui.Panel({
      layout: ui.Panel.Layout.flow("horizontal"),
      style: {position: 'top-center'},
      widgets: [PolL7,PolL8]
...
function buildImageSelect(items,uris,i,ivVis, reVis, nombreInd,recorte,Pr_Response,lis_S2A,prodVis, capa4, indSel,scala,lis_Regresion, Re_Response, nombreRegresion){ 
       var imageSelect = ui.Select({ 
           ...
           onChange: function (value){
                    var iv=ee.Image(uris.get(value)); //iNDICE DE VEGETACION
                    var ivR=ee.Image(lis_Regresion.get(value)); //REGRESION LINEAL 
                    ...
                    var capa5
                    if (Pr_Response===true) {
                        var prodrel = imp2.ProdRelativa(iv, recorte,nombreInd);//cALCULAR PRODUCTIVIDAD RELATIVA
                        var capa3=ui.Map.Layer(ee.Image(prodrel), prodVis, 'Weighting Factor');  //CAPA DE PROPDUCTIVIDAD RELATIVA
                        if (Re_Response===true) { 
                              capa5=ui.Map.Layer(ivR.select(nombreRegresion).clip(recorte), reVis, nombreRegresion);
              ...
              minBox_C1.onChange(function(value) {
                    ...
                    var pan1 = imp.Leyenda(nombreInd, ivVis);
...
function buildSelectDisplay(number) {
  if (number==1) {
    ...
    minBox_C1.setValue(0);
    maxBox_C1.setValue(15);
  }
  else if (number==9) {
    minBox_C1.setValue(-10);
    maxBox_C1.setValue(25);
  }
...
function Procesa()//*****************************************************************************************************************
{
  ...
  var PolL7_Response=PolL7.getValue();
  var PolL8_Response=PolL8.getValue();
  var ST_Response=StatisticalSelect.getValue();
  var recorte;
  if (VF_Response===true){
  ...
  if (SenSel===0) {
     if (PolL7_Response===true && PolL8_Response ===true){
          S2B = ColeccionImagenSR(fecha, recorte, umbral);
     }
     else if (PolL7_Response===true && PolL8_Response ===false){
          S2B = ColeccionImagenSR_L7(fecha, recorte, umbral);
     }
     else if (PolL7_Response===false && PolL8_Response ===true){
          S2B = ColeccionImagenSR_L89(fecha, recorte, umbral);     
     }
     scala=30;
  }
  else if  (SenSel==1) {
  ...
  if (indSel === 0) { //Atmospherically ResistanA GLOBAL CALCULATOR OF VEGETATION INDICES t Vegetation Index-ARVI
    ...
    ivs = ee.ImageCollection(S2A.map(imp2.GCVI));
    nombreInd = 'GCVI';
    buildSelectDisplay(8);
  }  
  else if (indSel == 25) { //
    ivs = ee.ImageCollection(S2A.map(imp2.AWEI));
    nombreInd = 'AWEI';
    buildSelectDisplay(7);
  }  
  else if (indSel == 26) { //
    ivs = ee.ImageCollection(S2A.map(imp2.MNDWI));
    nombreInd = 'MNDWI';
    buildSelectDisplay(1);
  } 
  else if (indSel == 27) { //
    ivs = ee.ImageCollection(S2A.map(imp2.WI));
    nombreInd = 'WI';
    buildSelectDisplay(9);
  }  
  else if (indSel == 28) { //
    ivs = ee.ImageCollection(S2A.map(imp2.ANDWI));
    nombreInd = 'ANDWI';
    buildSelectDisplay(1);
  }  
  ...
  if (ST_Response===0) {//**************************** entra para time series******************************
          titulo = ui.Label('Click inside a polygon to plot the time series');
          ...
          prodrel = imp2.ProdRelativa(iv, recorte,nombreInd); //Productividad
          ...
          Regresionfirst=RegresionValue.first();
          ...
          if (Pr_Response===true) {
            ...
            panL = imp.Leyenda(tituloPR, prodVis);
            Capa5=4;
            Map.add(panL);
            if (Re_Response===true) { 
                ...
                panL2 = imp.Leyenda(tituloPR2, reVis);
                Capa5=5;
                Map.add(panL2); 
          ...
          panL1 = imp.Leyenda(nombreInd, ivVis);
          panL = imp.Leyenda(tituloPR, prodVis);
          Map.add(panL1);
            ...
            if (Re_Response===true) { 
                ...
                GrafIV = ui.Chart.image.series(RegresionValue.select([nombreInd, nombreRegresion]),filtered,ee.Reducer.mean(), 10)
                    .setOptions({
                      ...
                      series:{
                            1: {targetAxisIndex: 1, curveType: 'function', pointSize: 4,lineWidth: 1, }, //Etapa (Derecho)
                            0: {targetAxisIndex: 0, curveType: 'function', pointSize: 4, lineWidth: 1}  // NDVI (Izquierdo)
                             },              
  ...
  else if (ST_Response===7) {
    var ndvi = ee.ImageCollection(ivs).map(function(image){return image.clip(recorte)});
    ivVis = {min :minPalete, max : maxPalete, palette : St.paletaIV};
    var rgbVis=St.TextImg(ndvi, nombreInd, parseFloat(Animation_in.getValue()), ivVis, recorte,nombreInd)
    var gifParams = {
      'region': recorte.geometry(),
      'dimensions': 500,
      'crs': 'EPSG:3857',
      'framesPerSecond': 1.5
    };
    //print(ui.Thumbnail(rgbVis, gifParams)); //aparece en la consola
    //**********************************************
    ...
    Map.add(panL1); 
        ...
        titulo_pol.add(ui.Label('click on a pixel inside the polygon to calculate its value'));
        Map.add(titulo_pol);
        var ivsNombre2;
        //var iv_P = ivs.first().clip(recorte.geometry());
        //print(iv_P)
        //var geo=ee.Geometry(iv_P.get('system:footprint'))
        if (ST_Response==1) {
           // ivs = ee.Image(ivs.max());
            ivs = ivs.reduce(ee.Reducer.max()).regexpRename('_max', '');
            ivsNombre2='Max';
        }
        else if (ST_Response==2) {
            ivs = ivs.min();
            ivsNombre2='Min'
        }
        else if (ST_Response==3) {
            ivs = ivs.mean();  
            ivsNombre2='Mean'
        }
        else if (ST_Response==4) {
            ivs = ivs.median();  
            ivsNombre2='Median'
        }
        else if (ST_Response==5) {
            ivs = ivs.reduce(ee.Reducer.percentile([25])).regexpRename('_p25', '');
            ivsNombre2='p25'        
        }
         else if (ST_Response==6) {
            ivs = ivs.reduce(ee.Reducer.percentile([75])).regexpRename('_p75', '');
            ivsNombre2='p75'        
        }       
        if (ReModel_Response===0){ 
              RegresionValue=ivs.expression(
                            'a*VI+b',{
                              VI : ivs.select(nombreInd),
                              a  : parseFloat(Ra_in.getValue()),
                              b  : parseFloat(Rb_in.getValue()),
              }),
              nombreRegresion='linealModel';
          ...
          //Se agregan las cuatro capas al mapa 
          tituloPR = 'Weighting Factor (adim.)';
          tituloPR2 = 'Regression values';
          prodVis = {min : 0.5, max : 1.5, palette : St.paletaProd};
          ivVis = {min :minPalete, max : maxPalete, palette : St.paletaIV};
          //Map.addLayer(S2A.median(), St.vizParams, 'RGB'); //RGB
          //Map.addLayer(ee.Image(ivs.clip(recorte)), ivVis, nombreInd+ivsNombre2); //Indice
          var capa1=ui.Map.Layer(ivs.clip(recorte), ivVis, nombreInd+ivsNombre2); 
          Map.layers().reset([capa1]);
          prodrel = imp2.ProdRelativa(ivs, recorte,nombreInd); //Productividad
          Capa5=2;
          if (Pr_Response===true) {
            Map.addLayer(ee.Image(prodrel), prodVis, 'Weighting Factor'); //Productividad Relativa
            ...
            if (Re_Response===true) { 
                reVis = {min :MinReg.getInfo(), max : MaxReg.getInfo(), palette : St.ETO};  
                Map.addLayer(RegresionValue.clip(recorte), reVis, nombreRegresion); //Productividad Relativa
                panL2 = imp.Leyenda(tituloPR2, reVis);
                Capa5=4;
                Map.add(panL2); 
                downloadRM.style().set('shown', true);
            ...
            if (Re_Response===true) { 
                reVis = {min :MinReg.getInfo(), max : MaxReg.getInfo(), palette :St.ETO};  
                Map.addLayer(RegresionValue.clip(recorte), reVis, nombreRegresion); //Productividad Relativa
                var capa6=ui.Map.Layer(RegresionValue.select(nombreRegresion).clip(recorte), reVis, nombreRegresion);
                panL2 = imp.Leyenda(tituloPR2, reVis);
                Capa5=3;
                Map.add(panL2); 
                downloadRM.style().set('shown', true);
          ...
          panL = imp.Leyenda(tituloPR, prodVis);
          Map.add(panL1);  
          DownloadC1_Button.setUrl(getURLNDVI2(ivs,nombreInd, recorte,scala,ivsNombre2 ));
          DownloadC3_Button.setUrl(getURLNDVI2(RegresionValue,nombreRegresion, recorte,scala,ivsNombre2));
          DownloadC2_Button.setUrl(getVectorFile(recorte));
          //Se activa el camb
          minBox_C1.setDisabled(false);
          maxBox_C1.setDisabled(false); 
          ...
          Map.onClick(function(coords) {
                ...
                var filtered = recorte.filterBounds(point);
                print(ivs)
                var temporalMean = ivs.reduce(ee.Reducer.mean());
                print(temporalMean)
                var sampledPoint = temporalMean.reduceRegion(ee.Reducer.mean(), filtered, 10);
               ...
               computedValue.evaluate(function(result) {
                                    ...
                                    titulo_pol.add(ui.Label({
                                      value: nombreInd+ivsNombre2 +': ' + result.toFixed(3),
                                      style: {stretch: 'vertical'}
                                    }));
              ...
              Map.layers().set(Capa5, filtered.style(St.HIGHLIGHT_STYLE));   
          });
  }
} //Termina Calculate*******************************************************************************************
var inifap_leyend = ui.Label('INIFAP');
