var geometry = ui.import && ui.import("geometry", "table", {
      "id": "users/diegopons06/inundaciones_historicas/deptos_Cofecyt_nucleo"
    }) || ee.FeatureCollection("users/diegopons06/inundaciones_historicas/deptos_Cofecyt_nucleo");
/*===========================================================================================
...
/* Now hit Run to start the demo! 
  ...
  *******************************************************************************************
   ...
   /* Algunas fechas y lugares para prueba
   Idiazabal:11/03/2015 
   Limite Bs.As, Cordoba y Santa Fe :04/04/2017
   departamento General Roca entre 01/01/2017 y 01/05/2017*/
var year = '/2016';
var before_start= '2016-11-01';
var before_end='2016-12-31';
// Now set the same parameters for AFTER the flood.
var after_start='2017-04-01';
var after_end='2017-05-31';
/********************************************************************************************
...
var difference_threshold = 1.10; /*threshodl to be applied on the difference image (after flood
                           ...
                           consider changing it! - ORIGINAL 1.25*/
...
/********************************************************************************************
  ...
  -----> The fi/al flood product will be ready for download on the right (under tasks) <-----
  ******************************************************************************************/
...
var population_count =  ee.Image('WorldPop/POP/ARG_2020_UNadj').clip(aoi);
...
print (population_exposed,'population_exposed');
...
//print (number_pp_exposed,'number_pp_exposed');
...
/*var LC = ee.ImageCollection('MODIS/006/MCD12Q1')
  ...
  .clip(aoi);
  */
  //------------------------------mapa agricultura copernicus
var LC = ee.Image("COPERNICUS/Landcover/100m/Proba-V-C3/Global/2016")
.select('discrete_classification').clip(aoi);
var agricultura = ee.Image("COPERNICUS/Landcover/100m/Proba-V-C3/Global/2016")
.select('discrete_classification').eq(40);
var pasturas = ee.Image("COPERNICUS/Landcover/100m/Proba-V-C3/Global/2016")
.select('discrete_classification').eq(30);
var urbano = ee.Image("COPERNICUS/Landcover/100m/Proba-V-C3/Global/2016")
.select('discrete_classification').eq(50);
var cropland= agricultura.add(pasturas);//se suman pasturas y cultivos
//Map.setCenter(-88.6, 26.4, 1);
Map.centerObject(geometry,5);
Map.addLayer(LC, {}, "Land Cover_2016");
Map.addLayer(cropland, {}, "Agricultura_2016",false);
Map.addLayer(urbano, {}, "Urbano_2016",false);
// landcover values: 40 = cropland
// 50: urban
 //------------------------------------ 
...
var MODISprojection = cropland.projection();
...
// Difference layer
Map.addLayer(difference,{min:0,max:2},"Differencia SAR",0);
// Flooded areas
Map.addLayer(flooded,{palette:"0000FF"},'Areas inundadas');
// Population Density
...
Map.addLayer(population_count, populationCountVis, 'Densidad Poblacional',0);
// Exposed Population
...
Map.addLayer(population_exposed, populationExposedVis, 'Poblacion expuesta');
// MODIS Land Cover
var LCVis = {
  min: 1.0,
  max: 17.0,
...
Map.addLayer(cropland, croplandVis, 'Cultivos y pasturas',0)
// Affected cropland
Map.addLayer(cropland_affected, croplandVis, 'Cultivos y Pasturas Afectadas'); 
// Urban
var urbanVis = {
  min: 0,
  max: 13.0,
...
Map.addLayer(urbano, urbanVis, 'Urbano',0)
// Affected urban
...
var text1 = ui.Label('Estado de inundaciones entre:',textVis);
...
var text2 = ui.Label('Superficie inundada estimada:',textVis);
var text2_2 = ui.Label('Espere por favor...',subTextVis);
dates(after_collection).evaluate(function(val){text2_2.setValue('basada en Senintel-1 imagery '+val)});
...
var text3 = ui.Label('Numero de personas potencialmente expuestas: ',textVis);
var text3_2 = ui.Label('basada en World Pop 2020',subTextVis);
...
var text4 = ui.Label('Area de cultivos y pasturas potencialmente afectadas:',textVis);
var text4_2 = ui.Label('Espere por favor...', subTextVis)
MODIS_date.evaluate(function(val){text4_2.setValue('basada en Copernicus '+val +' (100m)')}), subTextVis;
...
var text5 = ui.Label('Areas urbanas potencialmente afectadas:',textVis);
var text5_2 = ui.Label('Espere por favor...', subTextVis)
MODIS_date.evaluate(function(val){text5_2.setValue('basada en Copernicus '+val +' (100m)')}), subTextVis;
...
// Disclaimer
var text6 = ui.Label('ADVERTENCIA: Este producto es experimental y se encuentra en proceso de validacion. Tenga en cuenta las limitaciones de la informacion geografica utilizada (escala, resolucion, fechas e interpretacion). ',subTextVis)
// Produced by...
var text7 = ui.Label('Este es un producto derivado del original de UN-Spider (2019).', subTextVis)
...
// name of the legend
var names = ['Areas Potencialmente Inundadas','Cultivos y pasturas afectadas','Area urbana afectada'];
...
value: 'Densidad poblacional expuesta',
style: {
fontWeight: 'bold',
fontSize: '15px',
margin: '10px 0 0 0',
