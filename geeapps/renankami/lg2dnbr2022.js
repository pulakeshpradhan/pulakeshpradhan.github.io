...
var geometry = 
    ...
    ee.Geometry.Polygon(
        [[[-61.12561090834789, -6.0854677426728045],
          [-61.12561090834789, -6.381027208693164],
          [-60.84683283217601, -6.381027208693164],
          [-60.84683283217601, -6.0854677426728045]]], null, false),
    gpd = ee.FeatureCollection("users/renankami/PA_PAI_06_04032023_vfinal");
/***** End of imports. If edited, may not auto-convert in the playground. *****/
// NORMALIZED BURNED AREA INDEX - NBR - MEASRURING FIRE SEVERITY - REMOTE SENSING
// 2021 DRY SEASON - JULY (EX ANTE FIRE) / OCTOBER (EX POST)
...
var prefire_end = '2022-06-30';
// 2) SET TIME FRAME - TEMPORAL WINDOW - EX POST FIRE
var postfire_start = '2022-11-01';
var postfire_end = '2022-11-30';
...
//gpd = ee.FeatureCollection("users/renankami/PA_PAI_06_04032023_vfinal");
var area = ee.FeatureCollection(gpd);
...
Map.addLayer(pre_cm_mos, vis,'July 2022 - S2 False Color Image');
Map.addLayer(post_cm_mos, vis,'Nov 2022 - S2 False Color Image');
...
Map.addLayer(dNBR, {min: -100, max: 500, palette: grey}, 'dNBR continous');
...
// 18) ADD LAYER SHAPEFILE GPD's BOUNDARIES
var styling = {color: 'purple', fillColor: '00000000'};
Map.addLayer(gpd.style(styling));
//Map.addLayer(gpd, {color: 'blue'},'GPD Boundaries');
// 19) RECLASSIFING CONTINIUS DELTA NBR TO CATEGORICAL DATA (8 burn severity classes) 
var thresholds = ee.Image([-1000, -251, -101, 99, 269, 439, 659, 2000]);
var classified = dNBR.lt(thresholds).reduce('sum').toInt();
// 20) MEASURE AREA BURNED = COUNT NUMBER OF PIXELS IN EACH CLASS
...
// 21) Create an empty list to store area values in
var arealist = [];
// 22) Create a function to derive extent of one burn severity class // Arguments are class number and class name
...
var hect = pix.multiply(400).divide(10000);                              
...
// 23) Severity classes in different order
...
// 24) execute function for each class
for (var i = 0; i < 8; i++) {
...
//Export.image.toDrive({image: dNBR, scale: 20, description: 'dNBR_july_nov_2021', fileNamePrefix: 'dNBR_july_set_2021',
...
//Export.image.toDrive({image: postNBR, scale: 20, description: 'S2_post_NBR', fileNamePrefix: 'S2_post_NBR',
//region: area, maxPixels: 1e12});
// 33B) EXPORTING EX ANTE FIRE NBR
//Export.image.toDrive({image: preNBR, scale: 20, description: 'S2_pre_NBR', fileNamePrefix: 'S2_pre_NBR',
//region: area, maxPixels: 1e12});