var ahn = ui.import && ui.import("ahn", "image", {
    ...
    DPHM = ui.import && ui.import("DPHM", "image", {
      "id": "users/etiennekras/JIP_CALM_depth_proxy_heat_map"
    }) || ee.Image("users/etiennekras/JIP_CALM_depth_proxy_heat_map"),
...
App.addLayers = function() {
  ...
  // current dynamic bathymetry image
  ...
  App.layerDPHM = ui.Map.Layer (DPHMdata, {}, 'Depth Proxy Heat Map (static)', false)
  Map.layers().add(App.layerDPHM)
  App.layerObjects['Depth Proxy Heat Map'] = DPHM.select('b1').updateMask(water)
  ...
  App.layerGEBCO = ui.Map.Layer(gebcoRGB, {}, 'GEBCO', false)
  Map.layers().add(App.layerGEBCO)
  App.layerObjects['GEBCO'] = gebco
  // Load EMODnet
  var emodnetRGB =  hillshadeRGB(
          emodnet.visualize({min:min, max:max, palette: palette}),  // style
          emodnet, 
          weight, 200, azimuth, elevation, shadows)
  App.layerEMODnet = ui.Map.Layer(emodnetRGB, {}, 'EMODnet', false)
  Map.layers().add(App.layerEMODnet)
  App.layerObjects['EMODnet'] = emodnet
  // AHN3
  App.layerAHN3 = ui.Map.Layer(ee.Terrain.hillshade(ahn.multiply(4), 315-180, 25), { min: 68, max: 150 }, 'AHN3', false)
  ...
  App.aoiLayerA = ui.Map.Layer(null, {}, 'AoI');
  Map.layers().add(App.aoiLayerA); // adding aioLayerA to all the created mapLayer
}
App.createUI = function() {
  ...
  var panelAnalysis = ui.Panel({
    widgets: [
      ui.Label('Analysis (click to see charts)', { fontWeight: 'bold' }),
  ...
  var layerControlsEMODnet = createLayerUI('EMODnet', false, 'black', function(shown) { App.layerEMODnet.setShown(shown) }, function(opacity) { App.layerEMODnet.setOpacity(opacity) })
  var layerControlsGEBCO = createLayerUI('GEBCO', false, 'black', function(shown) { App.layerGEBCO.setShown(shown) }, function(opacity) { App.layerGEBCO.setOpacity(opacity) })
  ...
  var layerControlsDPHM = createLayerUI('Depth Proxy Heat Map', false, 'black', function(shown) { App.layerDPHM.setShown(shown) }, function(opacity) { App.layerDPHM.setOpacity(opacity) })
  ...
  var panelSide = ui.Panel({
    widgets: [
      ui.Label('JIP CALM Nearshore Bathymetry \nBased on Rijkswaterstaat KPP-CIP SDB', { fontSize: '16px', fontWeight: 'bold' , whiteSpace: 'pre'}),
      //ui.Label('Based on Rijkswaterstaat SDB', { fontSize: '16px', fontWeight: 'bold' }),
...
App.clearCorrelationChart = function() {
  ...
  App.corrChartPanel = null
  // hide click AOI
  App.aoiLayerA.setEeObject(ee.Image())
  App.aoiLayerA.setShown(false)
}
...
// Depth proxy heat map data preparation
var paletteInferno = ["000004", "1f0c48", "550f6d", "88226a", "a83655", "e35933", "f9950a", "f8c932", "fcffa4"]
var DPSDmos = depthProxySubtidalDynamic.mosaic()
var watermask = DPSDmos.select('water').gt(0)
var DPHMdata = DPHM.visualize({min:0, max:1, palette: paletteInferno}).updateMask(watermask)
// Start the App
App.run()
/***
...
function onMapClickShowCorrelationChart(pt) {
  ...
  App.aoiLayerA.setEeObject(ee.FeatureCollection([samplingArea]).style({
    color: '00ffff', 
    width: 2, 
    fillColor: '00ffff55'
  }))
  App.aoiLayerA.setShown(true)
  ...
  var percentiles = sample.reduceColumns(ee.Reducer.percentile([1, 99]), ['residual'])
  var chartOptions2 ={
    lineWidth: 0,
    pointSize: 1,
    dataOpacity: 0.3,
  ...
  }).setOptions(chartOptions2)
  ee.List([rmse, r2, percentiles]).evaluate(function(o) {
    // EE bug
    ...
    chartOptions2.vAxis = { viewWindow: { min: o[2].p1, max: o[2].p99 } }
    chart2.setOptions(chartOptions2)
  })
  panel.clear()
