...
var plotTimeSeries = function(x, y){  
  ...
  map.layers().set(1, ui.Map.Layer(pixel, {color: 'FF0000'}));
...
// index panel
var indexList = [['NBR',-1], ['NDVI',-1], ['NDSI',-1], ['TCB',1], ['TCG',-1],
...
var indexPanel = ui.Panel(
  [
    ui.Panel([indexBox[0], indexBox[3], indexBox[6], indexBox[9]], null, {stretch: 'horizontal'}),
    ui.Panel([indexBox[1], indexBox[4], indexBox[7], indexBox[10]], null, {stretch: 'horizontal'}),
    ui.Panel([indexBox[2], indexBox[5], indexBox[8], indexBox[11]], null, {stretch: 'horizontal'})
  ],
  ui.Panel.Layout.Flow('horizontal'), {stretch: 'horizontal'}
);
...
map.add(ui.Label({
  value: 'Click a point (be patient and do not keep clicking)',
...
hansen = hansen.updateMask(hansen.neq(2000));
map.addLayer(hansen,{'min':2000,'max':2017,'palette':'FF0,F00'},'Hansen Decline Year');