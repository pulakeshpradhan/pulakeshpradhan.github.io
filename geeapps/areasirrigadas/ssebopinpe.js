...
var instructionsPanel = ui.Panel({
  widgets:[
    ui.Label("Help?", {"margin": '8px 0px 8px 0px'}, "https://drive.google.com/file/d/1_2mj4AA-waknXYTcrbrzFrIr5mCPnRCO/view"),
  ],
  layout: ui.Panel.Layout.flow("horizontal"),
  style: extend(defaultStyle,{
    position: 'top-right',
...
var weatherDataset = ui.Select({
  items: [
    { label:"Xavier et al.", value: SSEBop.datasets.xavier },
    { label:"Xavier et al. (uses historical median)", value: SSEBop.datasets.historical_xavier},
...
searchButton = ui.Button({
  label: "SEARCH IMAGES IN THE CENTER OF MAP",
  onClick: function(){
    var start = startDate.getValue()
    var end = endDate.getValue()
    currentCollection = landsat5Collection.filterDate(start, end).filterBounds(map.getCenter())
              .merge(landsat52Collection.filterDate(start, end).filterBounds(map.getCenter()))
              .merge(landsat7Collection.filterDate(start, end).filterBounds(map.getCenter()))
              .merge(landsat72Collection.filterDate(start, end).filterBounds(map.getCenter()))
              .merge(landsat8Collection.filterDate(start, end).filterBounds(map.getCenter()))
              .merge(landsat82Collection.filterDate(start, end).filterBounds(map.getCenter()))
    panel.remove(products)
