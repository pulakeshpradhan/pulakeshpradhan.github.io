var umngi = ui.import && ui.import("umngi", "geometry", {
      "geometries": [
        {
          "type": "Polygon",
          "coordinates": [
    ...
    /* shown: false */
    ee.Geometry.Polygon(
        [[[91.36436936855615, 25.16414606297564],
          [91.41312119961084, 25.162436985789057],
          [91.40831468105615, 25.223172198524658],
          [91.34600160122217, 25.22255101979765]]]);
/*===========================================================================================
...
var before_start= '2022-05-25';
var before_end='2022-06-06';
// Now set the same parameters for AFTER the flood.
var after_start='2022-06-10';
var after_end='2022-06-24';
/********************************************************************************************
...
var difference_threshold = .90; /*threshodl to be applied on the difference image (after flood
...
var aoi = ee.FeatureCollection(umngi);
...
//7.Map.addLayer(before_filtered, {min:-25,max:0}, 'Before Flood',0);
//6.Map.addLayer(after_filtered, {min:-25,max:0}, 'After Flood',1);
// Difference layer
//5.Map.addLayer(difference,{min:0,max:2},"Difference Layer",0);
// Flooded areas
...
//4.Map.addLayer(population_count, populationCountVis, 'Population Density',0);
// Exposed Population
...
//10.Map.addLayer(population_exposed, populationExposedVis, 'Exposed Population');
// MODIS Land Cover
var LCVis = {
  min: 1.0,
  max: 17.0,
...
//9.Map.addLayer(LC, LCVis, 'Land Cover',0);
// Cropland
var croplandVis = {
  min: 0,
  max: 14.0,
...
//3.Map.addLayer(cropland, croplandVis, 'Cropland',0)
// Affected cropland
...
//2.Map.addLayer(urban, urbanVis, 'Urban',0)
// Affected urban
...
var title = ui.Label('UMNGI FLOOD ', titleTextVis);
...
MODIS_date.evaluate(function(val){text4_2.setValue('based on ESA/WorldCover'+val +' (10m)')}), subTextVis;
...
var text5 = ui.Label('Estimated affected Settelment areas:',textVis);
var text5_2 = ui.Label('Please wait', subTextVis)
MODIS_date.evaluate(function(val){text5_2.setValue('based on ESA/WorldCover'+val +' (10m)')}), subTextVis;
var number5 = ui.Label('Please wait...',numberVIS);
urban_area_ha.evaluate(function(val){number5.setValue(val+' hectares')}),numberVIS;
// Disclaimer
var text6 = ui.Label('MEGHALAYA WATER RESOURCES DEPARTMENT',titleTextVis)
// Produced by...
var text7 = ui.Label('FLOOD', titleTextVis)
...
// name of the legend
var names = ['potentially flooded areas','affected cropland','affected Habitation'];
