...
var instructionsPanel = ui.Panel({
  widgets:[
    ui.Label("Ajuda?", {"margin": '8px 0px 8px 0px'}, "https://drive.google.com/file/d/1pPswCa6bdbp6HxyCR6yImEgjyaCRkcsS/view"),
  ],
  layout: ui.Panel.Layout.flow("horizontal"),
  style: extend(defaultStyle,{
    position: 'top-right',
...
var datePanel = ui.Panel({
  widgets: [
    ui.Panel({
      widgets: [ui.Label("Data de interesse", extend(defaultStyle, {backgroundColor: "#337ab7", color: "#ffffff",  position:'top-center', textAlign: 'center'}))],
...
var locationPanel = ui.Panel({
  widgets: [
    ui.Panel({
      widgets: [ui.Label("Local de interesse", extend(defaultStyle,{backgroundColor: "#337ab7", color: "#ffffff",  position:'top-center', textAlign: 'center'}))],
      style: {backgroundColor: "#337ab7", position:'top-center'},
      layout: ui.Panel.Layout.flow("horizontal"),
    }),
...
var weatherDataset = ui.Select({
  items: [
    { label:"INMET- Inst. Nac. de Meteorologia", value: SSEBop.datasets.xavier },
    { label:"INMET- Inst. Nac. de Meteorologia (mediana histórica)", value: SSEBop.datasets.historical_xavier},
...
var parametersPanel = ui.Panel({
  widgets: [
    ui.Panel({
      widgets: [ui.Label("Parâmetros", extend(defaultStyle,{backgroundColor: "#337ab7", color: "#ffffff",  position:'top-center', textAlign: 'center'}))],
      style: {backgroundColor: "#337ab7", position:'top-center'},
      layout: ui.Panel.Layout.flow("horizontal"),
    }),
    ui.Panel({widgets: [ui.Label("Dados Meteorológicos:"), weatherDataset], layout: ui.Panel.Layout.flow("horizontal")}),
...
var footer = ui.Panel({
  widgets: [
    ...
    ui.Label("Instituições Parceiras:"),
    ui.Panel({
      widgets: [
...
var chartTimeSeries = ui.Label("Clique em qualquer ponto do mapa para ver as séries temporais da evapotranspiração.", {
  width: '100%',
  textAlign: 'center',
  fontSize: '20px',
});
...
function updateTimeSeries(){
  ...
  .setSeriesNames(['ETr', 'ETa', 'NDVI', 'Imagem LANDSAT'])
  .setOptions({
      title: title,
      titleTextStyle: {
         'fontSize': '14'
      ...
      series: {
        ...
        3: {targetAxisIndex:1, axis: 'Imagem Landsat', lineWidth: 0, pointSize: 4 },
      },
      vAxes: {
        0: {
