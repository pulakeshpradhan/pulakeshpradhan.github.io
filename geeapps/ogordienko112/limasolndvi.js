...
Map.add(uiText);
var textLabel = ui.Label('This map shows the NDVI for each season. Start of the season is September, and the end of the season is March. The map displays existing farmers, as well as the forests, highlighting the NDVI values that are not needed. The relief is represented by 4 layers. Slope shows the slope. Hillshade is represented by a shadow.Raw SRTM is a multi-coloured representation of the terrain.Contours are drawn every 10 metres to provide a detailed view. ', {
  fontWeight: 'bold',
  fontSize: '16px',
  margin: '0',
  whiteSpace: 'pre-wrap'
...
var lines = ee.List.sequence(0, 1000, 10)
...
Map.addLayer(contourlines, {min: 0, max: 5000, palette:['964B00']}, 'contours')
////////////////////////////
//see the information of the data
print (srtm);
//displaying the data in interactive map
Map.addLayer(srtm.clip(table), {min:0, max: 2000, 
...
Map.addLayer(hillshade.clip(table), {min:150, max:255,}, 'Hillshade');
print(hillshade);
var slope = ee.Terrain.slope(srtm);
Map.addLayer(slope.clip(table), {min:0, max:20, pallete: ['FFFFFF']},'Slope');
print(slope);