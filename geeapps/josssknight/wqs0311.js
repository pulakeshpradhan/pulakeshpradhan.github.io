...
function Normaliz(img){
  ...
  var norimg = (img.subtract(min).divide(max.subtract(min)).multiply(10000)).toUint16();
  return norimg;
}
function correction(img)
{
  ...
  var imgmedianpro = imgmedian.reproject(proj,null,10); 
...
function searchImage(roi,image1_start_time, image1_end_time)
{
  ...
  {labelMessage.setValue("没有满足条件的遥感影像！")}
    ...
    imgcolAA = image1col;
    print(imgcolAA);
    selector = ui.Select(getIds(imgcolAA))
    selector.setPlaceholder('选择一景影像')
    panelMain1.add(selector);
  }
}
...
function runDetection(roi,image1_start_time, image1_end_time)
{
  ...
  //AddLayerPercentStretch(imgDCstd,90,'dcstd')
//   var img1 = image1;
