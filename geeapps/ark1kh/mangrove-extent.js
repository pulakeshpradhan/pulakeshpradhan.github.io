var me2000 = ui.import && ui.import("me2000", "image", {
      "id": "users/ark1kh/IndusDelta2000"
    }) || ee.Image("users/ark1kh/IndusDelta2000"),
    me2010 = ui.import && ui.import("me2010", "image", {
      "id": "users/ark1kh/IndusDelta2010"
    }) || ee.Image("users/ark1kh/IndusDelta2010"),
    simard = ui.import && ui.import("simard", "imageCollection", {
      "id": "projects/mangrovescience/DAAC_Hba_Simard"
    }) || ee.ImageCollection("projects/mangrovescience/DAAC_Hba_Simard"),
    indusDelta = ui.import && ui.import("indusDelta", "table", {
      "id": "users/ark1kh/IndusDelta"
    }) || ee.FeatureCollection("users/ark1kh/IndusDelta"),
    viridis = ui.import && ui.import("viridis", "imageVisParam", {
      "params": {
        "min": 0,
        "max": 25,
        "palette": [
    ...
    }) || {"min":0,"max":25,"palette":["#481567FF","#482677FF","#453781FF","#404788FF","#39568CFF","#33638DFF","#2D708EFF","#287D8EFF","#238A8DFF","#1F968BFF","#20A387FF","#29AF7FFF","#3CBB75FF","#55C667FF","#73D055FF","#95D840FF","#B8DE29FF","#DCE319FF","#FDE725FF"]},
    me2020 = ui.import && ui.import("me2020", "image", {
      "id": "users/ark1kh/IndusDelta2020-1"
    }) || ee.Image("users/ark1kh/IndusDelta2020-1"),
...
var collection = ee.ImageCollection('COPERNICUS/S2')
    ...
    .filter(ee.Filter.bounds(indusDelta))
    .map(addIndices);
    // .map(maskS2clouds)
var composite = collection.median();
var rgbVis = {min: 0.0, max: 3000, bands: ['B4', 'B3', 'B2']};
var fccVis = {min: 0.0, max: 3000, bands: ['B8', 'B4', 'B3']};
var ndviVis = {min:0, max:0.5, bands: ['ndvi'] , palette: ['white', 'green']};
var mndwiVis = {min:0, max:0.5, bands: ['mndwi'], palette: ['white', 'blue']};
///////////////////////////////////////////////////////////////
...
//Center the map to indusDelta
Map.centerObject(indusDelta,9)
...
var ext2000 = ui.Map.Layer(me2000, {palette:['6D63EB'], min:1, max:1}, 'Extent 2000',false)
var ext2010 = ui.Map.Layer(me2010, {palette:['34BFDE'], min:1, max:1}, 'Extent 2010',false)
var ext2020 = ui.Map.Layer(me2020, {palette:['71F4B7'], min:1, max:1}, 'Extent 2020',false)
...
//App title
var header = ui.Label('Indus Delta Mangrove Height, Extent, and Loss Explorer', {fontSize: '25px', fontWeight: 'bold', color: '4A997E'});
//App summary
var text = ui.Label(
  'This tool maps mangrove extent in the Indus Delta in 2000, 2010, and 2020 using a Random Forest Classification derived from Landsat imagery. ' +
...
var get2000 = me2000.multiply(ee.Image.pixelArea()).divide(10000).reduceRegion({
...
var feature = ee.Feature(indusDelta)
...
//2010
//Calculate area in Hectares
var get2010 = me2010.multiply(ee.Image.pixelArea()).divide(10000).reduceRegion({
      reducer:ee.Reducer.sum(),
      geometry:indusDelta,
      scale: 1000,
      maxPixels:1e13,
...
//2020
//Calculate area in Hectares
var get2020 = me2020.multiply(ee.Image.pixelArea()).divide(10000).reduceRegion({
      reducer:ee.Reducer.sum(),
      geometry:indusDelta,
      scale: 1000,
      maxPixels:1e13,
...
//Construct Bar Chart
var chart2020 = ui.Chart.feature.byProperty([feature2020, feature2010, feature2000], ['2020', '2010', '2000'])
//Set up title and labels for chart
...
var rgb = 'Natural Color (RGB)';
var fcc = 'False Color (NRG)';
var ndvi = 'NDVI';
var mndwi = 'MNDWI';
//Construct Dropdown
var compositeSelect = ui.Select({
  items:[rgb, fcc, ndvi, mndwi],
...
function selectLayer(){
  var selected = compositeSelect.getValue() // get value from dropdown selection
  //We use "if else" statements to write instructions for drawing graphs
  if (selected == rgb){
    Map.addLayer(composite.clip(indusDelta), rgbVis, 'Natural Color (RGB)');
  }
  else if (selected == fcc){
     Map.addLayer(composite.clip(indusDelta), fccVis, 'False Color (NIR-R-G)');
  }
  else if (selected == ndvi){
     Map.addLayer(composite.clip(indusDelta), ndviVis, 'NDVI');
  }
  else if (selected == mndwi){
     Map.addLayer(composite.clip(indusDelta), mndwiVis, 'MNDWI');
  }
}
//Create a new label
...
panel.add(panelGraph).add(compositeSelect)
function reset() {
  Map.clear()
}
var button = ui.Button('Reset', reset, false);
panel.add(button)