...
var filter = ee.Filter.eq('ADM0_NAME','Spain');
var boundaries = Countries.filter(filter);
var date_0 = '2022-03-01';
var date_1 = '2022-03-31';
var date_2 = '2022-04-01';
var date_3 = '2022-04-11';
// var point = geometry;
// var geometry = /* color: #d63000 */ee.Geometry.Point([-71.24110221862793,-30.074813339923487]);
////////////////////////////////////////////////////////////////////////////////////////////////////////////// CLOUD PROB FUNCTION.
function prob_maskClouds(img) {
...
var clouds_prob_1 = ee.ImageCollection('COPERNICUS/S2_CLOUD_PROBABILITY')
...
///////////////////////////////////////SNOW1
var s2_snow_layer_1 = function(image){
  // 1.Locate SCL product
  var scl = image.select('SCL');
  // 2.Apply mask to the image
  // Select values: 1) greater than 3 and less than 7 or 2) equal to 11
  var wantedPixels = scl.eq(11);
  return wantedPixels.selfMask();
};
var snow_1 = coll1.map(s2_snow_layer_1).max();
/////////////////////////////WATER1
var s2_water_layer_1 = function(image){
  // 1.Locate SCL product
  var scl = image.select('SCL');
  // 2.Apply mask to the image
  // Select values: 1) greater than 3 and less than 7 or 2) equal to 11
  var wantedPixels = scl.eq(6);
  return wantedPixels.selfMask();
};
var water_1 = coll1.map(s2_water_layer_1).max();
////////////////////////////////////////////
...
var coll1 = coll1.map(maskEdges);
var coll1 = ee.Join.saveFirst('cloud_mask').apply({
  primary: coll1,
  secondary: clouds_prob_1,
  condition:
      ee.Filter.equals({leftField: 'system:index', rightField: 'system:index'})
});
var coll1 = ee.ImageCollection(coll1).map(prob_maskClouds);
////////////////////////////////////////////////////////////////////////////////////////////////
// NDVI Mask coll1
var coll1 = coll1.map(function(image) {
  var ndvi_mask = image.select().addBands(image.normalizedDifference(['B8', 'B4'])).rename('NDVI');
var ndvi_mask_app = ndvi_mask//.gte(0.150);
      return image.updateMask(ndvi_mask_app)//.resample('bicubic');
});
//////////////////////////// coll2
var clouds_prob_2 = ee.ImageCollection('COPERNICUS/S2_CLOUD_PROBABILITY')
            .filterBounds(boundaries)
...
/////////////////////////SNOW2
var s2_snow_layer_2 = function(image){
  // 1.Locate SCL product
  var scl = image.select('SCL');
  // 2.Apply mask to the image
  // Select values: 1) greater than 3 and less than 7 or 2) equal to 11
...
var snow_2 = coll2.map(s2_snow_layer_2).max();
/////////////////////////////WATER2
var s2_water_layer_2 = function(image){
  // 1.Locate SCL product
  var scl = image.select('SCL');
  // 2.Apply mask to the image
  // Select values: 1) greater than 3 and less than 7 or 2) equal to 11
...
var water_2 = coll2.map(s2_water_layer_2).max();
////////////////////////////////////////////////
...
var coll2 = coll2.map(maskEdges);
var coll2 = ee.Join.saveFirst('cloud_mask').apply({
  primary: coll2,
  secondary: clouds_prob_2,
  condition:
      ee.Filter.equals({leftField: 'system:index', rightField: 'system:index'})
});
var coll2 = ee.ImageCollection(coll2).map(prob_maskClouds);
////////////////////////////////////////////////////////////////////////////////////////////////
// NDVI Mask coll2
var coll2 = coll2.map(function(image) {
  var ndvi_mask = image.select().addBands(image.normalizedDifference(['B8', 'B4'])).rename('NDVI');
...
var img_0 = ee.Image(coll1.mean());
var img_1 = ee.Image(coll2.mean());
...
var img_ndvi_2 = ((img_ndvi_1.subtract(img_ndvi_0)).divide(img_ndvi_0)).multiply(100)
                        .rename('NDVI_diff');
...
var NAMES = ['Antes '+date_0+' a '+date_1,
             'Después '+date_2+' a '+date_3,
             'Cambio Porcentual'];
var vis_params = {opacity: 0.85, min: -0.2, max: 0.9, palette: [
  'blue','SaddleBrown ','DarkKhaki','GreenYellow ','DarkGreen ','Cyan'
]};
var vis_params_2 = {opacity: 0.8, min: -100, max: 100, palette: [
...
NAMES.forEach(function(name, index) {
  ...
  map.addLayer(SNOW[index],{palette:['white']},'Nieve');
  map.addLayer(WATER[index],{palette:['blue']},'Agua');
  // map.addLayer(canal,{color: '3399ff'}, 'Canales');
  // map.setControlVisibility(true);
  maps.push(map);
  map.centerObject(boundaries, 6, false);
...
// Create a title.
var title = ui.Label('Cambio en el indice vegetacional ['+country_name+']'+' [Efecto Heladas Abril 2022]', {
  stretch: 'horizontal',
  textAlign: 'center',
  fontWeight: 'bold',
  fontSize: '16px'
...
var credits = ui.Label('Desarrollo: Rodrigo Márquez A. Ingeniero Agrónomo, 2022 [rma@ASR-arvipo.com]', {
  stretch: 'horizontal',
  textAlign: 'left',
  fontWeight: 'bold',
  fontSize: '10px'
...
maps[0].centerObject(boundaries, 6, ui.root.setLayout(ui.Panel.Layout.Flow('vertical')));
//////////////////////////////////////////////////////////////////////////////
