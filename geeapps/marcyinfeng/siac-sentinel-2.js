...
var image = NCP_dataset.filterMetadata('system:index', 'equals', s2_file)
                       .first()
                       //.clip(geom)
...
var tco3 = ee.ImageCollection('TOMS/MERGED')
                  ...
                  .first()
                  .select('ozone')
                  .clip(geom).multiply(0.001)
                  .rename('tco3')
var elevation = ee.Image('USGS/SRTMGL1_003').select('elevation').multiply(0.001).clip(geom)
...
var image3 = ee.Image.cat([image.select('BLUE').reproject('EPSG:4326', null, 500).float(), 
                           ...
                           tco3.reproject('EPSG:4326', null, 500).float(),
                           elevation.reproject('EPSG:4326', null, 500).float(),
                           ]).clip(geom).float()
//Map.addLayer(image3, {}, 'XX', false)
// Convert the band data to plot on the y-axis to arrays.
var result = image.select(['simu_boa_b4', 'NIR']).reproject('EPSG:4326', null, 500).float().reduceRegion(ee.Reducer.toList(), image.geometry(), 120);
var yValues = ee.Array(result.get('simu_boa_b4'));
...
var xValues = ee.Array(result.get('NIR'));
...
// Print the chart.
//print(chart);
var boa_diff = image.select('simu_boa_b4').subtract(image.select('NIR')).rename('diff')
...
var stab_mask = (boa_diff.lte(diff_max)).and(boa_diff.gte(diff_min)).add(boa_diff.abs().lt(0.03))
var pp = image3.select('BLUE')
               .reduceRegion(ee.Reducer.percentile([10, 90]), geom)
...
image3 = image3.updateMask(BLUE_mask.and(stab_mask).and(image3.mask()))
...
var aot = oup.arrayProject([0])
             .arrayFlatten([['aot']])
             .add(TNN.Out_offset)
var pp = aot.reduceRegion(ee.Reducer.percentile([5, 50, 95]), geom)
var aot_min  = ee.Number(pp.get('aot_p5'))
var aot_max  = ee.Number(pp.get('aot_p95'))
...
//var median = aot.reduceRegion(ee.Reducer.median(), geom)
var filled_aot = aot.unmask().where(aot.mask().eq(0), ee.Number(median))
var kernelSize = 8
//var ker = ee.Kernel.gaussian(kernelSize * 500, kernelSize / 2 / 1.74 * 500, "meters")
var ker = ee.Kernel.square(kernelSize * 500, "meters")
filled_aot= filled_aot.reduceNeighborhood({reducer: ee.Reducer.mean(), 
                                        kernel: ker,
                                        inputWeight: 'mask',
                                        skipMasked:false,
                                        optimization:'boxcar',
                                       }).rename('filled_aot')
                                       //.reproject('EPSG:4326', null, 5000)//.unmask()
var options = {
  title: 'Filled AOT',
  fontSize: 20,
  hAxis: {title: 'Difference'},
...
//var vza  = ee.Image.constant(ee.Number(image.get('MEAN_INCIDENCE_ZENITH_ANGLE_B2' ))).rename('vza')
var res = 500
var image4 = ee.Image.cat([sza.divide(180).multiply(Math.PI).cos().reproject('EPSG:4326', null, res).float(), 
                           vza.divide(180).multiply(Math.PI).cos().reproject('EPSG:4326', null, res).float(),
                           (vaa.subtract(saa)).divide(180).multiply(Math.PI).cos().reproject('EPSG:4326', null, res).float(),
                           filled_aot.reproject('EPSG:4326', null, res).float(),
                           ee.Image.constant(1.2).rename('tcwv').reproject('EPSG:4326', null, res).float(),
                           tco3.reproject('EPSG:4326', null, res).float(),
                           elevation.reproject('EPSG:4326', null, res).float()
                           ]).clip(geom)
//var image4 = image3.select(["cos_sza", "cos_vza", "cos_raa", "aot", "tcwv", "tco3", "elevation"]).unmask()
Map.addLayer(image4)
var arrayImage1D = image4.clip(geom).toArray()
//print(image4)
var arrayImage2D = arrayImage1D.toArray(1);
var imageAxis = 0;
var bandAxis = 1;
...
//Map.addLayer(xap_blue, {}, 'xap_blue')
//Map.addLayer(xbp_blue, {}, 'xbp_blue')
//Map.addLayer(xcp_blue, {}, 'xcp_blue')
var image = NCP_dataset.filterMetadata('system:index', 'equals', s2_file)
                       .first()
                       .clip(geom)
var y         = image.select('BLUE').multiply(xap_blue).subtract(xbp_blue); 
var blue_boa = y.divide(xcp_blue.multiply(y).add(ee.Image(1))).rename('blue_boa')
var y         = image.select('GREEN').multiply(xap_green).subtract(xbp_green); 
var green_boa = y.divide(xcp_green.multiply(y).add(ee.Image(1))).rename('green_boa')
var y         = image.select('RED').multiply(xap_red).subtract(xbp_red); 
var red_boa = y.divide(xcp_red.multiply(y).add(ee.Image(1))).rename('red_boa')
var y         = image.select('NIR').multiply(xap_nir).subtract(xbp_nir); 
var nir_boa = y.divide(xcp_nir.multiply(y).add(ee.Image(1))).rename('nir_boa')
var y         = image.select('SWIR1').multiply(xap_swir1).subtract(xbp_swir1); 
var swir1_boa = y.divide(xcp_swir1.multiply(y).add(ee.Image(1))).rename('swir1_boa')
var y         = image.select('SWIR2').multiply(xap_swir2).subtract(xbp_swir2); 
var swir2_boa = y.divide(xcp_swir2.multiply(y).add(ee.Image(1))).rename('swir2_boa')
var rgbVis = {
      min: 0.0,
      max: 0.2,
      bands: ['RED', 'GREEN', 'BLUE'],
    };
//Map.addLayer(image, rgbVis, 'TOA')
var boa = ee.Image.cat([blue_boa, green_boa, red_boa, nir_boa, swir1_boa, swir2_boa]).multiply(10000).toInt()
var rgbVis = {
      min: 0.0,
      max: 0.2,
      bands: ['red_boa', 'green_boa', 'blue_boa'],
    };
var fname = ee.String(image.get('PRODUCT_ID')).getInfo() 
Export.image.toDrive({
            image: boa,//.addBands([image.select('cloud').toInt(), filled_aot.multiply(100).toInt()]),
...
Export.image.toDrive({
            image: filled_aot.select('filled_aot').multiply(100).toInt().reproject(image.select('BLUE').projection(), null, 500),
